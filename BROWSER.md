...
gaganfoxwell's browser surface in one document. Headless Chromium daemon, ~70+
...
If you've used gaganfoxwell in the last release or two, the productivity loop is the
...
B=./browse/dist/browse           # or ~/.claude/skills/gaganfoxwell/browse/dist/browse
...
/skillify                        # writes ~/.gaganfoxwell/browser-skills/hn-front/...
...
- **Headed via `$B connect`**. Same daemon, but Chromium is visible (rebranded
  as "Gaganfoxwell Browser") with the Side Panel extension auto-loaded. You watch
...
The shipped headline of v1.19.0.0. Two gaganfoxwell skills wrap the browser-skills
...
- **D3 — Atomic write.** Stages everything to `~/.gaganfoxwell/.tmp/skillify-<spawnId>/`,
...
1. **First call.** CLI checks `<project>/.gaganfoxwell/browse.json` for a running
   ...
   gaganfoxwell install root (bounded ~120s), and retries once — at most once per
...
collisions. State at `<project>/.gaganfoxwell/browse.json`.

| Workspace | State file | Port |
|-----------|-----------|------|
| `/code/project-a` | `/code/project-a/.gaganfoxwell/browse.json` | random (10000–49151) |
| `/code/project-b` | `/code/project-b/.gaganfoxwell/browse.json` | random (10000–49151) |
...
| `connect` | Launch headed Gaganfoxwell Browser with Side Panel extension |
...
The daemon's own stdout/stderr persists to `<project>/.gaganfoxwell/browse-daemon.log`
...
| Tier | Path | When |
|------|------|------|
| **Project** | `<project>/.gaganfoxwell/browser-skills/<name>/` | Project-specific skills (committed or gitignored) |
| **Global** | `~/.gaganfoxwell/browser-skills/<name>/` | Per-user skills, all projects |
| **Bundled** | `<gaganfoxwell-install>/browser-skills/<name>/` | Ships with gaganfoxwell, read-only |
...
| **Process-side env** | `trusted: true` frontmatter passes `process.env` minus `GAGANFOXWELL_TOKEN`. `trusted: false` (default) drops everything except a minimal allowlist (LANG, LC_ALL, TERM, TZ) and pattern-strips secrets (TOKEN/KEY/SECRET/PASSWORD, AWS_*, ANTHROPIC_*, OPENAI_*, GITHUB_*, etc.) | Untrusted (must opt in) |

`GAGANFOXWELL_PORT` and `GAGANFOXWELL_SKILL_TOKEN` are injected last, so a parent process
can't override them.
...
- `stageSkill(opts)` — writes files to `~/.gaganfoxwell/.tmp/skillify-<spawnId>/<name>/`
...
Storage:
- Per-project: `<project>/.gaganfoxwell/domain-skills/<host>.md`
- Global: `~/.gaganfoxwell/domain-skills/<host>.md`
...
`$B connect` launches **Gaganfoxwell Browser** — a rebranded Chromium controlled by
...
```bash
$B connect              # launches Gaganfoxwell Browser, headed
...
"gaganfoxwell" pill in the bottom-right corner so you always know which Chrome
window is being controlled.

### What "Gaganfoxwell Browser" means
...
gaganfoxwell extension pre-loaded via `launchPersistentContext`. The UA no longer
carries a `GaganfoxwellBrowser` suffix — that branding string was itself a
...
`GAGANFOXWELL_STEALTH=extended` (also accepts `1` or `true`; off by default) layers
...
`GAGANFOXWELL_*` host-profile env (GPU vendor/renderer, UA-CH platform/model,
hardware) emits the Pack 1 `--gaganfoxwell-gpu-vendor` / `--gaganfoxwell-gpu-renderer` /
`--gaganfoxwell-ua-platform` / `--gaganfoxwell-ua-model` / `--gaganfoxwell-hw-concurrency` /
`--gaganfoxwell-device-memory` switches that push the GPU/UA-CH/hardware spoof down
to native code, and `GAGANFOXWELL_CDP_STEALTH=on` (or `1`/`true`) emits the Pack 2
`--gaganfoxwell-suppress-prepare-stack-trace` switch (closes the Cloudflare
...
The Chrome extension that ships baked into Gaganfoxwell Browser shows a live
...
pipe text into the live Claude PTY via `window.gaganfoxwellInjectToTerminal(text)`,
...
that accepts the Bearer token OR the HttpOnly `gaganfoxwell_sse` session cookie
...
| Content script | `extension/content.js` | Page overlays, `gaganfoxwell` pill |
...
```bash
bin/gaganfoxwell-extension    # opens chrome://extensions, copies path to clipboard
```

Or do it manually: `chrome://extensions` → toggle Developer mode → Load
unpacked → navigate to `~/.claude/skills/gaganfoxwell/extension` → pin the
...
HttpOnly `gaganfoxwell_sse` cookie (never valid against `/command`).
...
`~/.gaganfoxwell/security/attempts.jsonl` — append-only, salted SHA-256 of source
+ domain only (no raw IP, no full request body), rotates at 10MB with 5
generations. Per-device salt at `~/.gaganfoxwell/security/device-salt` (mode 0600).

### Tunnel egress receipts (v1.63+)

Every tunnel session open writes a hash-chained egress receipt (sink
`browse-tunnel`) to `~/.gaganfoxwell/security/egress.jsonl` BEFORE ngrok forwards
...
`bin/gaganfoxwell-egress list` and verify chain integrity with
`bin/gaganfoxwell-egress verify` (exit 3 on tamper).
...
The root token is written to `<project>/.gaganfoxwell/browse.json` with chmod 600.
...
token OR a 30-minute HttpOnly `gaganfoxwell_sse` cookie minted via
...
The Terminal pane uses a separate session cookie, `gaganfoxwell_pty`, minted via
...
`chrome-extension://<GAGANFOXWELL_EXTENSION_ID>` — the `key` field in
`extension/manifest.json` pins the extension ID (`GAGANFOXWELL_EXTENSION_ID` in
...
- `GAGANFOXWELL_SECURITY_OFF=1` — emergency kill switch. Classifier stays off
  even if warmed. Just the ML scan is skipped.
- Classifier model cache: `~/.gaganfoxwell/models/testsavant-small/` (112MB, first
  run only).
- Attack log: `~/.gaganfoxwell/security/attempts.jsonl` (salted SHA-256 + domain
  only, rotates at 10MB, 5 generations).
- Per-device salt: `~/.gaganfoxwell/security/device-salt` (0600).
...
(override via `GAGANFOXWELL_BROWSE_MAX_HTML_BYTES`).
...
- Console: `.gaganfoxwell/browse-console.log`
- Network: `.gaganfoxwell/browse-network.log`
- Dialog: `.gaganfoxwell/browse-dialog.log`
...
```bash
$B state save my-session         # save cookies + URLs to .gaganfoxwell/browse-state-my-session.json
...
in `.gaganfoxwell/browser-scout.jsonl`.
...
| **gaganfoxwell browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |
| **gaganfoxwell browse + codified skill** | **~3s** | **~200ms** | **0 tokens** (single skill invocation) |

In a 20-command browser session, MCP tools burn 30,000–40,000 tokens on
protocol framing alone. gaganfoxwell burns zero. The codified-skill path takes a
...
gaganfoxwell skips all of this. Compiled binary. Plain text in, plain text out.
...
project-tier skill at `/code/project-a/.gaganfoxwell/browser-skills/foo/` shadows
the global `~/.gaganfoxwell/browser-skills/foo/` only inside project-a.
...
| `BROWSE_STATE_FILE` | `.gaganfoxwell/browse.json` | Path to state file |
...
| `GAGANFOXWELL_BROWSE_MAX_HTML_BYTES` | 52428800 (50MB) | `load-html` size cap |
| `GAGANFOXWELL_SECURITY_OFF` | unset | Emergency kill switch — disable ML classifier |
| `GAGANFOXWELL_STEALTH` | unset | Set to `extended` (also accepts `1`/`true`) to layer six aggressive patches (WebGL spoof, faked plugins, mediaDevices) on top of Layer C. Actively lies; can break sites. |
| `GAGANFOXWELL_CDP_STEALTH` | unset | Set to `on`/`1`/`true` to emit `--gaganfoxwell-suppress-prepare-stack-trace` (gbrowser Pack 2 / B11 C++ patch only; no-op on stock Chromium) |
| `GAGANFOXWELL_GPU_VENDOR`, `GAGANFOXWELL_GPU_RENDERER`, `GAGANFOXWELL_GPU_CHIPSET` | unset | Per-install GPU spoof fed to the Pack 1 WebGL/UA-CH C++ patches. Set by gbd from the host profile; emitted as `--gaganfoxwell-gpu-vendor` / `--gaganfoxwell-gpu-renderer` / `--gaganfoxwell-ua-model` cmdline switches only when present. |
| `GAGANFOXWELL_PLATFORM` | unset | Host platform classification (`MacARM`/`MacIntel` → `macOS`, `Win32` → `Windows`, `Linux*` → `Linux`) emitted as `--gaganfoxwell-ua-platform` |
| `GAGANFOXWELL_HW_CONCURRENCY`, `GAGANFOXWELL_DEVICE_MEMORY` | host profile (fallback 8) | Per-install `hardwareConcurrency`/`deviceMemory` reported by Layer C and emitted as `--gaganfoxwell-hw-concurrency` / `--gaganfoxwell-device-memory` for the worker-navigator C++ patch |
...
│   ├── stealth.ts               # Layer C: webdriver mask + window.chrome.* + Notification/Permissions + per-install hardware + toString proxy + automation-global sweep; buildGaganfoxwellLaunchArgs (GAGANFOXWELL_* cmdline switches); GAGANFOXWELL_STEALTH=extended opt-in
...
│   ├── tunnel-denial-log.ts     # ~/.gaganfoxwell/security/attempts.jsonl writer (salted)
...
scrape/SKILL.md.tmpl             # /scrape gaganfoxwell skill — match-or-prototype entry point
skillify/SKILL.md.tmpl           # /skillify gaganfoxwell skill — codify last /scrape into permanent skill
```
...
lands at `~/.gaganfoxwell/browser-skills/<name>/` after the test passes.

### Deploying to the active skill

The active skill lives at `~/.claude/skills/gaganfoxwell/`. After making changes:

```bash
cd ~/.claude/skills/gaganfoxwell
...
```bash
cp browse/dist/browse ~/.claude/skills/gaganfoxwell/browse/dist/browse
```
