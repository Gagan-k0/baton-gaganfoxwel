...
describe("substituteSlots", () => {
  test("replaces the <p>-wrapped token with slot HTML", () => {
    const slots = new Map([["gaganfoxwell-diagram-slot-ab-1", "<figure>X</figure>"]]);
    const html = "<h1>T</h1>\n<p>gaganfoxwell-diagram-slot-ab-1</p>\n<p>tail</p>";
    const out = substituteSlots(html, slots);
    expect(out).toContain("<figure>X</figure>");
    expect(out).not.toContain("gaganfoxwell-diagram-slot");
...
describe("inlineLocalImages", () => {
  ...
  test("local image becomes a data URI with probed dimensions", () => {
    ...
    expect(out).toContain('data-gaganfoxwell-px-width="40"');
    expect(out).toContain('data-gaganfoxwell-px-height="20"');
  ...
  test("existing data URI gets dimension annotations only", () => {
    ...
    expect(out).toContain('data-gaganfoxwell-px-width="33"');
    expect(out).toContain('data-gaganfoxwell-px-height="44"');
  });
  ...
  test("oversized raster without a tab inlines at full size with no downscale", () => {
    ...
    expect(out).toContain('data-gaganfoxwell-px-width="6000"');
  });
});
