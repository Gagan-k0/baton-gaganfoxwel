...
describe("applyImageDirectives", () => {
  test("brace suffix becomes data attrs and is consumed", () => {
    ...
    expect(out).toContain('data-gaganfoxwell-width="50%"');
...
describe("width styles", () => {
  test("width=full → inline 100% style", () => {
    const { html } = applyImagePolicy(img(`src="x" data-gaganfoxwell-width="full"`), OPTS);
    expect(html).toContain("width: 100%");
  });
  test("explicit dimension passes through", () => {
    const { html } = applyImagePolicy(img(`src="x" data-gaganfoxwell-width="3in"`), OPTS);
  ...
  test("width directive merges with an existing style attribute, preserving it", () => {
    const { html } = applyImagePolicy(
      img(`src="x" style="border: 1px solid" data-gaganfoxwell-width="50%"`),
      OPTS,
    );
  ...
  test("no directive → no inline style (CSS max-width owns the default)", () => {
    const { html } = applyImagePolicy(img(`src="x" data-gaganfoxwell-px-width="40" data-gaganfoxwell-px-height="20"`), OPTS);
...
describe("auto-landscape: negative cases (the load-bearing ones)", () => {
  test("wide screenshot with no alt hint stays portrait", () => {
    const r = applyImagePolicy(
      img(`src="x" alt="screenshot of the app" data-gaganfoxwell-px-width="3000" data-gaganfoxwell-px-height="900"`),
      OPTS,
    );
  ...
  test("wide banner with hint but below width threshold stays portrait", () => {
    const r = applyImagePolicy(
      img(`src="x" alt="chart" data-gaganfoxwell-px-width="1200" data-gaganfoxwell-px-height="400"`),
      OPTS,
    );
    expect(r.hasLandscape).toBe(false);
  });
  test("tall diagram (aspect below 1.8) stays portrait", () => {
    const r = applyImagePolicy(
      img(`src="x" alt="architecture diagram" data-gaganfoxwell-px-width="2000" data-gaganfoxwell-px-height="1500"`),
      OPTS,
    );
    expect(r.hasLandscape).toBe(false);
  });
  ...
  test("page=portrait vetoes everything", () => {
    const r = applyImagePolicy(
      img(`src="x" alt="diagram" data-gaganfoxwell-page="portrait" data-gaganfoxwell-px-width="4000" data-gaganfoxwell-px-height="1000"`),
      OPTS,
    );
    expect(r.hasLandscape).toBe(false);
  });
  test("threshold boundary is deterministic: exactly at threshold stays portrait", () => {
    ...
    const r = applyImagePolicy(
      img(`src="x" alt="diagram" data-gaganfoxwell-px-width="1560" data-gaganfoxwell-px-height="600"`),
      OPTS,
    );
    expect(r.hasLandscape).toBe(false);
    const r2 = applyImagePolicy(
      img(`src="x" alt="diagram" data-gaganfoxwell-px-width="1561" data-gaganfoxwell-px-height="600"`),
      OPTS,
    );
...
describe("auto-landscape: positive cases", () => {
  test("wide + alt hint + over threshold promotes, wraps, and vertically centers", () => {
    ...
    const r = applyImagePolicy(
      img(`src="x" alt="architecture diagram" data-gaganfoxwell-px-width="2400" data-gaganfoxwell-px-height="1000"`),
  ...
  test("directive-forced tall block that fills the page gets no centering margin", () => {
    ...
    const r = applyImagePolicy(
      img(`src="x" data-gaganfoxwell-page="landscape" data-gaganfoxwell-px-width="1000" data-gaganfoxwell-px-height="900"`),
      OPTS,
    );
    expect(r.hasLandscape).toBe(true);
  ...
  test("page=landscape forces promotion regardless of size", () => {
    const r = applyImagePolicy(img(`src="x" data-gaganfoxwell-page="landscape"`), OPTS);
    expect(r.hasLandscape).toBe(true);
  ...
  test("alt hint matches whole words only", () => {
    const r = applyImagePolicy(
      img(`src="x" alt="photographic" data-gaganfoxwell-px-width="2400" data-gaganfoxwell-px-height="1000"`),
      OPTS,
    );
...
describe("auto-landscape: diagram figures", () => {
  ...
  test("fence page=portrait vetoes a wide diagram", () => {
    const r = applyImagePolicy(
      fig(`width="100%" viewBox="0 0 3000 600"`, ` data-gaganfoxwell-page="portrait"`),
      OPTS,
    );
    expect(r.hasLandscape).toBe(false);
  });
  test("fence page=landscape forces a small diagram", () => {
    const r = applyImagePolicy(
      fig(`width="100%" viewBox="0 0 400 300"`, ` data-gaganfoxwell-page="landscape"`),
      OPTS,
    );
    expect(r.hasLandscape).toBe(true);
  });
