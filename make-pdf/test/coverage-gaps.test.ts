...
describe("rasterizeDiagramFigures (mock tab)", () => {
  ...
  test("figure rasterization failure surfaces the SOURCE as text (never silent loss)", () => {
    ...
    const srcFigure = figure.replace(
      ...
      `<figure class="diagram" data-gaganfoxwell-source="${Buffer.from("graph LR\n  A --> B").toString("base64")}"`,
    );
...
describe("pure-function stragglers", () => {
  ...
  test("substituteSlots bare-token fallback (token not <p>-wrapped)", () => {
    const slots = new Map([["gaganfoxwell-diagram-slot-x-1", "<figure>D</figure>"]]);
    const out = substituteSlots("<li>gaganfoxwell-diagram-slot-x-1</li>", slots);
  ...
  test("resolveBundlePath honors the env override", () => {
    ...
    try {
      expect(resolveBundlePath({ GAGANFOXWELL_DIAGRAM_BUNDLE: tmp } as NodeJS.ProcessEnv)).toBe(tmp);
    } finally {
      fs.unlinkSync(tmp);
    }
  });
  ...
  // GAGANFOXWELL_DIAGRAM_BUNDLE pointing at a missing file outside a repo.
