 ...
 *     `![alt](x.png){width=50%}` / `{page=landscape}` into data-gaganfoxwell-*
 ...
 *     annotates data-gaganfoxwell-px-width/-height from real bytes). Applies the
...
export function applyImageDirectives(html: string): string {
  return html.replace(IMG_WITH_SUFFIX_RE, (full, imgTag: string, body: string) => {
    ...
    let tag = imgTag;
    if (parsed.width) tag = addAttr(tag, "data-gaganfoxwell-width", parsed.width);
    if (parsed.page) tag = addAttr(tag, "data-gaganfoxwell-page", parsed.page);
    return tag;
  });
}
...
export function applyImagePolicy(html: string, opts: ImagePolicyOptions): ImagePolicyResult {
  ...
  let out = html.replace(/<img\b[^>]*>/gi, (tag) => {
    const width = attrValue(tag, "data-gaganfoxwell-width");
    if (!width) return tag;
  ...
  out = out.replace(/<p>\s*(<img\b[^>]*>)\s*<\/p>/gi, (full, tag: string) => {
    ...
    const w = num(attrValue(tag, "data-gaganfoxwell-px-width"));
    const h = num(attrValue(tag, "data-gaganfoxwell-px-height"));
...
function decideImagePromotion(tag: string, widthThresholdPx: number): PromotionDecision {
  const page = attrValue(tag, "data-gaganfoxwell-page");
...
function decideDiagramPromotion(figure: string, widthThresholdPx: number): PromotionDecision {
  const page = attrValue(figure, "data-gaganfoxwell-page");
  if (page === "portrait") return { promote: false, reason: "page=portrait veto" };
