 ...
 *   1. $GAGANFOXWELL_PDFTOTEXT_BIN env override (preferred, matches v1.24 GAGANFOXWELL_*_BIN pattern)
 ...
 *      and guessing causes false positives. Set GAGANFOXWELL_PDFTOTEXT_BIN explicitly.
...
export function resolvePdftotext(env: NodeJS.ProcessEnv = process.env): PdftotextInfo {
  // 1 + 2: env overrides (GAGANFOXWELL_PDFTOTEXT_BIN preferred, PDFTOTEXT_BIN back-compat).
  const overrideRaw = env.GAGANFOXWELL_PDFTOTEXT_BIN ?? env.PDFTOTEXT_BIN;
  ...
  // positives. Windows users set GAGANFOXWELL_PDFTOTEXT_BIN explicitly.
  ...
  throw new PdftotextUnavailableError([
    ...
    "",
    "Or set GAGANFOXWELL_PDFTOTEXT_BIN to an explicit path:",
    process.platform === "win32"
      ? '  setx GAGANFOXWELL_PDFTOTEXT_BIN "C:\\path\\to\\pdftotext.exe"'
      : "  export GAGANFOXWELL_PDFTOTEXT_BIN=/path/to/pdftotext",
  ].join("\n"));
}
 ...
 *   1. $GAGANFOXWELL_<TOOL>_BIN env override (e.g. GAGANFOXWELL_PDFFONTS_BIN)
...
export function resolvePopplerTool(
  ...
  const override = resolveOverride(env[`GAGANFOXWELL_${tool.toUpperCase()}_BIN`], env);
