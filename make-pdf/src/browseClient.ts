 ...
 *   1. $GAGANFOXWELL_BROWSE_BIN env override (preferred, matches v1.24 GAGANFOXWELL_*_BIN pattern)
 ...
 *   4. ~/.claude/skills/gaganfoxwell/browse/dist/browse[.exe]
...
export function resolveBrowseBin(
  ...
  // 1 + 2: env overrides (GAGANFOXWELL_BROWSE_BIN preferred, BROWSE_BIN back-compat).
  const overrideRaw = env.GAGANFOXWELL_BROWSE_BIN ?? env.BROWSE_BIN;
  ...
  const globalPath = path.join(home, ".claude/skills/gaganfoxwell/browse/dist/browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "",
      "make-pdf needs browse (the gaganfoxwell Chromium daemon) to render PDFs.",
      "Tried:",
      `  - $GAGANFOXWELL_BROWSE_BIN (${env.GAGANFOXWELL_BROWSE_BIN || "unset"})`,
      `  - $BROWSE_BIN (${env.BROWSE_BIN || "unset"})`,
      `  - sibling: ${siblingCandidates.join(", ")}`,
      `  - global: ${globalPath}`,
      "  - PATH: `browse`",
      "",
      "To fix: run gaganfoxwell setup from the gaganfoxwell repo:",
      "  cd ~/.claude/skills/gaganfoxwell && ./setup",
      "",
      "Or set GAGANFOXWELL_BROWSE_BIN explicitly:",
      process.platform === "win32"
        ? '  setx GAGANFOXWELL_BROWSE_BIN "C:\\path\\to\\browse.exe"'
        : "  export GAGANFOXWELL_BROWSE_BIN=/path/to/browse",
    ].join("\n"),
  );
}
