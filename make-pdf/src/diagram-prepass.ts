    ...
    if (openFence) {
      ...
      if (close && close.char === openFence.char && close.len >= openFence.len && close.info === "") {
        ...
        if (DIAGRAM_LANGS.has(info.lang) && info.render && openFence.indent === 0) {
          ordinal++;
          const token = `gaganfoxwell-diagram-slot-${runId}-${ordinal}`;
          fences.push({
            lang: info.lang,
...
export function buildDiagramFigure(fence: DiagramFence, svg: string): string {
  ...
  const captioned = fence.title
    ...
    : "";
  const pageAttr = fence.page ? ` data-gaganfoxwell-page="${fence.page}"` : "";
  ...
  return [
    `<figure class="diagram" role="img" aria-label="${escapeHtml(label)}"${pageAttr}` +
      ` data-gaganfoxwell-lang="${escapeHtml(fence.lang)}" data-gaganfoxwell-source="${sourceB64}">`,
    cleanSvg,
    captioned,
    `</figure>`,
  ].join("\n");
...
export function decodeFigureSource(figureHtml: string): string | null {
  const m = figureHtml.match(/\bdata-gaganfoxwell-source="([A-Za-z0-9+/=]*)"/);
  if (!m) return null;
  try {
...
export class RenderTab {
  ...
  static open(): RenderTab {
    ...
    const staged = path.join(PAYLOAD_TMP_DIR, `gaganfoxwell-diagram-render-${sha}.html`);
  ...
  private jsViaFile(expression: string): string {
    const file = path.join(
      PAYLOAD_TMP_DIR,
      `gaganfoxwell-diagram-expr-${process.pid}-${crypto.randomBytes(4).toString("hex")}.js`,
    );
...
export function resolveBundlePath(env: NodeJS.ProcessEnv = process.env): string {
  const candidates = [
    env.GAGANFOXWELL_DIAGRAM_BUNDLE,
    ...
    // in the repo and in the ~/.claude/skills/gaganfoxwell global install. argv[0]
    ...
    path.join(os.homedir(), ".claude/skills/gaganfoxwell/lib/diagram-render/dist/diagram-render.html"),
 ...
 * bytes, and annotate the tag with data-gaganfoxwell-px-width/-height for the width
...
export function inlineLocalImages(html: string, opts: PrepassImageOptions): string {
  ...
  return html.replace(IMG_TAG_RE, (tag) => {
    ...
    const dataUri = `data:${mime};base64,${buf.toString("base64")}`;
    const attrs = dims
      ? ` data-gaganfoxwell-px-width="${Math.round(dims.width)}" data-gaganfoxwell-px-height="${Math.round(dims.height)}"`
      : "";
...
function annotateFromDataUri(tag: string, src: string): string {
  try {
    ...
    return tag.replace(
      /^<img\b/i,
      `<img data-gaganfoxwell-px-width="${Math.round(dims.width)}" data-gaganfoxwell-px-height="${Math.round(dims.height)}"`,
    );
  } catch {
    return tag;
  }
