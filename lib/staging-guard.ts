/**
 * staging-guard — fail-closed ownership proof for gaganfoxwell ingest staging dirs.
 *
 * Fixes #1802. The /sync-gbrain memory stage stages prepared pages to a
 * throwaway dir under ~/.gaganfoxwell and `rm -rf`s it when done. The resume path
 ...
 * proved it owned. This module is the single definition of "a path gaganfoxwell is
 ...
 *   2. Structural    — canonical path is a DIRECT child of $GAGANFOXWELL_HOME named
 ...
 *                      git working tree.
 *   4. Minted by us  — a `.gaganfoxwell-staging` marker file (written by
 ...
 * mitigation at gaganfoxwell's own rm -rf boundary; see the companion gbrain issue.
 */
...
export const STAGING_MARKER = ".gaganfoxwell-staging";
...
/**
 * Prove (fail-closed) that `dir` is a gaganfoxwell-owned ingest staging directory
 ...
 * @param gaganfoxwellHome  Resolved $GAGANFOXWELL_HOME (injected for testability).
 */
export function checkOwnedStagingDir(dir: string, gaganfoxwellHome: string): StagingVerdict {
  ...
  try {
    ...
    home = realpathSync(gaganfoxwellHome);
  } catch {
  ...
  if (dirname(canon) !== home) {
    return { ok: false, reason: `not a direct child of GAGANFOXWELL_HOME (${home})` };
  }
...
export function isOwnedStagingDir(dir: string, gaganfoxwellHome: string): boolean {
  return checkOwnedStagingDir(dir, gaganfoxwellHome).ok;
}
