/**
 * gaganfoxwell-memory-helpers — shared helpers for the V1 memory ingest + retrieval pipeline.
 *
 * Imported by:
 *   - bin/gaganfoxwell-memory-ingest.ts (Lane A)
 *   - bin/gaganfoxwell-gbrain-sync.ts   (Lane B)
 *   - bin/gaganfoxwell-brain-context-load.ts (Lane C)
 ...
 *   ED1 — engine-tier cache lives in ~/.gaganfoxwell/.gbrain-engine-cache.json (60s TTL)
 *
 ...
 *
 * Examples:
 *   https://github.com/Gagan-k0/baton-gaganfoxwel.git → github.com/Gagan-k0/baton-gaganfoxwel
 *   git@github.com:Gagan-k0/baton-gaganfoxwel.git     → github.com/Gagan-k0/baton-gaganfoxwel
...
function gitleaksAvailable(): boolean {
  ...
  try {
    ...
    process.stderr.write(
      "[gaganfoxwell-memory-helpers] gitleaks not in PATH; secret scanning disabled. " +
 ...
 * Replaces the inadequate regex scanner in bin/gaganfoxwell-brain-sync (which only
...
function gaganfoxwellHome(): string {
  return process.env.GAGANFOXWELL_HOME || join(homedir(), ".gaganfoxwell");
}

function engineCachePath(): string {
  return join(gaganfoxwellHome(), ".gbrain-engine-cache.json");
}

function errorLogPath(): string {
  return join(gaganfoxwellHome(), ".gbrain-errors.jsonl");
}

/**
 * Detect which gbrain engine is active (PGLite vs Supabase) and cache the
 * answer for 60s in ~/.gaganfoxwell/.gbrain-engine-cache.json. Caching avoids
...
export function detectEngineTier(): EngineDetect {
  ...
  try {
    ...
    writeFileSync(
      engineCachePath(),
      JSON.stringify({ ...fresh, last_writer: "gaganfoxwell-memory-helpers.detectEngineTier" }, null, 2),
      "utf-8"
    );
  } catch {
...
// Best-effort JSONL append to ~/.gaganfoxwell/.gbrain-errors.jsonl. Never throws.
 ...
 * runtime read path used by gaganfoxwell-brain-context-load; it tolerates extra
...
const ERROR_LOG_PATH = join(gaganfoxwellHome(), ".gbrain-errors.jsonl");

/**
 * Wrap an op with structured error logging. Logs success/failure + duration
 * to ~/.gaganfoxwell/.gbrain-errors.jsonl for forensic debugging. Replaces ad-hoc
