 ...
 *
 *   GAGANFOXWELL_EVAL_MODEL_<KIND>  (e.g. GAGANFOXWELL_EVAL_MODEL_WARMUP) — per-kind
 *   GAGANFOXWELL_EVAL_MODEL                                        — global
...
const DEFAULTS = {
  ...
  // GAGANFOXWELL_EVAL_MODEL_CAPTURE.
...
export function resolveEvalModel(
  ...
  const perKind = env[`GAGANFOXWELL_EVAL_MODEL_${kind.toUpperCase()}`];
  if (perKind) return perKind;
  if (env.GAGANFOXWELL_EVAL_MODEL) return env.GAGANFOXWELL_EVAL_MODEL;
