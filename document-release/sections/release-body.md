...
(adapt to whatever project you're in — these are not gaganfoxwell-specific):
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/gaganfoxwell-pr-body-$$.md
cp /tmp/gaganfoxwell-pr-body-$$.md /tmp/gaganfoxwell-pr-body-orig-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/gaganfoxwell-pr-body-$$.md
cp /tmp/gaganfoxwell-pr-body-$$.md /tmp/gaganfoxwell-pr-body-orig-$$.md
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-issue-guard --stdin --source pr-body < /tmp/gaganfoxwell-pr-body-$$.md
```
...
4. Redaction scan-at-sink, then write the updated body back. The body is already
   in a temp file (`/tmp/gaganfoxwell-pr-body-$$.md`); scan THAT file before editing so
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-redact --from-file /tmp/gaganfoxwell-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
...
if [ ! -f /tmp/gaganfoxwell-pr-body-orig-$$.md ] || [ ! -f /tmp/gaganfoxwell-pr-body-$$.md ]; then
...
_ORIG_BANNERS=$(grep -c "UNTRUSTED TRACKER CONTENT" /tmp/gaganfoxwell-pr-body-orig-$$.md 2>/dev/null)
_ORIG_BANNERS=${_ORIG_BANNERS:-0}
_NEW_BANNERS=$(grep -c "UNTRUSTED TRACKER CONTENT" /tmp/gaganfoxwell-pr-body-$$.md 2>/dev/null)
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/gaganfoxwell-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/gaganfoxwell-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
...
```bash
rm -f /tmp/gaganfoxwell-pr-body-$$.md /tmp/gaganfoxwell-pr-body-orig-$$.md
```
...
```bash
NEW_TITLE=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
