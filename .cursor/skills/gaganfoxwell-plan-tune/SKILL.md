---
name: gaganfoxwell-plan-tune
preamble-tier: 2
version: 1.0.0
description: "Self-tuning question sensitivity + developer psychographic for gaganfoxwell (v1: observational). (gaganfoxwell)"
triggers:
  - tune questions
  - stop asking me that
  - too many questions
...
Review which AskUserQuestion prompts fire across gaganfoxwell skills, set per-question preferences
...
Proactively suggest when the user says the same gaganfoxwell question has come up before,
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-tune","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"plan-tune","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-log '{"skill":"plan-tune","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
1. **Consent gate.** If `question_tuning` is `false` AND
   `~/.gaganfoxwell/.question-tuning-prompted` is missing → run `Consent + opt-in`
   below. Honor the answer with a marker write either way; do not re-prompt.
2. **Setup gate.** If `question_tuning` is `true` AND
   `~/.gaganfoxwell/developer-profile.json`'s `declared` object is empty AND
   `~/.gaganfoxwell/.declared-setup-prompted` is missing → run `5-Q setup` below.
   Touch the marker after setup completes OR is declined.
3. **Dream-cycle gate (Layer 8 / cathedral T10/T11).** If
   `~/.gaganfoxwell/projects/<slug>/distillation-proposals.json` exists AND has
...
9. **"Dream cycle" / "distill" / "what have I been free-texting"** →
   run `Dream cycle distill` below (triggers `gaganfoxwell-distill-free-text`).
10. **"Turn it off" / "disable"** → `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set question_tuning false`
11. **"Turn it on" / "enable"** → `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set question_tuning true && touch ~/.gaganfoxwell/.question-tuning-prompted`
...
`~/.gaganfoxwell/.question-tuning-prompted` is missing. The user has never been
asked.

**Privacy note.** gaganfoxwell defaults `question_tuning` to `false` for every user.
...
rationale). If the user is a contributor (`gaganfoxwell_contributor: true`), the
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get gaganfoxwell_contributor 2>/dev/null || echo "false")
...
2. AskUserQuestion (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **General framing:**
   > Question tuning is off. gaganfoxwell can learn which of its prompts you find
   > valuable vs noisy — so over time, gaganfoxwell stops asking questions you've
   > already answered the same way. It takes about 2 minutes to set up your
   > initial profile. v1 is observational: gaganfoxwell tracks your preferences
   > and shows you a profile, but doesn't silently change skill behavior yet.
   > Logs stay local (`~/.gaganfoxwell/projects/<slug>/question-log.jsonl`).
   >
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   > You're a gaganfoxwell contributor. Question tuning isn't on by default for
   ...
   > `~/.gaganfoxwell/projects/<slug>/question-log.jsonl` — nothing leaves your
...
3. ALWAYS touch the marker, regardless of choice:
   ```bash
   touch ~/.gaganfoxwell/.question-tuning-prompted
   ```

4. If A or B: enable:
   ```bash
   ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set question_tuning true
   ```

5. If C: do nothing else. Tell the user: "Question tuning stays off. Re-enable
   any time with `/plan-tune enable` or `gaganfoxwell-config set question_tuning true`."
...
- Standalone via Step 0's setup gate: `question_tuning` is already `true`
  (user opted in via gaganfoxwell-config or earlier `/plan-tune enable`) AND
  `declared` is empty AND `~/.gaganfoxwell/.declared-setup-prompted` is missing.
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   `~/.gaganfoxwell/developer-profile.json` under `declared.{dimension}`:

   ```bash
   # Ensure profile exists
   ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-developer-profile --read >/dev/null
   # Update declared dimensions atomically
   eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-paths)"
   _PROFILE="$GAGANFOXWELL_STATE_ROOT/developer-profile.json"
   bun -e "
     const fs = require('fs');
...
2. Touch the marker so the Setup gate doesn't re-fire:
   ```bash
   touch ~/.gaganfoxwell/.declared-setup-prompted
   ```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-developer-profile --profile
```
...
- Show the vibe (archetype) from `gaganfoxwell-developer-profile --vibe` — the
...
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)"
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-paths)"
_LOG="$GAGANFOXWELL_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ ! -f "$_LOG" ]; then
  echo "NO_LOG"
else
  bun -e "
...
If `NO_LOG`, tell the user: "No questions logged yet. As you use gaganfoxwell skills,
gaganfoxwell will log them here."
...
4. Write:
   ```bash
   ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-developer-profile --gap
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-preference --stats
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)"
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-paths)"
_LOG="$GAGANFOXWELL_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-distill-free-text --status
```
...
Run `gaganfoxwell-question-preference --write '{"question_id":"<id>","preference":
...
hook captured but cannot enforce against (no `<gaganfoxwell-qid:foo>` marker in
...
1. Show the proposals:
   ```bash
   ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-distill-apply --list
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-distill-apply --proposal N
   ```
...
4. **On decline**: skip without marking. User can re-decide later (the
   ...
   `gaganfoxwell-distill-apply --proposal N --dismiss` (not implemented in T11;
...
1. Run distill:
   ```bash
   ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-distill-free-text
   ```
...
3. If `NO_FREE_TEXT`: tell the user "No free-text answers since the last
   distill. Keep using gaganfoxwell — `Other` responses on AskUserQuestion feed
   this loop."
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-distill-free-text --background
```
