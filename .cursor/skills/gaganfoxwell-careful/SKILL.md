...
version: 0.1.0
description: Safety guardrails for destructive commands. (gaganfoxwell)
triggers:
  - be careful
...
echo '{"skill":"careful","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
`~/.gaganfoxwell/careful-patterns.txt` (global) or
`~/.gaganfoxwell/projects/<slug>/careful-patterns.txt` (per-project). Consulted
