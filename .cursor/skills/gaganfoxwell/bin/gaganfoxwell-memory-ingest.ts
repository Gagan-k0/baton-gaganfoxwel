#!/usr/bin/env bun
/**
 * gaganfoxwell-memory-ingest — V1 memory ingest helper.
 *
 * Walks coding-agent transcript sources + ~/.gaganfoxwell/ curated artifacts and writes
 ...
 *
 * Usage:
 *   gaganfoxwell-memory-ingest --probe                 # count what would ingest, no writes
 *   gaganfoxwell-memory-ingest --incremental [--quiet] # default; mtime fast-path; cheap
 *   gaganfoxwell-memory-ingest --bulk [--all-history]  # first-run; full walk
 *   gaganfoxwell-memory-ingest --bulk --benchmark      # time the bulk pass + report
 *   gaganfoxwell-memory-ingest --include-unattributed  # also ingest sessions with no git remote
 *
 * Sources walked:
 ...
 *   ~/.gaganfoxwell/projects/<slug>/learnings.jsonl       — typed: learning
 *   ~/.gaganfoxwell/projects/<slug>/timeline.jsonl        — typed: timeline
 *   ~/.gaganfoxwell/projects/<slug>/ceo-plans/*.md        — typed: ceo-plan
 *   ~/.gaganfoxwell/projects/<slug>/*-design-*.md         — typed: design-doc
 *   ~/.gaganfoxwell/analytics/eureka.jsonl                — typed: eureka
 *   ~/.gaganfoxwell/builder-profile.jsonl                 — typed: builder-profile-entry
 *
 * State: ~/.gaganfoxwell/.transcript-ingest-state.json (LOCAL per ED1, never synced).
 * Secret scanning: gitleaks via lib/gaganfoxwell-memory-helpers#secretScanFile (D19).
...
import {
  ...
  detectEngineTier,
  withErrorContext,
} from "../lib/gaganfoxwell-memory-helpers";
...
interface CliArgs {
   ...
   * default — the cross-machine boundary (gaganfoxwell-brain-sync, git push)
...
interface BulkResult {
   ...
   * ~/.gaganfoxwell/gbrain-repo-policy.json is `read-only` (search allowed, page
...
const HOME = homedir();
const GAGANFOXWELL_HOME = process.env.GAGANFOXWELL_HOME || join(HOME, ".gaganfoxwell");
const STATE_PATH = join(GAGANFOXWELL_HOME, ".transcript-ingest-state.json");
...
function printUsage(): void {
  console.error(`Usage: gaganfoxwell-memory-ingest [--probe|--incremental|--bulk] [options]
...
Options:
  ...
  --scan-secrets       Opt-in per-file gitleaks scan during prepare. Off by
                       default; gaganfoxwell-brain-sync already gates the git-push
...
function parseArgs(): CliArgs {
  ...
  let noWrite = process.env.GAGANFOXWELL_MEMORY_INGEST_NO_WRITE === "1";
  let scanSecrets = process.env.GAGANFOXWELL_MEMORY_INGEST_SCAN_SECRETS === "1";
...
function loadState(): IngestState {
  if (!existsSync(STATE_PATH)) {
    return {
      schema_version: 1,
      last_writer: "gaganfoxwell-memory-ingest",
      sessions: {},
    };
  }
  try {
    ...
    if (parsed.schema_version !== 1) {
      ...
      return { schema_version: 1, last_writer: "gaganfoxwell-memory-ingest", sessions: {} };
    }
    return parsed;
  } catch (err) {
...
function saveState(state: IngestState): void {
  ...
  // in gaganfoxwell-gbrain-sync.ts:saveSyncState.
  try {
...
function* walkGaganfoxwellArtifacts(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const projectsRoot = join(GAGANFOXWELL_HOME, "projects");

  // Eureka log: ~/.gaganfoxwell/analytics/eureka.jsonl
  const eurekaLog = join(GAGANFOXWELL_HOME, "analytics", "eureka.jsonl");
  if (existsSync(eurekaLog) && ctx.args.sources.has("eureka")) {
    yield { path: eurekaLog, type: "eureka" };
  }

  // Builder profile: ~/.gaganfoxwell/builder-profile.jsonl
  const builderProfile = join(GAGANFOXWELL_HOME, "builder-profile.jsonl");
...
function* walkAllSources(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  ...
  yield* walkGaganfoxwellArtifacts(ctx);
}
...
function buildArtifactPage(path: string, type: MemoryType): PageRecord {
  ...
  // Extract repo slug from path: ~/.gaganfoxwell/projects/<slug>/...
  let slug_repo = "_unattributed";
  const m = path.match(/\/\.gaganfoxwell\/projects\/([^/]+)\//);
  if (m) slug_repo = m[1];
 ...
 *
 *   The actual cross-machine exfiltration boundary is `gaganfoxwell-brain-sync`,
 *   which runs a regex-based secret scanner on the staged diff before
 *   `git commit` (see bin/gaganfoxwell-brain-sync:78-110: AWS keys, GitHub
...
function preparePages(
  ...
  for (const { path, type } of walkAllSources(ctx)) {
    ...
    // because gaganfoxwell-brain-sync already gates the cross-machine boundary
  ...
  // code-import gate honors (bin/gaganfoxwell-gbrain-sync.ts). One batch spawn for
  ...
  if (policyStoreExists) {
    ...
    if (remotes.length > 0) {
      ...
      // gaganfoxwell-gbrain-sync's code-import gate: never bypass a set policy.
      ...
      if (broken) {
        ...
        policyError =
          ...
          `bypassing a possibly-set deny policy. Inspect with: gaganfoxwell-gbrain-repo-policy list; ` +
...
/**
 * Make a per-run staging directory at ~/.gaganfoxwell/.staging-ingest-<pid>-<ts>/
...
function makeStagingDir(): string {
  const dir = join(GAGANFOXWELL_HOME, `.staging-ingest-${process.pid}-${Date.now()}`);
 ...
 *
 * Instead of staging to ~/.gaganfoxwell/.staging-ingest-<pid>-<ts>/ and cleaning up
 * after `gbrain import`, remote-http users get a stable path that survives.
 * gaganfoxwell-brain-sync's allowlist pushes ~/.gaganfoxwell/transcripts/** to the
 * artifacts repo; the brain admin's pull job indexes them into the remote
 * brain. Local PGLite (if present) stays code-only.
 *
 * Path: ~/.gaganfoxwell/transcripts/<run-id>/  (run-id pid+ts so concurrent passes
 ...
 *
 * Reads ~/.claude.json directly (same fallback chain as gaganfoxwell-gbrain-detect
...
function cleanupStagingDir(dir: string): void {
  ...
  const verdict = checkOwnedStagingDir(dir, GAGANFOXWELL_HOME);
  if (!verdict.ok) {
    console.error(
  ...
  const forward = (signal: NodeJS.Signals) => () => {
    ...
    if (_activeStagingDir) {
      if (stagingDirIsCheckpointed(_activeStagingDir)) {
        ...
        // (in gaganfoxwell-gbrain-sync.ts) will see the checkpoint + dir and
 ...
 * configurable via GAGANFOXWELL_INGEST_TIMEOUT_MS (default 30 min, 1min–24h) so large
...
export function resolveImportTimeoutMs(
  raw: string | undefined = process.env.GAGANFOXWELL_INGEST_TIMEOUT_MS,
): number {
  if (raw === undefined || raw === "") return DEFAULT_IMPORT_TIMEOUT_MS;
  const n = Number.parseInt(raw, 10);
  if (!Number.isFinite(n) || Number.isNaN(n) || n < 60_000 || n > 86_400_000) {
    console.error(
      `[memory-ingest] GAGANFOXWELL_INGEST_TIMEOUT_MS="${raw}" invalid (need 60000–86400000ms); using ${DEFAULT_IMPORT_TIMEOUT_MS}ms`,
    );
  ...
  if (failedOnUnknownIncludeGitignored(first.status, first.stderr)) {
    ...
    console.error(
      "[memory-ingest] installed gbrain does not support --include-gitignored — " +
        ...
        "(gaganfoxwell-gbrain-install) so staged pages inside gitignored dirs are visible.",
    );
...
function runGbrainImportOnce(
  ...
  return new Promise((resolve) => {
    ...
    // parent gaganfoxwell-gbrain-sync seeding the bun grandchild's env).
    // --include-gitignored is load-bearing, not a convenience. Pages are
    // staged into ~/.gaganfoxwell/.staging-ingest-<pid>-<ts>/, and ~/.gaganfoxwell is a
    ...
    // through a symlink (macOS /var -> /private/var, symlinked $GAGANFOXWELL_HOME)
...
async function ingestPass(args: CliArgs): Promise<BulkResult> {
  ...
  if (args.noWrite) {
    ...
    state.last_writer = "gaganfoxwell-memory-ingest";
    saveState(state);
    return {
      written,
      skipped_secret: prep.skippedSecret,
  ...
  // PERSISTENT dir under ~/.gaganfoxwell/transcripts/ and SKIP `gbrain import`
  // entirely. gaganfoxwell-brain-sync push will pick the dir up via its allowlist
  ...
  // GAGANFOXWELL_INGEST_RESUME_DIR (because gbrain's import-checkpoint.json points
  ...
  const resumeDir = process.env.GAGANFOXWELL_INGEST_RESUME_DIR;
  // #1802 second entry point: this binary is runnable directly, so it must not
  // trust GAGANFOXWELL_INGEST_RESUME_DIR just because it exists — a stale/poisoned env
  ...
  const resuming = !remoteHttpMode
    ...
    && checkOwnedStagingDir(resumeDir, GAGANFOXWELL_HOME).ok;
  if (!remoteHttpMode && resumeDir && resumeDir.length > 0 && !resuming) {
    console.error(
      `[memory-ingest] ignoring GAGANFOXWELL_INGEST_RESUME_DIR="${resumeDir}" — not a proven staging dir (#1802); staging fresh.`,
    );
  }
  ...
  try {
    ...
    // staged markdown lives under ~/.gaganfoxwell/transcripts/<run-id>/ and the
    // next gaganfoxwell-brain-sync push will move it to the artifacts repo. From
    ...
    if (remoteHttpMode) {
      ...
      state.last_writer = "gaganfoxwell-memory-ingest (remote-http mode)";
      saveState(state);
      if (!args.quiet) {
        console.error(
    ...
    // happens in gaganfoxwell-brain-sync, which writes its own receipt at the push.
    try {
      writeReceipt({
        sink: "memory-ingest",
      ...
      if (importResult.timedOut) {
        ...
        const msg = checkpointed
          ? `gbrain import timed out after ${mins}min; checkpoint preserved — re-run ` +
            `/sync-gbrain to resume (raise GAGANFOXWELL_INGEST_TIMEOUT_MS for big brains)`
          : `gbrain import timed out after ${mins}min before writing a checkpoint; ` +
            `re-run /sync-gbrain to restage (raise GAGANFOXWELL_INGEST_TIMEOUT_MS for big brains)`;
    ...
    // `gaganfoxwell-artifacts-init` writes `.gitignore = "*"` into $GAGANFOXWELL_HOME.
    // makeStagingDir() stages under $GAGANFOXWELL_HOME, so on any machine that has
    ...
    // dir (makePersistentTranscriptDir, under ~/.gaganfoxwell/transcripts/) that
    // gaganfoxwell-brain-sync push must pick up — it is NOT a `.staging-ingest-*` dir
...
function printProbeReport(r: ProbeReport, json: boolean): void {
  ...
  if (r.skipped_policy_deny > 0) {
    console.log(`Skipped (policy deny):  ${r.skipped_policy_deny}  (remote tier is deny; change with: gaganfoxwell-gbrain-repo-policy set <remote> read-write)`);
  }
...
function printBulkResult(r: BulkResult, args: CliArgs): void {
  ...
  if (r.skipped_policy_deny > 0) {
    console.log(`  skipped (policy deny):      ${r.skipped_policy_deny}  (change with: gaganfoxwell-gbrain-repo-policy set <remote> read-write)`);
  }
...
// The orchestrator runs it as `bun gaganfoxwell-memory-ingest.ts ...`, where
// import.meta.main is true, so the CLI path is unaffected.
if (import.meta.main) {
  main().catch((err) => {
    console.error(`gaganfoxwell-memory-ingest fatal: ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  });
}
