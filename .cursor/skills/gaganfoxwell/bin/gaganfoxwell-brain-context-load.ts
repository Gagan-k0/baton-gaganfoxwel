#!/usr/bin/env bun
/**
 * gaganfoxwell-brain-context-load — V1 retrieval surface (Lane C).
 *
 * Called from the gaganfoxwell preamble at every skill start. Reads the active skill's
 ...
 *
 * Usage:
 *   gaganfoxwell-brain-context-load --skill office-hours --repo Gagan-k0-gaganfoxwell
 *   gaganfoxwell-brain-context-load --skill-file ./SKILL.md --repo X --user Y
 *   gaganfoxwell-brain-context-load --window 14d --explain
 *   gaganfoxwell-brain-context-load --quiet
 */
...
import { parseSkillManifest, type GbrainManifest, type GbrainManifestQuery, withErrorContext } from "../lib/gaganfoxwell-memory-helpers";
...
const MCP_TIMEOUT_MS = Math.max(1, parseInt(process.env.GAGANFOXWELL_BRAIN_TIMEOUT_MS || "", 10) || 500);
const PAGE_SIZE_CAP = 10 * 1024; // 10KB per query result before truncation

// ── CLI ────────────────────────────────────────────────────────────────────

function printUsage(): void {
  console.error(`Usage: gaganfoxwell-brain-context-load [options]
...
function resolveSkillFile(args: CliArgs): string | null {
  ...
  // Look in common gaganfoxwell skill locations
  const candidates = [
    join(HOME, ".claude", "skills", args.skill, "SKILL.md"),
    join(HOME, ".claude", "skills", "gaganfoxwell", args.skill, "SKILL.md"),
...
async function loadContext(args: CliArgs): Promise<{ rendered: string; results: QueryResult[]; mode: "manifest" | "default" }> {
  ...
  for (const q of manifest.context_queries) {
    ...
    }, "gaganfoxwell-brain-context-load");
    results.push(r);
  }
...
main().catch((err) => {
  console.error(`gaganfoxwell-brain-context-load fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
