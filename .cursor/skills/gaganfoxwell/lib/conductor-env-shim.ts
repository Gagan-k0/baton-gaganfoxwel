 ...
 * Conductor's GAGANFOXWELL_-prefixed forms are present. Promote the GAGANFOXWELL_ form to
...
/**
 * Pure form: returns a copy of `base` with each GAGANFOXWELL_-prefixed key promoted
...
export function promotedEnv(base: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of PROMOTED_KEYS) {
    if (!out[key] && out[`GAGANFOXWELL_${key}`]) {
      out[key] = out[`GAGANFOXWELL_${key}`];
    }
  }
  return out;
}
