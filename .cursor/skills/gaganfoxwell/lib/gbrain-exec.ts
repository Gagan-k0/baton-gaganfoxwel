 ...
 *
 * Every `gbrain ...` spawn from `bin/gaganfoxwell-gbrain-sync.ts` and
 * `bin/gaganfoxwell-memory-ingest.ts` MUST go through `spawnGbrain` (or
 ...
 *      → /tmp/x/.gbrain/config.json). Every gaganfoxwell-side read goes through
 *      `gbrainConfigDir()` below so gaganfoxwell and gbrain agree on which
 *      config file matters.
 *
 * **Escape hatch:** `GAGANFOXWELL_RESPECT_ENV_DATABASE_URL=1` returns the
 ...
 * Unset → ~/.gbrain. Every gaganfoxwell-side gbrain-config read MUST resolve
 * through this helper, or gaganfoxwell classifies engine status from a file
 * gbrain never reads.
 */
 ...
 * unchanged when:
 *   - `GAGANFOXWELL_RESPECT_ENV_DATABASE_URL=1` (intentional opt-out),
...
export function buildGbrainEnv(opts: BuildGbrainEnvOptions = {}): NodeJS.ProcessEnv {
  ...
  if (baseEnv.GAGANFOXWELL_RESPECT_ENV_DATABASE_URL === "1") return out;
 ...
 * `gbrain.cmd`/`.ps1` shim) or a shebang script like the bash `gaganfoxwell-brain-sync`
...
/**
 * How to invoke a **bash shebang script** (`gaganfoxwell-brain-sync`) on this platform.
 *
 ...
 * `~/.gaganfoxwell` and were never committed, while `/sync-gbrain` printed one red
 ...
 * repo. `GAGANFOXWELL_BASH` overrides everything for unusual installs.
 *
...
export function bashScriptInvocation(
  ...
  const override = env.GAGANFOXWELL_BASH?.trim();
 ...
 * listeners (e.g., `gbrain import` in `gaganfoxwell-memory-ingest.ts`). Always
