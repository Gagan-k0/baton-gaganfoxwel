 ...
 * trust store (bin/gaganfoxwell-gbrain-repo-policy, a bash CLI that owns URL
 ...
 * veto; bin/gaganfoxwell-gbrain-sync.ts code-import gate) each spawnSync'd the script
...
/** Absolute path of the policy store for this env (GAGANFOXWELL_HOME-aware). */
export function repoPolicyStorePath(env: NodeJS.ProcessEnv = process.env): string {
  const home = env.GAGANFOXWELL_HOME || join(env.HOME || homedir(), ".gaganfoxwell");
...
const POLICY_SCRIPT = join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-repo-policy");

/**
 * Trust tier for a remote URL, via `gaganfoxwell-gbrain-repo-policy get <url>`.
 *
...
export function repoPolicyTier(url: string | null, env: NodeJS.ProcessEnv = process.env): RepoPolicyResult {
  ...
  const res = spawnSync(cmd, args, {
    ...
    // process.env mutations (e.g. tests redirecting GAGANFOXWELL_HOME).
...
/**
 * Bulk trust-tier lookup via `gaganfoxwell-gbrain-repo-policy get --batch` — ONE
