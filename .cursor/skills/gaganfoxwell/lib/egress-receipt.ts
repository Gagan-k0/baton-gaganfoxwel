 ...
 * gaganfoxwell-initiated off-machine send (`~/.gaganfoxwell/security/egress.jsonl`, 0600).
 *
...
export interface WriteReceiptOptions {
  /** gaganfoxwell home; resolved from env when omitted */
  home?: string;
  /** env for home resolution (tests) */
  env?: Env;
  /** which gaganfoxwell component is sending */
  sink: string;
...
/**
 * Same resolution order as the rest of gaganfoxwell (shell sinks, selection code):
 * GAGANFOXWELL_HOME, legacy GAGANFOXWELL_STATE_DIR, then $HOME/.gaganfoxwell.
 */
export function resolveEgressHome(env: Env = process.env): string {
  const configured = env.GAGANFOXWELL_HOME || env.GAGANFOXWELL_STATE_DIR;
  if (configured) return path.resolve(configured);
  return path.join(env.HOME || os.homedir(), '.gaganfoxwell');
}
 ...
 * gaganfoxwell ATTEMPTS to send off-machine), how to inspect it, and that
 * trimming arrives with rotation.
 */
export function ledgerSizeWarning(ledger: string, size: number): string {
  const mb = (size / (1024 * 1024)).toFixed(1);
  return (
    `gaganfoxwell: egress ledger is large (${mb}MB): ${ledger}. ` +
    `This file records what gaganfoxwell ATTEMPTS to send off-machine (content-free receipts, for auditing). ` +
    `Inspect it with 'gaganfoxwell-egress list'. Trimming arrives with ledger rotation (TODO); until then it only grows.`
  );
}
