/**
 * bin-context — tiny shared helpers for non-interactive gaganfoxwell bins that need the
 * project slug, current branch, and argv flags. Extracted from the decision bins
 * (gaganfoxwell-decision-log / gaganfoxwell-decision-search) so the slug/branch/flag plumbing
...
/** Keep the slug inside the [a-zA-Z0-9._-] alphabet gaganfoxwell-slug promises (`tr -cd`). */
 ...
 * `C:\Users\j\foo` → `/c/Users/j/foo`. gaganfoxwell-slug keys its cache on THAT form
 * (`tr '/' '_'`), so a native lookup must reproduce it exactly or it misses the very
 * entry gaganfoxwell-slug wrote and silently re-derives instead of staying consistent.
...
// Marker tiers mirror bin/gaganfoxwell-slug's `_outermost_project_root` exactly.
...
/**
 * Native port of bin/gaganfoxwell-slug's `_outermost_project_root`: walk UP
...
/**
 * Native port of bin/gaganfoxwell-slug's `_outermost_remote_repo` (step 1a): walk UP
...
/**
 * Native port of bin/gaganfoxwell-slug's resolution order, used when that script cannot be
 ...
 * test/bin-context-windows-slug.test.ts against test/gaganfoxwell-slug-cwd-walk-up.test.ts
 * and test/gaganfoxwell-slug-parity.test.ts):
 *   0. $GAGANFOXWELL_PROJECT_SLUG env override — wins over everything, never cached.
 ...
 *      like `url = ..` must never escape ~/.gaganfoxwell/projects/<slug>.
 *   4. Project root's basename; else basename(cwd) for plain non-project folders.
 */
export function slugFromEnvironment(gaganfoxwellHome?: string, cwd: string = process.cwd()): string {
  const home = gaganfoxwellHome || process.env.GAGANFOXWELL_HOME || join(homedir(), ".gaganfoxwell");
  ...
  const envSlug = sanitizeSlug((process.env.GAGANFOXWELL_PROJECT_SLUG || "").trim());
  if (envSlug) return envSlug;

  // 1. outermost project root along the cwd ancestor chain (may be "").
  const projectRoot = outermostProjectRoot(cwd);

  // Lazy, memoized remote discovery (mirrors gaganfoxwell-slug's _resolve_remote):
  ...
  // 3. canonical remote-derived slug from the outermost remote-bearing repo.
  //    Parse mirrors bin/gaganfoxwell-slug step 2 exactly (byte-parity with
  ...
  if (!slug) {
    ...
    if (url) {
      ...
      // Dot-only / degenerate guard (mirrors bin/gaganfoxwell-slug): a hostile
      // origin like `url = ..` yields "." or ".." here, which would file
      // state OUTSIDE ~/.gaganfoxwell/projects/. Reject and let the basename
  ...
  // 5. cache it, as gaganfoxwell-slug does — atomic, self-healing (only rewrites when
 ...
 *  PATHEXT match for an explicit path), so gaganfoxwell-slug spawns ENOENT there. */
export const NEEDS_NATIVE_SLUG_ON_WINDOWS = process.platform === "win32";

/**
 * Resolve the project slug via the `gaganfoxwell-slug` helper (parses `SLUG=...`).
 *
 ...
 * ~/.gaganfoxwell/projects/unknown/ — one bucket shared by every project on the machine —
