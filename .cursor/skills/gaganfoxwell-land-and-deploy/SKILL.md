---
name: gaganfoxwell-land-and-deploy
preamble-tier: 4
version: 1.0.0
description: Land and deploy workflow. (gaganfoxwell)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-log '{"skill":"land-and-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)"
if [ ! -f ~/.gaganfoxwell/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat ~/.gaganfoxwell/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-read 2>/dev/null
```
...
```bash
mkdir -p ~/.gaganfoxwell/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.gaganfoxwell/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
```bash
cat ~/.claude/skills/gaganfoxwell/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-evidence check --label tests --expect-cmd '<the project test command>' --max-age 24 --allow-paths CHANGELOG.md,VERSION,package.json
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-evidence run --label tests -- 'bun test 2>&1'
```
...
ls -t ~/.gaganfoxwell-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.gaganfoxwell-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-issue-guard pr-body
```
...
Then run `gaganfoxwell-diff-scope` to classify the changes:

```bash
eval $(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".gaganfoxwell/deploy-reports/post-deploy.png"
```
...
```bash
mkdir -p .gaganfoxwell/deploy-reports
```
...
Save report to `.gaganfoxwell/deploy-reports/{date}-pr{number}-deploy.md`.
