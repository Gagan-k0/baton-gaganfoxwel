---
name: gaganfoxwell-setup-gbrain
preamble-tier: 2
version: 1.0.0
description: "Set up gbrain for this coding agent: install the CLI, initialize a local PGLite or Supabase brain, register MCP, capture per-remote trust policy. (gaganfoxwell)"
triggers:
  - setup gbrain
  - install gbrain
  - connect gbrain
...
```bash
_UPD=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-update-check 2>/dev/null || .claude/skills/gaganfoxwell/bin/gaganfoxwell-update-check 2>/dev/null || true)
...
_PROACTIVE=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.gaganfoxwell/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_SESSION_KIND=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-session-kind 2>/dev/null || echo "interactive")
...
if [ "$_ACTIVATED" = "no" ] && [ "$_SESSION_KIND" != "headless" ]; then
  _FIRST_TASK=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-first-task-detect 2>/dev/null || true)
fi
echo "FIRST_TASK: $_FIRST_TASK"
_LAKE_SEEN=$([ -f ~/.gaganfoxwell/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.gaganfoxwell/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get question_tuning 2>/dev/null || echo "false")
echo "QUESTION_TUNING: $_QUESTION_TUNING"
_UPDATE_CHECK=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get update_check 2>/dev/null || echo "true")
echo "UPDATE_CHECK: $_UPDATE_CHECK"
mkdir -p ~/.gaganfoxwell/analytics
if [ "$_TEL" != "off" ]; then
echo '{"skill":"setup-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
fi
for _PF in $(find ~/.gaganfoxwell/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "$HOME/.claude/skills/gaganfoxwell/bin/gaganfoxwell-telemetry-log" ]; then
      ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${GAGANFOXWELL_HOME:-$HOME/.gaganfoxwell}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
_ROUTING_DECLINED=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
_VENDORED="no"
if [ -d ".claude/skills/gaganfoxwell" ] && [ ! -L ".claude/skills/gaganfoxwell" ]; then
  if [ -f ".claude/skills/gaganfoxwell/VERSION" ] || [ -d ".claude/skills/gaganfoxwell/.git" ]; then
    _VENDORED="yes"
  fi
fi
echo "VENDORED_GAGANFOXWELL: $_VENDORED"
echo "MODEL_OVERLAY: claude"
_CHECKPOINT_MODE=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get checkpoint_push 2>/dev/null || echo "false")
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gaganfoxwell-*` names. Disk paths stay `~/.claude/skills/gaganfoxwell/[skill-name]/SKILL.md`.
...
If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.claude/skills/gaganfoxwell/gaganfoxwell-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).
...
- Missing `~/.claude/skills/gaganfoxwell/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set checkpoint_mode continuous`. Always touch marker.
- Missing `~/.claude/skills/gaganfoxwell/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set explain_level terse`.
...
If A: run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set telemetry community`
...
If B→A: run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set telemetry anonymous`
If B→B: run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set telemetry off`
...
If A: run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set proactive true`
If B: run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set proactive false`
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-telemetry-log --event-type first_task_scaffold_shown --skill "TASK_TOKEN" --outcome shown 2>/dev/null || true
...
If B: run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set routing_declined true` and say they can re-enable with `gaganfoxwell-config set routing_declined false`.
...
> This project has gaganfoxwell vendored in `.claude/skills/gaganfoxwell/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .claude/skills/gaganfoxwell/`
2. Run `echo '.claude/skills/gaganfoxwell/' >> .gitignore`
3. Run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate gaganfoxwell from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd ~/.claude/skills/gaganfoxwell && ./setup --team`"
...
_BRAIN_SYNC_BIN="$HOME/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-sync"
_BRAIN_CONFIG_BIN="$HOME/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config"
...
```bash
"$HOME/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-sync" --discover-new 2>/dev/null || true
"$HOME/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-sync" --once 2>/dev/null || true
```
...
```bash
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)"
...
if [ -d "$_PROJ" ]; then
  ...
  if [ -f "$_PROJ/decisions.active.json" ]; then
    ...
    ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-decision-search --recent 5 2>/dev/null
    echo "--- END DECISIONS ---"
  fi
  echo "--- END ARTIFACTS ---"
fi
...
Curated jargon list lives at `~/.claude/skills/gaganfoxwell/scripts/jargon-list.json` (80+ terms). On the first jargon term you encounter this session, Read that file once; treat the `terms` array as the canonical list. The list is repo-owned and may grow between releases.
...
Before each AskUserQuestion, choose `question_id` from `~/.claude/skills/gaganfoxwell/scripts/question-registry.ts` or `{skill}-{slug}`, then run `printf '%s' "<question summary>" | ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-preference --check "<id>" --summary-stdin` (piped summary feeds the one-way keyword net, #2024). `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-telemetry-log ]; then
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-telemetry-log \
...
You are setting up gbrain (https://github.com/Gagan-k0/gbrain), a persistent
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-detect
```
...
`gaganfoxwell_brain_sync_mode`, `gaganfoxwell_brain_git`, `gaganfoxwell_artifacts_remote`, and
...
>   ❌ Destructive — existing config moved to ~/.gbrain/config.json.gaganfoxwell-bak-{ts}
...
**If A (Retry)**: re-run `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-detect`
with `GAGANFOXWELL_DETECT_NO_CACHE=1` (busts the 60s cache). If the new
...
```bash
BACKUP="$HOME/.gbrain/config.json.gaganfoxwell-bak-$(date +%s)"
mv "$HOME/.gbrain/config.json" "$BACKUP"
# gaganfoxwell default: voyage-code-3 (1024d) when VOYAGE_API_KEY is set — best for
...
```bash
[ -f ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-code-intelligence ] \
  && bun ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-code-intelligence select gbrain 2>/dev/null \
  || true
```
...
- `"offer": false` with reason `bin-absent` → the installed gaganfoxwell predates
...
- `"offer": true` → present the returned options ONCE via AskUserQuestion:
  **GBrain** (recommended — semantic memory + code, sends repo content to
  ...
  the choice: `gaganfoxwell-code-intelligence select <provider|none>` — `none`
  persists the decline so NO skill ever asks again, on any repo
  (re-enable: `gaganfoxwell-code-intelligence select <provider>`). Local-compute
  and remote-send providers are separate consents — never bundle them.
- Per-repo send consent (GBrain/Sourcebot) is recorded with
  `gaganfoxwell-code-intelligence consent <repo> yes|no` and is ALWAYS vetoed by a
  `deny` tier in gaganfoxwell-gbrain-repo-policy — the trust store is the single
  authority for whether code leaves a repo.

If the user picked GBrain (or asked for this skill directly), continue below.
If they picked Sourcebot/Graphify, run `gaganfoxwell-code-intelligence index <repo>`
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-install
```
...
```bash
. ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-supabase-provision list-orgs --json)
```
...
```bash
trap 'echo ""; echo "gaganfoxwell-gbrain: interrupted. In-flight ref: $INFLIGHT_REF"; \
...
```bash
result=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-supabase-provision \
...
```bash
# gaganfoxwell default: voyage-code-3 (1024d) when VOYAGE_API_KEY is set — code
...
**4c. Verify via gaganfoxwell-gbrain-mcp-verify.** Run the helper; capture the
classified JSON output:

```bash
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-mcp-verify "$MCP_URL")
...
- `URL_FORM_SUPPORTED` (`true|false`) — passed to `gaganfoxwell-artifacts-init` in
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-install || exit $?
...
# gaganfoxwell default for local code-search PGLite: voyage-code-3 (1024d) when
...
When A was picked, Step 3 already ran (via gaganfoxwell-gbrain-install) and Step 4
...
("Migration didn't complete in 3 minutes — another gaganfoxwell session may be
...
```bash
current_tier=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
Separate AskUserQuestion: "Also sync your gaganfoxwell artifacts (CEO plans,
...
`gaganfoxwell-artifacts-$USER` (private), and writes the canonical HTTPS URL to
`~/.gaganfoxwell-artifacts-remote.txt`. Pass `--url-form-supported` from Step 4c's
verify output (Path 4) or `false` (Paths 1/2/3 — local mode doesn't probe):

```bash
URL_FORM=${URL_FORM_SUPPORTED:-false}
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-artifacts-init --url-form-supported "$URL_FORM"
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set artifacts_sync_mode artifacts-only
# or "full" if user picked yes-full
```

`gaganfoxwell-artifacts-init` always prints a "Send this to your brain admin" block
...
In remote mode, the local `gaganfoxwell-gbrain-source-wireup` helper does NOT run
...
any gbrain client. The helper creates a `git worktree` of `~/.gaganfoxwell/`,
...
try:
    ...
    pass
")
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-source-wireup --strict \
  ${GBRAIN_URL:+--database-url "$GBRAIN_URL"}
```

`--strict` exits non-zero on missing prereqs (gbrain not installed, < 0.18.0,
or no `~/.gaganfoxwell/.git` yet) so the user sees the failure rather than silently
...
this machine's transcripts indexed, they pull from your `gaganfoxwell-artifacts-$USER`
repo (set up in Step 7) on whatever schedule they prefer. Set
`gaganfoxwell-config set transcript_ingest_mode off` and continue to Step 8.
...
curated `~/.gaganfoxwell/` artifacts into gbrain so the retrieval surface
(per-skill manifests, salience block) has data to surface.

Run the probe to size the operation:
```bash
bun run ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-memory-ingest.ts --probe
```

Read the output. If `Total files in window: 0`, skip — there's nothing
to ingest. Set `gaganfoxwell-config set transcript_ingest_mode incremental`
silently and continue to Step 8.

If `New (never ingested)` is < 200 AND total bytes are < 100MB: silent
bulk via `bun run ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-memory-ingest.ts --bulk --quiet`. Set
...
>
> What you get after this: every gaganfoxwell skill auto-loads recent salience
...
> it in prior commits. Use `gaganfoxwell-transcript-prune` to delete in bulk;
...
After answer:
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set transcript_ingest_mode <choice>
bun run ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-sync.ts --full --no-brain-sync
```
...
hook runs `bun run ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-sync.ts --incremental --quiet` on every skill
...
- Artifacts repo: {gaganfoxwell_artifacts_remote URL or "none"}
...
<!-- gaganfoxwell-gbrain-search-guidance:start -->

GBrain is set up and synced on this machine. The agent should prefer gbrain
over Grep when the question is semantic or when you don't know the exact
identifier yet. Two indexed corpora available via the `gbrain` CLI:
- This repo's code (registered as `gaganfoxwell-code-<repo>` source).
- `~/.gaganfoxwell/` curated memory (registered as `gaganfoxwell-brain-<user>` source via
...
- "What did we decide last time?" / past plans, retros, learnings:
    `gbrain search "<terms>" --source gaganfoxwell-brain-<user>`

Grep is still right for known exact strings, regex, multiline patterns, and
file globs. The brain auto-syncs incrementally on every gaganfoxwell skill start.
Run `/sync-gbrain` to force-refresh, `/sync-gbrain --full` for full reindex.

<!-- gaganfoxwell-gbrain-search-guidance:end -->
```
...
The brain trust policy controls whether gaganfoxwell auto-pushes `~/.gaganfoxwell/`
...
```bash
_HASH=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set brain_trust_policy@$_HASH personal
...
> shared/team brain?
>
> Personal: gaganfoxwell auto-pushes ~/.gaganfoxwell/ artifacts (CEO plans, design
...
>
> Shared/team: read-only by default. gaganfoxwell reads context but prompts
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set artifacts_sync_mode full
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-detect 2>/dev/null || true
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get artifacts_sync_mode 2>/dev/null || echo "off"
[ -f ~/.gaganfoxwell/.gbrain-sync-state.json ] && cat ~/.gaganfoxwell/.gbrain-sync-state.json || echo "{}"
```
...
gbrain status: GREEN  (mode: remote-http)
  ...
  Artifacts repo .. OK   {gaganfoxwell_artifacts_remote URL}
...
- If user picked B (No): `N/A declined at Step 4d` — `gaganfoxwell-config set local_code_index_offered true` to silence future migration notices.

The **Transcripts** row changed in v1.34.0.0: in remote-http mode,
gaganfoxwell-memory-ingest now persists staged transcripts to
`~/.gaganfoxwell/transcripts/run-<pid>-<ts>/` and gaganfoxwell-brain-sync pushes them
...
- **Concurrent-run lock.** At skill start, `mkdir ~/.gaganfoxwell/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `/setup-gbrain` instance
  is running. Wait for it, or `rm -rf ~/.gaganfoxwell/.setup-gbrain.lock.d` if
