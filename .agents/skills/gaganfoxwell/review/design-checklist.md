...
**Trigger:** Only run this checklist if the diff touches frontend files. Use `gaganfoxwell-diff-scope` to detect:

```bash
source <(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-diff-scope <base> 2>/dev/null)
```
