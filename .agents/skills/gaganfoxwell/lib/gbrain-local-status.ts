 ...
 *
 * Shared between bin/gaganfoxwell-gbrain-detect (preamble probe on every skill start)
 * and bin/gaganfoxwell-gbrain-sync.ts (orchestrator SKIP-when-not-ok semantics).
 ...
 *   - Cache: 60s TTL at ~/.gaganfoxwell/.gbrain-local-status-cache.json, keyed on
 ...
 * Timeout → probe exceeded GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS (default 15s) with no
...
interface CacheEntry {
  // Local-cache schema version, controlled by gaganfoxwell. Not to be confused
  // with `gbrain doctor --json` output schema_version (gbrain v0.25+ emits
  // schema_version: 2). Doctor-output parsing lives in
  // lib/gaganfoxwell-memory-helpers.ts:freshDetectEngineTier and accepts both
...
/**
 * Effective probe timeout. `GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS` overrides the
...
export function probeTimeoutMs(env?: NodeJS.ProcessEnv): number {
  const raw = (env ?? process.env).GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS;
...
/** Cache path computed fresh on each call so tests can mutate GAGANFOXWELL_HOME per case. */
export function cacheFilePath(): string {
  return join(
    process.env.GAGANFOXWELL_HOME || join(userHome(), ".gaganfoxwell"),
...
export function hasRemoteOnlyGbrainMcp(
  ...
  // Same classification rules as gaganfoxwell-gbrain-detect's detectMcpMode tier 3,
