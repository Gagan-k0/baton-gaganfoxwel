/**
 * gaganfoxwell context-bill — token bill-of-materials for an installed gaganfoxwell skills tree.
 *
 ...
 * envelope subtracted. Regenerate with `gaganfoxwell-context-bill <tree> --exact
...
function totalMd(dir: string, tokensOf: TokensOf): { bytes: number; tokens: number } {
  ...
  for (const p of walkMd(dir)) {
    // A skill dir that CONTAINS other skill dirs (the gaganfoxwell root skill wraps
 ...
 * installed ~/.claude/skills tree contains flat skill dirs PLUS a full gaganfoxwell
 * repo checkout (`gaganfoxwell/`, with .git). Its nested SKILL.md files are the
...
const USAGE =
  "Usage:\n" +
  "  gaganfoxwell-context-bill [TREE] [--json] [--skill <name>]\n" +
  "  gaganfoxwell-context-bill --diff <treeA> <treeB> [--json]\n" +
  "  gaganfoxwell-context-bill [TREE] --budget <budget.json> [--json]\n" +
  "\n" +
  ...
  "                       gaganfoxwell-egress); if it cannot be written, the run falls\n" +
