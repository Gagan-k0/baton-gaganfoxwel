 ...
 *
 * gbrain (the separate CLI gaganfoxwell shells out to) can rm-rf a user's working tree
 * during an autopilot race (its own bug, upstream gbrain #1526). gaganfoxwell can't fix
...
export function gbrainHome(env: NodeJS.ProcessEnv = process.env): string {
  ...
  // gbrain appends `.gbrain` itself, so gaganfoxwell must too.
