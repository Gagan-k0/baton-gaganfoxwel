 ...
 * bin/gaganfoxwell-gbrain-supabase-provision (thin bun-shebang entry).
 *
 ...
 * mirroring `_receipted_curl closed` in bin/gaganfoxwell-egress-lib.sh.
 *
 * Exit codes:
 *   0 — success
...
const PROG = 'gaganfoxwell-gbrain-supabase-provision';
 ...
 * is not reproduced.
 */
export const HELP_TEXT = `gaganfoxwell-gbrain-supabase-provision — Supabase Management API wrapper for
 ...
 * and refusal message as _receipted_curl closed in gaganfoxwell-egress-lib.sh).
 */
  ...
  for (;;) {
    ...
    try {
      const receipt = writeReceipt({
        ...
        consent: 'user ran gaganfoxwell-gbrain-supabase-provision',
      });
      receiptId = receipt.id;
    } catch (error) {
      // Refused — the send never happens. Same problem/cause/fix contract as
      // _gaganfoxwell_egress_refusal in gaganfoxwell-egress-lib.sh, then exit 8.
      const home = resolveEgressHome(ctx.env);
      const cause = `EGRESS_RECEIPT_FAILED: ${(error as Error)?.message ?? error}`.replace(/\n/g, ' ');
      ctx.stderr(
        `gaganfoxwell: supabase-provision NOT sent — the egress receipt could not be written (${cause}). ` +
          `Fix: chmod -R u+w ${path.join(home, 'security')} (or check GAGANFOXWELL_HOME). ` +
          `What this is: gaganfoxwell records everything it ATTEMPTS to send off-machine; see gaganfoxwell-egress.\n`,
      );
...
async function cmdPoolerUrl(ctx: Ctx, args: string[]): Promise<void> {
  ...
  // Override with GAGANFOXWELL_SUPABASE_TRUST_API_PORT=1 if a future API version
  // starts returning a working transaction port and this rewrite is wrong.
  if ((ctx.env.GAGANFOXWELL_SUPABASE_TRUST_API_PORT ?? '0') !== '1' && poolMode === 'transaction' && dbPort === '6543') {
    ctx.stderr(
      'pooler-url: API returned transaction pooler (port 6543); shared pooler for new projects listens on session port 5432 — rewriting (set GAGANFOXWELL_SUPABASE_TRUST_API_PORT=1 to disable)\n',
    );
    dbPort = '5432';
    poolMode = 'session';
  }
...
export async function runProvision(argv: string[], options: ProvisionOptions = {}): Promise<number> {
  ...
  try {
    switch (cmd) {
      ...
      case undefined:
      case '':
        die(ctx, 'usage: gaganfoxwell-gbrain-supabase-provision {list-orgs|create|wait|pooler-url|list-orphans|delete-project|--help}');
        break;
      default:
