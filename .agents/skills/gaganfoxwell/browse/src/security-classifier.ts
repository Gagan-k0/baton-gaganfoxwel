 ...
 * layout at ~/.gaganfoxwell/models/testsavant-small/ on first use.
 *
 ...
 */
const MODELS_DIR = path.join(os.homedir(), '.gaganfoxwell', 'models');
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.GAGANFOXWELL_SECURITY_OFF === '1') {
    testsavantState = 'failed';
    testsavantLoadError = 'GAGANFOXWELL_SECURITY_OFF=1 — ML classifier kill switch engaged';
