 ...
 * and passes it via GAGANFOXWELL_SKILL_TOKEN. The skill never sees the daemon root
 ...
 * GAGANFOXWELL_PORT, so a synchronous wait would deadlock it.
 */
async function runToFiles(cmd: string[], opts: RunToFilesOptions): Promise<RunToFilesResult> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-skill-'));
 ...
 *    GAGANFOXWELL_PORT and GAGANFOXWELL_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get GAGANFOXWELL_PORT +
 * GAGANFOXWELL_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'GAGANFOXWELL_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip GAGANFOXWELL_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.GAGANFOXWELL_PORT = String(opts.port);
  out.GAGANFOXWELL_SKILL_TOKEN = opts.skillToken;
