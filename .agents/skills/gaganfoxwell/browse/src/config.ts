 ...
 *   2. git rev-parse --show-toplevel → projectDir/.gaganfoxwell/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .gaganfoxwell/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.gaganfoxwell');
...
/**
 * Create the .gaganfoxwell/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // redundant safety, kept so `.gaganfoxwell/` still reads as ignored in git status.
  try {
  ...
  // Ensure .gaganfoxwell/ is in the project's .gitignore
  // First, check if git already ignores .gaganfoxwell/ (via global excludes, .git/info/exclude, or parent .gitignore)
  if (isIgnoredByGit(config.projectDir, '.gaganfoxwell/')) return;

  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.gaganfoxwell\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.gaganfoxwell/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
...
/**
 * Resolve the gaganfoxwell home directory.
 *
 * Honors the existing convention used by telemetry.ts and domain-skills.ts:
 *   1. GAGANFOXWELL_HOME env (explicit override)
 *   2. $HOME/.gaganfoxwell (default)
 */
export function resolveGaganfoxwellHome(): string {
  return process.env.GAGANFOXWELL_HOME || path.join(os.homedir(), '.gaganfoxwell');
}

/**
 * Read one key from the flat-YAML config store at <gaganfoxwell home>/config.yaml
 * (the shape bin/gaganfoxwell-config writes: `key: value` lines). Tolerates
 ...
 */
export function readGaganfoxwellConfigYamlKey(key: string): string | null {
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  try {
    const yaml = fs.readFileSync(path.join(resolveGaganfoxwellHome(), 'config.yaml'), 'utf-8');
    // Last match wins: bin/gaganfoxwell-config's `get` reads duplicates with
 ...
 * stays OFF unless the user explicitly ran `gaganfoxwell-config set pair_agent on`
 ...
 *
 * Env override `GAGANFOXWELL_PAIR_AGENT=on|off` wins (used by tests and as an
 * emergency knob), mirroring the telemetry env-hint convention.
 */
export function isPairAgentEnabled(): boolean {
  const env = process.env.GAGANFOXWELL_PAIR_AGENT;
  if (env === 'on') return true;
  if (env === 'off') return false;
  // Canonical store: ~/.gaganfoxwell/config.yaml (flat `key: value` lines, written
  // by bin/gaganfoxwell-config — which is what the /pair-agent consent step runs).
  ...
  const yamlValue = readGaganfoxwellConfigYamlKey('pair_agent');
  if (yamlValue === 'on') return true;
  if (yamlValue === 'off') return false;
  try {
    const raw = fs.readFileSync(path.join(resolveGaganfoxwellHome(), 'config.json'), 'utf-8');
