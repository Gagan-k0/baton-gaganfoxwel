 ...
 *     suggested-action. Matches the gaganfoxwell house style.
 *
...
function formatSavedOk(row: DomainSkillRow, slug: string): string {
  return [
    ...
    `Version: ${row.version}`,
    `Stored at: ~/.gaganfoxwell/projects/${slug}/learnings.jsonl`,
    '',
...
async function handleEdit(args: string[]): Promise<string> {
  ...
  const tmpFile = path.join(os.tmpdir(), `gaganfoxwell-domain-skill-${process.pid}-${Date.now()}.md`);
...
async function handlePromoteToGlobal(args: string[]): Promise<string> {
  ...
  return [
    ...
    `Stored at: ~/.gaganfoxwell/global-domain-skills.jsonl`,
    '',
