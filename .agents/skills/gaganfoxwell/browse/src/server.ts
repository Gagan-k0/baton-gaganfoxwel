/**
 * gaganfoxwell browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.gaganfoxwell/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.gaganfoxwell/browse-{console,network,dialog}.log
 ...
 * `/Users/Gagan-k0/.claude/plans/system-instruction-you-are-working-swirling-fountain.md`
 * Part 1.
 */
export interface ServerConfig {
  ...
  startTime: number;
  /**
   * Overlay hook. Runs AFTER gaganfoxwell resolves auth and BEFORE route dispatch.
   * Invalid tokens are auto-rejected at the gaganfoxwell layer (401 returned
   * before hook fires), so the hook only ever sees valid TokenInfo or null
   * (no token presented). Returning a Response short-circuits gaganfoxwell
   * dispatch; returning null falls through.
   */
  beforeRoute?: (req: Request, surface: Surface, auth: TokenInfo | null) => Promise<Response | null>;
  /**
   * Whether gaganfoxwell owns the lifecycle of the terminal-agent process and its
   ...
   *      matched sibling gaganfoxwell sessions on the same host — see
   ...
   *
   * This is correct for gaganfoxwell's CLI path, which spawns `terminal-agent.ts` as
...
/**
 * Build a ServerConfig-shaped object from process.env. Used by gaganfoxwell's
...
/**
 * The gaganfoxwell sidebar extension's pinned Chrome extension ID. Derived from
 ...
 */
export const GAGANFOXWELL_EXTENSION_ID = 'dgbkdbjebeiblbajiilljmhjdpmiglep';
...
/**
 * Read ngrok authtoken from env var, ~/.gaganfoxwell/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.gaganfoxwell', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['gaganfoxwell browse — headless browser for AI agents', '', 'Commands:'];
...
// so this branch is the normal path for /open-gaganfoxwell-browser.
...
function parentWatchdogTick(parentPid: number = BROWSE_PARENT_PID): void {
  try {
    ...
    //    /open-gaganfoxwell-browser sessions.
...
// fighting with gaganfoxwell's. CLI path is unchanged.
if (import.meta.main) {
...
export function buildFetchHandler(cfg: ServerConfig): ServerHandle {
  ...
  // to gaganfoxwell-owns. Matches the "default-true preserves CLI bit-for-bit"
  ...
  const AGENT_WATCHDOG_TICK_MS = parseInt(
    process.env.GAGANFOXWELL_AGENT_WATCHDOG_TICK_MS || '60000',
    10,
  );
  ...
  async function shutdown(exitCode: number = 0) {
    ...
    if (ownsTerminalAgent) {
      ...
      // terminal-agent\.ts` regex teardown which matched sibling gaganfoxwell
  ...
  // block gaganfoxwell shutdown — defensive symmetry with the safeUnlinkQuiet /
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // Welcome page — served when Gaganfoxwell Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate GAGANFOXWELL_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.gaganfoxwell/projects/../../etc/passwd/... — path
          ...
          const rawSlug = process.env.GAGANFOXWELL_SLUG || 'unknown';
          const slug = /^[a-z0-9_-]+$/.test(rawSlug) ? rawSlug : 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.gaganfoxwell/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from gaganfoxwell install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the GAGANFOXWELL_SLUG regex above.  Not exploitable today
          // (env set at install time), but the gate is one check.
          const rawSkillRoot = process.env.GAGANFOXWELL_SKILL_ROOT || `${homeDir}/.claude/skills/gaganfoxwell`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>Gaganfoxwell Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>Gaganfoxwell Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      // exactly `chrome-extension://<GAGANFOXWELL_EXTENSION_ID>`, where the ID
      ...
      if (url.pathname === '/extension-token' && req.method === 'POST') {
        ...
        const originOk =
          req.headers.get('origin') === `chrome-extension://${GAGANFOXWELL_EXTENSION_ID}`;
      ...
      // extension's gaganfoxwellInjectToTerminal callers. The extension routes
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        if (!isPairAgentEnabled()) {
          ...
          return new Response(JSON.stringify({
            ...
            hint: 'enable once with: gaganfoxwell-config set pair_agent on — or run /pair-agent, which asks for consent and sets it',
          }), { status: 403, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        // 1) Resolve ngrok authtoken from env / .gaganfoxwell / native config
...
export async function start() {
  ...
  if (process.env.BROWSE_TUNNEL === '1' && !isPairAgentEnabled()) {
    console.error('[browse] BROWSE_TUNNEL=1 ignored: pair-agent is off. Enable once with: gaganfoxwell-config set pair_agent on');
  } else if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.gaganfoxwell/ngrok.env');
    } else {
