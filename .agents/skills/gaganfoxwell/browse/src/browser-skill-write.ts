 ...
 *
 * /skillify stages a candidate skill into ~/.gaganfoxwell/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.gaganfoxwell/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.gaganfoxwell', '.tmp');
