...
const TMP_HOME = path.join(os.tmpdir(), `gaganfoxwell-telemetry-test-${process.pid}-${Date.now()}`);
const TELEMETRY_FILE = path.join(TMP_HOME, 'analytics', 'browse-telemetry.jsonl');

// Use GAGANFOXWELL_HOME env to redirect telemetry writes (read each call,
...
describe('telemetry: signals fire to ~/.gaganfoxwell/analytics/browse-telemetry.jsonl', () => {
  ...
  it('GAGANFOXWELL_TELEMETRY_OFF=1 silences all events', async () => {
