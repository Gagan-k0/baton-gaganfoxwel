...
describe('findPlaywrightRevisionDir', () => {
  ...
  it('returns null outside the Playwright cache layout', () => {
    expect(findPlaywrightRevisionDir('/Applications/Gaganfoxwell Browser.app/Contents/MacOS/Chromium')).toBe(null);
  });
});
...
describe('findGaganfoxwellInstallRoot (ENG-OV3: revision-matched roots only)', () => {
  let tmpRoot: string;
  ...
  it('resolves the dev checkout by default (its node_modules pins our revision)', () => {
    ...
    const registryRevision = browsersJson.browsers.find((b: { name: string }) => b.name === 'chromium').revision;
    const root = findGaganfoxwellInstallRoot(registryRevision);
...
describe('clearQuarantineOnPlaywrightCache', () => {
  let tmpCache: string;
  let execPath: string;
  const savedCustomPath = process.env.GAGANFOXWELL_CHROMIUM_PATH;
  ...
  it('NEVER touches a GAGANFOXWELL_CHROMIUM_PATH bundle (embedder scope contract)', () => {
    process.env.GAGANFOXWELL_CHROMIUM_PATH = execPath;
    const cleared: string[] = [];
    const ok = clearQuarantineOnPlaywrightCache(execPath, (target) => {
      cleared.push(target);
      return 0;
...
function makeDeps(counters: { installs: number; quarantines: number }, overrides: Record<string, unknown> = {}) {
  return {
    ...
    installRoot: () => '/tmp/fake-gaganfoxwell-root',
...
describe('maybeHealXProtectKill', () => {
  ...
  it('never heals over a custom executable (GAGANFOXWELL_CHROMIUM_PATH scope)', async () => {
    const counters = { installs: 0, quarantines: 0 };
