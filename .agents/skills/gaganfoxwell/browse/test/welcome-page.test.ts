...
describe('welcome page served via HTTP', () => {
  ...
  test('has Gaganfoxwell Browser title and branding', () => {
    expect(html).toContain('<title>Gaganfoxwell Browser</title>');
    expect(html).toContain('Gaganfoxwell Browser');
  });
  ...
  test('hides sidebar prompt when extension is detected', () => {
    ...
    expect(html).toContain("'gaganfoxwell-extension-ready'");
  ...
  test('does NOT auto-hide based on extension detection alone', () => {
    ...
    expect(html).not.toContain('gaganfoxwell-status-pill');
  ...
  test('has footer with attribution', () => {
    expect(html).toContain('Gaganfoxwell');
    expect(html).toContain('github.com/Gagan-k0/baton-gaganfoxwel');
  });
});
