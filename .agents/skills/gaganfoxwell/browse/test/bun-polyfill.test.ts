...
describe('bun-polyfill', () => {
  ...
  // GAGANFOXWELL_SPAWN_MAX_BUFFER caps the drain so a runaway child can't OOM the
  ...
  test('Bun.spawn caps buffered output at GAGANFOXWELL_SPAWN_MAX_BUFFER', async () => {
    const result = Bun.spawnSync(['node', '-e', `
      process.env.GAGANFOXWELL_SPAWN_MAX_BUFFER = '${1024}';
      require(${JSON.stringify(polyfillPath)});
      (async () => {
