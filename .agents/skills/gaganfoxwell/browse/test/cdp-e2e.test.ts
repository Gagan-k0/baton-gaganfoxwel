...
const TMP_HOME = path.join(os.tmpdir(), `gaganfoxwell-cdp-e2e-${process.pid}-${Date.now()}`);
...
// exact leak once pointed a sibling test's GAGANFOXWELL_HOME at our temp dir,
// which then got baked into artifacts that outlived it (dangling symlinks
// into a deleted render dir). Save + restore in afterAll.
const ORIGINAL_GAGANFOXWELL_HOME = process.env.GAGANFOXWELL_HOME;
process.env.GAGANFOXWELL_HOME = TMP_HOME;
process.env.GAGANFOXWELL_TELEMETRY_OFF = '1'; // don't pollute analytics during tests
...
afterAll(async () => {
  if (ORIGINAL_GAGANFOXWELL_HOME === undefined) delete process.env.GAGANFOXWELL_HOME;
  else process.env.GAGANFOXWELL_HOME = ORIGINAL_GAGANFOXWELL_HOME;
