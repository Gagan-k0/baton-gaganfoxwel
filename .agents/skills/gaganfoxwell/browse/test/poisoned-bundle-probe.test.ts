 ...
 * pre-v1.64 in-place rebrand (Info.plist contains "Gaganfoxwell Browser"),
 ...
 *   - GAGANFOXWELL_CHROMIUM_PATH bundles (custom/embedder) are NEVER deleted:
...
const POISONED_PLIST =
  '<plist><dict><key>CFBundleName</key><string>Gaganfoxwell Browser</string></dict></plist>';
const CLEAN_PLIST =
...
beforeEach(() => {
  ...
  savedCustomPath = process.env.GAGANFOXWELL_CHROMIUM_PATH;
  delete process.env.GAGANFOXWELL_CHROMIUM_PATH;
});

afterEach(() => {
  fs.rmSync(tmpDir, { recursive: true, force: true });
  if (savedCustomPath === undefined) delete process.env.GAGANFOXWELL_CHROMIUM_PATH;
  else process.env.GAGANFOXWELL_CHROMIUM_PATH = savedCustomPath;
});
...
describe('probePoisonedChromiumBundle — GAGANFOXWELL_CHROMIUM_PATH is never deleted', () => {
  test('probe refuses to act on the GAGANFOXWELL_CHROMIUM_PATH executable, even when poisoned', () => {
    // A custom/embedder bundle (Gaganfoxwell Browser.app wrapper) legitimately
    // contains "Gaganfoxwell Browser" in its plist — that is its branding, not
    // cache poison. Deleting it would destroy the embedder's product.
    const { appDir, exe } = makeApp(path.join(tmpDir, 'Gaganfoxwell Browser.app-parent'), POISONED_PLIST);
    process.env.GAGANFOXWELL_CHROMIUM_PATH = exe;
  ...
  test('caller contract: both headed launch paths probe chromium.executablePath() only', () => {
    ...
    expect(SRC).not.toMatch(/probePoisonedChromiumBundle\([^)]*GAGANFOXWELL_CHROMIUM_PATH/);
  });
});
