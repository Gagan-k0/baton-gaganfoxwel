...
function loadBackground() {
  ...
  const run = new Function('chrome', 'importScripts', 'gaganfoxwellSenderAuth', 'fetch', BG_SRC);
...
describe('background.js ↔ sender-auth.js wiring', () => {
  ...
  test('background.js consults gaganfoxwellSenderAuth.denialFor in the message listener', () => {
    expect(BG_SRC).toContain('gaganfoxwellSenderAuth.denialFor(msg.type, sender, chrome.runtime.id)');
  });
