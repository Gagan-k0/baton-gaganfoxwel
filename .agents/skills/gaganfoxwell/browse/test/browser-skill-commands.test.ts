...
beforeEach(() => {
  ...
  tiers = {
    project: path.join(tmpRoot, 'project', '.gaganfoxwell', 'browser-skills'),
    global: path.join(tmpRoot, 'home', '.gaganfoxwell', 'browser-skills'),
    bundled: path.join(tmpRoot, 'gaganfoxwell-install', 'browser-skills'),
  };
...
describe('buildSpawnEnv', () => {
  ...
  beforeEach(() => {
    ...
    process.env.GAGANFOXWELL_TOKEN = 'root-token';
  ...
  it('untrusted: drops $HOME and secrets', () => {
    ...
    expect(env.GAGANFOXWELL_TOKEN).toBeUndefined();
  });
  ...
  it('untrusted: injects GAGANFOXWELL_PORT + GAGANFOXWELL_SKILL_TOKEN', () => {
    const env = buildSpawnEnv({ trusted: false, port: 1234, skillToken: 'tok-xyz' });
    expect(env.GAGANFOXWELL_PORT).toBe('1234');
    expect(env.GAGANFOXWELL_SKILL_TOKEN).toBe('tok-xyz');
  });
  ...
  it('trusted: still strips GAGANFOXWELL_TOKEN (defense in depth)', () => {
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'tok' });
    expect(env.GAGANFOXWELL_TOKEN).toBeUndefined();
  });
  ...
  it('GAGANFOXWELL_PORT/GAGANFOXWELL_SKILL_TOKEN can never be overridden by parent env', () => {
    process.env.GAGANFOXWELL_PORT = '99999'; // attacker-set
    process.env.GAGANFOXWELL_SKILL_TOKEN = 'attacker-tok';
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'real-tok' });
    expect(env.GAGANFOXWELL_PORT).toBe('1234');
    expect(env.GAGANFOXWELL_SKILL_TOKEN).toBe('real-tok');
  });
});
...
describe.skipIf(SKIP_SPAWN)('spawnSkill: lifecycle', () => {
  ...
  it('untrusted spawn: GAGANFOXWELL_SKILL_TOKEN visible, root env scrubbed', async () => {
    const dir = makeSkillDir(tiers.bundled, 'env-probe',
      'name: env-probe\nhost: x.com',  // trusted defaults to false
      `console.log(JSON.stringify({
        port: process.env.GAGANFOXWELL_PORT,
        token: process.env.GAGANFOXWELL_SKILL_TOKEN,
        home: process.env.HOME ?? null,
        gh: process.env.GITHUB_TOKEN ?? null,
        gaganfoxwell: process.env.GAGANFOXWELL_TOKEN ?? null,
      }));`,
    );
    const origEnv = { ...process.env };
    process.env.GITHUB_TOKEN = 'gh-secret';
    process.env.GAGANFOXWELL_TOKEN = 'root';
    try {
      ...
      expect(parsed.gaganfoxwell).toBeNull();
    } finally {
