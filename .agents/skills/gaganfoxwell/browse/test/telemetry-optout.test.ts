 ...
 * ~/.gaganfoxwell/config.yaml). An absent key, an absent config file, an explicit
 * `off`, or any unrecognized value all mean DISABLED — the same default
 * bin/gaganfoxwell-config's DEFAULTS table reports for an unset key, so a daemon
 * spawned outside a skill preamble (direct $B use, embedders) can never
 * emit while `gaganfoxwell-config get telemetry` tells the user 'off'.
 *
 * The persistent tier reads through the shared flat-YAML helper in
 * config.ts (readGaganfoxwellConfigYamlKey), same parser as the pair-agent gate.
 * Env tier: GAGANFOXWELL_TELEMETRY_OFF=1 always disables; =0 is a harness-side
 * consent assertion that covers the no-config default only — it never
 * overrides an explicit `telemetry: off`.
 *
 * Harness mirrors pair-agent-optin-gate.test.ts: GAGANFOXWELL_HOME → temp dir,
...
const savedEnv = {
  ...
  GAGANFOXWELL_TELEMETRY_OFF: process.env.GAGANFOXWELL_TELEMETRY_OFF,
};
const tmpHomes: string[] = [];

/** Fresh GAGANFOXWELL_HOME with the given config.yaml body (null = no file). */
function tmpHomeWith(configYaml: string | null): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-telemetry-optout-'));
  tmpHomes.push(dir);
  if (configYaml !== null) {
    fs.writeFileSync(path.join(dir, 'config.yaml'), configYaml);
  }
  process.env.GAGANFOXWELL_HOME = dir;
  delete process.env.GAGANFOXWELL_TELEMETRY_OFF;
  _resetTelemetryCache();
  return dir;
}

afterEach(() => {
  for (const k of ['GAGANFOXWELL_HOME', 'GAGANFOXWELL_TELEMETRY_OFF'] as const) {
...
describe('telemetry persistent opt-out tier (config.yaml)', () => {
  ...
  test('DISABLED when the key is absent — consent was never granted', () => {
    // bin/gaganfoxwell-config's DEFAULTS table reports 'off' for an unset telemetry
...
describe('telemetry env tier + cache semantics', () => {
  test('GAGANFOXWELL_TELEMETRY_OFF=1 disables even when config says anonymous', () => {
    tmpHomeWith('telemetry: anonymous\n');
    process.env.GAGANFOXWELL_TELEMETRY_OFF = '1';
    _resetTelemetryCache();
    expect(isTelemetryDisabled()).toBe(true);
  });

  test('GAGANFOXWELL_TELEMETRY_OFF=0 never overrides an explicit `telemetry: off`', () => {
    ...
    tmpHomeWith('telemetry: off\n');
    process.env.GAGANFOXWELL_TELEMETRY_OFF = '0';
    _resetTelemetryCache();
    expect(isTelemetryDisabled()).toBe(true);
  });

  test('GAGANFOXWELL_TELEMETRY_OFF=0 enables when no config store exists (harness seam)', () => {
    tmpHomeWith(null);
    process.env.GAGANFOXWELL_TELEMETRY_OFF = '0';
    _resetTelemetryCache();
    expect(isTelemetryDisabled()).toBe(false);
