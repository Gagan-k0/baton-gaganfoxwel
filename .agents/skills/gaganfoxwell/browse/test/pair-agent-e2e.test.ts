...
import { GAGANFOXWELL_EXTENSION_ID } from '../src/server';
...
describe('pair-agent flow end-to-end (HTTP only, no ngrok)', () => {
  ...
  test('GET /health returns daemon status and NEVER includes a token (even for chrome-extension origins)', async () => {
    const resp = await fetch(`${daemon.baseUrl}/health`, {
      headers: { Origin: `chrome-extension://${GAGANFOXWELL_EXTENSION_ID}` },
    });
  ...
  test('POST /sse-session with root Bearer returns a Set-Cookie for gaganfoxwell_sse', async () => {
    ...
    expect(setCookie!).toContain('gaganfoxwell_sse=');
  ...
  test('/welcome regex gate: safe slug resolves; dangerous slug does not path-traverse', async () => {
    // The regex gate lives in server.ts — we can't easily flip GAGANFOXWELL_SLUG
