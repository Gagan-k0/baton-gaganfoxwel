# gaganfoxwell-gbrain-lib.sh — shared helpers for setup-gbrain bin scripts.
#
# This file is NOT executable; source it:
#
#   . "$(dirname "$0")/gaganfoxwell-gbrain-lib.sh"
#
# Provides:
...
# _gaganfoxwell_gbrain_validate_varname <name> — returns 0 if usable, 2 otherwise.
...
_gaganfoxwell_gbrain_validate_varname() {
  local name="$1"
  local LC_ALL=C
  case "$name" in
...
read_secret_to_env() {
  ...
  if ! _gaganfoxwell_gbrain_validate_varname "$varname"; then
