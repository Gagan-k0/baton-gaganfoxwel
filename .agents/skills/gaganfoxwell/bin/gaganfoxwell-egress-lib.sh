# gaganfoxwell-egress-lib.sh — shared egress-receipt helpers for bash sinks.
#
# This file is NOT executable; source it:
#
#   . "$(dirname "$0")/gaganfoxwell-egress-lib.sh"
#
...
#     the receipt id is exported as _GAGANFOXWELL_EGRESS_LAST_RECEIPT so callers
...
#
# NO EXIT traps in this file, ever: callers (gaganfoxwell-telemetry-sync) own
...
case "${BASH_SOURCE[0]}" in
  */*) _gaganfoxwell_egress_lib_dir="$(cd "${BASH_SOURCE[0]%/*}" && pwd)" ;;
  *) _gaganfoxwell_egress_lib_dir="$(pwd)" ;;
esac

_gaganfoxwell_egress_home() {
  if [ -n "${GAGANFOXWELL_HOME:-}" ]; then
    printf '%s' "$GAGANFOXWELL_HOME"
  elif [ -n "${GAGANFOXWELL_STATE_DIR:-}" ]; then
    printf '%s' "$GAGANFOXWELL_STATE_DIR"
  else
    printf '%s' "$HOME/.gaganfoxwell"
  fi
}

# Problem + cause + fix, in plain language (DX contract for every
# fail-closed refusal). $1 = sink, $2 = cause text from the receipt bridge.
_gaganfoxwell_egress_refusal() {
  local home
  home="$(_gaganfoxwell_egress_home)"
  echo "gaganfoxwell: $1 NOT sent — the egress receipt could not be written (${2:-unknown cause}). Fix: chmod -R u+w $home/security (or check GAGANFOXWELL_HOME). What this is: gaganfoxwell records everything it ATTEMPTS to send off-machine; see gaganfoxwell-egress." >&2
}
...
_gaganfoxwell_egress_run() {
  ...
  _GAGANFOXWELL_EGRESS_LAST_RECEIPT="$receipt_id"

  if [ -z "$receipt_id" ]; then
    if [ "$policy" = "closed" ]; then
      _gaganfoxwell_egress_refusal "$sink" "$receipt_err"
      [ "$payload" != "--no-payload" ] && rm -f "$payload"
      return 3
    fi
    echo "gaganfoxwell: egress receipt could not be written for $sink (${receipt_err:-unknown cause}) — sending anyway (fail-open). gaganfoxwell normally records everything it ATTEMPTS to send off-machine; see gaganfoxwell-egress." >&2
  fi
...
_receipted_git() {
  ...
  shift 5
  _gaganfoxwell_egress_run "$policy" "$sink" "$host" "$class" "$consent" --no-payload "$@"
}
