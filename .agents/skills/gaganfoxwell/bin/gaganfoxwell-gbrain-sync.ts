#!/usr/bin/env bun
/**
 * gaganfoxwell-gbrain-sync — V1 unified sync verb.
 *
 ...
 *   2. Transcripts + curated memory → gaganfoxwell-memory-ingest (typed put_page)
 *   3. Curated artifacts to git    → gaganfoxwell-brain-sync (existing pipeline)
 *
 * Modes:
 ...
 *   - Lock file at ~/.gaganfoxwell/.sync-gbrain.lock (PID + start ts).
 ...
 * than building a gaganfoxwell-side daemon.
 */
...
import { detectEngineTier, withErrorContext, canonicalizeRemote } from "../lib/gaganfoxwell-memory-helpers";
...
const STATE_PATH = join(GAGANFOXWELL_HOME, ".gbrain-sync-state.json");
const LOCK_PATH = join(GAGANFOXWELL_HOME, ".sync-gbrain.lock");
 ...
 * GAGANFOXWELL_HOME at runtime — same pattern as cacheFilePath() in
 * lib/gbrain-local-status.ts. Avoids the ESM static-import hoist trap where a
 * module-load-time const captures the real ~/.gaganfoxwell before a test can redirect.
 */
export function dreamMarkerPath(): string {
  return join(process.env.GAGANFOXWELL_HOME || join(homedir(), ".gaganfoxwell"), ".dream-in-progress");
}

// Default 35-minute timeout for code-walk + memory-ingest stages. Override via
// GAGANFOXWELL_SYNC_CODE_TIMEOUT_MS / GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS. Bounds-checked
 ...
 */
export function decideResume(gaganfoxwellHome: string = GAGANFOXWELL_HOME): ResumeVerdict {
  const cp = readGbrainCheckpoint();
  if (!cp || !cp.dir) return { kind: "no-checkpoint" };
  const stagingDir = cp.dir;
  // #1802: only resume into a path we can PROVE is a gaganfoxwell-minted staging dir.
  ...
  const verdict = checkOwnedStagingDir(stagingDir, gaganfoxwellHome);
  if (!verdict.ok) {
...
function printUsage(): void {
  console.error(`Usage: gaganfoxwell-gbrain-sync [--incremental|--full|--dry-run] [options]
...
Options:
  ...
  --no-memory          Skip the gaganfoxwell-memory-ingest stage (transcripts + artifacts).
  --no-brain-sync      Skip the gaganfoxwell-brain-sync git pipeline stage.
  ...
  --dream              Force the source-scoped dream cycle that builds this
                       ...
                       timeout 45min, override GAGANFOXWELL_SYNC_DREAM_TIMEOUT_MS.
 ...
 *
 * Pattern: `gaganfoxwell-code-<slug>-<hostpathhash8>` where slug comes from origin
 ...
 *
 * `GAGANFOXWELL_HOSTNAME` env override is honored for deterministic tests; in
...
function deriveCodeSourceId(repoPath: string): string {
  const host = process.env.GAGANFOXWELL_HOSTNAME || hostname();
  ...
  if (remote) {
    ...
    const fullId = constrainSourceId("gaganfoxwell-code", `${slugSource}-${hostPathHash}`);
    ...
    return constrainSourceId("gaganfoxwell-code", `${repoOnly}-${hostPathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("gaganfoxwell-code", `${base}-${hostPathHash}`);
}
 ...
 *
 * Earlier /sync-gbrain versions registered `gaganfoxwell-code-<slug>` (no pathhash
...
function deriveLegacyCodeSourceId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("gaganfoxwell-code", slugSource);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("gaganfoxwell-code", base);
}
 ...
 * repo path: `gaganfoxwell-code-<slug>-<sha1(path).slice(0,8)>`. After #1468 the
...
export function derivePathOnlyHashLegacyId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("gaganfoxwell-code", `${slugSource}-${pathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("gaganfoxwell-code", `${base}-${pathHash}`);
}
...
function acquireLock(): boolean {
  mkdirSync(GAGANFOXWELL_HOME, { recursive: true });
...
/**
 * Acquire the dream marker (`~/.gaganfoxwell/.dream-in-progress`). Returns false when
...
function skipStageForLocalStatus(
  ...
  const reasons: Record<Exclude<LocalEngineStatus, "ok">, string> = {
    ...
    "timeout":
      "engine probe timed out; raise GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS if your pooler is slow",
    "thin-client":
      "thin client (remote-HTTP MCP brain, no local engine by design, #2051); " +
...
function warnProbeTimeout(stage: "code" | "memory" | "dream"): void {
  process.stderr.write(
    `[gaganfoxwell-gbrain-sync] ${stage}: engine probe timed out — proceeding anyway; ` +
      `raise GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS if your pooler is slow\n`,
  );
}


/**
 * Per-repo trust tier from ~/.gaganfoxwell/gbrain-repo-policy.json, read through
 * the bin/gaganfoxwell-gbrain-repo-policy CLI (which owns URL normalization and
...
export function repoPolicyTier(url: string | null): "read-write" | "read-only" | "deny" | "unset" | "error" {
  ...
  if (res.error === "spawn-failed") {
    process.stderr.write(
      "[gaganfoxwell-gbrain-sync] the repo-policy helper could not be spawned (bash missing from PATH?) — " +
...
async function runCodeImport(args: CliArgs): Promise<StageResult> {
  ...
  if (tier === "read-only") {
    ...
    return {
      ...
      ok: true,
      duration_ms: Date.now() - t0,
      summary: `skipped — repo policy is read-only for ${policyUrl} (code ingest writes pages). Change with: gaganfoxwell-gbrain-repo-policy set ${policyUrl} read-write`,
      detail: { source_id: sourceId, source_path: root, status: "skipped-policy-read-only" },
    };
  }
  if (tier === "deny" || tier === "error") {
    const why = tier === "deny"
      ? `repo policy is deny for ${policyUrl} — no gbrain ingest for this repo. Change with: gaganfoxwell-gbrain-repo-policy set ${policyUrl} read-write`
      : "repo policy store exists but could not be read (gaganfoxwell-gbrain-repo-policy get failed) — refusing ingest rather than bypassing a set policy";
    return {
      name: "code",
      ran: true,
      ok: false,
  ...
  // Earlier /sync-gbrain versions registered `gaganfoxwell-code-<slug>` (no path
  ...
  const codeTimeoutMs = resolveStageTimeoutMs(
    process.env.GAGANFOXWELL_SYNC_CODE_TIMEOUT_MS,
    "GAGANFOXWELL_SYNC_CODE_TIMEOUT_MS",
  );
  ...
  // gaganfoxwell's own .gitignore. Without the consumer-side entry, the pin gets
...
function runMemoryIngest(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "memory", ran: false, ok: true, duration_ms: 0, summary: "would: gaganfoxwell-memory-ingest --probe" };
  }

  // Split-engine pre-flight (per plan D12). gaganfoxwell-memory-ingest shells out
  ...
  if (resume.kind === "resume") {
    ...
    childEnv.GAGANFOXWELL_INGEST_RESUME_DIR = resume.stagingDir;
  ...
  const ingestPath = join(import.meta.dir, "gaganfoxwell-memory-ingest.ts");
  ...
  // .env.local footgun affects gaganfoxwell-memory-ingest.ts too, not just the
  ...
  const memoryTimeoutMs = resolveStageTimeoutMs(
    process.env.GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS,
    "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS",
  );
...
function runBrainSyncPush(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "would: gaganfoxwell-brain-sync --discover-new --once" };
  }

  const brainSyncPath = join(import.meta.dir, "gaganfoxwell-brain-sync");
  if (!existsSync(brainSyncPath)) {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "skipped (gaganfoxwell-brain-sync not installed)" };
  }

  // gaganfoxwell-brain-sync is a bash shebang script, so it needs an INTERPRETER, not
  ...
  if (!discover || !once) {
    return {
      ...
      ok: true,
      duration_ms: Date.now() - t0,
      summary: "skipped (no bash found; set GAGANFOXWELL_BASH to your Git bash.exe)",
    };
  }
  ...
  return {
    ...
    ok: result.status === 0,
    duration_ms: Date.now() - t0,
    summary: result.status === 0 ? "curated artifacts pushed" : `gaganfoxwell-brain-sync exited ${result.status}`,
  };
}
...
export async function runDream(args: CliArgs): Promise<StageResult> {
  ...
  try {
    const dreamTimeoutMs = resolveStageTimeoutMs(
      process.env.GAGANFOXWELL_SYNC_DREAM_TIMEOUT_MS,
      "GAGANFOXWELL_SYNC_DREAM_TIMEOUT_MS",
...
function loadSyncState(): SyncState {
  if (!existsSync(STATE_PATH)) {
    return { schema_version: 1, last_writer: "gaganfoxwell-gbrain-sync" };
  }
  try {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!args.noCode) {
      stages.push(await withErrorContext("sync:code", () => runCodeImport(args), "gaganfoxwell-gbrain-sync"));
    }
    if (!args.noMemory) {
      stages.push(await withErrorContext("sync:memory", () => runMemoryIngest(args), "gaganfoxwell-gbrain-sync"));
    }
    if (!args.noBrainSync) {
      stages.push(await withErrorContext("sync:brain-sync", () => runBrainSyncPush(args), "gaganfoxwell-gbrain-sync"));
    }
  ...
  if (!args.quiet || args.mode === "dry-run") {
    ...
    console.log(`\ngaganfoxwell-gbrain-sync (${args.mode}):`);
...
if (import.meta.main) {
  main().catch((err) => {
    console.error(`gaganfoxwell-gbrain-sync fatal: ${err instanceof Error ? err.message : String(err)}`);
    releaseLock();
    process.exit(1);
  });
}
