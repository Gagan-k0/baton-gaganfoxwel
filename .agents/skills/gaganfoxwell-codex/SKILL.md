---
name: gaganfoxwell-codex
preamble-tier: 3
version: 1.0.0
description: OpenAI Codex CLI wrapper — three modes. (gaganfoxwell)
triggers:
  - codex review
  - second opinion
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"codex","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"codex","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-log '{"skill":"codex","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
source ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-codex-probe 2>/dev/null && _gaganfoxwell_codex_log_event "codex_cli_missing" 2>/dev/null || true
```
...
known-bad list. Sourcing `gaganfoxwell-codex-probe` loads the shared helpers that both
...
if [ "${GAGANFOXWELL_FORCE_CODEX_REVIEW:-0}" != "1" ] && { [ -n "${CODEX_THREAD_ID:-}" ] || [ -n "${CODEX_SANDBOX:-}" ]; }; then
  echo "UNDER_CODEX"
elif ! _gaganfoxwell_codex_auth_probe >/dev/null; then
  _gaganfoxwell_codex_log_event "codex_auth_failed"
  echo "AUTH_FAILED"
else
  _gaganfoxwell_codex_model_probe   # ~10s round trip on first run, cached 1h (#2477)
fi
_gaganfoxwell_codex_version_check   # warns if known-bad, non-blocking
```
...
cost; skipped. Set `GAGANFOXWELL_FORCE_CODEX_REVIEW=1` to force.]" The whole value
...
**Update the known-bad list** in `bin/gaganfoxwell-codex-probe` when a new Codex CLI version
...
(where ephemeral codex stderr / response captures land) via `bin/gaganfoxwell-paths`.
This keeps the skill working whether installed as a Claude Code plugin
(`CLAUDE_PLANS_DIR` set), a global `~/.claude/skills/gaganfoxwell/` install, or a CI
container where `HOME` may be unset and `/tmp` may be read-only.

```bash
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-paths)"
```
...
_gaganfoxwell_codex_timeout_wrapper 330 codex review --base <base> -c 'sandbox_mode="read-only"' -c 'model_reasoning_effort="high"' -c 'web_search="cached"' < /dev/null 2>"$TMPERR"
_CODEX_EXIT=$?
if [ "$_CODEX_EXIT" = "124" ]; then
  _gaganfoxwell_codex_log_event "codex_timeout" "330"
  _gaganfoxwell_codex_log_hang "review" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
  ...
  _gaganfoxwell_codex_log_event "codex_nonzero_exit" "review:$_CODEX_EXIT"
fi
```
...
_gaganfoxwell_codex_timeout_wrapper 330 codex exec -s read-only "$(cat "$_PROMPT_FILE")" -c 'model_reasoning_effort="high"' -c 'web_search="cached"' < /dev/null 2>"$TMPERR"
_CODEX_EXIT=$?
rm -f "$_PROMPT_FILE"
if [ "$_CODEX_EXIT" = "124" ]; then
  _gaganfoxwell_codex_log_event "codex_timeout" "330"
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
line).** After VERDICT, end the report (content under the \`## GAGANFOXWELL REVIEW REPORT\`
...
1. Read the plan file (Read tool) to see its full current content. Search the read
   output for a \`## GAGANFOXWELL REVIEW REPORT\` heading anywhere in the file.
2. If found, use the Edit tool to DELETE the entire existing section. Match from
   \`## GAGANFOXWELL REVIEW REPORT\` through either the next \`## \` heading or end of
   file, whichever comes first. Replace with the empty string. This applies
   regardless of where the section currently lives — mid-file deletion is
   intentional, not a special case. If the Edit fails (e.g., concurrent edit
   changed the content), re-read the plan file and retry once.
3. After the delete (or skipped, if no section existed), append the new
   \`## GAGANFOXWELL REVIEW REPORT\` section at the END of the file. Use the Edit
   tool to match the file's current last paragraph and add the section after it,
   or use Write to re-emit the whole file with the section at the end.
4. Verify with the Read tool that \`## GAGANFOXWELL REVIEW REPORT\` is the last
...
2. Confirm the LAST `## ` heading in the file is `## GAGANFOXWELL REVIEW REPORT`.
   In-body prose that mentions "outside voice", "codex findings", or similar
   does NOT count — only the structured `## GAGANFOXWELL REVIEW REPORT` section
...
5. If a plan file is in context for this skill invocation: confirm
   `gaganfoxwell-review-log` was called and `gaganfoxwell-review-read` was run at least
...
_gaganfoxwell_codex_timeout_wrapper 600 codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' -c 'web_search="cached"' --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
import sys, json
...
if [ "$_CODEX_EXIT" = "124" ]; then
  ...
  _gaganfoxwell_codex_log_hang "challenge" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
  ...
  echo "[codex exit $_CODEX_EXIT] $(head -1 "$TMPERR" 2>/dev/null || echo "no stderr captured")"
  head -20 "$TMPERR" 2>/dev/null | sed 's/^/  /' || true
  _gaganfoxwell_codex_log_event "codex_nonzero_exit" "challenge:$_CODEX_EXIT"
fi
...
_gaganfoxwell_codex_timeout_wrapper 600 codex exec "<prompt>" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' -c 'web_search="cached"' --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
import sys, json
for line in sys.stdin:
    line = line.strip()
    if not line: continue
...
if [ "$_CODEX_EXIT" = "124" ]; then
  _gaganfoxwell_codex_log_event "codex_timeout" "600"
  _gaganfoxwell_codex_log_hang "consult" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
  echo "Codex stalled past 10 minutes. Common causes: model API stall, long prompt, network issue. Try re-running. If persistent, split the prompt or check ~/.codex/logs/."
elif [ "$_CODEX_EXIT" != "0" ]; then
  # Surface non-zero exits so the calling agent doesn't read "no output" as
  # a silent model/API stall. See #1327.
  echo "[codex exit $_CODEX_EXIT] $(head -1 "$TMPERR" 2>/dev/null || echo "no stderr captured")"
  head -20 "$TMPERR" 2>/dev/null | sed 's/^/  /' || true
  _gaganfoxwell_codex_log_event "codex_nonzero_exit" "consult:$_CODEX_EXIT"
fi
```
...
# Fix 1: wrap with timeout (gtimeout/timeout fallback chain via probe helper)
_gaganfoxwell_codex_timeout_wrapper 600 codex exec resume <session-id> "<prompt>" -c 'sandbox_mode="read-only"' -c 'model_reasoning_effort="medium"' -c 'web_search="cached"' --json < /dev/null 2>"$TMPERR" | PYTHONUNBUFFERED=1 "$PYTHON_CMD" -u -c "
...
if [ "$_CODEX_EXIT" = "124" ]; then
  _gaganfoxwell_codex_log_event "codex_timeout" "600"
  _gaganfoxwell_codex_log_hang "consult-resume" "$(wc -c < "$TMPERR" 2>/dev/null || echo 0)"
  echo "Codex stalled past 10 minutes. Common causes: model API stall, long prompt, network issue. Try re-running. If persistent, split the prompt or check ~/.codex/logs/."
elif [ "$_CODEX_EXIT" != "0" ]; then
  # Surface non-zero exits so the calling agent doesn't read "no output" as
  # a silent model/API stall. See #1327.
  echo "[codex exit $_CODEX_EXIT] $(head -1 "$TMPERR" 2>/dev/null || echo "no stderr captured")"
  head -20 "$TMPERR" 2>/dev/null | sed 's/^/  /' || true
  _gaganfoxwell_codex_log_event "codex_nonzero_exit" "consult-resume:$_CODEX_EXIT"
fi
...
- **Bash gate above the wrapper.** Every Bash call to codex sets its `timeout`
  parameter ABOVE the inner `_gaganfoxwell_codex_timeout_wrapper` budget (Review:
...
- **Detect skill-file rabbit holes.** After receiving Codex output, scan for signs
  that Codex got distracted by skill files: `gaganfoxwell-config`, `gaganfoxwell-update-check`,
  `SKILL.md`, or `skills/gaganfoxwell`. If any of these appear in the output, append a
  warning: "Codex appears to have read gaganfoxwell skill files instead of reviewing your
