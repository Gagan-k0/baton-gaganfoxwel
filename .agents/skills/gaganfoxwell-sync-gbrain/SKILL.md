---
name: gaganfoxwell-sync-gbrain
preamble-tier: 2
version: 1.0.0
description: Keep gbrain current with this repo's code and refresh agent search guidance in CLAUDE.md. (gaganfoxwell)
triggers:
  - sync gbrain
  - refresh gbrain
  - reindex repo
...
Wraps the gaganfoxwell-gbrain-sync orchestrator with
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"sync-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"sync-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
It does NOT touch `~/.gaganfoxwell/` indexing (the existing `gaganfoxwell-gbrain-source-wireup`
...
- `/sync-gbrain --refresh-cache` — force-rebuild brain-aware planning cache (v1.48; replaces /brain-refresh-context per D1 fold). Skips code + memory stages; routes to `gaganfoxwell-brain-cache refresh --project <slug>`.
- `/sync-gbrain --audit` — emit summary of gaganfoxwell-owned pages per project + sensitive-content audit (v1.48 / D10 lifecycle). Read-only.

Pass-through args go straight to the orchestrator at
`~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-sync.ts`.

**`--refresh-cache` short-circuit:** when this flag is present, the skill
runs ONLY the cache refresh (`gaganfoxwell-brain-cache refresh --project <slug>`
for the current worktree's slug, plus a cross-project refresh of
user-profile if `gaganfoxwell/user-profile/<user-slug>` exists). Code +
memory + brain-sync stages are skipped. Useful when the user knows the
brain has new info gaganfoxwell should pick up before the next planning skill.

**`--audit` short-circuit:** when this flag is present, the skill runs
`gaganfoxwell-brain-cache list --project <slug> --json`, summarizes by page
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-detect 2>/dev/null
```
...
also runs locally** in local-stdio MCP mode — `gaganfoxwell-memory-ingest` shells
...
`~/.gaganfoxwell/transcripts/<run-id>/` and the artifacts pipeline pushes it to
the brain admin's pull job (plan D11). Brain-sync (the `gaganfoxwell-brain-sync`
...
Also check the per-repo trust policy. If `gaganfoxwell-gbrain-repo-policy get` for
...
- **`timeout`**: proceed to Step 2 — the engine is most likely healthy but
  ...
  probe timed out (>15s) — proceeding; raise `GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS`
...
```bash
bun run ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-sync.ts <user-args>
```
...
still run. State is persisted to `~/.gaganfoxwell/.gbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`~/.gaganfoxwell/.sync-gbrain.lock` (5-min stale-takeover).
...
```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.gaganfoxwell/.gbrain-sync-state.json 2>/dev/null \
...
from `<!-- gaganfoxwell-gbrain-search-guidance:start -->` through
`<!-- gaganfoxwell-gbrain-search-guidance:end -->`. If those markers are missing,
...
gbrain status: GREEN
  ...
  CWD source ...... OK   <gaganfoxwell-code-{repo_slug}> (page_count=<N>)
  Call graph ...... OK   <N> edges resolved (code-callers/callees live)
  ~/.gaganfoxwell source. OK   <gaganfoxwell-brain-{user}> (page_count=<N>) — managed by /setup-gbrain
...
Mac. The orchestrator acquires a lock at `~/.gaganfoxwell/.sync-gbrain.lock` before
...
and travels with `git push`/`git pull` — NOT through `~/.gaganfoxwell/.brain-allowlist`
(which is for `~/.gaganfoxwell/` brain-sync only). On a different Mac with a synced
