...
(function () {
  ...
  if (!Terminal) {
    console.error('[gaganfoxwell terminal] xterm not loaded');
    return;
  }
   ...
   * /pty-session and surfaced back via window.gaganfoxwellPtySession so the
   ...
   * exposes the relevant fields on window.gaganfoxwellHealth (set below in init()).
   * If terminalPort is missing, the agent isn't ready yet.
   */
  function getHealth() {
    return window.gaganfoxwellHealth || {};
  }

  function getServerPort() {
    return window.gaganfoxwellServerPort || null;
  }

  function getAuthToken() {
    return window.gaganfoxwellAuthToken || null;
  }
  ...
  function startReattachLoop(prevSessionId) {
    ...
    const tick = async () => {
      ...
      if (resp.status === 401) {
        ...
        setState(STATE.IDLE, {
          message: 'Auth invalid — reload the sidebar or restart your gaganfoxwell session.',
        });
        return;
      }
      if (!resp.ok) {
  ...
  function openReattachWebSocket(terminalPort, attachToken, sessionId) {
    ...
    try { window.gaganfoxwellPtySession = currentSessionId; } catch {}
    setState(STATE.LIVE);
    ensureXterm();
    nextBinaryIsReplay = false;
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`gaganfoxwell-pty.${attachToken}`]);
    ...
    ws.addEventListener('error', (err) => {
      console.error('[gaganfoxwell terminal] reattach ws error', err);
    });
  }
   ...
   * derived input MUST be pre-scanned via window.gaganfoxwellScanForPTYInject()
   ...
   * sync `const ok = gaganfoxwellInjectToTerminal?.(text)` pattern. Making the
   ...
   */
  window.gaganfoxwellInjectToTerminal = function (text) {
   ...
   * Closes #1370.
   */
  window.gaganfoxwellScanForPTYInject = async function (text, origin) {
  ...
  // sidepanel uses for /pty-session — window.gaganfoxwellAuthToken, set by
  ...
  async function connect() {
    ...
    ws.addEventListener('error', (err) => {
      console.error('[gaganfoxwell terminal] ws error', err);
    });
  }
  ...
  async function forceRestart() {
    ...
    if (!nextTuple) {
      ...
      try { window.gaganfoxwellPtySession = null; } catch {}
      tryAutoConnect();
      return;
    }
    ...
    setState(STATE.LIVE);
    ensureXterm();
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`gaganfoxwell-pty.${token}`]);
    ws.binaryType = 'arraybuffer';
  ...
  function init() {
    ...
    document.addEventListener('gaganfoxwell:tab-state', (ev) => {
      if (!ws || ws.readyState !== WebSocket.OPEN) return;
      try {
        ws.send(JSON.stringify({
          type: 'tabState',
   ...
   * window.gaganfoxwellServerPort + window.gaganfoxwellAuthToken (which it does as soon
