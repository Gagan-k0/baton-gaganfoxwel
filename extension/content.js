/**
 * gaganfoxwell browse — content script
 *
...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'gaganfoxwell-status-pill';
  ...
  statusPill.innerHTML = `<span class="gaganfoxwell-pill-dot"></span> gaganfoxwell${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'gaganfoxwell-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'gaganfoxwell-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'gaganfoxwell-ref-panel';

  const header = document.createElement('div');
  header.className = 'gaganfoxwell-ref-panel-header';
  header.textContent = `gaganfoxwell refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'gaganfoxwell-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'gaganfoxwell-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'gaganfoxwell-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'gaganfoxwell-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'gaganfoxwell-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'gaganfoxwell-ref-panel-more';
...
// Do NOT dispatch gaganfoxwell-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('gaganfoxwell-extension-ready'));
    return;
  }
