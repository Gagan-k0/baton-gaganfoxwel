/**
 * gaganfoxwell browse — background service worker
 *
...
// Classic (non-module) service worker: importScripts puts gaganfoxwellSenderAuth on
...
async function loadAuthToken() {
  ...
  try {
    ...
    if (resp.status === 403) {
      console.error('[gaganfoxwell bg] /extension-token 403 — extension identity not trusted by server');
      authToken = null;
      setDisconnected();
      return false;
    }
    ...
    console.error('[gaganfoxwell bg] Failed to load auth token:', err.message);
  }
  return false;
}
...
async function checkHealth() {
  ...
  try {
    ...
    console.error('[gaganfoxwell bg] Health check failed:', err.message);
    setDisconnected();
  }
}
...
function setConnected(healthData) {
  ...
  chrome.runtime.sendMessage({ type: 'health', data: healthData }).catch((err) => {
    console.debug('[gaganfoxwell bg] No listener for health broadcast:', err.message);
  });
...
function setDisconnected() {
  ...
  chrome.runtime.sendMessage({ type: 'health', data: null }).catch((err) => {
    console.debug('[gaganfoxwell bg] No listener for disconnect broadcast:', err.message);
  });
...
async function notifyContentScripts(type) {
  try {
    ...
    console.error('[gaganfoxwell bg] Failed to query tabs for notification:', err.message);
  }
}
...
async function fetchAndRelayRefs() {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn(`[gaganfoxwell bg] Refs endpoint returned ${resp.status}`);
      return;
    }
    const data = await resp.json();
    ...
    console.error('[gaganfoxwell bg] Failed to fetch/relay refs:', err.message);
  }
}
...
async function injectInspector(tabId) {
  ...
  try {
    ...
    try {
      ...
      console.debug('[gaganfoxwell bg] Inspector CSS injection failed (non-fatal):', err.message);
    }
    // Send startPicker to the injected inspector.js
    try {
      await chrome.tabs.sendMessage(tabId, { type: 'startPicker' });
    } catch (err) {
      console.warn('[gaganfoxwell bg] Failed to send startPicker:', err.message);
    }
    inspectorMode = 'full';
    ...
    try {
      ...
      console.error('[gaganfoxwell bg] Inspector injection failed completely:', err.message, '| Basic fallback:', err2.message);
      inspectorMode = 'full';
...
async function stopInspector(tabId) {
  try {
    ...
    console.debug('[gaganfoxwell bg] Failed to stop picker on tab', tabId, ':', err.message);
  }
  return { ok: true };
}
...
async function postInspectorPick(selector, frameInfo, basicData, activeTabUrl) {
  ...
  try {
    ...
    console.debug('[gaganfoxwell bg] Inspector pick server unavailable, using basic mode:', err.message);
    return { mode: 'basic', selector, basicData, frameInfo };
  }
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (sender.id !== chrome.runtime.id) {
    console.warn('[gaganfoxwell] Rejected message from unknown sender:', sender.id);
    return;
  }
  ...
  if (!ALLOWED_TYPES.has(msg.type)) {
    console.warn('[gaganfoxwell] Rejected unknown message type:', msg.type);
    return;
  }
  ...
  const denial = gaganfoxwellSenderAuth.denialFor(msg.type, sender, chrome.runtime.id);
  if (denial) {
    console.warn('[gaganfoxwell] Rejected privileged message from unauthorized sender:', msg.type, sender.url || '(no sender url)');
    sendResponse(denial);
    return true;
  }
  ...
  if (msg.type === 'openSidePanel') {
    if (chrome.sidePanel?.open && sender.tab) {
      chrome.sidePanel.open({ tabId: sender.tab.id }).catch((err) => {
        console.warn('[gaganfoxwell bg] Failed to open side panel:', err.message);
      });
    }
    return;
  }
      ...
      postInspectorPick(msg.selector, frameInfo, msg.basicData, activeTabUrl)
        .then(result => {
          ...
          chrome.runtime.sendMessage({
            ...
            console.warn('[gaganfoxwell bg] Failed to forward inspectResult to sidepanel:', err.message);
          });
          sendResponse({ ok: true });
        });
    });
  ...
  if (msg.type === 'pickerCancelled') {
    chrome.runtime.sendMessage({ type: 'pickerCancelled' }).catch((err) => {
      console.debug('[gaganfoxwell bg] No listener for pickerCancelled:', err.message);
    });
    return;
  }
...
if (chrome.sidePanel && chrome.sidePanel.setPanelBehavior) {
  chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true }).catch((err) => {
    console.warn('[gaganfoxwell bg] Failed to set panel behavior:', err.message);
  });
}
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      if (wins.length > 0) {
        ...
        console.log(`[gaganfoxwell] Side panel opened on attempt ${attempt + 1}`);
        return; // success
      }
    } catch (e) {
      // May throw if window isn't ready or user gesture required
      console.log(`[gaganfoxwell] Side panel open attempt ${attempt + 1} failed:`, e.message);
    }
    // Backoff: 500ms, 1000ms, 2000ms, 3000ms, 5000ms
    await new Promise(r => setTimeout(r, [500, 1000, 2000, 3000, 5000][attempt]));
  }
  console.log('[gaganfoxwell] Side panel auto-open failed after 5 attempts');
}
...
async function announceIdentityPinOnce() {
  try {
    const data = await chrome.storage.local.get('gaganfoxwell_id_pin_migrated');
    if (data.gaganfoxwell_id_pin_migrated) return;
    console.log('[gaganfoxwell] gaganfoxwell sidebar: extension identity pinned in v1.63 — panel state reset once.');
    chrome.runtime.sendMessage({
      type: 'gaganfoxwell-migration-notice',
      message: 'gaganfoxwell sidebar: extension identity pinned in v1.63 — panel state reset once.',
    }).catch(() => {
      // Expected: panel not open. The console line above still lands.
    });
    await chrome.storage.local.set({ gaganfoxwell_id_pin_migrated: true });
  } catch (err) {
    console.debug('[gaganfoxwell] identity-pin notice failed (non-fatal):', err.message);
  }
}
...
loadPort().then(() => {
  ...
  const startupCheck = setInterval(async () => {
    ...
    if (isConnected || startupAttempts >= 15) {
      ...
      if (!isConnected) {
        console.log('[gaganfoxwell] Startup health checks failed after 15 attempts, falling back to 10s polling');
      }
    }
  }, 1000);
});
