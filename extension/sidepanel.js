/**
 * gaganfoxwell browse — Side Panel
 *
...
document.getElementById('chat-cookies-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('[gaganfoxwell sidebar] Failed to open cookie picker:', err.message);
  }
});
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn('[gaganfoxwell sidebar] Failed to mint SSE session cookie:', err && err.message);
    return false;
  }
}
...
async function connectSSE() {
  ...
  eventSource.addEventListener('activity', (e) => {
    try { addEntry(JSON.parse(e.data)); } catch (err) {
      console.error('[gaganfoxwell sidebar] Failed to parse activity event:', err.message);
    }
  });
  ...
  eventSource.addEventListener('gap', (e) => {
    try {
      ...
      console.error('[gaganfoxwell sidebar] Failed to parse gap event:', err.message);
    }
  });
}
...
async function pollMemoryOnce() {
  ...
  try {
    ...
    try { evaluateMemToast(snapshot); } catch (err) {
      console.debug('[gaganfoxwell sidebar] mem-toast evaluation failed:', err && err.message);
    }
    ...
    if (elapsed > MEM_POLL_SLOW_THRESHOLD_MS) {
      console.debug('[gaganfoxwell sidebar] /memory poll slow/failed:', elapsed, 'ms', err && err.message);
    }
...
async function loadSnoozeState() {
  ...
  try {
    ...
    console.debug('[gaganfoxwell sidebar] mem-toast snooze load failed:', err && err.message);
  }
}
...
async function saveSnoozeState() {
  ...
  try {
    ...
    console.debug('[gaganfoxwell sidebar] mem-toast snooze save failed:', err && err.message);
  }
}
  ...
  closeBtn.onclick = async () => {
    ...
    for (const id of ids) {
      try {
        ...
        console.warn('[gaganfoxwell sidebar] mem-toast closetab failed:', id, err && err.message);
      }
    }
  };
}
  ...
  if (snooze) {
    snooze.addEventListener('click', async () => {
      ...
      try {
        ...
        console.debug('[gaganfoxwell sidebar] mem-toast snooze fetch failed:', err && err.message);
      }
      dismissMemToast();
    });
  }
...
async function fetchRefs() {
  ...
  try {
    ...
    console.error('[gaganfoxwell sidebar] Failed to fetch refs:', err.message);
  }
}
...
inspectorSendBtn.addEventListener('click', async () => {
  ...
  // gaganfoxwellScanForPTYInject is async; gaganfoxwellInjectToTerminal stays sync.
  const verdict = await window.gaganfoxwellScanForPTYInject?.(message + '\n', 'inspector-send');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[gaganfoxwell sidebar] Inspector send BLOCKED by /pty-inject-scan:', verdict.reasons);
    return;
  }
  ...
  const ok = window.gaganfoxwellInjectToTerminal?.(message + '\n');
  if (!ok) {
    console.warn('[gaganfoxwell sidebar] Inspector send needs an active Terminal session.');
  }
});
...
async function runCleanup(...buttons) {
  ...
  // confirms every call site goes through gaganfoxwellScanForPTYInject first.
  const verdict = await window.gaganfoxwellScanForPTYInject?.(cleanupPrompt + '\n', 'cleanup-button');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[gaganfoxwell sidebar] Cleanup BLOCKED by /pty-inject-scan:', verdict.reasons);
  ...
  const sent = window.gaganfoxwellInjectToTerminal?.(cleanupPrompt + '\n');
  if (!sent) {
    console.warn('[gaganfoxwell sidebar] Cleanup needs an active Terminal session.');
  }
...
async function runScreenshot(...buttons) {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn('[gaganfoxwell sidebar] Screenshot failed:', text);
    } else {
      console.log('[gaganfoxwell sidebar] Screenshot:', text);
    }
  } catch (err) {
    console.error('[gaganfoxwell sidebar] Screenshot error:', err.message);
  } finally {
  ...
  try {
    ...
    inspectorSSE.addEventListener('inspectResult', (e) => {
      try {
        ...
        console.error('[gaganfoxwell sidebar] Failed to parse inspectResult:', err.message);
      }
    });
    ...
    console.debug('[gaganfoxwell sidebar] Inspector SSE not available:', err.message);
  }
}
...
function updateConnection(url, token) {
  ...
  if (url) {
    try { window.gaganfoxwellServerPort = parseInt(new URL(url).port, 10); } catch {}
    window.gaganfoxwellAuthToken = token || null;
  } else {
    window.gaganfoxwellServerPort = null;
    window.gaganfoxwellAuthToken = null;
  }
  if (url) {
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/open-gaganfoxwell-browser').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/open-gaganfoxwell-browser'; }, 2000);
  });
});
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'gaganfoxwell-migration-notice' && msg.message) {
    console.log('[gaganfoxwell sidebar]', msg.message);
    try {
      const banner = document.createElement('div');
      ...
      console.debug('[gaganfoxwell sidebar] migration banner failed:', err && err.message);
    }
  }
  ...
  if (msg.type === 'browserTabState') {
    document.dispatchEvent(new CustomEvent('gaganfoxwell:tab-state', {
...
window.addEventListener('pagehide', () => {
  const sessionId = window.gaganfoxwellPtySession;
  const authToken = window.gaganfoxwellAuthToken;
  const port = window.gaganfoxwellServerPort;
