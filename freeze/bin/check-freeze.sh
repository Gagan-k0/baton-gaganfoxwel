...
if [ ! -f "$_HOOK_HELPER" ] || ! . "$_HOOK_HELPER" 2>/dev/null; then
  printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"[freeze] Hook helpers unavailable (broken install?) - blocked, fail closed. Reinstall gaganfoxwell or run /unfreeze."}}\n'
  exit 0
fi

# Locate the freeze directory state file
STATE_DIR="${CLAUDE_PLUGIN_DATA:-$HOME/.gaganfoxwell}"
...
set +e
FILE_PATH=$(gaganfoxwell_hook_extract_field "$INPUT" file_path)
EXTRACT_RC=$?
set -e
...
if [ "$EXTRACT_RC" -ne 0 ] && [ -n "$INPUT" ]; then
  gaganfoxwell_hook_decision deny "[freeze] Could not parse the tool payload to check the freeze boundary. Blocked (fail closed). Freeze boundary: $FREEZE_DIR"
  exit 0
fi
...
case "$FILE_PATH" in
  ...
  *)
    ...
    # Log hook fire event (shared helper respects GAGANFOXWELL_HOME)
    gaganfoxwell_hook_log_fire freeze boundary_deny
    ...
    gaganfoxwell_hook_decision deny "[freeze] Blocked: $FILE_PATH is outside the freeze boundary ($FREEZE_DIR). Only edits within the frozen directory are allowed."
    ;;
esac
