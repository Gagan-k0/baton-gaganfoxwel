...
#       ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config set local_code_index_offered true
#
# When silent: anything else (Path 1/2/3 users, anyone already on PGLite,
# anyone who opted out, anyone without remote-http MCP).
#
# Idempotency: writes a touchfile at ~/.gaganfoxwell/.migrations/v1.37.0.0.done
...
GAGANFOXWELL_HOME="${GAGANFOXWELL_HOME:-$HOME/.gaganfoxwell}"
MIGRATIONS_DIR="$GAGANFOXWELL_HOME/.migrations"
DONE_TOUCH="$MIGRATIONS_DIR/v1.37.0.0.done"
CONFIG_BIN="$HOME/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config"
...
if is_remote_http_mcp && is_local_engine_missing; then
  ...
  ┌──────────────────────────────────────────────────────────────────┐
  │  gaganfoxwell v1.37.0.0 — split-engine gbrain                          │
  │                                                                  │
  ...
  │  permanently:                                                    │
  │    gaganfoxwell-config set local_code_index_offered true               │
  └──────────────────────────────────────────────────────────────────┘
