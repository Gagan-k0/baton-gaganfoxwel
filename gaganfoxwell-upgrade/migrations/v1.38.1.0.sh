...
#
# Why a migration: gaganfoxwell-artifacts-init regenerates these files but also
# does `git commit + push` on ~/.gaganfoxwell/, which would clobber user state on
...
if [ -f "${PRIVACY}" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    echo "  [v1.38.1.0] WARN: jq not found; skipping privacy-map repair. Install jq and re-run gaganfoxwell-upgrade, or run gaganfoxwell-artifacts-init manually." >&2
  fi
fi
...
# patches ship into their federated artifacts repo (next gaganfoxwell-brain-sync
