...
# (~/.claude/skills/gaganfoxwell), rewriting ~16 TRACKED SKILL.md files. The
# checkout stayed permanently dirty, and every /gaganfoxwell-upgrade `git stash`
...
# version. v1.67 renders to an untracked out-dir (~/.gaganfoxwell/render/claude)
...
#
# Affected: global-git installs that ever ran ./setup or `gaganfoxwell-config
# gbrain-refresh` with gbrain detected, before v1.67.0.0.
#
# Idempotent: a clean checkout is a no-op. Non-fatal throughout.
set -u

INSTALL_DIR="${GAGANFOXWELL_INSTALL_DIR:-$HOME/.claude/skills/gaganfoxwell}"

# Only operate on a real (non-symlink) git checkout that looks like gaganfoxwell.
...
if [ "$_RESTORED" -gt 0 ]; then
  ...
  echo "  Brain-aware blocks now render to ~/.gaganfoxwell/render/claude — the checkout stays clean from here on."
fi
if [ "$_LEFT" -gt 0 ]; then
