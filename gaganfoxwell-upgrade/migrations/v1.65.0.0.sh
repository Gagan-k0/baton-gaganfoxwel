...
# Info.plist ("Google Chrome for Testing" → "Gaganfoxwell Browser") and overwrote
...
#
# Idempotent: detection is content-based (plist contains "Gaganfoxwell Browser");
...
if [ -d "${PW_CACHE}" ]; then
  ...
  while IFS= read -r plist; do
    if grep -q "Gaganfoxwell Browser" "${plist}" 2>/dev/null; then
...
if [ "${REMOVED}" = "1" ]; then
  ...
  # headed use the same bundle). Run from the gaganfoxwell install root so bunx
