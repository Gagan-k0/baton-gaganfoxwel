...
# so the migration runner retries on the next /gaganfoxwell-upgrade.

set -u

GAGANFOXWELL_HOME="${HOME}/.gaganfoxwell"
ALLOWLIST="${GAGANFOXWELL_HOME}/.brain-allowlist"
PRIVACY="${GAGANFOXWELL_HOME}/.brain-privacy-map.json"
GITATTRS="${GAGANFOXWELL_HOME}/.gitattributes"
...
if [ -f "${PRIVACY}" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    if ! jq -e . "${PRIVACY}" >/dev/null 2>&1; then
      echo "  [v1.40.0.0] WARN: ${PRIVACY} is not valid JSON; skipping privacy-map repair. Fix manually or run gaganfoxwell-artifacts-init." >&2
      incomplete=1
    else
      for PATTERN in "${NEW_PATTERNS[@]}"; do
    ...
    echo "  [v1.40.0.0] WARN: jq not found; skipping privacy-map repair. Install jq and re-run gaganfoxwell-upgrade, or run gaganfoxwell-artifacts-init manually." >&2
    incomplete=1
  fi
fi
...
if [ "${incomplete}" = "0" ]; then
  ...
  # unnecessary. A fresh-init user's bin/gaganfoxwell-artifacts-init now writes the
  ...
  echo "  [v1.40.0.0] INFO: marker not written; gaganfoxwell-upgrade will retry once prerequisites are met." >&2
fi
