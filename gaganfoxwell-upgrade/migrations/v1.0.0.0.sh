...
# the older V0 prose can set `gaganfoxwell-config set explain_level terse`.
#
...
PROMPTED_FLAG="$GAGANFOXWELL_HOME/.writing-style-prompted"
PENDING_FLAG="$GAGANFOXWELL_HOME/.writing-style-prompt-pending"

mkdir -p "$GAGANFOXWELL_HOME"
...
EXPLAIN_LEVEL_SET="$("${HOME}/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config" get explain_level 2>/dev/null || true)"
