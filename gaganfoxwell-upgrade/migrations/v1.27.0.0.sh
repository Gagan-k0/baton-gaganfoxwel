#!/usr/bin/env bash
# Migration: v1.27.0.0 — rename gaganfoxwell-brain-* → gaganfoxwell-artifacts-*
#
# Phase C of the v1.27.0.0 plan. Hard-rename, no compat shim. Steps:
#   1. gh_repo_renamed       — gh/glab repo rename gaganfoxwell-brain-$USER →
#                              gaganfoxwell-artifacts-$USER (skipped on user opt-out)
#   2. remote_txt_renamed    — mv ~/.gaganfoxwell-brain-remote.txt → artifacts-remote.txt
#   3. config_key_renamed    — rewrite gbrain_sync_mode → artifacts_sync_mode
#                              in ~/.gaganfoxwell/config.yaml
...
#
# Interruption-safe via journal at ~/.gaganfoxwell/.migrations/v1.27.0.0.journal:
# each step writes its name on success; re-entry resumes from the next un-done
# step. Done touchfile at ~/.gaganfoxwell/.migrations/v1.27.0.0.done.
#
...
# Configuration
# ---------------------------------------------------------------------------
GAGANFOXWELL_HOME="${HOME}/.gaganfoxwell"
SKILLS_DIR="${HOME}/.claude/skills"
BIN_DIR="${SKILLS_DIR}/gaganfoxwell/bin"
CONFIG_BIN="${BIN_DIR}/gaganfoxwell-config"
URL_BIN="${BIN_DIR}/gaganfoxwell-artifacts-url"

MIGRATION_DIR="${GAGANFOXWELL_HOME}/.migrations"
...
RERUN_CMD="GAGANFOXWELL_MIGRATE_ASSUME_YES=1 bash ${SELF_PATH}"

USER_NAME="${USER:-$(whoami 2>/dev/null || echo unknown)}"
OLD_REPO_NAME="gaganfoxwell-brain-${USER_NAME}"
NEW_REPO_NAME="gaganfoxwell-artifacts-${USER_NAME}"
OLD_REMOTE_TXT="${HOME}/.gaganfoxwell-brain-remote.txt"
NEW_REMOTE_TXT="${HOME}/.gaganfoxwell-artifacts-remote.txt"
...
#   - presence of ~/.gaganfoxwell-brain-remote.txt (legacy file)
#   - presence of ~/.gaganfoxwell/.git (brain-init ever ran)
HAS_LEGACY_STATE=0
[ -f "$OLD_REMOTE_TXT" ] && HAS_LEGACY_STATE=1
[ -d "$GAGANFOXWELL_HOME/.git" ] && HAS_LEGACY_STATE=1

# If nothing to migrate, finalize silently.
if [ "$HAS_LEGACY_STATE" = "0" ]; then
  echo "  [v1.27.0.0] no legacy gaganfoxwell-brain state detected — nothing to migrate." >&2
  touch "$DONE"
...
if [ ! -f "$JOURNAL" ]; then
  ...
  [v1.27.0.0] gaganfoxwell-brain has been renamed to gaganfoxwell-artifacts.
  ...
  This migration will:
    ...
    2. mv ~/.gaganfoxwell-brain-remote.txt → ~/.gaganfoxwell-artifacts-remote.txt
    3. Rename gbrain_sync_mode → artifacts_sync_mode in ~/.gaganfoxwell/config.yaml
...
EOF
  if [ -t 0 ]; then
    ...
    # proceeds unattended.
    if [ "${GAGANFOXWELL_MIGRATE_ASSUME_YES:-0}" = "1" ]; then
      echo "  (non-interactive: proceeding — GAGANFOXWELL_MIGRATE_ASSUME_YES=1)" >&2
    else
...
# ---------------------------------------------------------------------------
# Step 2: rename ~/.gaganfoxwell-brain-remote.txt → ~/.gaganfoxwell-artifacts-remote.txt
# ---------------------------------------------------------------------------
if ! journal_done "remote_txt_renamed"; then
  echo "  [v1.27.0.0] step 2: rename ~/.gaganfoxwell-brain-remote.txt → ~/.gaganfoxwell-artifacts-remote.txt" >&2
  if [ -f "$OLD_REMOTE_TXT" ] && [ ! -f "$NEW_REMOTE_TXT" ]; then
    # Update the URL inside if the rename happened on the host: replace
    # gaganfoxwell-brain-$USER with gaganfoxwell-artifacts-$USER in the URL.
...
if ! journal_done "config_key_renamed"; then
  ...
  CFG="$GAGANFOXWELL_HOME/config.yaml"
  if [ -f "$CFG" ]; then
...
if ! journal_done "claude_md_block_rewritten"; then
  ...
  # Look in cwd's CLAUDE.md (where /setup-gbrain wrote it) and ~/.gaganfoxwell/CLAUDE.md
  ...
  for CMD in "$PWD/CLAUDE.md" "$GAGANFOXWELL_HOME/CLAUDE.md"; do
    [ -f "$CMD" ] || continue
...
EOF
  ...
  elif command -v gbrain >/dev/null 2>&1 && [ -d "$GAGANFOXWELL_HOME/.git" ]; then
    ...
    WORKTREE="${GAGANFOXWELL_BRAIN_WORKTREE:-$HOME/.gaganfoxwell-brain-worktree}"
    ...
    echo "    gbrain CLI not available or no ~/.gaganfoxwell/.git — skipping" >&2
    mark_done "sources_swapped"
  fi
fi
