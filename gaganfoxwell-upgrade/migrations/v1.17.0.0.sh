...
#
# Pre-1.17.0.0 /setup-gbrain wrote ~/.gaganfoxwell/consumers.json with a placeholder
...
#   - No ~/.gaganfoxwell/.git (brain-init never ran)
...
# CLI is a benign skip rather than blocking the rest of /gaganfoxwell-upgrade.
set -euo pipefail
...
WIREUP_BIN="${BIN_DIR}/gaganfoxwell-gbrain-source-wireup"

# Skip if user never opted into brain-sync.
SYNC_MODE=""
if [ -x "$CONFIG_BIN" ]; then
  # Trim whitespace defensively: gaganfoxwell-config can emit trailing newlines,
...
if [ ! -d "${HOME}/.gaganfoxwell/.git" ]; then
  exit 0
fi
