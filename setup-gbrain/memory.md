# gaganfoxwell memory ingest — what it does, what stays local, what you can do with it
...
| Eureka log | `eureka` | `~/.gaganfoxwell/analytics/eureka.jsonl` | Medium — your insights, often non-secret |
| Project learnings | `learning` | `~/.gaganfoxwell/projects/<slug>/learnings.jsonl` | Medium |
| Project timeline | `timeline` | `~/.gaganfoxwell/projects/<slug>/timeline.jsonl` | Low |
| CEO plans | `ceo-plan` | `~/.gaganfoxwell/projects/<slug>/ceo-plans/*.md` | Medium |
| Design docs | `design-doc` | `~/.gaganfoxwell/projects/<slug>/*-design-*.md` | Medium |
| Retros | `retro` | `~/.gaganfoxwell/projects/<slug>/retros/*.md` | Medium |
| Builder profile | `builder-profile-entry` | `~/.gaganfoxwell/builder-profile.jsonl` | Low |

## What stays local

- **State files** (`~/.gaganfoxwell/.gbrain-sync-state.json`,
  `~/.gaganfoxwell/.transcript-ingest-state.json`,
  `~/.gaganfoxwell/.gbrain-engine-cache.json`,
  `~/.gaganfoxwell/.gbrain-errors.jsonl`) are local-only per ED1 (state file
...
(`~/.gaganfoxwell/gbrain-repo-policy.json`, managed by
`gaganfoxwell-gbrain-repo-policy`). Each transcript's git remote is checked
...
- **Corrupted or unreadable store** — ingestion aborts before any writes
  ...
  `gaganfoxwell-gbrain-repo-policy list`; re-run `/setup-gbrain` if it's corrupt.
...
The cross-machine secret boundary is `gaganfoxwell-brain-sync` (the git push
...
```bash
bun run bin/gaganfoxwell-memory-ingest.ts --bulk --scan-secrets
# or
GAGANFOXWELL_MEMORY_INGEST_SCAN_SECRETS=1 bun run bin/gaganfoxwell-memory-ingest.ts --bulk
```
...
was skipped via `bun run bin/gaganfoxwell-memory-ingest.ts --probe` (which
...
- **Supabase configured:** code + transcripts go to Supabase Storage
  ...
  brain-linked git repo via `gaganfoxwell-brain-sync`.
...
- **Read a specific page:**
  ```bash
  gbrain get_page transcripts/claude-code/Gagan-k0-gaganfoxwell/2026-05-01-abc123
  ```
...
- **Bulk-delete by criteria** (V1.0.1 follow-up — `gaganfoxwell-transcript-prune`
  helper). For V1.0, use `gbrain delete_page <slug>` per-page or write
  a small loop over `gbrain list_pages` output.

- **Disable entirely:**
  ```bash
  gaganfoxwell-config set transcript_ingest_mode off
  gaganfoxwell-config set gbrain_context_load off  # also disables retrieval
  ```

## How the agent uses it

At every gaganfoxwell skill start, the preamble runs
`gaganfoxwell-brain-context-load` which:
...
- **Salience block is empty** — your transcripts may not be ingested
  yet. Run `bun run bin/gaganfoxwell-gbrain-sync.ts --full` to do a full pass.
...
- **A page has stale or wrong content** — `gbrain delete_page <slug>`,
  then re-run `bun run bin/gaganfoxwell-gbrain-sync.ts --incremental` to re-ingest from
  source if the source file is still on disk and unchanged.

## Privacy + audit

- Every `secretScanFile` finding is logged to stderr at ingest time.
- Every gbrain put/delete is logged to `~/.gaganfoxwell/.gbrain-errors.jsonl`
  with `{ts, op, duration_ms, outcome}` for forensic tracing.
- `~/.gaganfoxwell/.gbrain-engine-cache.json` shows which storage tier is
...
1. Verifies the URL + token via `gaganfoxwell-gbrain-mcp-verify`. Three failure
...
3. Skips local install, local doctor, transcript ingest, and federated
   ...
   doesn't install.
4. Optionally provisions a `gaganfoxwell-artifacts-$USER` private repo on
...
`gaganfoxwell-artifacts-init` always prints the `gbrain sources add` command
...
- Artifacts repo: github.com/Gagan-k0/baton-gaganfoxwel-artifacts-Gagan-k0 (private)
...
(See `gaganfoxwell/setup-gbrain/SKILL.md.tmpl` for the full Path 4 flow plus
the gbrain enhancement requests around scoped tokens that would let
gaganfoxwell auto-rotate in V2.)
