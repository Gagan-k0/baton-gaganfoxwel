#!/usr/bin/env bash
# hook-extract.sh — SHARED JSON helpers for gaganfoxwell PreToolUse hooks.
...
# gaganfoxwell_hook_extract_field PAYLOAD FIELD
...
gaganfoxwell_hook_extract_field() {
  _ghef_payload="$1"
  _ghef_field="$2"
...
# gaganfoxwell_hook_json_string TEXT
...
# gaganfoxwell_hook_decision DECISION REASON
...
gaganfoxwell_hook_decision() {
  _ghd_decision="$1"
  _ghd_reason="$2"
  _ghd_encoded=$(gaganfoxwell_hook_json_string "$_ghd_reason")
  printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"%s","permissionDecisionReason":%s}}\n' "$_ghd_decision" "$_ghd_encoded"
}

# gaganfoxwell_hook_log_fire SKILL PATTERN
#   Append a hook_fire analytics record (pattern name only, never command
#   content). Respects GAGANFOXWELL_HOME so tests never pollute the operator's real
#   analytics file. Best-effort: failures never affect the hook decision.
gaganfoxwell_hook_log_fire() {
  _ghlf_dir="${GAGANFOXWELL_HOME:-$HOME/.gaganfoxwell}/analytics"
  ...
  printf '{"event":"hook_fire","skill":%s,"pattern":%s,"ts":"%s","repo":%s}\n' \
    "$(gaganfoxwell_hook_json_string "$1")" \
    "$(gaganfoxwell_hook_json_string "$2")" \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    "$(gaganfoxwell_hook_json_string "$_ghlf_repo")" >> "$_ghlf_dir/skill-usage.jsonl" 2>/dev/null || true
}
