...
# unreadable input.
set +e
CMD=$(gaganfoxwell_hook_extract_field "$INPUT" command)
EXTRACT_RC=$?
set -e

# No parser available, or the payload is not parseable JSON. Fail closed.
if [ "$EXTRACT_RC" -ne 0 ] && [ -n "$INPUT" ]; then
  gaganfoxwell_hook_decision ask "[careful] Could not parse the tool payload to safety-check this command. Approve only if you know what it does."
  exit 0
fi
...
# Shared helper respects GAGANFOXWELL_HOME, so tests never write real analytics.
_careful_log_fire() { gaganfoxwell_hook_log_fire careful "$1"; }
...
if printf '%s' "$CMD" | grep -qE '\$\{IFS\}|\$IFS|\$\(echo[^)]*base64[^)]*\)|base64[[:space:]]+(-d|--decode)[^|]*\|[[:space:]]*(sh|bash)' 2>/dev/null; then
  gaganfoxwell_hook_decision ask "[careful] Shell obfuscation detected (IFS word-splitting or base64-to-shell). Read the command carefully before approving."
  exit 0
fi
...
# installer flows, including gaganfoxwell's own setup pattern.
_IS_SIMPLE=1
case "$CMD" in
  *';'*|*'&&'*|*'||'*|*'|'*|*$'\n'*) _IS_SIMPLE=0 ;;
esac
if [ "$_IS_SIMPLE" -eq 1 ]; then
  ...
  if printf '%s' "$CMD" | grep -qE '^[[:space:]]*(sudo[[:space:]]+)?rm[[:space:]]' 2>/dev/null \
    ...
    if [ "$_ROOT_TARGETS" -eq 1 ] && [ "$_SAFE_TARGETS" -eq 0 ]; then
      ...
      gaganfoxwell_hook_decision deny "[careful][HIGH] Recursive delete of / or the home directory is blocked while /careful is active. If you truly mean it, end the /careful session first."
      exit 0
    fi
  fi
    ...
    if [ "$_HAS_FORCE" -eq 1 ]; then
      ...
      if [ -n "$_DEFAULT_BRANCH" ]; then
        ...
        if [ "$_TARGETS_DEFAULT" -eq 1 ]; then
          ...
          gaganfoxwell_hook_decision deny "[careful][HIGH] Force-push to the default branch ($_DEFAULT_BRANCH) is blocked while /careful is active. Use --force-with-lease on a feature branch, or end the /careful session if you truly mean it."
          exit 0
        fi
      fi
    fi
...
if [ -z "$WARN" ]; then
  _GAGANFOXWELL_HOME_DIR="${GAGANFOXWELL_HOME:-$HOME/.gaganfoxwell}"
  _PATTERN_FILES="$_GAGANFOXWELL_HOME_DIR/careful-patterns.txt"
  ...
  _ANY_PROJ_PAT=$(find "$_GAGANFOXWELL_HOME_DIR/projects" -maxdepth 2 -name careful-patterns.txt -print -quit 2>/dev/null || true)
  if [ -n "$_ANY_PROJ_PAT" ]; then
    eval "$("$_HOOK_DIR/../../bin/gaganfoxwell-slug" 2>/dev/null)" 2>/dev/null || true
    if [ -n "${SLUG:-}" ]; then
      _PATTERN_FILES="$_PATTERN_FILES
$_GAGANFOXWELL_HOME_DIR/projects/$SLUG/careful-patterns.txt"
    fi
  fi
...
if [ -n "$WARN" ]; then
  ...
  gaganfoxwell_hook_decision ask "[careful] $WARN"
else
  echo '{}'
fi
