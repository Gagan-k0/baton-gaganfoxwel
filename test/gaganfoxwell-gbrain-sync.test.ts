/**
 * Unit tests for bin/gaganfoxwell-gbrain-sync.ts (Lane B).
 *
...
} from "../bin/gaganfoxwell-gbrain-sync";

const SCRIPT = join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-sync.ts");

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "gaganfoxwell-gbrain-sync-"));
}
...
describe("gaganfoxwell-gbrain-sync CLI", () => {
  it("--help exits 0 with usage text", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: gaganfoxwell-gbrain-sync");
  ...
  it("--dry-run with --code-only reports the code import preview only", () => {
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });

    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    expect(r.exitCode).toBe(0);
    ...
    expect(r.stdout).not.toContain("gaganfoxwell-memory-ingest --probe");
    expect(r.stdout).not.toContain("gaganfoxwell-brain-sync --discover-new");
  ...
  it("--dry-run with all stages shows previews for all three", () => {
    ...
    const r = runScript(["--dry-run"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("would: gbrain sources add");
    expect(r.stdout).toContain("gbrain sync --strategy code");
    expect(r.stdout).toContain("would: gaganfoxwell-memory-ingest");
    expect(r.stdout).toContain("would: gaganfoxwell-brain-sync");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("--no-code skips the code import stage", () => {
    ...
    const r = runScript(["--dry-run", "--no-code"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    expect(r.exitCode).toBe(0);
  ...
  it("dry-run derives a stable source id from the canonical git remote", () => {
    // The source id pattern is `gaganfoxwell-code-<canonicalized-remote>`. For this
    // repo (github.com/garrytan/gaganfoxwell), the slug should appear in the dry-run
    // preview line. We don't pin the exact slug — just verify the prefix +
    // that the preview command would target a source with id gaganfoxwell-code-*.
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    ...
    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/gbrain sources add gaganfoxwell-code-[a-z0-9-]+/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source gaganfoxwell-code-[a-z0-9-]+/);
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("uses a local .gbrain-source in dry-run without spawning gbrain", () => {
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    const bindir = mkdtempSync(join(tmpdir(), "gaganfoxwell-pinned-source-bin-"));
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-pinned-source-repo-"));
...
`);
    ...
    const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
      ...
      env: {
        ...process.env,
        HOME: home,
        GAGANFOXWELL_HOME: gaganfoxwellHome,
        GAGANFOXWELL_TEST_GBRAIN_LOG: commandLog,
  ...
  it("keeps a symlink-equivalent pinned source registered as-is", () => {
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-pinned-source-repo-"));
    const linkDir = mkdtempSync(join(tmpdir(), "gaganfoxwell-pinned-source-link-"));
...
`);
  ...
  it("uses a local pin for a dry-run dream without spawning gbrain", () => {
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    const bindir = mkdtempSync(join(tmpdir(), "gaganfoxwell-pinned-dream-bin-"));
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-pinned-dream-repo-"));
    mkdirSync(gaganfoxwellHome, { recursive: true });
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    writeFileSync(join(repo, ".gbrain-source"), "client-acme-app\n");
    writeFileSync(join(bindir, "gbrain"), "#!/bin/sh\nexit 99\n");
    ...
    const r = spawnSync("bun", [SCRIPT, "--dry-run", "--dream", "--no-code", "--no-memory", "--no-brain-sync", "--quiet"], {
      ...
      timeout: 60000,
      cwd: repo,
      env: { ...process.env, HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome, PATH: `${bindir}:${process.env.PATH || ""}` },
    });
  ...
  it("falls back to a derived source when .gbrain-source cannot be read", () => {
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-unreadable-pin-repo-"));
    mkdirSync(gaganfoxwellHome, { recursive: true });
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
      ...
      timeout: 60000,
      cwd: repo,
      env: { ...process.env, HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome },
    });

    expect(r.status).toBe(0);
    expect(r.stdout).toMatch(/gbrain sources add gaganfoxwell-code-/);
    rmSync(repo, { recursive: true, force: true });
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("derived source ids are gbrain-valid (≤32 chars, alnum + interior hyphens, no dots) for any remote", () => {
    ...
    // For `github.com/<org>/<repo>`, the id was `gaganfoxwell-code-github.com-<org>-<repo>`,
    ...
    const cases = [
      ...
      "git@github.com:garrytan/gaganfoxwell.git",            // SCP-style remote
    ...
    for (const remote of cases) {
      ...
      const gaganfoxwellHome = join(home, ".gaganfoxwell");
      mkdirSync(gaganfoxwellHome, { recursive: true });
      const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-source-id-repo-"));
      spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
      ...
      expect(id.startsWith("gaganfoxwell-code-")).toBe(true);
  ...
  it("derives a gbrain-valid source id when the cwd repo has NO origin remote", () => {
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-no-origin-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
  ...
  it("derives a gbrain-valid source id when the basename sanitizes to empty", () => {
    ...
    // returned "gaganfoxwell-code-" (invalid trailing hyphen) and was patched to
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const parent = mkdtempSync(join(tmpdir(), "gaganfoxwell-empty-base-"));
  ...
  it("derives distinct source ids for the same absolute path on different hosts", () => {
    ...
    // key keeps them distinct. `GAGANFOXWELL_HOSTNAME` env var is the test-only knob;
    // production uses `os.hostname()`.
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-host-collide-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    const bindir = mkdtempSync(join(tmpdir(), "gaganfoxwell-host-collide-bin-"));
    ...
    const runAs = (host: string) =>
      spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        ...
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome, GAGANFOXWELL_HOSTNAME: host, PATH },
      });
  ...
  it("dry-run does NOT acquire the lock file (lock is for write paths only)", () => {
    ...
    const lockPath = join(gaganfoxwellHome, ".sync-gbrain.lock");
  ...
  it("writes a state file with schema_version: 1 after a non-dry run", () => {
    ...
    const statePath = join(gaganfoxwellHome, ".gbrain-sync-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("gaganfoxwell-gbrain-sync");
  ...
  it("records stage results in state file", () => {
    ...
    const state = JSON.parse(readFileSync(join(gaganfoxwellHome, ".gbrain-sync-state.json"), "utf-8"));
  ...
  it("brain-sync stage resolves the sibling binary, not a HOME-rooted path", () => {
    ...
    // ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-sync, which silently no-op'd
    // on Codex installs and dev workspaces with the misleading summary
    // "skipped (gaganfoxwell-brain-sync not installed)". Post-fix it resolves
    ...
    const r = runScript(
      ...
      { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome },
    );
    ...
    expect(combined).not.toContain("skipped (gaganfoxwell-brain-sync not installed)");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("worktree-aware source ID: two worktrees of the same repo get DIFFERENT ids", () => {
    ...
    // ID was slug-only so both worktrees collapsed onto `gaganfoxwell-code-<slug>` and
    ...
    const remote = "https://github.com/garrytan/gaganfoxwell.git";
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });

    const repoA = mkdtempSync(join(tmpdir(), "gaganfoxwell-worktree-a-"));
    const repoB = mkdtempSync(join(tmpdir(), "gaganfoxwell-worktree-b-"));
    ...
    expect(idA).not.toBe(idB);
    expect(idA.startsWith("gaganfoxwell-code-")).toBe(true);
    expect(idB.startsWith("gaganfoxwell-code-")).toBe(true);
  ...
  it("worktree-aware source ID: same path produces the same id across runs (deterministic)", () => {
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-worktree-stable-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", remote], { cwd: repo });
  ...
  it("dry-run preview includes legacy-source removal + attach (post-codex-review hardening)", () => {
    // Codex adversarial flagged: pre-pathhash `gaganfoxwell-code-<slug>` sources stay
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-legacy-cleanup-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", "https://github.com/garrytan/gaganfoxwell.git"], { cwd: repo });
    ...
    // remove the legacy source via `gbrain sources remove gaganfoxwell-code-<slug>
    ...
    expect(r.stdout).toMatch(/gbrain sources add gaganfoxwell-code-/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source gaganfoxwell-code-/);
    expect(r.stdout).toMatch(/gbrain sources attach gaganfoxwell-code-/);
  ...
  it("dry-run preview includes the `sources attach` step (kubectl-style CWD pin)", () => {
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-attach-preview-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", "https://github.com/garrytan/gaganfoxwell.git"], { cwd: repo });
...
// creating a data-loss window. See bin/gaganfoxwell-gbrain-sync.ts and the
...
describe("derivePathOnlyHashLegacyId", () => {
  it("returns the pre-#1468 form (path-only sha1, no hostname)", () => {
    ...
    // legacy id regardless of $GAGANFOXWELL_HOSTNAME, because the pre-#1468 hash
    // didn't include hostname.
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-legacy-id-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    try {
      ...
      process.env.GAGANFOXWELL_HOSTNAME = "machine-a";
      const b = derivePathOnlyHashLegacyId(repo);
      process.env.GAGANFOXWELL_HOSTNAME = "machine-b";
      const c = derivePathOnlyHashLegacyId(repo);
      expect(a).toBe(b);
      expect(b).toBe(c);
      expect(a.startsWith("gaganfoxwell-code-")).toBe(true);
      expect(a.length).toBeLessThanOrEqual(32);
    } finally {
      delete process.env.GAGANFOXWELL_HOSTNAME;
      process.chdir(cwd);
      rmSync(repo, { recursive: true, force: true });
    }
  });
  ...
  it("produces a different id than the new hostname-folded form", () => {
    ...
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-legacy-id-distinct-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    try {
      process.chdir(repo);
      process.env.GAGANFOXWELL_HOSTNAME = "machine-x";
      const legacy = derivePathOnlyHashLegacyId(repo);
      // Drive the new id through the CLI so we use the same code path users hit.
      const home = makeTestHome();
      const gaganfoxwellHome = join(home, ".gaganfoxwell");
      mkdirSync(gaganfoxwellHome, { recursive: true });
      const bindir = mkdtempSync(join(tmpdir(), "gaganfoxwell-legacy-id-distinct-bin-"));
      makeShim(bindir, { "--help": { stdout: "gbrain\n" } });
      const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        encoding: "utf-8",
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome, GAGANFOXWELL_HOSTNAME: "machine-x", PATH: `${bindir}:${process.env.PATH || ""}` },
      });
...
describe("planHostnameFoldMigration", () => {
  ...
  beforeEach(() => {
    bindir = mkdtempSync(join(tmpdir(), "gaganfoxwell-mig-plan-bin-"));
    _resetGbrainSupportsRenameCache();
  });
  afterEach(() => {
    rmSync(bindir, { recursive: true, force: true });
  ...
  it("returns ids-match when legacy == new (degenerate case)", () => {
    const result = planHostnameFoldMigration("/repo/path", "gaganfoxwell-code-same-abc12345", "gaganfoxwell-code-same-abc12345");
...
describe("constrainSourceId truncation (hyphen-boundary cut)", () => {
  ...
  //
  // Exercised via the dry-run preview (`gbrain sources add gaganfoxwell-code-…`),
  // since constrainSourceId is module-private.
  it("never produces mid-word truncation artifacts like `kill` (from `skill`)", () => {
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-hyphen-cut-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
  ...
  it("produces a period-free source id for HTTPS remotes ending in .git (#1357)", () => {
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "gaganfoxwell-https-period-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
...
describe("sourceLocalPath", () => {
  let bindir: string;
  beforeEach(() => {
    bindir = mkdtempSync(join(tmpdir(), "gaganfoxwell-source-lp-bin-"));
  });
  afterEach(() => {
    rmSync(bindir, { recursive: true, force: true });
  });
