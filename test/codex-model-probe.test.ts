/**
 * _gaganfoxwell_codex_model_probe — round-trip model readiness (#2477).
 *
...
function makeFixture(): Fixture {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-model-probe-'));
  const stubDir = path.join(home, 'stub-bin');
  const codexHome = path.join(home, '.codex');
  const gaganfoxwellHome = path.join(home, '.gaganfoxwell');
...
describe('codex model probe (#2477)', () => {
  test('successful round trip -> MODEL_OK, cached, no re-invocation', () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(f.gaganfoxwellHome, '.codex-model-probe'))).toBe(true);
  ...
  test('TTL expiry: a cached MODEL_OK older than 3600s re-probes (T5)', () => {
    ...
    try {
      ...
      const cachePath = path.join(f.gaganfoxwellHome, '.codex-model-probe');
