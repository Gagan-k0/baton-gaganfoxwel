...
beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-egress-'));
});
...
describe('egress receipt library', () => {
  ...
  test('WARN-at-size: >25MB ledger emits one self-explanatory stderr warning per process', () => {
    ...
    expect(warnings[0]).toContain('gaganfoxwell-egress list');
...
describe('gaganfoxwell-egress-receipt shell bridge', () => {
  const bin = path.join(ROOT, 'bin', 'gaganfoxwell-egress-receipt');
  ...
  test('write hashes the exact payload file, prints the receipt id; outcome joins', () => {
    ...
    const write = spawnSync(bin, ['write', '--sink', 'telemetry-sync', '--host', '127.0.0.1:8399',
      ...
      { encoding: 'utf-8', env: { ...process.env, GAGANFOXWELL_HOME: home } });
