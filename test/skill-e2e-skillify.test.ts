 ...
 *      Skill written to ~/.gaganfoxwell/browser-skills/<name>/ with full
 ...
 *   - A scoped GAGANFOXWELL_HOME under the workdir so on-disk artifacts are
...
function setupSkillifyWorkdir(suffix: string, installSkills: string[] = ['scrape', 'skillify']): Workdir {
  ...
  for (const script of [
    'gaganfoxwell-timeline-log', 'gaganfoxwell-slug', 'gaganfoxwell-config',
    'gaganfoxwell-update-check', 'gaganfoxwell-repo-mode',
...
Environment:
- GAGANFOXWELL_HOME="${gaganfoxwellHome}" for all gaganfoxwell bin scripts.
- bin scripts are at ./bin/ relative to this directory.
- Browse binary is at ${browseBin} — assign to $B (e.g., \`B=${browseBin}\`).
`);

  return { workDir, gaganfoxwellHome, skillsDir };
}
 ...
 * has to live under <workdir>/.gaganfoxwell/browser-skills/ for the project-tier
 * lookup to find it (gaganfoxwell's bundled tier resolves from the install dir,
...
function installBundledHackernewsSkill(workDir: string) {
  ...
  const dst = path.join(workDir, '.gaganfoxwell', 'browser-skills', 'hackernews-frontpage');
  copyDirSync(src, dst);
}
...
describeIfSelected('Browser-skills Phase 2a E2E (/scrape + /skillify)', [
  ...
  testConcurrentIfSelected('scrape-match-path', async () => {
    const { workDir, gaganfoxwellHome } = setupSkillifyWorkdir('match', ['scrape']);
...
Do NOT enter the prototype phase. Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('scrape-prototype-path', async () => {
    const { workDir, gaganfoxwellHome } = setupSkillifyWorkdir('prototype', ['scrape']);
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('skillify-happy-path', async () => {
    const { workDir, gaganfoxwellHome } = setupSkillifyWorkdir('happy', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
...
(translates to ~/.gaganfoxwell/browser-skills/<name>/ via $HOME).

Do NOT halt for clarification.`,
      workingDirectory: workDir,
      env: {
        GAGANFOXWELL_HOME: gaganfoxwellHome,
        HOME: workDir,  // /skillify writes to $HOME/.gaganfoxwell/browser-skills/
      },
      maxTurns: 40,
    ...
    // The skill should land in $HOME/.gaganfoxwell/browser-skills/<name>/
    const skillsRoot = path.join(workDir, '.gaganfoxwell', 'browser-skills');
  ...
  testConcurrentIfSelected('skillify-provenance-refusal', async () => {
    const { workDir, gaganfoxwellHome } = setupSkillifyWorkdir('refusal', ['skillify']);
...
write any files.`,
    ...
    const skillsRoot = path.join(workDir, '.gaganfoxwell', 'browser-skills');
    const stagingRoot = path.join(workDir, '.gaganfoxwell', '.tmp');
  ...
  testConcurrentIfSelected('skillify-approval-reject', async () => {
    const { workDir, gaganfoxwellHome } = setupSkillifyWorkdir('reject', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
