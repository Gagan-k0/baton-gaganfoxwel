/**
 * Static invariant: every gaganfoxwellInjectToTerminal call in extension/*.js
 * must be preceded by an await on gaganfoxwellScanForPTYInject on the same code
 * path (#1370 / D6).
 *
 ...
 *
 *   Rule 1: every file that calls gaganfoxwellInjectToTerminal must also call
 *           gaganfoxwellScanForPTYInject.
 *
 *   Rule 2: in any function that calls gaganfoxwellInjectToTerminal, an
 *           `await ... gaganfoxwellScanForPTYInject` MUST appear before the
...
const INJECT_FN = 'gaganfoxwellInjectToTerminal';
const SCAN_FN = 'gaganfoxwellScanForPTYInject';
...
function findInjectCallSites(content: string): number[] {
  // Find positions of `gaganfoxwellInjectToTerminal(` or `gaganfoxwellInjectToTerminal?.(`
  // — but exclude the function DEFINITION (window.gaganfoxwellInjectToTerminal = ).
  const sites: number[] = [];
  const callRe = /window\.gaganfoxwellInjectToTerminal\s*\??\.?\s*\(/g;
  let match: RegExpExecArray | null;
  while ((match = callRe.exec(content)) !== null) {
    // Look back ~30 chars; if "window.gaganfoxwellInjectToTerminal =" appears
    ...
    if (window30.includes('gaganfoxwellInjectToTerminal =')) continue;
...
describe('extension/* PTY injection invariant (#1370 / D6)', () => {
  ...
  test('sidepanel-terminal.js defines both gaganfoxwellInjectToTerminal and gaganfoxwellScanForPTYInject', () => {
    const file = join(EXTENSION_DIR, 'sidepanel-terminal.js');
    const content = readFileSync(file, 'utf-8');
    expect(content).toContain('window.gaganfoxwellInjectToTerminal');
    expect(content).toContain('window.gaganfoxwellScanForPTYInject');
  });
  ...
  test('inject function stays synchronous (D6 contract preservation)', () => {
    ...
    const match = content.match(/window\.gaganfoxwellInjectToTerminal\s*=\s*(async\s+)?function/);
