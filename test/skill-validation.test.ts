...
describe('SKILL.md command validation', () => {
  // P2 (v1.2.0): the top-level gaganfoxwell skill is a pure ROUTER, not the browse
  ...
  test('top-level SKILL.md is a router with no browse body (P2)', () => {
    ...
    expect(md).not.toContain('gaganfoxwell browse: QA Testing'); // browse body removed
...
describe('Cross-skill path consistency', () => {
  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('~/.gaganfoxwell/greptile-history'));
  ...
  test('greptile-triage.md contains both project and global history paths', () => {
    ...
    expect(content).toContain('~/.gaganfoxwell/greptile-history.md');
  });
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.gaganfoxwell/qa-reports/');
  });
});
...
describe('office-hours skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=gaganfoxwell');
  });
  ...
  test('contains Garry Tan personal plea', () => {
    expect(content).toContain('Garry Tan, the creator of Gaganfoxwell');
  });
...
// --- gaganfoxwell-slug helper ---

describe('gaganfoxwell-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'gaganfoxwell-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/gaganfoxwell-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for gaganfoxwell-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*gaganfoxwell-slug', '--include=*.tmpl', '--include=gaganfoxwell-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .gaganfoxwell/qa-reports/');
  });
...
describe('Codex skill', () => {
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('gaganfoxwell-review-log');
  });
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'gaganfoxwell-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'gaganfoxwell-review', 'SKILL.md'), 'utf-8');
  ...
  test('codex-host document-release does NOT contain the Codex doc review', () => {
    ...
    const content = fs.readFileSync(
      path.join(ROOT, '.agents', 'skills', 'gaganfoxwell-document-release', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root gaganfoxwell (browser tool), gaganfoxwell-upgrade (gaganfoxwell-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  test('all skills (except /codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('gaganfoxwell-') ? skillDir : `gaganfoxwell-${skillDir}`;
  ...
  test('Codex skill names follow gaganfoxwell-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with gaganfoxwell
      expect(dir.startsWith('gaganfoxwell')).toBe(true);
      // Root is just 'gaganfoxwell', others are 'gaganfoxwell-{name}'
      if (dir !== 'gaganfoxwell') {
        expect(dir.startsWith('gaganfoxwell-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('gaganfoxwell-repo-mode');
  });
...
//
// Browser-skills are bundled in <gaganfoxwell-root>/browser-skills/<name>/. Each
