...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    gaganfoxwellHome = path.join(workDir, '.gaganfoxwell-home');
    ...
    // Compute slug (same logic as gaganfoxwell-slug without git remote)
  ...
  // Write a timeline event via gaganfoxwell-timeline-log, read it back via gaganfoxwell-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    const projectDir = path.join(gaganfoxwellHome, 'projects', slug);
    fs.mkdirSync(projectDir, { recursive: true });

    // Write two events via the binary
    const logBin = path.join(workDir, 'bin', 'gaganfoxwell-timeline-log');
    const readBin = path.join(workDir, 'bin', 'gaganfoxwell-timeline-read');
    const env = { ...process.env, GAGANFOXWELL_HOME: gaganfoxwellHome };
    ...
    // Read via gaganfoxwell-timeline-read
    ...
    // Verify gaganfoxwell-timeline-read output includes the events
...
IMPORTANT:
- Use GAGANFOXWELL_HOME="${gaganfoxwellHome}" as an environment variable when running bin scripts.
- The bin scripts are at ./bin/ (relative to this directory), not at ~/.claude/skills/gaganfoxwell/bin/.
  Replace any references to ~/.claude/skills/gaganfoxwell/bin/ with ./bin/ when running commands.
