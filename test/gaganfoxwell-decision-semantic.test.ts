/**
 * Tests for lib/gaganfoxwell-decision-semantic.ts — the OPTIONAL gbrain enhancement.
 *
...
import {
  ...
  semanticRecall,
} from "../lib/gaganfoxwell-decision-semantic";
...
describe("end-to-end with a fake gbrain shim", () => {
  ...
  test("resolves the worktree-backed source and scopes search to it", () => {
    writeShim(
      `#!/usr/bin/env bash
if [ "$1" = "sources" ]; then
  echo '{"sources":[{"id":"code","local_path":"/repo","page_count":100},{"id":"default","local_path":"/u/.gaganfoxwell-brain-worktree","page_count":3}]}'
  exit 0
fi
if [ "$1" = "search" ]; then
