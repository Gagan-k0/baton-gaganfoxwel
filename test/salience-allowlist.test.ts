...
const ORIGINAL_ENV = process.env.GAGANFOXWELL_SALIENCE_ALLOWLIST;

beforeEach(() => {
  delete require.cache[require.resolve('../bin/gaganfoxwell-brain-cache')];
});

afterEach(() => {
  if (ORIGINAL_ENV) process.env.GAGANFOXWELL_SALIENCE_ALLOWLIST = ORIGINAL_ENV;
  else delete process.env.GAGANFOXWELL_SALIENCE_ALLOWLIST;
});

async function importCache(): Promise<typeof import('../bin/gaganfoxwell-brain-cache')> {
  return (await import('../bin/gaganfoxwell-brain-cache')) as typeof import('../bin/gaganfoxwell-brain-cache');
}

describe('salience allowlist gate', () => {
  test('default allowlist permits projects/ + gaganfoxwell/ + concepts/', async () => {
    const mod = await importCache();
    expect(mod.isSalienceSlugAllowed('projects/myrepo', SALIENCE_DEFAULT_ALLOWLIST)).toBe(true);
    expect(mod.isSalienceSlugAllowed('gaganfoxwell/product/helsinki', SALIENCE_DEFAULT_ALLOWLIST)).toBe(true);
  ...
  test('getSalienceAllowlist returns default when env unset and config silent', async () => {
    delete process.env.GAGANFOXWELL_SALIENCE_ALLOWLIST;
    const mod = await importCache();
    ...
    expect(list).toContain('gaganfoxwell/');
  });

  test('GAGANFOXWELL_SALIENCE_ALLOWLIST env override is honored', async () => {
    process.env.GAGANFOXWELL_SALIENCE_ALLOWLIST = 'custom-a/,custom-b/,custom-c/';
    const mod = await importCache();
    const list = mod.getSalienceAllowlist();
    expect(list).toEqual(['custom-a/', 'custom-b/', 'custom-c/']);
  });

  test('GAGANFOXWELL_SALIENCE_ALLOWLIST with whitespace is trimmed', async () => {
    process.env.GAGANFOXWELL_SALIENCE_ALLOWLIST = ' projects/ , gaganfoxwell/ , concepts/ ';
    const mod = await importCache();
    const list = mod.getSalienceAllowlist();
    expect(list).toEqual(['projects/', 'gaganfoxwell/', 'concepts/']);
  });

  test('empty env value falls through to default (not empty list)', async () => {
    process.env.GAGANFOXWELL_SALIENCE_ALLOWLIST = '';
    const mod = await importCache();
    const list = mod.getSalienceAllowlist();
    expect(list.length).toBeGreaterThan(0);
  });
