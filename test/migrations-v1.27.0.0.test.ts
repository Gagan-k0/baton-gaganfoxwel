/**
 * v1.27.0.0 migration — gaganfoxwell-brain → gaganfoxwell-artifacts rename.
 *
...
const MIGRATION = path.join(ROOT, 'gaganfoxwell-upgrade', 'migrations', 'v1.27.0.0.sh');
  ...
  repo)
    ...
    case "$1" in
      view)
        # gh repo view <name>
        shift
        ${alreadyRenamed ? `if echo "$@" | grep -q gaganfoxwell-artifacts; then exit 0; else exit 1; fi` : `exit 1`}
        ;;
...
case "$1 $2" in
  "sources list")
    ${hasOld ? `echo "gaganfoxwell-brain-testuser ~/.gaganfoxwell-brain-worktree"` : 'true'}
    exit 0
    ;;
...
function run(extraEnv: Record<string, string> = {}, input = ''): { code: number; stdout: string; stderr: string } {
  const r = spawnSync(MIGRATION, [], {
    env: {
      ...
      GAGANFOXWELL_MIGRATE_ASSUME_YES: '1',
      ...extraEnv,
    },
    encoding: 'utf-8',
    input,
...
beforeEach(() => {
  ...
  fs.mkdirSync(path.join(tmpHome, '.gaganfoxwell'), { recursive: true });
});
...
describe('v1.27.0.0 migration — nothing to migrate', () => {
  test('no legacy state → exits 0, writes done touchfile, no journal', () => {
    // Fresh HOME: no brain-remote.txt, no .gaganfoxwell/.git
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toContain('nothing to migrate');
    expect(fs.existsSync(path.join(tmpHome, '.gaganfoxwell/.migrations/v1.27.0.0.done'))).toBe(true);
    expect(fs.existsSync(path.join(tmpHome, '.gaganfoxwell/.migrations/v1.27.0.0.journal'))).toBe(false);
  });

  test('done touchfile present → exits 0 silently (no re-prompt)', () => {
    fs.mkdirSync(path.join(tmpHome, '.gaganfoxwell/.migrations'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.gaganfoxwell/.migrations/v1.27.0.0.done'), '');
    const r = run();
    expect(r.code).toBe(0);
  ...
  test('skipped-by-user touchfile → exits 0 silently', () => {
    fs.mkdirSync(path.join(tmpHome, '.gaganfoxwell/.migrations'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.gaganfoxwell/.migrations/v1.27.0.0.skipped-by-user'), '');
    fs.writeFileSync(path.join(tmpHome, '.gaganfoxwell-brain-remote.txt'), 'https://github.com/x/gaganfoxwell-brain-testuser');
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toBe('');
  });
...
describe('v1.27.0.0 migration — GitHub host (non-interactive)', () => {
  beforeEach(() => {
    fs.writeFileSync(
      ...
      'https://github.com/testuser/gaganfoxwell-brain-testuser\n'
    );
    fs.writeFileSync(
      path.join(tmpHome, '.gaganfoxwell/config.yaml'),
  ...
  test('renames repo, mvs remote.txt, rewrites config key, writes done', () => {
    ...
    expect(fs.existsSync(path.join(tmpHome, '.gaganfoxwell-brain-remote.txt'))).toBe(false);
    const newUrl = fs.readFileSync(path.join(tmpHome, '.gaganfoxwell-artifacts-remote.txt'), 'utf-8').trim();
    expect(newUrl).toBe('https://github.com/testuser/gaganfoxwell-artifacts-testuser');
    // Config key renamed.
    const cfg = fs.readFileSync(path.join(tmpHome, '.gaganfoxwell/config.yaml'), 'utf-8');
...
describe('v1.27.0.0 migration — #1383 consent + failure-stays-pending contract', () => {
  ...
  test('non-interactive without opt-in: skips for now, touches NOTHING', () => {
    const r = run({ GAGANFOXWELL_MIGRATE_ASSUME_YES: '0' });
    expect(r.code).toBe(0);
    expect(r.stderr).toContain('skipping for now');
    expect(r.stderr).toContain('GAGANFOXWELL_MIGRATE_ASSUME_YES=1');
    ...
    expect(fs.existsSync(path.join(tmpHome, '.gaganfoxwell-brain-remote.txt'))).toBe(true);
    expect(fs.existsSync(path.join(tmpHome, '.gaganfoxwell-artifacts-remote.txt'))).toBe(false);
    expect(fs.existsSync(path.join(tmpHome, '.gaganfoxwell/.migrations/v1.27.0.0.done'))).toBe(false);
  });
  ...
  test('gh rename failure: step stays pending, migration exits INCOMPLETE, retry succeeds', () => {
    ...
    const journal = fs.readFileSync(
      path.join(tmpHome, '.gaganfoxwell/.migrations/v1.27.0.0.journal'),
      'utf-8'
    );
...
describe('v1.27.0.0 migration — interruption resume', () => {
  ...
  test('partial journal: skips already-done steps', () => {
    ...
    const migDir = path.join(tmpHome, '.gaganfoxwell/.migrations');
...
describe('v1.27.0.0 migration — local CLI sources swap (codex Finding #6 ordering)', () => {
  test('add-new before remove-old (verify by call order in log)', () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.gaganfoxwell/.git'), { recursive: true }); // brain repo present
    makeFakeGh({});
    ...
    const log = fs.readFileSync(path.join(fakeBinDir, 'gbrain-calls.log'), 'utf-8');
    const addIdx = log.indexOf('gbrain sources add gaganfoxwell-artifacts-testuser');
    const removeIdx = log.indexOf('gbrain sources remove gaganfoxwell-brain-testuser');
  ...
  test('add fails → old source stays registered (no silent loss)', () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.gaganfoxwell/.git'), { recursive: true });
    makeFakeGh({});
