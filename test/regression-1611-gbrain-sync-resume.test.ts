 ...
 *   - resolveStageTimeoutMs (gaganfoxwell-gbrain-sync.ts) — env parsing + bounds
 *   - decideResume          (gaganfoxwell-gbrain-sync.ts) — checkpoint+staging detection
 *   - SIGTERM staging preservation invariants in gaganfoxwell-memory-ingest.ts
 *
...
import { stagedRelPath, readNewFailures } from "../bin/gaganfoxwell-memory-ingest";
...
describe("#1611 resolveStageTimeoutMs — env parsing + bounds", () => {
  test("undefined env → default 2_100_000ms (unchanged from prior behavior)", () => {
    expect(resolveStageTimeoutMs(undefined, "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("empty string env → default", () => {
    expect(resolveStageTimeoutMs("", "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("non-numeric env → warn + default", () => {
    expect(resolveStageTimeoutMs("not-a-number", "GAGANFOXWELL_SYNC_CODE_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("zero env → warn + default (not positive)", () => {
    expect(resolveStageTimeoutMs("0", "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("negative env → warn + default", () => {
    expect(resolveStageTimeoutMs("-1000", "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("below 60_000ms floor (1min) → warn + default", () => {
    expect(resolveStageTimeoutMs("30000", "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
    expect(resolveStageTimeoutMs(`${MIN_MS - 1}`, "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("above 86_400_000ms ceiling (24h) → warn + default", () => {
    expect(resolveStageTimeoutMs(`${MAX_MS + 1}`, "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
    expect(resolveStageTimeoutMs("999999999999", "GAGANFOXWELL_SYNC_CODE_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("at floor (60_000ms exactly) → accepted", () => {
    expect(resolveStageTimeoutMs(`${MIN_MS}`, "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(MIN_MS);
  });

  test("at ceiling (86_400_000ms exactly) → accepted", () => {
    expect(resolveStageTimeoutMs(`${MAX_MS}`, "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(MAX_MS);
  });

  test("valid mid-range (2h = 7_200_000ms) → returns value", () => {
    expect(resolveStageTimeoutMs("7200000", "GAGANFOXWELL_SYNC_MEMORY_TIMEOUT_MS")).toBe(7_200_000);
  });
});
...
describe("#1611 decideResume — checkpoint + staging detection", () => {
  ...
  beforeEach(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-1611-"));
  ...
  test("checkpoint + minted staging dir exists → resume verdict", () => {
    ...
    // gaganfoxwellHome is injected so the ownership check anchors on the test home.
...
describe("#1611 SIGTERM staging preservation — static invariants", () => {
  test("memory-ingest signal handler checks stagingDirIsCheckpointed before cleanup", () => {
    const body = fs.readFileSync(
      path.join(ROOT, "bin", "gaganfoxwell-memory-ingest.ts"),
      "utf-8",
    );
  ...
  test("memory-ingest reads GAGANFOXWELL_INGEST_RESUME_DIR env to reuse staging dir", () => {
    const body = fs.readFileSync(
      path.join(ROOT, "bin", "gaganfoxwell-memory-ingest.ts"),
      "utf-8",
    );
    expect(body).toMatch(/process\.env\.GAGANFOXWELL_INGEST_RESUME_DIR/);
    expect(body).toMatch(/skipping prepare phase/);
  });

  test("gbrain-sync orchestrator passes GAGANFOXWELL_INGEST_RESUME_DIR to grandchild on resume", () => {
    const body = fs.readFileSync(
      path.join(ROOT, "bin", "gaganfoxwell-gbrain-sync.ts"),
      "utf-8",
    );
    expect(body).toMatch(/GAGANFOXWELL_INGEST_RESUME_DIR/);
...
describe("#1802 checkOwnedStagingDir — ownership matrix", () => {
  ...
  beforeEach(() => {
    home = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-1802-"));
  });
  afterEach(() => {
  ...
  test("cleanupStagingDir + decideResume both call the guard (static invariant)", () => {
    const ingest = fs.readFileSync(path.join(ROOT, "bin", "gaganfoxwell-memory-ingest.ts"), "utf-8");
    const sync = fs.readFileSync(path.join(ROOT, "bin", "gaganfoxwell-gbrain-sync.ts"), "utf-8");
    expect(ingest).toMatch(/checkOwnedStagingDir\(dir, GAGANFOXWELL_HOME\)/);
    expect(ingest).toMatch(/staging cleanup REFUSED/);
    expect(sync).toMatch(/checkOwnedStagingDir\(stagingDir, gaganfoxwellHome\)/);
  });
});
...
// (makePersistentTranscriptDir, under ~/.gaganfoxwell/transcripts/) that
// gaganfoxwell-brain-sync push consumes. The finally runs on the remote-http `return`,
...
describe("#1802 C4 — resume failure mapping (behavioral)", () => {
  ...
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-1802c4-"));
    cpHome = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-1802c4-fail-"));
  });
  afterEach(() => {
