...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root gaganfoxwell' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('package.json version matches VERSION file (npm-valid translation)', () => {
    ...
    // (grandfathered until the next write), matching gaganfoxwell-version-bump's
    // own drift contract.
  ...
  test('generated SKILL.md contains operational self-improvement (replaced contributor mode)', () => {
    ...
    expect(content).not.toContain('gaganfoxwell_contributor');
    expect(content).not.toContain('contributor-logs');
    expect(content).toContain('Operational Self-Improvement');
    expect(content).toContain('gaganfoxwell-learnings-log');
    expect(content).toContain('gaganfoxwell-learnings-search --limit 3');
  });
  ...
  test('generated SKILL.md contains telemetry line', () => {
    ...
    expect(content).toContain('~/.gaganfoxwell/analytics');
  });
  ...
  test('plan-review generated preambles stay under the Option A budget', () => {
    ...
    // resolver gained the <gaganfoxwell-qid:...> marker convention + the
  ...
  test('slim voice section preserves the gaganfoxwell voice contract', () => {
    const content = readSkillUnion('plan-eng-review'); // carved: review body moved to section
    const voice = extractMarkdownSection(content, '## Voice');
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      expect(content).toContain("find ~/.gaganfoxwell/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('gaganfoxwell-review');
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('GAGANFOXWELL REVIEW REPORT');
    });
  }
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = readSkillUnion('office-hours'); // carved: Phase 5/6 prose moved to section
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'gaganfoxwell-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('gaganfoxwell-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('gaganfoxwell-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/gaganfoxwell' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/gaganfoxwell');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('gaganfoxwell')) {
        skills.push({ dir: '.', codexName: 'gaganfoxwell' });
      }
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      ...
      const codexName = entry.name.startsWith('gaganfoxwell-') ? entry.name : `gaganfoxwell-${entry.name}`;
  ...
  test('root gaganfoxwell bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "gaganfoxwell"');
    expect(content).toContain('Use $gaganfoxwell to locate the bundled gaganfoxwell skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('externalSkillName mapping: root is gaganfoxwell, others are gaganfoxwell-{dir}', () => {
    // Root → gaganfoxwell
    expect(fs.existsSync(path.join(AGENTS_DIR, 'gaganfoxwell', 'SKILL.md'))).toBe(true);
    // Subdirectories → gaganfoxwell-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'gaganfoxwell-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'gaganfoxwell-ship', 'SKILL.md'))).toBe(true);
    // gaganfoxwell-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'gaganfoxwell-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: gaganfoxwell-gaganfoxwell-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'gaganfoxwell-gaganfoxwell-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'gaganfoxwell-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'gaganfoxwell-codex'))).toBe(false);
  });

  test('Codex output includes Claude outside-voice skill with read-only boundary', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-claude', 'SKILL.md'), 'utf-8');
    expect(content).toContain('claude -p');
    expect(content).toContain('mktemp /tmp/gaganfoxwell-claude-prompt-');
    expect(content).toContain('mktemp /tmp/gaganfoxwell-claude-response-XXXXXX');
    expect(content).toContain('mktemp /tmp/gaganfoxwell-claude-error-XXXXXX');
    expect(content).toContain('mktemp /tmp/gaganfoxwell-claude-diff-');
    expect(content).not.toMatch(/gaganfoxwell-claude-(?:prompt|response|error|diff)-X{6,}\.\w+/);
    expect(content).not.toContain('/tmp/gaganfoxwell-claude-diff-$$');
  ...
  test('Claude temp file templates are accepted by host mktemp', () => {
    for (const template of [
      '/tmp/gaganfoxwell-claude-prompt-XXXXXX',
      '/tmp/gaganfoxwell-claude-response-XXXXXX',
      '/tmp/gaganfoxwell-claude-error-XXXXXX',
      '/tmp/gaganfoxwell-claude-diff-XXXXXX',
    ]) {
  ...
  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['gaganfoxwell-careful', 'gaganfoxwell-freeze', 'gaganfoxwell-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global gaganfoxwell roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('GAGANFOXWELL_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/gaganfoxwell');
    expect(content).toContain('$GAGANFOXWELL_BIN/gaganfoxwell-config');
    expect(content).toContain('$GAGANFOXWELL_ROOT/gaganfoxwell-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/gaganfoxwell/bin/gaganfoxwell-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths resolve through $GAGANFOXWELL_ROOT (not gaganfoxwell-review/)', () => {
    // #2518: templates now anchor sidecars at the installed skill root
    // (~/.claude/skills/gaganfoxwell/review/...), which the codex path rewrite turns
    // into $GAGANFOXWELL_ROOT/review/... — resolved by the preamble against the
    // repo-local .agents root or the global install. The old repo-relative
    // form (.claude/skills/review/) only resolved inside gaganfoxwell's own checkout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('$GAGANFOXWELL_ROOT/review/checklist.md');
    // design-checklist.md is now referenced via Review Army specialist (Claude only, stripped for Codex)
    // Wrong: must NOT reference gaganfoxwell-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/gaganfoxwell-review/checklist.md');
  });

  test('sidecar paths in ship skill point to gaganfoxwell/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/gaganfoxwell/review/');
      expect(content).not.toContain('.agents/skills/gaganfoxwell-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('$GAGANFOXWELL_ROOT/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/gaganfoxwell-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/gaganfoxwell → $GAGANFOXWELL_ROOT
    expect(content).not.toContain('~/.claude/skills/gaganfoxwell');
    expect(content).toContain('$GAGANFOXWELL_ROOT');

    // Rule 2: .claude/skills/gaganfoxwell → .agents/skills/gaganfoxwell
    expect(content).not.toContain('.claude/skills/gaganfoxwell');

    // Rule 3: .claude/skills/review → .agents/skills/gaganfoxwell/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('gaganfoxwell-config') || content.includes('gaganfoxwell-update-check') || content.includes('gaganfoxwell-telemetry-log')) {
        expect(content).toContain('$GAGANFOXWELL_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('gaganfoxwell-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output uses installed-root review paths (#2518)', () => {
    ...
    // resolved inside gaganfoxwell's own checkout.
    const content = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('~/.claude/skills/gaganfoxwell/review/checklist.md');
    expect(content).toContain('~/.claude/skills/gaganfoxwell');
    ...
    // codex skill), not the gaganfoxwell Codex host install path.
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      // not the gaganfoxwell Codex host install path. ~/.codex/sessions/ (rollout
      ...
      // gaganfoxwell-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'gaganfoxwell-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-ship', 'SKILL.md'), 'utf-8');
  ...
  test('explicit --model overrides the codex host default', () => {
    ...
    const restored = fs.readFileSync(path.join(AGENTS_DIR, 'gaganfoxwell-ship', 'SKILL.md'), 'utf-8');
...
describe('Factory generation (--host factory)', () => {
  ...
  const FACTORY_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('gaganfoxwell')) skills.push({ dir: '.', factoryName: 'gaganfoxwell' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue;
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const factoryName = entry.name.startsWith('gaganfoxwell-') ? entry.name : `gaganfoxwell-${entry.name}`;
  ...
  test('sensitive skills have disable-model-invocation', () => {
    const SENSITIVE = ['gaganfoxwell-ship', 'gaganfoxwell-land-and-deploy', 'gaganfoxwell-guard', 'gaganfoxwell-careful', 'gaganfoxwell-freeze', 'gaganfoxwell-unfreeze'];
  ...
  test('non-sensitive skills lack disable-model-invocation', () => {
    const NON_SENSITIVE = ['gaganfoxwell-qa', 'gaganfoxwell-review', 'gaganfoxwell-investigate', 'gaganfoxwell-browse'];
  ...
  test('/codex skill excluded from Factory output', () => {
    expect(fs.existsSync(path.join(FACTORY_DIR, 'gaganfoxwell-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(FACTORY_DIR, 'gaganfoxwell-codex'))).toBe(false);
  });

  test('Factory keeps Codex integration blocks', () => {
    // Factory users CAN use Codex second opinions (codex exec is a standalone binary)
    const shipContent = fs.readFileSync(path.join(FACTORY_DIR, 'gaganfoxwell-ship', 'SKILL.md'), 'utf-8');
  ...
  test('Factory preamble uses .factory paths', () => {
    const content = fs.readFileSync(path.join(FACTORY_DIR, 'gaganfoxwell-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('GAGANFOXWELL_ROOT');
    expect(content).toContain('$_ROOT/.factory/skills/gaganfoxwell');
    expect(content).toContain('$GAGANFOXWELL_BIN/gaganfoxwell-config');
  });
});
    ...
    describe(`${hostConfig.displayName} (--host ${hostConfig.name})`, () => {
      ...
      test('no .claude/skills path leakage outside repo-root sidecar symlinks', () => {
        ...
        for (const skill of skills) {
          // Dev installs may mount the repo root at host/skills/gaganfoxwell as a runtime
      ...
      if (hostConfig.generation.skipSkills?.includes('codex')) {
        test('/codex skill excluded', () => {
          expect(fs.existsSync(path.join(hostDir, 'gaganfoxwell-codex', 'SKILL.md'))).toBe(false);
        });
      }
    });
  }
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('_link_or_copy "$GAGANFOXWELL_DIR" "$CODEX_GAGANFOXWELL"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_GAGANFOXWELL_DIR=');
    expect(setupContent).toContain('SOURCE_GAGANFOXWELL_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_GAGANFOXWELL="$INSTALL_GAGANFOXWELL_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_GAGANFOXWELL_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_GAGANFOXWELL_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('gaganfoxwell*');
  });
  ...
  test('link_claude_skill_dirs creates real directories with absolute SKILL.md symlinks', () => {
    ...
    // to ensure Claude Code discovers them as top-level skills (not nested under gaganfoxwell/)
    ...
    // rendered :user variant from ${GAGANFOXWELL_HOME}/render/claude when present.
    expect(fnBody).toContain('_skill_md_src="$gaganfoxwell_dir/$dir_name/SKILL.md"');
  ...
  test('setup links root gaganfoxwell skill through a thin Claude wrapper alias', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('_gaganfoxwell-command');
    // #2511: the alias must be a rewritten COPY (unique frontmatter name),
    // never a verbatim symlink of the canonical SKILL.md.
    expect(fnBody).toContain('_install_alias_skill_md "$gaganfoxwell_dir/SKILL.md" "$target" "_gaganfoxwell-command"');
    expect(fnBody).not.toContain('_link_or_copy "$gaganfoxwell_dir/SKILL.md"');

    const claudeSection = setupContent.slice(
      setupContent.indexOf('# 4. Install for Claude'),
      setupContent.indexOf('# 5. Install for Codex')
    );
    expect(claudeSection).toContain('link_claude_root_skill_alias "$SOURCE_GAGANFOXWELL_DIR" "$INSTALL_SKILLS_DIR"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/gaganfoxwell from being linked as a skill
  test('link_codex_skill_dirs skips the gaganfoxwell sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "gaganfoxwell" ] && continue');
  });

  // T2: Dynamic $GAGANFOXWELL_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic GAGANFOXWELL_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'gaganfoxwell-ship');
    ...
    expect(content).toContain('GAGANFOXWELL_ROOT=');
    expect(content).toContain('$GAGANFOXWELL_BIN/');
  });
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/gaganfoxwell');
    expect(setupContent).toContain('$KIRO_GAGANFOXWELL/lib');
  });
  ...
  test('setup supports --host opencode with install section and OpenCode skill path vars', () => {
    ...
    expect(setupContent).toContain('OPENCODE_GAGANFOXWELL="$OPENCODE_SKILLS/gaganfoxwell"');
  });
  ...
  test('setup supports --host cursor with install section and Cursor skill path vars', () => {
    ...
    expect(setupContent).toContain('CURSOR_GAGANFOXWELL="$CURSOR_SKILLS/gaganfoxwell"');
    ...
    expect(setupContent).toContain('gaganfoxwell ready (cursor).');
  });
  ...
  test('create_cursor_runtime_root exposes only Cursor runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('gaganfoxwell/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('gaganfoxwell-upgrade/SKILL.md');
    ...
    expect(fnBody).toContain('$cursor_gaganfoxwell/lib');
    ...
    expect(fnBody).not.toContain('_link_or_copy "$gaganfoxwell_dir" "$cursor_gaganfoxwell"');
  });
  ...
  test('create_cursor_sidecar plants runtime assets without wiping generated SKILL.md', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('.cursor/skills/gaganfoxwell');
  ...
  test('link_cursor_skill_dirs skips the gaganfoxwell runtime root directory', () => {
  ...
  test('Cursor install links generated skills before planting the sidecar', () => {
    ...
    const linkCall = cursorInstall.indexOf('link_cursor_skill_dirs "$SOURCE_GAGANFOXWELL_DIR"');
    const sidecarCall = cursorInstall.indexOf('create_cursor_sidecar "$SOURCE_GAGANFOXWELL_DIR"');
    expect(linkCall).toBeGreaterThan(-1);
    expect(sidecarCall).toBeGreaterThan(-1);
    expect(linkCall).toBeLessThan(sidecarCall);
  });

  test('setup installs OpenCode skills into a nested gaganfoxwell runtime root', () => {
    ...
    expect(setupContent).toContain('$opencode_gaganfoxwell/lib');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('gaganfoxwell/SKILL.md');
    expect(fnBody).toContain('$codex_gaganfoxwell/lib');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('gaganfoxwell-upgrade/SKILL.md');
    ...
    expect(fnBody).toContain('TODOS-format.md');
    expect(fnBody).not.toContain('_link_or_copy "$gaganfoxwell_dir" "$codex_gaganfoxwell"');
  });
  ...
  test('create_factory_runtime_root links shared lib modules beside bin', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('$factory_gaganfoxwell/bin');
    expect(fnBody).toContain('$factory_gaganfoxwell/lib');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/gaganfoxwell', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.gaganfoxwell/repos/gaganfoxwell');
    expect(setupContent).toContain('avoid duplicate skill discovery');
  });

  // --- Symlink prefix tests (PR #503) ---

  test('link_claude_skill_dirs applies gaganfoxwell- prefix by default', () => {
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('SKILL_PREFIX');
    expect(fnBody).toContain('link_name="gaganfoxwell-$skill_name"');
  });
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // gaganfoxwell-* dirs should keep their name (e.g., gaganfoxwell-upgrade stays gaganfoxwell-upgrade)
    expect(fnBody).toContain('gaganfoxwell-*) link_name="$skill_name"');
  });
  ...
  test('cleanup_old_claude_symlinks removes only gaganfoxwell-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('gaganfoxwell-*) continue');
  });
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('GAGANFOXWELL_CONFIG');
  });
  ...
  test('cleanup_prefixed_claude_symlinks exists and uses readlink', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('readlink');
    expect(fnBody).toContain('gaganfoxwell-$skill_name');
  });
  ...
  test('welcome message references SKILL_PREFIX', () => {
    // gaganfoxwell-upgrade is always called gaganfoxwell-upgrade (it's the actual dir name)
    // but the welcome section should exist near the prefix logic
    expect(setupContent).toContain('Run /gaganfoxwell-upgrade anytime');
  });
});

describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-discover-'));
    try {
...
describe('telemetry', () => {
  test('generated SKILL.md contains telemetry start block', () => {
    ...
    expect(content).toContain('gaganfoxwell-config get telemetry');
  });

  test('generated SKILL.md contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('.telemetry-prompted');
    expect(content).toContain('Help gaganfoxwell get better');
    expect(content).toContain('gaganfoxwell-config set telemetry community');
    expect(content).toContain('gaganfoxwell-config set telemetry anonymous');
    expect(content).toContain('gaganfoxwell-config set telemetry off');
  });

  test('generated SKILL.md contains telemetry epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Telemetry (run last)');
    expect(content).toContain('gaganfoxwell-telemetry-log');
...
describe('community fixes wave', () => {
  ...
  // #594 — Discoverability: every SKILL.md.tmpl description contains "gaganfoxwell"
  test('every SKILL.md.tmpl description contains "gaganfoxwell"', () => {
    for (const skill of ALL_SKILLS) {
      const tmplPath = skill.dir === '.' ? path.join(ROOT, 'SKILL.md.tmpl') : path.join(ROOT, skill.dir, 'SKILL.md.tmpl');
      const content = fs.readFileSync(tmplPath, 'utf-8');
      const desc = extractDescription(content);
      expect(desc.toLowerCase()).toContain('gaganfoxwell');
    }
  });
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/gaganfoxwell symlink back to ROOT
...
describe('LEARNINGS_SEARCH resolver', () => {
  ...
  for (const skill of SEARCH_SKILLS) {
    test(`${skill} generated SKILL.md contains learnings search`, () => {
      ...
      expect(content).toContain('gaganfoxwell-learnings-search');
    });
  }
...
describe('gen-skill-docs prefix warning (#620/#578)', () => {
  ...
  test('warns about skill_prefix when config has prefix=true', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-prefix-warn-'));
    try {
      // Create a fake ~/.gaganfoxwell/config.yaml with skill_prefix: true
      const fakeHome = tmpDir;
      const fakeGaganfoxwell = path.join(fakeHome, '.gaganfoxwell');
      fs.mkdirSync(fakeGaganfoxwell, { recursive: true });
      fs.writeFileSync(path.join(fakeGaganfoxwell, 'config.yaml'), 'skill_prefix: true\n');
      ...
      expect(output).toContain('gaganfoxwell-relink');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  test('no warning when skill_prefix is false or absent', () => {
    ...
    try {
      ...
      const fakeGaganfoxwell = path.join(fakeHome, '.gaganfoxwell');
      fs.mkdirSync(fakeGaganfoxwell, { recursive: true });
      fs.writeFileSync(path.join(fakeGaganfoxwell, 'config.yaml'), 'skill_prefix: false\n');
...
// GAGANFOXWELL REVIEW REPORT report-at-bottom contract — verifies the prompt-text
...
describe('GAGANFOXWELL REVIEW REPORT delete-then-append flow', () => {
...
describe('LEARNINGS_SEARCH resolver: query parameter', () => {
  ...
  test('claude host + query=foo bar → both cross-project and project-scoped branches contain --query', () => {
    ...
    const lines = out.split('\n').filter(l => l.includes('gaganfoxwell-learnings-search'));
  ...
  test('codex host + query=foo bar → codex bash variant contains --query', () => {
    ...
    expect(out).toContain('$GAGANFOXWELL_BIN/gaganfoxwell-learnings-search');
  });
...
describe('EXIT PLAN MODE GATE placement', () => {
  ...
  // already contains `## GAGANFOXWELL REVIEW REPORT` inside a markdown example fence,
  // and the gate text itself shows `## GAGANFOXWELL REVIEW REPORT` inside a fence too.
...
describe('GAGANFOXWELL REVIEW REPORT mandatory unresolved-decisions status', () => {
...
describe('brain-sync block reads project-scoped MCP registrations (#2499)', () => {
  ...
  test('rendered jq lines FUNCTION: project-scoped registration resolves for a cwd inside the project', () => {
    ...
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-2499-home-'));
    const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-2499-proj-'));
