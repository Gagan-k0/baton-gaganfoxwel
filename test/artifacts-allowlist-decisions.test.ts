...
const INIT = fs.readFileSync(path.join(ROOT, "bin", "gaganfoxwell-artifacts-init"), "utf-8");

/** Pull a quoted heredoc body out of gaganfoxwell-artifacts-init by target filename. */
function heredoc(target: string): string {
  const re = new RegExp(`cat > "\\$GAGANFOXWELL_HOME/${target}" <<'EOF'\\n([\\s\\S]*?)\\nEOF\\n`);
  const m = INIT.match(re);
  if (!m) throw new Error(`heredoc for ${target} not found in gaganfoxwell-artifacts-init`);
  return m[1];
}
...
/**
 * gaganfoxwell-decision-log:40 enqueues projects/<slug>/decisions.jsonl after EVERY write,
 ...
 *
 * Source-level rather than end-to-end: gaganfoxwell-artifacts-init.test.ts drives the real
...
describe("the artifacts allowlist covers the decision store", () => {
  ...
  test("the allowlist still ends with the user-additions marker", () => {
    ...
    // overwritten the next time gaganfoxwell-artifacts-init runs.
