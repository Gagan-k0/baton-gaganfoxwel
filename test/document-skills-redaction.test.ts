...
describe("/document-release redaction", () => {
  test("scans the PR-body temp file before gh pr edit", () => {
    const scanIdx = RELEASE.indexOf("gaganfoxwell-redact --from-file /tmp/gaganfoxwell-pr-body");
    const editIdx = RELEASE.indexOf("gh pr edit --body-file /tmp/gaganfoxwell-pr-body");
...
describe("/document-generate redaction", () => {
  test("scans staged doc diff before commit", () => {
    const scanIdx = GENERATE.indexOf("gaganfoxwell-redact --repo-visibility");
  ...
  test("scans added lines of the staged diff", () => {
    expect(GENERATE).toMatch(/git diff --cached[\s\S]{0,80}gaganfoxwell-redact/);
  });
