 ...
 * for /rewind, silently shadowing the gaganfoxwell /checkpoint skill. Users
 ...
 *
 * This file is the "never again" test. It enumerates every gaganfoxwell skill name
 * from every SKILL.md.tmpl file in the repo and cross-checks against a
 * per-host list of known built-in slash commands. If any gaganfoxwell skill name
 ...
 * below. If a gaganfoxwell skill needs to coexist with a built-in anyway (e.g.,
...
const KNOWN_BUILTINS: Record<string, string[]> = {
  ...
  // invoke codex from gaganfoxwell, we don't install skills INTO codex the same
...
interface GaganfoxwellSkill {
  name: string;
  templatePath: string;
}

function enumerateGaganfoxwellSkills(): GaganfoxwellSkill[] {
  const skills: GaganfoxwellSkill[] = [];
...
describe('skill-collision-sentinel', () => {
  const skills = enumerateGaganfoxwellSkills();
  ...
  test('no duplicate skill names within gaganfoxwell', () => {
  ...
  // Hard check: no gaganfoxwell skill name collides with a known host built-in
    ...
    test(`no skill name collides with a ${host} built-in (or has written justification)`, () => {
      ...
      if (collisions.length > 0) {
        const msg = collisions.map(c =>
          ...
          `    Fix: rename the gaganfoxwell skill (precedent: /checkpoint → /context-save+/context-restore),\n` +
  ...
  test('KNOWN_COLLISIONS_TOLERATED entries are all still active collisions', () => {
    ...
    for (const name of Object.keys(KNOWN_COLLISIONS_TOLERATED)) {
      if (!skillNames.has(name)) {
        stale.push(`  "${name}" is in KNOWN_COLLISIONS_TOLERATED but no gaganfoxwell skill has that name — remove the exception`);
