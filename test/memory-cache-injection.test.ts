 ...
 *
 * Verifies the PreToolUse hook reads ~/.gaganfoxwell/free-text-memory.json and
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-memcache-'));
  ...
  test('injects matching nugget into additionalContext on pass-through', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:ship-todos-reorganize> Reorganize?',
...
describe('per-session memory cache', () => {
  test('first read writes cache; subsequent reads use cache', () => {
    ...
    runHook({
      ...
      tool_input: {
        questions: [
          { question: '<gaganfoxwell-qid:ship-todos-reorganize> Q', options: ['A', 'B'] },
        ],
      },
    });
