...
// gaganfoxwell-config accepts arbitrary keys (free-form YAML store), so we can't
  ...
  while (stack.length) {
    ...
    for (const ent of fs.readdirSync(cur, { withFileTypes: true })) {
      ...
      lines.forEach((line, idx) => {
        // Match `gaganfoxwell-config set <key>` or `gaganfoxwell-config get <key>`.
        for (const m of line.matchAll(/gaganfoxwell-config\s+(?:set|get)\s+([a-z][a-z0-9_]*)/g)) {
...
function runConfig(args: string[], tmpHome: string) {
  return spawnSync(CONFIG_BIN, args, {
    encoding: 'utf-8',
    env: { ...process.env, HOME: tmpHome, GAGANFOXWELL_HOME: tmpHome },
    timeout: 5000,
  });
}

describe('docs ↔ gaganfoxwell-config key drift guard', () => {
  ...
  // gaganfoxwell-config is a bash script; Windows can't exec it via spawnSync
  ...
  test.skipIf(process.platform === 'win32')('`explain_level` is exposed as a documented default', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'gaganfoxwell-cfg-'));
    try {
  ...
  test.skipIf(process.platform === 'win32')('`gaganfoxwell-config get artifacts_sync_mode` returns a value (the rename landed)', () => {
