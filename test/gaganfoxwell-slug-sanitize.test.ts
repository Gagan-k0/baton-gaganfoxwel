/**
 * gaganfoxwell-slug cache-read sanitization.
 *
 * `eval "$(gaganfoxwell-slug)"` is how callers load SLUG/BRANCH. The compute and
...
describe('gaganfoxwell-slug cache-read sanitization', () => {
  test('a poisoned cache file cannot inject shell metacharacters into output', () => {
    ...
    try {
      const cacheDir = path.join(home, '.gaganfoxwell', 'slug-cache');
...
// The GAGANFOXWELL_PROJECT_SLUG escape hatch is per-invocation, never durable: a
...
// The cache is also GAGANFOXWELL_HOME-aware now, matching lib/bin-context.ts's
// native port — temp-home runs must not litter the real ~/.gaganfoxwell (observed:
  ...
  test('an env-override run never writes the cwd cache', () => {
    ...
    try {
      const r = spawnSync(['bash', SLUG_BIN], {
        cwd: os.tmpdir(),
        env: { ...process.env, GAGANFOXWELL_HOME: home, GAGANFOXWELL_PROJECT_SLUG: 'override-slug' },
      });
