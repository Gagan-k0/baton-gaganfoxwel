...
function run(argv: string[], env: Record<string, string> = {}) {
  ...
  const res = spawnSync(full, argv.slice(1), {
    env: { ...process.env, HOME: tmpHome, GAGANFOXWELL_HOME: tmpHome, ...env },
    encoding: 'utf-8',
    cwd: ROOT,
  });
...
const worktreePath = () => path.join(tmpHome, '.gaganfoxwell-brain-worktree');
...
describe('gaganfoxwell-gbrain-source-wireup --advance-only (#2516)', () => {
  ...
  test('REFUSES a path that is not a worktree of the artifacts repo', () => {
    ...
    try {
      ...
      const r = run(['gaganfoxwell-gbrain-source-wireup', '--advance-only'], {
        GAGANFOXWELL_BRAIN_WORKTREE: userRepo,
      });
      expect(r.status).toBe(0);
  ...
  test('never contains a force-remove on the advance-only path (static pin)', () => {
    ...
    const src = fs.readFileSync(path.join(BIN, 'gaganfoxwell-gbrain-source-wireup'), 'utf-8');
...
describe('brain-sync --once daily advance wiring (#2516)', () => {
  test('once advances the worktree behind a 24h attempt stamp', () => {
    makeArtifactsRepoWithWorktree();
    run(['gaganfoxwell-config', 'set', 'artifacts_sync_mode', 'artifacts-only']);
    const second = commit(tmpHome, 'second');

    const r1 = run(['gaganfoxwell-brain-sync', '--once']);
    ...
    const r2 = run(['gaganfoxwell-brain-sync', '--once']);
    ...
    const r3 = run(['gaganfoxwell-brain-sync', '--once']);
