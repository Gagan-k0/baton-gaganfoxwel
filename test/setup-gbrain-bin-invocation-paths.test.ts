...
//
// Pins the correct bun-run + .ts invocation form for gaganfoxwell-memory-ingest
// and gaganfoxwell-gbrain-sync wherever setup-gbrain's docs instruct the agent
...
// The negative lookahead means `gaganfoxwell-memory-ingest.ts --probe` (correct)
// does NOT match, while `gaganfoxwell-memory-ingest --probe` (stale) does.
// `(?:\s|\\\r?\n)+` also spans a backslash line-continuation between the
// name and its flag (e.g. `gaganfoxwell-memory-ingest \` newline `  --probe`),
...
const bareMemoryIngest = /\bgaganfoxwell-memory-ingest\b(?!\.ts)(?:\s|\\\r?\n)+--/;
const bareGbrainSync = /\bgaganfoxwell-gbrain-sync\b(?!\.ts)(?:\s|\\\r?\n)+--/;

describe('setup-gbrain/SKILL.md.tmpl — bin invocation paths', () => {
  test('no bare gaganfoxwell-memory-ingest invocation remains', () => {
    expect(tmpl).not.toMatch(bareMemoryIngest);
  });

  test('no bare gaganfoxwell-gbrain-sync invocation remains', () => {
  ...
  test('the silent-bulk mention uses bun run + .ts (R2)', () => {
    expect(tmpl).toContain(
      'bun run ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-memory-ingest.ts --bulk --quiet'
    );
  });
  ...
  test('the preamble-hook incremental-sync mention uses bun run + .ts (R4)', () => {
    expect(tmpl).toContain(
      'bun run ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-sync.ts --incremental --quiet'
    );
  });

  test('the neighboring gaganfoxwell-config line in the post-answer block is untouched (bash script, no extension)', () => {
    expect(tmpl).toContain(
  ...
  test('the prose-only mention naming the tool as a sentence subject is left unchanged (KTD4 — not a literal invocation)', () => {
    expect(tmpl).toContain('gaganfoxwell-memory-ingest now persists staged transcripts to');
  });
});
...
describe('setup-gbrain/memory.md — bin invocation paths', () => {
  ...
  test('the secret-scanning example uses bun run + .ts (R5)', () => {
    expect(memoryDoc).toContain('bun run bin/gaganfoxwell-memory-ingest.ts --bulk --scan-secrets');
  ...
  test('the troubleshooting full-pass mention uses bun run + .ts (R5)', () => {
    expect(memoryDoc).toContain('Run `bun run bin/gaganfoxwell-gbrain-sync.ts --full` to do a full pass.');
  });

  test('the troubleshooting incremental-reingest mention uses bun run + .ts (R5)', () => {
    expect(memoryDoc).toContain(
      're-run `bun run bin/gaganfoxwell-gbrain-sync.ts --incremental` to re-ingest from'
    );
  });

  test('the already-correct reference line at the top of the file is unchanged', () => {
    expect(memoryDoc).toContain('bun run bin/gaganfoxwell-memory-ingest.ts --probe` (which');
  });
});
...
describe('bare-invocation regex — backslash line-continuation coverage', () => {
  ...
  test('catches a bare invocation split across a backslash continuation', () => {
    const staleContinuation = 'gaganfoxwell-memory-ingest \\\n  --probe';
  ...
  test('does not flag a correct invocation split across a backslash continuation', () => {
    const fixedContinuation = 'bun run bin/gaganfoxwell-gbrain-sync.ts \\\n  --incremental';
