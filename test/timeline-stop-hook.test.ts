...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-stop-hook-home-'));
  projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-stop-hook-proj-'));
...
function runHook(stdin: string): { exitCode: number; stdout: string; stderr: string } {
  const r = spawnSync('bash', [HOOK], {
    ...
    env: {
      ...
      GAGANFOXWELL_PROJECT_SLUG: SLUG, // deterministic slug, no git required
    },
    timeout: 15_000,
  });
...
describe('timeline-stop-hook wiring', () => {
  ...
  test('setup registers the Stop hook with its own source tag and tears it down on --no-team', () => {
    ...
    expect(setup).toContain('--source gaganfoxwell-timeline-stop');
    ...
    expect(teardown).toContain('remove-source --source gaganfoxwell-timeline-stop');
  });
  ...
  test('setup surfaces a settings-hook refusal instead of swallowing it', () => {
    ...
    expect(setup).not.toMatch(/ensure-event[\s\S]{0,220}--source gaganfoxwell-timeline-stop[\s\S]{0,40}2>\/dev\/null/);
  });
  ...
  test('setup routes the Stop hook through ensure-event, not presence-only dedup', () => {
    ...
    expect(setup).toMatch(/ensure-event[\s\S]{0,220}--source gaganfoxwell-timeline-stop/);
    ...
    expect(setup).not.toMatch(/list-sources 2>\/dev\/null \| grep -q "gaganfoxwell-timeline-stop"/);
  });
  ...
  test('hook path resolution is canonical-only: global install or skip, never the worktree', () => {
    ...
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-hookpath-'));
    try {
      const canonicalRoot = path.join(fakeHome, '.claude', 'skills', 'gaganfoxwell');
      ...
      const env = {
        ...process.env,
        HOME: fakeHome,
        SOURCE_GAGANFOXWELL_DIR: '/some/dev/worktree',
        CANONICAL_GAGANFOXWELL_ROOT: canonicalRoot,
      };
  ...
  test('ensure-event re-points a stale absolute path and leaves exactly one registration', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-ensure-'));
    try {
      ...
      const r = spawnSync('bash', [
        ...
        '--event', 'Stop',
        '--command', HOOK,
        '--source', 'gaganfoxwell-timeline-stop',
        '--timeout', '5',
  ...
  test('ensure-event is a true no-op when the registration already matches (no write, no backup churn)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-ensure-noop-'));
    try {
      const settingsFile = path.join(dir, 'settings.json');
      const args = [
        SETTINGS_HOOK, 'ensure-event',
      ...
      const env = { ...process.env, GAGANFOXWELL_SETTINGS_FILE: settingsFile };
  ...
  test('corrupt settings.json: ensure-event refuses (exit 3) and never rewrites the file', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-ensure-corrupt-'));
    try {
      const settingsFile = path.join(dir, 'settings.json');
  ...
  test('a matcher change updates the tagged entry in place — still exactly one registration', () => {
    // Identity key is (event, source): an existing gaganfoxwell entry with a STALE
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-ensure-matcher-'));
    try {
      const settingsFile = path.join(dir, 'settings.json');
      fs.writeFileSync(settingsFile, JSON.stringify({
        hooks: {
          PreToolUse: [{
            _gaganfoxwell_source: 'gaganfoxwell-plan-tune',
            matcher: 'OldMatcher',
      ...
      expect(s.hooks.PreToolUse[0]._gaganfoxwell_source).toBe('gaganfoxwell-plan-tune');
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test('a failed update leaves exactly one registration — never zero, never two', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-ensure-fail-'));
    try {
      const settingsFile = path.join(dir, 'settings.json');
      fs.writeFileSync(settingsFile, JSON.stringify({
        hooks: {
  ...
  test('gaganfoxwell-uninstall removes the Stop hook registration', () => {
    const uninstall = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-uninstall'), 'utf-8');
    expect(uninstall).toContain('remove-source --source gaganfoxwell-timeline-stop');
  });
  ...
  test('the bash shim is fail-open: exits 0 even when bun is unavailable', () => {
    const r = spawnSync('bash', [HOOK], {
      input: '{}',
      encoding: 'utf-8',
      env: { HOME: tmpHome, PATH: '/usr/bin:/bin', GAGANFOXWELL_HOME: tmpHome },
      timeout: 15_000,
    });
    expect(r.status).toBe(0);
  });
