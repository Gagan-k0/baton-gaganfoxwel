...
// shebang script gaganfoxwell-brain-sync without a shell; the fix gates `shell: true`
...
describe("#1731 gbrain spawns carry the Windows shell flag", () => {
  ...
  // and has no concept of a shebang, so gaganfoxwell-brain-sync came back as "is not
  ...
  test("orchestrator invokes brain-sync through bash, never a raw spawn", () => {
    const src = read("bin/gaganfoxwell-gbrain-sync.ts");
...
describe("bashScriptInvocation", () => {
  ...
  test("POSIX execs the script directly, no interpreter needed", () => {
    const inv = bashScriptInvocation("/home/u/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-sync", ["--once"], {
      platform: "linux",
    });
    expect(inv).toEqual({
      cmd: "/home/u/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-sync",
      argv: ["--once"],
      shell: false,
    });
  });
  ...
  test("Windows routes through Git bash with the script as argv[0]", () => {
    const inv = bashScriptInvocation("C:\\Users\\u\\.claude\\skills\\gaganfoxwell\\bin\\gaganfoxwell-brain-sync", ["--once"], {
      platform: "win32",
      exists: (p) => p === WIN_BASH,
      env: {},
    });
  ...
  test("Windows forward-slashes the script path", () => {
    ...
    const inv = bashScriptInvocation("C:\\Users\\u\\bin\\gaganfoxwell-brain-sync", [], {
      platform: "win32",
      exists: (p) => p === WIN_BASH,
      env: {},
    });
    expect(inv?.argv[0]).toBe("C:/Users/u/bin/gaganfoxwell-brain-sync");
    expect(inv?.argv[0]).not.toContain("\\");
  });

  test("never asks for a shell — cmd.exe is what broke this", () => {
    const inv = bashScriptInvocation("C:\\x\\gaganfoxwell-brain-sync", [], {
      platform: "win32",
      exists: (p) => p === WIN_BASH,
      env: {},
    });
  ...
  test("GAGANFOXWELL_BASH overrides the search for unusual installs", () => {
    const custom = "D:\\tools\\git\\bin\\bash.exe";
    const inv = bashScriptInvocation("C:\\x\\gaganfoxwell-brain-sync", [], {
      platform: "win32",
      exists: (p) => p === custom || p === WIN_BASH,
      env: { GAGANFOXWELL_BASH: custom },
    });
