...
// GAGANFOXWELL_HAS_IOS_DEVICE=1; the signing/install/interaction smoke test has the
// separate, explicit GAGANFOXWELL_IOS_DEVICE_DEPLOY=1 opt-in.
//
// Runs only when:
...
//
// GAGANFOXWELL_IOS_DEVICE_DEPLOY=1 additionally generates the fixture Xcode project,
// signs it with GAGANFOXWELL_IOS_DEVELOPMENT_TEAM + GAGANFOXWELL_IOS_BUNDLE_ID, installs
...
const HAS_DEVICE = process.env.GAGANFOXWELL_HAS_IOS_DEVICE === '1';
const DEPLOY_TO_DEVICE = process.env.GAGANFOXWELL_IOS_DEVICE_DEPLOY === '1';
...
function requireDeployEnv(name: 'GAGANFOXWELL_IOS_DEVELOPMENT_TEAM' | 'GAGANFOXWELL_IOS_BUNDLE_ID'): string {
  const value = process.env[name]?.trim();
  if (!value) {
    throw new Error(`${name} is required when GAGANFOXWELL_IOS_DEVICE_DEPLOY=1`);
  }
  return value;
}
...
describe('ios device deployment (explicit opt-in)', () => {
  testIfDeploy('generates, signs, installs, launches, and drives the fixture through the daemon', async () => {
    const developmentTeam = requireDeployEnv('GAGANFOXWELL_IOS_DEVELOPMENT_TEAM');
    const bundleId = requireDeployEnv('GAGANFOXWELL_IOS_BUNDLE_ID');
    const devices = listDevices();
    ...
    if (!device) {
      ...
      throw new Error([
        'GAGANFOXWELL_IOS_DEVICE_DEPLOY=1 requires an available, paired iPhone; stale unavailable devices are never selected.',
        summary,
      ].join('\n'));
    }

    const workDir = mkdtempSync(join(tmpdir(), 'gaganfoxwell-ios-device-deploy-'));
    ...
    try {
      ...
      runChecked(join(ROOT, 'bin/gaganfoxwell-ios-qa-regen'), [
