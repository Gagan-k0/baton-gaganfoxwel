...
const CONTAMINATED: NodeJS.ProcessEnv = {
  ...
  GAGANFOXWELL_HOME: '/Users/op/.gaganfoxwell',
  GAGANFOXWELL_HEADLESS_DEFAULT: 'x',
  MCP_TIMEOUT: '5000',
...
const HERMETIC_VARS = { CLAUDE_CONFIG_DIR: '/x/.claude', GAGANFOXWELL_HOME: '/x/gaganfoxwell-home' };
...
describe('buildHermeticEnv allowlist', () => {
  ...
  test('drops session-context and operator-credential vars', () => {
    for (const k of [
      ...
      'CLAUDE_CODE_ENTRYPOINT', 'GAGANFOXWELL_HEADLESS_DEFAULT', 'MCP_TIMEOUT',
  ...
  test('redirects CLAUDE_CONFIG_DIR and GAGANFOXWELL_HOME to hermetic values', () => {
    expect(env.CLAUDE_CONFIG_DIR).toBe('/x/.claude');
    expect(env.GAGANFOXWELL_HOME).toBe('/x/gaganfoxwell-home');
  });
  ...
  test('overrides merge last — per-test re-contamination is deliberate', () => {
    const e = buildHermeticEnv(CONTAMINATED, HERMETIC_VARS, {
      ...
      GAGANFOXWELL_HOME: '/tmp/test-home',
      GAGANFOXWELL_HEADLESS: '',
    });
    expect(e.CONDUCTOR_WORKSPACE_PATH).toBe('/tmp/test-ws');
    expect(e.GAGANFOXWELL_HOME).toBe('/tmp/test-home');
    expect(e.GAGANFOXWELL_HEADLESS).toBe('');
  });

  test('promotes GAGANFOXWELL_ANTHROPIC_API_KEY when canonical absent (shared shim fn)', () => {
    const base = { ...CONTAMINATED } as NodeJS.ProcessEnv;
    delete base.ANTHROPIC_API_KEY;
    base.GAGANFOXWELL_ANTHROPIC_API_KEY = 'sk-ant-promoted-9876543210';
    const e = buildHermeticEnv(base, HERMETIC_VARS);
    expect(e.ANTHROPIC_API_KEY).toBe('sk-ant-promoted-9876543210');
    expect(e.GAGANFOXWELL_ANTHROPIC_API_KEY).toBeUndefined(); // GAGANFOXWELL_* still dropped
  });
...
describe('EVALS_HERMETIC=0 escape hatch', () => {
  test('returns byte-identical legacy env, overrides still last', () => {
    ...
    const e = buildHermeticEnv(base, HERMETIC_VARS, { GAGANFOXWELL_HEADLESS: '1' });
    ...
    expect(e.GAGANFOXWELL_HOME).toBe('/Users/op/.gaganfoxwell');
    expect(e.GAGANFOXWELL_HEADLESS).toBe('1');
    expect(e).toEqual({ ...(base as Record<string, string>), GAGANFOXWELL_HEADLESS: '1' });
  });
...
describe('gcStaleHermeticDirs', () => {
  test('removes dead-pid dirs, keeps live-pid and foreign dirs', () => {
    ...
    const dead = path.join(tmp, `gaganfoxwell-hermetic-${deadPid}-abc`);
    const live = path.join(tmp, `gaganfoxwell-hermetic-${process.pid}-abc`);
    const foreign = path.join(tmp, 'unrelated-dir');
    const malformed = path.join(tmp, 'gaganfoxwell-hermetic-notapid-abc');
  ...
  test('keeps a fresh dead-pid dir (PID-reuse grace window)', () => {
    ...
    const freshDead = path.join(tmp, 'gaganfoxwell-hermetic-99999999-xyz');
...
describe('hermeticChildEnv composition', () => {
  test('hermetic by default: redirects config dirs, drops contamination', () => {
    ...
    const e = hermeticChildEnv({ GAGANFOXWELL_HEADLESS: '1' });
    const dirs = getHermeticDirs();
    expect(e.CLAUDE_CONFIG_DIR).toBe(dirs.configDir);
    expect(e.GAGANFOXWELL_HOME).toBe(dirs.gaganfoxwellHome);
    expect(e.GAGANFOXWELL_HEADLESS).toBe('1');
