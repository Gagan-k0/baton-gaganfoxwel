...
const GAGANFOXWELL_DEV_DIR = path.join(os.homedir(), '.gaganfoxwell-dev');
const HEARTBEAT_PATH = path.join(GAGANFOXWELL_DEV_DIR, 'e2e-live.json'); // heartbeat stays global
const PROJECT_DIR = path.dirname(getProjectEvalDir()); // ~/.gaganfoxwell/projects/$SLUG/
...
export async function runSkillTest(options: {
   ...
   *  per-test GAGANFOXWELL_HOME overrides so the test doesn't have to spell out
  ...
  const proc = Bun.spawn(['claude', ...args], {
    ...
    // session context (CONDUCTOR_*, CLAUDECODE, ~/.claude config, ~/.gaganfoxwell)
    // never reaches the child; EVALS_HERMETIC=0 restores the legacy env.
    // Default GAGANFOXWELL_HEADLESS=1 so eval/E2E runs classify as headless (BLOCK on an
    ...
    // `env: { GAGANFOXWELL_HEADLESS: '' }` — extraEnv wins because it spreads last.
    env: hermeticChildEnv({ GAGANFOXWELL_HEADLESS: '1', ...extraEnv }),
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.gaganfoxwell', 'test-transcripts');
