...
// --- GAGANFOXWELL_EVAL_DIR + shard slug tests ---

describe('EvalCollector eval-dir resolution', () => {
  const savedEnv = process.env.GAGANFOXWELL_EVAL_DIR;

  afterEach(() => {
    if (savedEnv === undefined) delete process.env.GAGANFOXWELL_EVAL_DIR;
    else process.env.GAGANFOXWELL_EVAL_DIR = savedEnv;
  });

  test('honors GAGANFOXWELL_EVAL_DIR set after import — no --preload needed', async () => {
    // The default eval dir must resolve lazily at construction, not at module
    // load: the sharded runner sets GAGANFOXWELL_EVAL_DIR in each shard child's env
    // and shard tests import this module long before any collector exists.
    const envDir = path.join(tmpDir, 'env-dir');
    process.env.GAGANFOXWELL_EVAL_DIR = envDir;
  ...
  test('explicit constructor arg beats GAGANFOXWELL_EVAL_DIR', async () => {
    process.env.GAGANFOXWELL_EVAL_DIR = path.join(tmpDir, 'env-dir');
