 ...
 * ~/.gaganfoxwell/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.gaganfoxwell-dev', 'evals');

/**
 * Detect project-scoped eval dir via gaganfoxwell-slug.
 * Falls back to legacy ~/.gaganfoxwell-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local gaganfoxwell-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null || ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.gaganfoxwell', 'projects', slugMatch[1], 'evals');
...
/**
 * Lazy + memoized so importing this module never spawns the gaganfoxwell-slug
 * subprocess. Callers that pass an explicit dir or set GAGANFOXWELL_EVAL_DIR
 ...
 * directory (the sharded paid runner's per-shard GAGANFOXWELL_EVAL_DIR layout),
 ...
 * `GAGANFOXWELL_BUDGET_RATIO` env var. New tests with no prior data are skipped.
 */
export function findBudgetRegressions(
  comparison: ComparisonResult,
  opts?: { ratioCap?: number; minPriorTools?: number; minPriorTurns?: number },
): BudgetRegression[] {
  const envRatio = Number(process.env.GAGANFOXWELL_BUDGET_RATIO);
...
export function assertNoBudgetRegression(
  ...
  const cap = opts?.ratioCap ?? (Number(process.env.GAGANFOXWELL_BUDGET_RATIO) || 2.0);
  ...
  throw new Error(
    ...
    lines.join('\n') +
    `\n(Override per run: GAGANFOXWELL_BUDGET_RATIO=<n>. ${comparison.before_file} vs ${comparison.after_file})`,
  );
}
...
export class EvalCollector {
  ...
  constructor(tier: 'e2e' | 'llm-judge', evalDir?: string) {
    this.tier = tier;
    this.evalDir = evalDir || process.env.GAGANFOXWELL_EVAL_DIR || defaultEvalDir();
