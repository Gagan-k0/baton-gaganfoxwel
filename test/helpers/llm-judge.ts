...
// scoped work. Override per run with GAGANFOXWELL_EVAL_MODEL_JUDGE; Haiku remains
// the right default for classifier-grade duties (pty hung/working, warmup,
// distill — see lib/eval-model.ts).
export async function callJudge<T>(prompt: string, model: string = process.env.GAGANFOXWELL_EVAL_MODEL_JUDGE || 'claude-sonnet-4-6'): Promise<T> {
...
Rubric:
- 5: Reason names a SPECIFIC TRADEOFF that distinguishes the chosen option from at least one alternative (e.g. "because hybrid ships V1 in gaganfoxwell-only without blocking on cross-repo gbrain coordination", "because Postgres preserves ACID guarantees the workflow already depends on").
