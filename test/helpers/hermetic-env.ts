 ...
 * skills), ~/.gaganfoxwell decision logs, and CONDUCTOR_-/CLAUDECODE-style env vars.
 ...
 *   │   AUTH_TOKEN                │
 *   │ GAGANFOXWELL_ANTHROPIC_API_KEY    │── promotedEnv() ─────► ANTHROPIC_API_KEY
 *   │ CONDUCTOR_*, CLAUDECODE,    │
 *   │ CLAUDE_*, GAGANFOXWELL_*, MCP_*,  │── dropped ───────────► ∅
 *   │ GBRAIN_*, GH_TOKEN, ...     │
 *   └─────────────────────────────┘
 *      + per-runner extraAllow (codex: OpenAI vars; gemini: Google vars)
 *      + CLAUDE_CONFIG_DIR=<runRoot>/.claude  GAGANFOXWELL_HOME=<runRoot>/gaganfoxwell-home
 ...
 * GAGANFOXWELL_* / MCP_* / GBRAIN_* — session-context contamination; and operator
 ...
 * (GAGANFOXWELL_HOME, CONDUCTOR_WORKSPACE_PATH, GAGANFOXWELL_HEADLESS opt-out) always
...
const DIR_PREFIX = 'gaganfoxwell-hermetic-';
...
export function getHermeticDirs(): HermeticDirs {
  ...
  const gaganfoxwellHome = path.join(runRoot, 'gaganfoxwell-home');
  ...
  try {
    ...
    const seed = buildSeedConfig({
      apiKey: process.env.ANTHROPIC_API_KEY ?? process.env.GAGANFOXWELL_ANTHROPIC_API_KEY,
  ...
  cachedDirs = { configDir, gaganfoxwellHome, runRoot };
  return cachedDirs;
}
 ...
 * (dir-name fallback), NO gaganfoxwell- prefix; the root SKILL.md router registers
 * as `_gaganfoxwell-command`. skillCensus().registryEntries is the authoritative
 ...
 *   `~/.claude/skills/gaganfoxwell/...` preamble references inside each SKILL.md
...
export function hermeticSkillsConfigDir(): string {
  ...
  for (const rel of skillCensus(root).physicalSkillFiles) {
    ...
    // symlink to open-gaganfoxwell-browser) shares its target's frontmatter name,
...
export function hermeticChildEnv(
  ...
  return buildHermeticEnv(
    process.env,
    { CLAUDE_CONFIG_DIR: dirs.configDir, GAGANFOXWELL_HOME: dirs.gaganfoxwellHome },
    overrides,
    opts,
  );
}
