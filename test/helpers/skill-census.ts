 ...
 *
 *   1. `connect-chrome/` is a directory SYMLINK to `open-gaganfoxwell-browser/`.
 ...
 *      alias name `_gaganfoxwell-command`, not as an authored skill.
 *
...
export interface SkillCensus {
   ...
   * plus the `_gaganfoxwell-command` root alias when the root router exists.
...
export function skillCensus(root: string): SkillCensus {
  ...
  if (fs.existsSync(path.join(root, 'SKILL.md'))) {
    ...
    registrySet.add('_gaganfoxwell-command');
  }
