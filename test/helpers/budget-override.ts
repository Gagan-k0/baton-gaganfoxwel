 ...
 *
 * Records uses of GAGANFOXWELL_SIZE_BUDGET_OVERRIDE_REASON or
 * EVALS_BUDGET_OVERRIDE_REASON so a reviewer can see what was waived,
 * by whom, and why. Append-only JSONL at ~/.gaganfoxwell/analytics/spend-overrides.jsonl.
 *
...
function getAuditPath(): string {
  const base = process.env.GAGANFOXWELL_HOME || path.join(os.homedir(), '.gaganfoxwell');
