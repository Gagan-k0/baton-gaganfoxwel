 ...
 * `GAGANFOXWELL_SIZE_BUDGET_OVERRIDE_REASON` — if the logger silently drops events,
...
describe('logBudgetOverride', () => {
  ...
  test('survives an unwritable audit path (logs warning, does not throw)', () => {
    // Point GAGANFOXWELL_HOME at a path inside a file (illegal directory location)
    const originalHome = process.env.GAGANFOXWELL_HOME;
    const bogusFile = path.join(TMP_HOME, 'not-a-dir.txt');
    fs.writeFileSync(bogusFile, 'just a file');
    process.env.GAGANFOXWELL_HOME = bogusFile;
    expect(() => logBudgetOverride({ scope: 'unwritable', reason: 'fs error path' })).not.toThrow();
    process.env.GAGANFOXWELL_HOME = originalHome;
  });
});
