 ...
 *   - telemetry JSONL under $HOME/.gaganfoxwell/analytics/ (same walk, but called
...
export interface SinkResult {
  ...
  /** Subset of filesWritten matching .gaganfoxwell/analytics/*.jsonl. */
...
export async function runWithSecretSink(opts: SecretSinkOptions): Promise<SinkResult> {
  ...
  // Make sure .gaganfoxwell exists so bins that append to analytics have somewhere to write.
  fs.mkdirSync(path.join(tmpHome, '.gaganfoxwell', 'analytics'), { recursive: true });
  ...
  for (const [rel, content] of Object.entries(filesWritten)) {
    if (rel.startsWith('.gaganfoxwell/analytics/') && rel.endsWith('.jsonl')) {
  ...
  for (const seed of opts.seeds) {
    ...
    for (const { rule, matchType } of rules) {
      ...
      for (const [rel, content] of Object.entries(filesWritten)) {
        ...
        if (hit !== null) {
          const channel = rel.startsWith('.gaganfoxwell/analytics/') ? 'telemetry' : 'file';
