...
describe('isPermissionDialogVisible', () => {
  test('matches "Bash command requires permission" prompts', () => {
    const sample = `
      ...
      Bash command \`gaganfoxwell-config get telemetry\` requires permission to run.
  ...
  test('matches "allow all edits" file-edit prompts', () => {
    ...
    const sample = `
      Edit to ~/.gaganfoxwell/config.yaml
...
describe('classifyVisible (runtime path through the runner classifier)', () => {
  ...
  test('permission dialog (Bash) → returns null (skip, keep polling)', () => {
    const visible = `
      Bash command \`gaganfoxwell-update-check\` requires permission to run.
...
describe('COMPLETION_SUMMARY_RE', () => {
  test('matches GAGANFOXWELL REVIEW REPORT heading', () => {
    expect(COMPLETION_SUMMARY_RE.test('## GAGANFOXWELL REVIEW REPORT')).toBe(true);
  });
...
`;
    ...
    expect(r.reason).toMatch(/no GAGANFOXWELL REVIEW REPORT/);
  });
...
`;
  ...
  test('passes when only ### subheadings follow REVIEW REPORT (deeper nesting allowed)', () => {
    const content = `## GAGANFOXWELL REVIEW REPORT
