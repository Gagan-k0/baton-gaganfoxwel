/**
 * gaganfoxwell-config explain_level round-trip + validation tests.
 *
 * Coverage:
...
const BIN_CONFIG = path.join(ROOT, 'bin', 'gaganfoxwell-config');
...
function run(...args: string[]): { stdout: string; stderr: string; status: number } {
  // gaganfoxwell-config precedence is `${GAGANFOXWELL_HOME:-${GAGANFOXWELL_STATE_DIR:-$HOME/.gaganfoxwell}}`,
  // so GAGANFOXWELL_HOME from the developer's parent env wins over the test's
  // GAGANFOXWELL_STATE_DIR. Override both to isolate from the real ~/.gaganfoxwell.
  const res = spawnSync(BIN_CONFIG, args, {
    env: { ...process.env, GAGANFOXWELL_STATE_DIR: tmpHome, GAGANFOXWELL_HOME: tmpHome },
    encoding: 'utf-8',
    cwd: ROOT,
  });
  return {
...
describe('gaganfoxwell-config explain_level', () => {
  ...
  test('get with unset explain_level returns the documented default', () => {
    // gaganfoxwell-config returns the documented default ("default") when the
    // key is absent from config.yaml — see bin/gaganfoxwell-config:103. Earlier
...
describe('gaganfoxwell-config values with spaces', () => {
