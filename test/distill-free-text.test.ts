/**
 * gaganfoxwell-distill-free-text — Layer 8 dream cycle (plan-tune cathedral T10).
 *
...
const BIN = path.join(ROOT, 'bin', 'gaganfoxwell-distill-free-text');
const QLOG_BIN = path.join(ROOT, 'bin', 'gaganfoxwell-question-log');
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-dist-'));
