/**
 * Unit tests for bin/gaganfoxwell-memory-ingest.ts (Lane A).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "gaganfoxwell-memory-ingest.ts");

// ── Helpers ────────────────────────────────────────────────────────────────

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "gaganfoxwell-memory-ingest-"));
}
...
describe("gaganfoxwell-memory-ingest CLI", () => {
  it("prints usage on --help and exits 0", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: gaganfoxwell-memory-ingest");
  ...
  it("--probe on empty home reports 0 files", () => {
    ...
    const r = runScript(["--probe"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    expect(r.exitCode).toBe(0);
  ...
  it("--probe finds Claude Code sessions", () => {
    ...
    const r = runScript(["--probe", "--include-unattributed"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    expect(r.exitCode).toBe(0);
  ...
  it("--probe finds gaganfoxwell artifacts (learnings, eureka, ceo-plan)", () => {
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(join(gaganfoxwellHome, "analytics"), { recursive: true });
    mkdirSync(join(gaganfoxwellHome, "projects", "foo-bar", "ceo-plans"), { recursive: true });

    writeFileSync(join(gaganfoxwellHome, "analytics", "eureka.jsonl"), '{"insight":"lake first"}\n');
    writeFileSync(join(gaganfoxwellHome, "projects", "foo-bar", "learnings.jsonl"), '{"key":"a","insight":"b"}\n');
    writeFileSync(join(gaganfoxwellHome, "projects", "foo-bar", "ceo-plans", "2026-05-01-test.md"), "# Plan\n");
  ...
  it("--sources filter limits the walk to specific types", () => {
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(join(gaganfoxwellHome, "analytics"), { recursive: true });
    mkdirSync(join(gaganfoxwellHome, "projects", "foo", "ceo-plans"), { recursive: true });

    writeFileSync(join(gaganfoxwellHome, "analytics", "eureka.jsonl"), '{"insight":"x"}\n');
    writeFileSync(join(gaganfoxwellHome, "projects", "foo", "learnings.jsonl"), '{"key":"a"}\n');

    const r = runScript(["--probe", "--sources", "eureka"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
    expect(r.stdout).toContain("eureka");
...
describe("gaganfoxwell-memory-ingest state file", () => {
  it("--incremental on empty home creates state file with schema_version: 1", () => {
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const r = runScript(["--incremental", "--quiet"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    expect(r.exitCode).toBe(0);
    const statePath = join(gaganfoxwellHome, ".transcript-ingest-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("gaganfoxwell-memory-ingest");
    rmSync(home, { recursive: true, force: true });
  });
...
describe("gaganfoxwell-memory-ingest security: untrusted cwd cannot trigger shell substitution", () => {
  it("does not invoke /bin/sh when a transcript record contains $() in cwd", () => {
    ...
    const markerDir = mkdtempSync(join(tmpdir(), "gaganfoxwell-mi-cwd-marker-"));
    ...
    const r = runScript(["--incremental", "--quiet"], {
      ...
      GAGANFOXWELL_MEMORY_INGEST_NO_WRITE: "1",
    });
...
describe("internal: parseTranscriptJsonl + buildTranscriptPage shape", () => {
  it("parses a Claude Code JSONL session", async () => {
    const dir = mkdtempSync(join(tmpdir(), "gaganfoxwell-mi-parse-"));
    ...
    const r = runScript(["--probe", "--include-unattributed"], { HOME: home, GAGANFOXWELL_HOME: join(home, ".gaganfoxwell") });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
...
describe("gaganfoxwell-memory-ingest --limit", () => {
  it("respects --limit by stopping after N writes (mocked via --probe shortcut)", () => {
    ...
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(gaganfoxwellHome, { recursive: true });
    const r = runScript(["--probe", "--limit", "1"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
...
describe("gaganfoxwell-memory-ingest writer (gbrain v0.20+ batch `import` interface)", () => {
  ...
  // names. Uses the REAL bin/gaganfoxwell-gbrain-repo-policy (resolved by the client
  ...
  it("a deny policy keyed _unattributed applies to unattributable transcripts (#2353)", () => {
    ...
    const POLICY = join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-repo-policy");
    ...
    expect(existsSync(join(gaganfoxwellHome, "gbrain-repo-policy.json"))).toBe(true);
  ...
  // (real-world cause: gaganfoxwell-artifacts-init writes `.gitignore = "*"` into
  // $GAGANFOXWELL_HOME, and `gbrain import` honours .gitignore, so every file staged
  // under $GAGANFOXWELL_HOME is invisible to it).
  //
...
describe("#2105 codex response_item rollout shape", () => {
  it("extracts messages from response_item records", async () => {
    const { parseTranscriptJsonl } = await import("../bin/gaganfoxwell-memory-ingest");
...
describe("#2394: probe applies the same attribution gate as prepare", () => {
  ...
  it("parity: probe post-attribution count equals what prepare actually processes", () => {
    ...
    const probe = runScript(["--probe"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
    ...
    const inc = runScript(["--incremental", "--quiet"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
  ...
  it("parity: a Codex cwd appearing only on a LATER record is unattributed in probe AND prepare", () => {
    ...
    const inc = runScript(["--incremental"], { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome });
...
//
// The same store the code-import gate honors (bin/gaganfoxwell-gbrain-sync.ts):
...
// The policy store is seeded through the REAL bin/gaganfoxwell-gbrain-repo-policy
// script (its `set` verb owns the file schema + URL normalization).

describe("#2392: transcript ingest honors per-remote trust policy", () => {
  const POLICY_BIN = join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-repo-policy");
  ...
  function setPolicy(gaganfoxwellHome: string, url: string, tier: string): void {
    const r = spawnSync(POLICY_BIN, ["set", url, tier], {
      encoding: "utf-8",
      env: { ...process.env, GAGANFOXWELL_HOME: gaganfoxwellHome },
    });
    expect(r.status).toBe(0);
  }

  function stateSessions(gaganfoxwellHome: string): string[] {
    const statePath = join(gaganfoxwellHome, ".transcript-ingest-state.json");
  ...
  it("(a) deny remote's transcript is skipped and counted as skipped_policy_deny", () => {
    ...
    setPolicy(gaganfoxwellHome, "https://github.com/denyme/denied.git", "deny");
    ...
    expect(existsSync(logFile)).toBe(true);
    const sessions = stateSessions(gaganfoxwellHome);
  ...
  it("(b) read-only remote's transcript is skipped and counted as skipped_policy_readonly", () => {
    ...
    writeSessionForRepo(home, "work-allowed", "oksess1", okCwd);
    setPolicy(gaganfoxwellHome, "https://github.com/roorg/rorepo.git", "read-only");
  ...
  it("(c) read-write remote's transcript is ingested (reaches gbrain import)", () => {
    ...
    setPolicy(gaganfoxwellHome, "https://github.com/rworg/rwrepo.git", "read-write");
    ...
    const calls = readFileSync(logFile, "utf-8").trim().split("\n").filter(Boolean);
    expect(calls.length).toBe(1);
    expect(stateSessions(gaganfoxwellHome).length).toBe(1);
  ...
  it("(d) corrupted store: hard error before any writes, message names recovery", () => {
    ...
    writeFileSync(join(gaganfoxwellHome, "gbrain-repo-policy.json"), "not valid json{", "utf-8");
    ...
    expect(r.stderr).toContain("gaganfoxwell-gbrain-repo-policy list");
    expect(r.stderr).toContain("/setup-gbrain");
    // Nothing written: no gbrain import call, no state file, store untouched.
    expect(existsSync(logFile)).toBe(false);
    expect(stateSessions(gaganfoxwellHome).length).toBe(0);
    expect(readFileSync(join(gaganfoxwellHome, "gbrain-repo-policy.json"), "utf-8")).toBe("not valid json{");
  ...
  it("artifacts are never policy-filtered, even when their project's remote is denied", () => {
    const home = makeTestHome();
    const gaganfoxwellHome = join(home, ".gaganfoxwell");
    mkdirSync(join(gaganfoxwellHome, "projects", "denyme-denied"), { recursive: true });
    const { binDir } = installFakeGbrain(home);
    ...
    writeFileSync(join(gaganfoxwellHome, "projects", "denyme-denied", "learnings.jsonl"), '{"key":"a","insight":"b"}\n');
    setPolicy(gaganfoxwellHome, "https://github.com/denyme/denied.git", "deny");
