...
const scriptPath = join(import.meta.dir, "..", "bin", "gaganfoxwell-global-discover.ts");

describe("gaganfoxwell-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/gaganfoxwell-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:garrytan/gaganfoxwell.git");
      const https = normalizeRemoteUrl("https://github.com/garrytan/gaganfoxwell.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/garrytan/gaganfoxwell");
  ...
  describe("codex large session_meta parsing", () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), "gaganfoxwell-codex-test-"));
