 ...
 * on PATH), runs a cheap gaganfoxwell skill through the Agent SDK, intercepts
...
describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('gaganfoxwell skill preamble fires the 3-option AskUserQuestion when gbrain is detected', async () => {
    // Stage a fresh GAGANFOXWELL_HOME with artifacts_sync_mode_prompted=false.
    const gaganfoxwellHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-gaganfoxwell-'));
    ...
    fs.writeFileSync(
      path.join(gaganfoxwellHome, 'config.yaml'),
    ...
    // break auth). Ambient process.env.GAGANFOXWELL_HOME mutation does NOT work
    // here — hermetic-env scrubs GAGANFOXWELL_* and repoints GAGANFOXWELL_HOME at its
    ...
    try {
      ...
      await runAgentSdkTest({
        ...
        workingDirectory: gaganfoxwellHome,
        maxTurns: 10,
  ...
  test('privacy gate does NOT fire when artifacts_sync_mode_prompted is already true', async () => {
    ...
    const gaganfoxwellHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-off-'));
    ...
    // first test — ambient GAGANFOXWELL_HOME mutation is scrubbed by hermetic-env).
    const childEnv = {
      GAGANFOXWELL_HOME: gaganfoxwellHome,
      HOME: tempHome,
      PATH: `${fakeBinDir}:${process.env.PATH ?? '/usr/bin:/bin:/opt/homebrew/bin'}`,
