...
describe('setup Codex model activation', () => {
  ...
  test('resolves the profile once, fails closed, and passes it as quoted argv', () => {
    ...
    const guardAt = setup.indexOf('gaganfoxwell setup failed: Codex model resolver returned no model');
  ...
  test('Kiro rewrites the codex-rendered SETUP_COMMAND and never symlinks gaganfoxwell-upgrade', () => {
    // The artifact Kiro copies was rendered for the codex host, so its
    // gaganfoxwell-upgrade skill bakes in './setup --host codex'. Every copy path
    // must rewrite it to '--host kiro', and the KIRO_GAGANFOXWELL gaganfoxwell-upgrade
    ...
    expect(block).not.toContain('_link_or_copy "$AGENTS_DIR/gaganfoxwell-upgrade/SKILL.md"');
  });
...
describe('Sol E2E tree hygiene', () => {
  ...
  test('snapshots and restores the exact prior .agents tree around the Sol render', () => {
    ...
    // default profile.
    const backupAt = solTest.indexOf('gaganfoxwell-agents-backup-');
