...
const UNINSTALL = path.join(ROOT, 'bin', 'gaganfoxwell-uninstall');

describe('gaganfoxwell-uninstall', () => {
  ...
  test('--help prints usage and exits 0', () => {
    ...
    expect(output).toContain('gaganfoxwell-uninstall');
  ...
  describe('integration tests with mock layout', () => {
    ...
    beforeEach(() => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-uninstall-test-'));
      mockHome = path.join(tmpDir, 'home');
      mockGitRoot = path.join(tmpDir, 'repo');

      // Create mock gaganfoxwell install layout
      fs.mkdirSync(path.join(mockHome, '.claude', 'skills', 'gaganfoxwell'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.claude', 'skills', 'gaganfoxwell', 'SKILL.md'), 'test');

      // Create per-skill symlinks (both old unprefixed and new prefixed)
      fs.symlinkSync('gaganfoxwell/review', path.join(mockHome, '.claude', 'skills', 'review'));
      fs.symlinkSync('gaganfoxwell/ship', path.join(mockHome, '.claude', 'skills', 'gaganfoxwell-ship'));

      // Create a non-gaganfoxwell symlink (should NOT be removed)
      fs.mkdirSync(path.join(mockHome, '.claude', 'skills', 'other-tool'), { recursive: true });

      // Create state directory
      fs.mkdirSync(path.join(mockHome, '.gaganfoxwell', 'projects'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.gaganfoxwell', 'config.json'), '{}');
    ...
    test('--force removes global Claude skills and state', () => {
      const result = spawnSync('bash', [UNINSTALL, '--force'], {
        ...
        env: {
          ...process.env,
          HOME: mockHome,
          GAGANFOXWELL_DIR: path.join(mockHome, '.claude', 'skills', 'gaganfoxwell'),
          GAGANFOXWELL_STATE_DIR: path.join(mockHome, '.gaganfoxwell'),
        },
        cwd: mockGitRoot,
      });
      ...
      expect(result.status).toBe(0);
      const output = result.stdout.toString();
      expect(output).toContain('gaganfoxwell uninstalled');

      // Global skill dir should be removed
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'gaganfoxwell'))).toBe(false);

      // Per-skill symlinks pointing into gaganfoxwell/ should be removed
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'review'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'gaganfoxwell-ship'))).toBe(false);

      // Non-gaganfoxwell tool should still exist
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'other-tool'))).toBe(true);

      // State should be removed
      expect(fs.existsSync(path.join(mockHome, '.gaganfoxwell'))).toBe(false);
    });
    ...
    test('--keep-state preserves state directory', () => {
      ...
      expect(fs.existsSync(path.join(mockHome, '.gaganfoxwell'))).toBe(true);
      expect(fs.existsSync(path.join(mockHome, '.gaganfoxwell', 'config.json'))).toBe(true);
    });
    ...
    test('clean system outputs nothing to remove', () => {
      ...
      const result = spawnSync('bash', [UNINSTALL, '--force'], {
        ...
        env: {
          ...process.env,
          HOME: cleanHome,
          GAGANFOXWELL_DIR: path.join(cleanHome, 'nonexistent'),
          GAGANFOXWELL_STATE_DIR: path.join(cleanHome, '.gaganfoxwell'),
        },
        cwd: mockGitRoot,
      });
    ...
    test('upgrade path: prefixed install + uninstall cleans both old and new symlinks', () => {
      ...
      // (mockHome already has both 'review' and 'gaganfoxwell-ship' symlinks)
      ...
      // Both old (review) and new (gaganfoxwell-ship) symlinks should be gone
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'review'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'gaganfoxwell-ship'))).toBe(false);

      // Non-gaganfoxwell should survive
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'other-tool'))).toBe(true);
    });

    test('--force removes Cursor gaganfoxwell skills and leaves other Cursor skills', () => {
      ...
      fs.mkdirSync(path.join(mockHome, '.cursor', 'skills', 'gaganfoxwell'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.cursor', 'skills', 'gaganfoxwell', 'SKILL.md'), banner);
      fs.mkdirSync(path.join(mockHome, '.cursor', 'skills', 'gaganfoxwell-review'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.cursor', 'skills', 'gaganfoxwell-review', 'SKILL.md'), banner);
      ...
      fs.mkdirSync(path.join(mockGitRoot, '.cursor', 'skills', 'gaganfoxwell-ship'), { recursive: true });
      fs.writeFileSync(path.join(mockGitRoot, '.cursor', 'skills', 'gaganfoxwell-ship', 'SKILL.md'), banner);
      ...
      expect(fs.existsSync(path.join(mockHome, '.cursor', 'skills', 'gaganfoxwell'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.cursor', 'skills', 'gaganfoxwell-review'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.cursor', 'skills', 'frontend-design'))).toBe(true);
      expect(fs.existsSync(path.join(mockGitRoot, '.cursor', 'skills', 'gaganfoxwell-ship'))).toBe(false);
      expect(fs.existsSync(path.join(mockGitRoot, '.cursor', 'rules', 'keep.md'))).toBe(true);
    });

    test("a user's own gaganfoxwell-prefixed Cursor dir (no banner) survives and is listed", () => {
      // S5: the bare gaganfoxwell* glob must not sweep a dir that merely starts
      // with "gaganfoxwell" — provenance comes from the generated banner, and a
      // hand-written SKILL.md never carries it.
      const foreign = path.join(mockHome, '.cursor', 'skills', 'gaganfoxwell-fork-notes');
      fs.mkdirSync(foreign, { recursive: true });
      fs.writeFileSync(path.join(foreign, 'SKILL.md'), '# my own notes\n');

      const foreignLocal = path.join(mockGitRoot, '.cursor', 'skills', 'gaganfoxwell-my-rules');
      ...
      expect(stderr).toContain('gaganfoxwell-fork-notes');
      expect(stderr).toContain('gaganfoxwell-my-rules');
    });
  });
});
...
// cleanup ran AFTER `rm -rf $CLAUDE_SKILLS/gaganfoxwell` — and SETTINGS_HOOK
...
describe('hook cleanup runs before the install root is deleted', () => {
  test('uninstall executed FROM the install root still removes hook entries', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-uninstall-order-'));
    try {
      const mockHome = path.join(tmp, 'home');
      const installRoot = path.join(mockHome, '.claude', 'skills', 'gaganfoxwell');
      ...
      // the root it deletes.
      for (const b of ['gaganfoxwell-uninstall', 'gaganfoxwell-settings-hook', 'gaganfoxwell-session-update', 'gaganfoxwell-config']) {
      ...
      fs.writeFileSync(settingsFile, JSON.stringify({
        hooks: {
          PostToolUse: [{
            ...
            _gaganfoxwell_source: 'auq-error-fallback',
      ...
      fs.mkdirSync(path.join(mockHome, '.gaganfoxwell'), { recursive: true });

      const result = spawnSync('bash', [path.join(installBin, 'gaganfoxwell-uninstall'), '--force', '--keep-state'], {
        stdio: 'pipe',
        env: {
          ...process.env,
          HOME: mockHome,
          GAGANFOXWELL_SETTINGS_FILE: settingsFile,
          GAGANFOXWELL_STATE_ROOT: path.join(mockHome, '.gaganfoxwell'),
        },
        cwd: tmp,
      });
      expect(result.status).toBe(0);
...
describe('hook cleanup under lock contention is loud, never silent (review-army)', () => {
  test('a held foreign lock during uninstall surfaces the give-up warning on stderr', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-uninstall-lock-'));
    try {
      const mockHome = path.join(tmp, 'home');
      const installRoot = path.join(mockHome, '.claude', 'skills', 'gaganfoxwell');
      const installBin = path.join(installRoot, 'bin');
      ...
      fs.writeFileSync(settingsFile, JSON.stringify({
        hooks: {
          Stop: [{
            _gaganfoxwell_source: 'gaganfoxwell-timeline-stop',
