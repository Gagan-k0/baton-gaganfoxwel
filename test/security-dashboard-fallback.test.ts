...
const SEC_BIN = join(ROOT, "bin", "gaganfoxwell-security-dashboard");
const COMM_BIN = join(ROOT, "bin", "gaganfoxwell-community-dashboard");
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), "gaganfoxwell-dash-test-"));
...
function run(
  ...
  return spawnSync(BASH, opts.json ? [bin, "--json"] : [bin], {
    ...
    env: {
      ...process.env,
      PATH: pathEnv,
      GAGANFOXWELL_DIR: ROOT,
      GAGANFOXWELL_SUPABASE_URL: "https://stub.supabase.test",
      GAGANFOXWELL_SUPABASE_ANON_KEY: "stub-key",
...
describe("gaganfoxwell-security-dashboard — never reports fake zeros (#1947)", () => {
...
describe("gaganfoxwell-community-dashboard — never reports fake zeros (#1947)", () => {
