/**
 * gaganfoxwell-session-kind — classifies the session so skills know whether a human can
...
const BIN = path.resolve(__dirname, '..', 'bin', 'gaganfoxwell-session-kind');
...
describe('gaganfoxwell-session-kind', () => {
  test('OPENCLAW_SESSION → spawned (highest precedence)', () => {
    ...
    expect(kind({ OPENCLAW_SESSION: '1', GAGANFOXWELL_HEADLESS: '1', CONDUCTOR_PORT: '5' })).toBe('spawned');
  });

  test('GAGANFOXWELL_HEADLESS → headless', () => {
    expect(kind({ GAGANFOXWELL_HEADLESS: '1' })).toBe('headless');
  });
  ...
  test('GAGANFOXWELL_HEADLESS beats CONDUCTOR (explicit override wins)', () => {
    expect(kind({ GAGANFOXWELL_HEADLESS: '1', CONDUCTOR_PORT: '5' })).toBe('headless');
  });
  ...
  test('empty GAGANFOXWELL_HEADLESS is treated as unset (interactive)', () => {
    ...
    expect(kind({ GAGANFOXWELL_HEADLESS: '' })).toBe('interactive');
  });
});
