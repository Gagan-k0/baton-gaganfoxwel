/**
 * Unit tests for bin/gaganfoxwell-brain-context-load.ts (Lane C).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "gaganfoxwell-brain-context-load.ts");
...
function prependPath(binDir: string): Record<string, string> {
  ...
  return {
    ...
    GAGANFOXWELL_BRAIN_TIMEOUT_MS: "10000",
  };
}

describe("gaganfoxwell-brain-context-load CLI", () => {
  it("--help exits 0 with usage", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: gaganfoxwell-brain-context-load");
...
describe("gaganfoxwell-brain-context-load — manifest dispatch", () => {
  ...
  it("uses skill manifest when --skill-file points at a valid SKILL.md", () => {
    const dir = mkdtempSync(join(tmpdir(), "gaganfoxwell-bcl-"));
...
describe("gaganfoxwell-brain-context-load — graceful gbrain absence", () => {
...
`,
  ...
  it("availability survives a 1ms query budget — detection is not subject to GAGANFOXWELL_BRAIN_TIMEOUT_MS", () => {
...
`,
    ...
    try {
      const env = { ...prependPath(binDir), GAGANFOXWELL_BRAIN_TIMEOUT_MS: "1" };
