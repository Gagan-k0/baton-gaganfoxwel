 ...
 * Step 0:
 *   - Consent gate:  question_tuning=false AND ~/.gaganfoxwell/.question-tuning-prompted missing
 *                    → run "Consent + opt-in".
 *   - Setup gate:    question_tuning=true AND declared empty AND
 *                    ~/.gaganfoxwell/.declared-setup-prompted missing → run "5-Q setup".
 *
 ...
 *
 * Strategy: exercise the helpers the gates depend on (gaganfoxwell-config get,
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-gate-'));
});
