 ...
 * reference docs must flow through bin/gaganfoxwell-issue-guard. Same posture as
 ...
 *
 * A line is compliant when it mentions gaganfoxwell-issue-guard, or when the
...
const SCANNER_EXEMPT: { file: string; pattern: string; reason: string }[] = [
  {
    ...
    reason:
      'raw fetch lands in /tmp json FILES (metadata/body split); body text is read into context only via the gaganfoxwell-issue-guard --stdin pipes documented in the same file',
  },
  {
...
describe('tracker-text wiring scanner', () => {
  test('every tracker-text read flows through gaganfoxwell-issue-guard (or carries a reasoned exemption)', () => {
    ...
    for (const rel of trackedFiles()) {
      ...
      lines.forEach((line, i) => {
        for (const { name, re } of READ_PATTERNS) {
          ...
          if (line.includes('gaganfoxwell-issue-guard')) continue;
          ...
          if (line.trimEnd().endsWith('\\')) {
            ...
            if (continuation.includes('gaganfoxwell-issue-guard')) continue;
          }
    ...
    if (violations.length > 0) {
      throw new Error(
        `Raw tracker-text read(s) outside gaganfoxwell-issue-guard:\n  ${violations.join('\n  ')}\n\n` +
          `Fix: pipe the read through bin/gaganfoxwell-issue-guard (--stdin for pre-fetched text), or — ` +
  ...
  test('exemption entries stay live (a stale exemption means the site moved — re-audit it)', () => {
    for (const e of SCANNER_EXEMPT) {
      ...
      const hasMatch = content.split('\n').some((l) => pat.re.test(l) && !l.includes('gaganfoxwell-issue-guard'));
  ...
  test('the guarded sites actually mention the guard (wiring, not just lib existence)', () => {
    ...
    for (const rel of mustMention) {
      ...
      expect(content).toContain('gaganfoxwell-issue-guard');
    }
  });
});
