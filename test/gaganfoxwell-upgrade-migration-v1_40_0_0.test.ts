/**
 * gaganfoxwell-upgrade/migrations/v1.40.0.0.sh — migration script unit tests.
 *
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-mig-v1400-"));
  gaganfoxwellHome = path.join(tmpHome, ".gaganfoxwell");
  migrationDir = path.join(gaganfoxwellHome, ".migrations");
  donePath = path.join(migrationDir, "v1.40.0.0.done");
  allowlistPath = path.join(gaganfoxwellHome, ".brain-allowlist");
  privacyPath = path.join(gaganfoxwellHome, ".brain-privacy-map.json");
  gitattrsPath = path.join(gaganfoxwellHome, ".gitattributes");
  fs.mkdirSync(gaganfoxwellHome, { recursive: true });
});

afterEach(() => {
  try {
    fs.chmodSync(gaganfoxwellHome, 0o755);
...
function makeCuratedPath(opts: { jq?: "missing" | "shim-fail" | "real" } = {}): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-mig-path-"));
  const utils = [
    "bash",
    "sh",
    "mkdir",
