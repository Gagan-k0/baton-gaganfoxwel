 ...
 *   spec-archive            — gaganfoxwell-paths eval + atomic tmp/mv + PID suffix
 ...
 *   spec-plan-mode-detection — reads GAGANFOXWELL_PLAN_MODE env
 */
...
describe('/spec fail-closed redaction (shared engine)', () => {
  ...
  test('redaction routes through the shared gaganfoxwell-redact bin, not inline regex', () => {
    expect(GEN).toContain('gaganfoxwell-redact');
...
describe('/spec archive', () => {
  test('uses eval $(gaganfoxwell-paths) not hardcoded ~/.gaganfoxwell/', () => {
    expect(TMPL).toMatch(/eval "\$\(.+gaganfoxwell-paths\)"/);
    expect(TMPL).toMatch(/\$GAGANFOXWELL_STATE_ROOT\/projects\/\$SLUG\/specs/);
    // No hardcoded ~/.gaganfoxwell/projects path:
    expect(TMPL).not.toMatch(/~\/\.gaganfoxwell\/projects\/\$SLUG\/specs/);
  });
...
describe('/spec plan-mode-aware Phase 5 (DX7/DX11/F1)', () => {
  test('reads GAGANFOXWELL_PLAN_MODE env at Phase 5 dispatch', () => {
    expect(TMPL).toMatch(/GAGANFOXWELL_PLAN_MODE/);
    expect(TMPL).toMatch(/plan-mode-aware default/i);
  });
  test('plan-mode active → file-only path; inactive → file + spawn', () => {
    expect(TMPL).toMatch(/GAGANFOXWELL_PLAN_MODE=active.*file-only path/);
    expect(TMPL).toMatch(/GAGANFOXWELL_PLAN_MODE=inactive.*file \+ spawn/);
  });
