...
Report whether it worked.`,
  ...
  testConcurrentIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/gaganfoxwell/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with gaganfoxwell installed globally, the fallback path
    // ~/.claude/skills/gaganfoxwell/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const gaganfoxwellHome = path.join(opDir, '.gaganfoxwell-home');
    ...
    // Copy bin scripts + the lib module they import. gaganfoxwell-learnings-log
    ...
    for (const script of ['gaganfoxwell-learnings-log', 'gaganfoxwell-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }
    // gaganfoxwell-learnings-log imports $SCRIPT_DIR/../lib/jsonl-store.ts (shared
    ...
    // gaganfoxwell-learnings-log will create the project dir automatically via gaganfoxwell-slug
...
\`\`\`bash
GAGANFOXWELL_HOME="${gaganfoxwellHome}" ${binDir}/gaganfoxwell-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(gaganfoxwellHome, 'projects');
  ...
  testConcurrentIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a gaganfoxwell skill. The session preamble detected _SESSIONS=4 (the user has 4 gaganfoxwell windows open).
