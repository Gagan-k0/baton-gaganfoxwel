...
const EVIDENCE = path.join(ROOT, 'bin', 'gaganfoxwell-evidence');
...
function ledgerFile(): string {
  const found = findFilesBySuffix(path.join(gaganfoxwellHome, 'projects'), '-evidence.jsonl');
...
beforeEach(() => {
  gaganfoxwellHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-evidence-home-'));
  repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-evidence-repo-'));
  git('init -q -b main');
...
describe('gaganfoxwell-evidence run', () => {
...
describe('gaganfoxwell-evidence check', () => {
  ...
  test('check never errors outside a git repo — degrades to STALE', () => {
    expect(run(['run', '--label', 'tests', '--', 'echo green']).status).toBe(0);
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-evidence-nongit-'));
    try {
...
describe('gaganfoxwell-evidence run — bun dotenv autoload must not reach the child', () => {
  ...
  function runWith(env: Record<string, string | undefined>, cmd: string) {
    return spawnSync(EVIDENCE, ['run', '--label', 'envprobe', '--', cmd], {
      cwd: repoDir,
      env: { ...process.env, GAGANFOXWELL_HOME: gaganfoxwellHome, ...env },
      encoding: 'utf-8',
      timeout: 60000,
    });
  }
  ...
  test('GAGANFOXWELL_EVIDENCE_KEEP_DOTENV=1 restores the old pass-through behaviour', () => {
    fs.writeFileSync(path.join(repoDir, '.env'), 'ZZ_OPTOUT_PROBE=kept\n');
    const r = runWith({ GAGANFOXWELL_EVIDENCE_KEEP_DOTENV: '1' }, 'echo "saw=[${ZZ_OPTOUT_PROBE:-absent}]"');
