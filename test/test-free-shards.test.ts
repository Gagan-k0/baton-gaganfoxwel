...
describe('test-free-shards: strict shard execution', () => {
  ...
  test('the log-file path is announced once at start and the PASS epilogue repeats it', async () => {
    ...
    const outcome = await runFreeShard(['pass'], 1, 1, { commandFor, quiet: true, log: (l) => lines.push(l) });
    expect(outcome.status).toBe('passed');
    const announced = lines.filter((l) => /^\[test:free\] full log: .+gaganfoxwell-free-test-.+\.log$/.test(l));
  ...
  test('spawned shard gets throwaway TMPDIR but NEVER an injected GAGANFOXWELL_HOME', async () => {
    // GAGANFOXWELL_HOME injection was tried and reverted: one shared scratch home
    ...
    try {
      const script =
        ...
        + `  home: process.env.GAGANFOXWELL_HOME ?? null, tmp: process.env.TMPDIR,`
      ...
      // GAGANFOXWELL_HOME passes through untouched (whatever the parent had, incl. unset).
      expect(seen.home).toBe(process.env.GAGANFOXWELL_HOME ?? null);
      // TMPDIR is a per-shard throwaway, cleaned up once the shard finishes.
      expect(seen.tmp).toContain('gaganfoxwell-free-shard-');
