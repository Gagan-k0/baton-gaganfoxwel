/**
 * gaganfoxwell-gbrain-detect — gbrain_mcp_mode + gaganfoxwell_artifacts_remote tests.
 *
...
const DETECT_BIN = path.join(ROOT, 'bin', 'gaganfoxwell-gbrain-detect');
...
function runDetect(extraEnv: Record<string, string> = {}): { code: number; json: any; stderr: string } {
  ...
  const r = spawnSync(DETECT_BIN, [], {
    env: {
      ...
      HOME: tmpHome,
      GAGANFOXWELL_HOME: path.join(tmpHome, '.gaganfoxwell'),
      ...extraEnv,
    },
    encoding: 'utf-8',
  });
...
describe('gaganfoxwell_artifacts_remote', () => {
  test('reads ~/.gaganfoxwell-artifacts-remote.txt when present', () => {
    fs.writeFileSync(
      path.join(tmpHome, '.gaganfoxwell-artifacts-remote.txt'),
      'https://github.com/garrytan/gaganfoxwell-artifacts-garrytan\n'
    );
    expect(runDetect().json.gaganfoxwell_artifacts_remote).toBe(
      'https://github.com/garrytan/gaganfoxwell-artifacts-garrytan'
    );
  });

  test('migration-window fallback: reads ~/.gaganfoxwell-brain-remote.txt if artifacts file is missing', () => {
    fs.writeFileSync(
      path.join(tmpHome, '.gaganfoxwell-brain-remote.txt'),
      'git@github.com:garrytan/gaganfoxwell-brain-garrytan.git\n'
    );
    expect(runDetect().json.gaganfoxwell_artifacts_remote).toBe(
      'git@github.com:garrytan/gaganfoxwell-brain-garrytan.git'
    );
  ...
  test('artifacts file wins over brain file when both exist', () => {
    ...
    expect(runDetect().json.gaganfoxwell_artifacts_remote).toBe('https://github.com/x/new');
  });
...
describe('schema regression', () => {
  test('output JSON has all expected keys (sync-gbrain compat)', () => {
    ...
    expect(keys).toEqual([
      ...
      'gbrain_pooler_mode',
      'gbrain_version',
      'gaganfoxwell_artifacts_remote',
      'gaganfoxwell_brain_git',
      'gaganfoxwell_brain_sync_mode',
    ]);
  });
});
