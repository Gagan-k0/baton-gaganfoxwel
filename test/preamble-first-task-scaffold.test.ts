...
//   1. bin/gaganfoxwell-first-task-detect — classifies a repo into ONE enum bucket.
//   2. The unified first-run-guidance preamble wiring (generated into SKILL.md).

const ROOT = path.join(import.meta.dir, '..');
const DETECT = path.join(ROOT, 'bin', 'gaganfoxwell-first-task-detect');
...
describe('gaganfoxwell-first-task-detect — bucket classification', () => {
...
describe('gaganfoxwell-first-task-detect — contract', () => {
...
describe('first-run-guidance preamble wiring (generated)', () => {
  ...
  test('detection is gated to the first-ever run only (ACTIVATED=no, not headless)', () => {
    ...
    expect(md).toContain('gaganfoxwell-first-task-detect');
  });
  ...
  test('marks activated + logs the scaffold telemetry only on the shown path', () => {
    ...
    expect(md).toContain('touch ~/.gaganfoxwell/.activated');
    expect(md).toContain('touch ~/.gaganfoxwell/.first-loop-tip-shown');
  });
});
