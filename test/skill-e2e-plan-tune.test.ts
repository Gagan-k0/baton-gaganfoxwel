...
describeIfSelected('PlanTune E2E', ['plan-tune-inspect'], () => {
  ...
  beforeAll(() => {
    ...
    // gaganfoxwell-developer-profile --derive imports from scripts/ — copy those too.
    ...
    const cfgDir = path.join(gaganfoxwellHome);
...
IMPORTANT:
- Use GAGANFOXWELL_HOME="${gaganfoxwellHome}" as an environment variable for all bin calls.
- Replace any ~/.claude/skills/gaganfoxwell/bin/ references with ./bin/ (relative path).
- Replace any ~/.claude/skills/gaganfoxwell/scripts/ references with ./scripts/.
