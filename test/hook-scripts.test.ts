...
function withGitRepo(defaultBranch: string, currentBranch: string, fn: (repoDir: string) => void) {
  const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-careful-git-'));
  try {
...
function withFreezeDir(freezePath: string, fn: (stateDir: string) => void) {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-freeze-test-'));
  ...
  test.each(['investigate/SKILL.md', 'careful/SKILL.md', 'freeze/SKILL.md', 'guard/SKILL.md'])(
    ...
    (rel) => {
      ...
      for (const line of commandLines) {
        ...
        expect(line).toContain('$HOME/.claude/skills/gaganfoxwell/');
      }
    },
  );
});
...
describe('check-careful.sh', () => {
  ...
  describe('HIGH tier (hard deny)', () => {
    ...
    test('missing origin/HEAD symbolic ref falls back to origin/main probe (Conductor worktrees)', () => {
      const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-careful-nohead-'));
      try {
        const git = (args: string[]) => gitArgvIn(repoDir, args);
        git(['init', '-q', '-b', 'main']);
        git(['commit', '--allow-empty', '-q', '-m', 'init']);
  ...
  describe('additive project patterns', () => {
    function withPatternFile(content: string, fn: (gaganfoxwellHome: string) => void) {
      const gaganfoxwellHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-careful-pat-'));
      fs.writeFileSync(path.join(gaganfoxwellHome, 'careful-patterns.txt'), content);
      try {
        fn(gaganfoxwellHome);
      } finally {
    ...
    test('a project pattern adds an ask rule', () => {
      withPatternFile('# infra safety\nterraform\\s+destroy\n', (gaganfoxwellHome) => {
        const { exitCode, output } = runHook(CAREFUL_SCRIPT, carefulInput('terraform destroy -auto-approve'), { GAGANFOXWELL_HOME: gaganfoxwellHome });
        expect(exitCode).toBe(0);
        expect(output.hookSpecificOutput?.permissionDecision).toBe('ask');
    ...
    test('a garbage pattern file cannot suppress a baseline match (additive invariant)', () => {
      withPatternFile('# override: allow everything\nallow-everything\nignore baseline\n', (gaganfoxwellHome) => {
        const { exitCode, output } = runHook(CAREFUL_SCRIPT, carefulInput('rm -rf /var/data'), { GAGANFOXWELL_HOME: gaganfoxwellHome });
        expect(exitCode).toBe(0);
        expect(output.hookSpecificOutput?.permissionDecision).toBe('ask');
        expect(output.hookSpecificOutput?.permissionDecisionReason).toContain('recursive delete');
      });
    ...
    test('an invalid regex line is skipped without breaking the hook', () => {
      withPatternFile('([unclosed\nterraform\\s+destroy\n', (gaganfoxwellHome) => {
        const { exitCode, output } = runHook(CAREFUL_SCRIPT, carefulInput('terraform destroy'), { GAGANFOXWELL_HOME: gaganfoxwellHome });
        expect(exitCode).toBe(0);
        expect(output.hookSpecificOutput?.permissionDecision).toBe('ask');
        expect(output.hookSpecificOutput?.permissionDecisionReason).toContain('Project rule');
      });
    ...
    test('safe commands still allow with a pattern file present', () => {
      withPatternFile('terraform\\s+destroy\n', (gaganfoxwellHome) => {
        const { exitCode, output } = runHook(CAREFUL_SCRIPT, carefulInput('ls -la'), { GAGANFOXWELL_HOME: gaganfoxwellHome });
        expect(exitCode).toBe(0);
        expect(output.hookSpecificOutput?.permissionDecision).toBeUndefined();
      });
    });
...
describe('check-freeze.sh', () => {
  ...
  describe('space-bearing freeze boundary', () => {
    ...
    test('a boundary containing spaces allows edits inside it', () => {
      const base = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-freeze-space-'));
  ...
  describe('broken install fails closed', () => {
    test('a missing hook-extract helper DENIES instead of proceeding', () => {
      ...
      const base = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-freeze-broken-'));
  ...
  describe('symlink boundary escape', () => {
    ...
    test('an in-boundary symlink to an outside target denies', () => {
      const base = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-freeze-link-'));
