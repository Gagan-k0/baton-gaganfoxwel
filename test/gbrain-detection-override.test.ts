 ...
 * with --respect-detection, it reads ~/.gaganfoxwell/gbrain-detection.json and
 ...
 *
 * Tests drive gen-skill-docs as a subprocess against a temp GAGANFOXWELL_HOME
...
/**
 * Run gen-skill-docs with --respect-detection and an isolated GAGANFOXWELL_HOME.
...
function regenAndSnapshot(opts: {
  ...
  try {
    execFileSync('bun', args, {
      cwd: REPO_ROOT,
      env: { ...process.env, GAGANFOXWELL_HOME: opts.tmpHome },
...
describe('gbrain detection override → gen-skill-docs', () => {
  ...
  test('with status "timeout" (slow-but-healthy, #1964), brain blocks render like "ok"', () => {
    ...
    try {
      ...
      // treatment as "ok" (matches gaganfoxwell-gbrain-detect --is-ok).
      expect(content).toContain('## Save Results to Brain');
      expect(content).toContain('gbrain put "office-hours/');
    } finally {
      cleanup();
