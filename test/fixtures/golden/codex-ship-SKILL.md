...
GAGANFOXWELL_ROOT="$HOME/.codex/skills/gaganfoxwell"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/gaganfoxwell" ] && GAGANFOXWELL_ROOT="$_ROOT/.agents/skills/gaganfoxwell"
...
_UPD=$($GAGANFOXWELL_BIN/gaganfoxwell-update-check 2>/dev/null || .agents/skills/gaganfoxwell/bin/gaganfoxwell-update-check 2>/dev/null || true)
...
_VENDORED="no"
if [ -d ".agents/skills/gaganfoxwell" ] && [ ! -L ".agents/skills/gaganfoxwell" ]; then
  if [ -f ".agents/skills/gaganfoxwell/VERSION" ] || [ -d ".agents/skills/gaganfoxwell/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
> gaganfoxwell works best when your project's AGENTS.md includes skill routing rules.
...
Then commit the change: `git add AGENTS.md && git commit -m "chore: add gaganfoxwell skill routing rules to AGENTS.md"`
...
> This project has gaganfoxwell vendored in `.agents/skills/gaganfoxwell/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .agents/skills/gaganfoxwell/`
2. Run `echo '.agents/skills/gaganfoxwell/' >> .gitignore`
3. Run `$GAGANFOXWELL_BIN/gaganfoxwell-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore AGENTS.md && git commit -m "chore: migrate gaganfoxwell from vendored to team mode"`
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.agents/skills/gaganfoxwell/review/TODOS-format.md`).
...
2. **Read `.agents/skills/gaganfoxwell/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
Read `.agents/skills/gaganfoxwell/review/TODOS-format.md` for the canonical format reference.
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.agents/skills/gaganfoxwell/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
