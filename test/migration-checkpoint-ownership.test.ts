...
const MIGRATION = path.join(ROOT, 'gaganfoxwell-upgrade', 'migrations', 'v1.1.3.0.sh');
...
function setupFakeGaganfoxwellRoot(tmpHome: string): string {
  // A real target that the gaganfoxwell symlink can resolve into.
  const gaganfoxwellDir = path.join(tmpHome, '.claude', 'skills', 'gaganfoxwell');
  fs.mkdirSync(path.join(gaganfoxwellDir, 'checkpoint'), { recursive: true });
  fs.writeFileSync(path.join(gaganfoxwellDir, 'checkpoint', 'SKILL.md'), '# fake gaganfoxwell checkpoint\n');
  return gaganfoxwellDir;
}
...
describe('migration v1.1.3.0 — checkpoint ownership guard', () => {
  ...
  beforeEach(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-migration-ownership-'));
  });
  ...
  test('scenario A: directory symlink into gaganfoxwell → removed', () => {
    setupFakeGaganfoxwellRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const gaganfoxwellCheckpoint = path.join(skillsDir, 'gaganfoxwell', 'checkpoint');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.symlinkSync(gaganfoxwellCheckpoint, topLevel);

    const result = runMigration(tmpHome);
    expect(result.exitCode).toBe(0);
    expect(fs.existsSync(topLevel)).toBe(false);
    // Also removes the gaganfoxwell-owned inner copy (Shape 2 cleanup).
    expect(fs.existsSync(gaganfoxwellCheckpoint)).toBe(false);
    expect(result.stdout).toContain('Removed stale /checkpoint symlink');
  });

  test('scenario B: directory with SKILL.md symlinked into gaganfoxwell → removed', () => {
    setupFakeGaganfoxwellRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const gaganfoxwellSKILL = path.join(skillsDir, 'gaganfoxwell', 'checkpoint', 'SKILL.md');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.mkdirSync(topLevel, { recursive: true });
    fs.symlinkSync(gaganfoxwellSKILL, path.join(topLevel, 'SKILL.md'));
  ...
  test('scenario C: user-owned regular directory with custom content → preserved', () => {
    ...
    expect(result.stdout).toContain('not a gaganfoxwell-owned install');
  });

  test('scenario D: symlink pointing outside gaganfoxwell → preserved', () => {
    setupFakeGaganfoxwellRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const topLevel = path.join(skillsDir, 'checkpoint');
    ...
    expect(result.stdout).toContain('Leaving');
    expect(result.stdout).toContain('outside gaganfoxwell');
  });

  test('scenario E: nothing to do → no-op exit 0 (idempotent)', () => {
    // No checkpoint install at all. First run: nothing removed.
    setupFakeGaganfoxwellRoot(tmpHome);
    // Delete the inner gaganfoxwell/checkpoint to simulate post-upgrade state.
    fs.rmSync(path.join(tmpHome, '.claude', 'skills', 'gaganfoxwell', 'checkpoint'), { recursive: true, force: true });
  ...
  test('scenario F: gaganfoxwell not installed → no-op exit 0', () => {
    // No ~/.claude/skills/gaganfoxwell/ at all. Also no checkpoint install.
  ...
  test('scenario G: SKILL.md is a symlink pointing outside gaganfoxwell → preserved', () => {
    setupFakeGaganfoxwellRoot(tmpHome);
    const skillsDir = path.join(tmpHome, '.claude', 'skills');
    const topLevel = path.join(skillsDir, 'checkpoint');
    fs.mkdirSync(topLevel, { recursive: true });
    // A directory containing SKILL.md that's a symlink pointing outside gaganfoxwell.
