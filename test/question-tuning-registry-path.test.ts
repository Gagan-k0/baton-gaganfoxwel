 ...
 * project cwd (the file lives only under the gaganfoxwell install root). The
 ...
 * $GAGANFOXWELL_ROOT on env-var hosts).
 */
...
describe('question-tuning registry path is absolute (#2489)', () => {
  test('claude host renders the installed registry path', () => {
    ...
    expect(out).toContain('`~/.claude/skills/gaganfoxwell/scripts/question-registry.ts`');
  });

  test('env-var host renders $GAGANFOXWELL_ROOT-anchored registry path', () => {
    const out = generateQuestionTuning(makeCtx('codex'));
    expect(out).toContain('`$GAGANFOXWELL_ROOT/scripts/question-registry.ts`');
  });
  ...
  test('rendered SKILL.md carries the absolute path', () => {
    ...
    expect(rendered).toContain('~/.claude/skills/gaganfoxwell/scripts/question-registry.ts');
