  ...
  testConcurrentIfSelected('diagram-authoring-quality', async () => {
    ...
    try {
      const result = await runSkillTest({
        prompt: basePrompt(
          dir,
          'how gaganfoxwell renders diagrams in PDFs: markdown containing mermaid fences goes through a pre-pass that extracts the fences, renders them in a browse daemon tab using an offline bundle, substitutes the SVG back in, inlines local images, and prints via Chromium. Failures become visible diagnostic blocks.',
        ),
        workingDirectory: dir,
        maxTurns: 25,
        allowedTools: ['Bash', 'Read', 'Write'],
...
THE ASK: a diagram of gaganfoxwell's PDF diagram-rendering flow — mermaid fences are
