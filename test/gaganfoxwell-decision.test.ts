/**
 * Unit tests for lib/gaganfoxwell-decision.ts — event-sourced decision memory model.
 */
...
import {
  ...
  type ActiveDecision,
  type DecisionPaths,
} from "../lib/gaganfoxwell-decision";
...
describe("decisionPaths", () => {
  it("derives log/snapshot/archive under the project slug", () => {
    const p = decisionPaths("garrytan-gaganfoxwell", "/tmp/gs");
    expect(p.log).toBe("/tmp/gs/projects/garrytan-gaganfoxwell/decisions.jsonl");
    expect(p.snapshot).toBe("/tmp/gs/projects/garrytan-gaganfoxwell/decisions.active.json");
    expect(p.archive).toBe("/tmp/gs/projects/garrytan-gaganfoxwell/decisions.archive.jsonl");
  });
});
