/**
 * GAGANFOXWELL_STATE_ROOT override — verifies the 3 plan-tune bins honor
 * GAGANFOXWELL_STATE_ROOT as a higher-priority override over GAGANFOXWELL_HOME.
 *
 * Surfaced by plan-tune cathedral D16 (Codex outside voice): tests can't
 * isolate from real ~/.gaganfoxwell today because the bins ignore STATE_ROOT.
 * Without this override, the cathedral's E2E + integration tests would
 * silently pollute the user's real profile.
 *
 * Contract:
 *   - GAGANFOXWELL_STATE_ROOT set → bins write under STATE_ROOT (HOME ignored).
 *   - Only GAGANFOXWELL_HOME set → bins write under HOME (existing behavior).
 *   - Neither set → falls back to $HOME/.gaganfoxwell (existing behavior).
...
const BIN_LOG = path.join(ROOT, 'bin', 'gaganfoxwell-question-log');
const BIN_PREF = path.join(ROOT, 'bin', 'gaganfoxwell-question-preference');
const BIN_DEV = path.join(ROOT, 'bin', 'gaganfoxwell-developer-profile');

let stateRoot: string;
let homeRoot: string;

beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-state-'));
  homeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-home-'));
});
...
function runBin(
  ...
  if (env.GAGANFOXWELL_STATE_ROOT === undefined) delete cleaned.GAGANFOXWELL_STATE_ROOT;
  if (env.GAGANFOXWELL_HOME === undefined) delete cleaned.GAGANFOXWELL_HOME;
...
describe('gaganfoxwell-question-log honors GAGANFOXWELL_STATE_ROOT', () => {
  test('STATE_ROOT set, HOME unset → writes under STATE_ROOT', () => {
    const r = runBin(BIN_LOG, [JSON.stringify(SAMPLE_LOG)], {
      GAGANFOXWELL_STATE_ROOT: stateRoot,
      GAGANFOXWELL_HOME: undefined,
    });
  ...
  test('only HOME set → preserves existing behavior (writes under HOME)', () => {
    const r = runBin(BIN_LOG, [JSON.stringify(SAMPLE_LOG)], {
      GAGANFOXWELL_STATE_ROOT: undefined,
      GAGANFOXWELL_HOME: homeRoot,
    });
    expect(r.status).toBe(0);
...
describe('gaganfoxwell-question-preference honors GAGANFOXWELL_STATE_ROOT', () => {
  test('STATE_ROOT set → preferences file lives under STATE_ROOT', () => {
    const write = runBin(
      ...
      { GAGANFOXWELL_STATE_ROOT: stateRoot, GAGANFOXWELL_HOME: undefined },
    );
...
describe('gaganfoxwell-developer-profile honors GAGANFOXWELL_STATE_ROOT', () => {
