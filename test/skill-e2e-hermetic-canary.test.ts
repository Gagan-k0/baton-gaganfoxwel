 ...
 *    `/.claude` config dir, a temp GAGANFOXWELL_HOME, and none of the planted
    ...
    try {
      const result = await runSkillTest({
        ...
        prompt: 'Run exactly this bash command and then stop: ' +
          'echo "CFG=${CLAUDE_CONFIG_DIR:-}"; echo "GH=${GAGANFOXWELL_HOME:-}"; ' +
      ...
      expect(cfg).toMatch(/gaganfoxwell-hermetic-.*\/\.claude$/);
      expect(bashOut).toMatch(/GH=\S*gaganfoxwell-home/);
