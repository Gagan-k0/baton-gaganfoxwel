...
const PROBE = path.join(ROOT, 'bin/gaganfoxwell-codex-probe');
...
function tempHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-codex-probe-home-'));
}

describe('gaganfoxwell-codex-probe: auth probe', () => {
  test('CODEX_API_KEY set → AUTH_OK', () => {
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: '_gaganfoxwell_codex_auth_probe',
  ...
  test('${CODEX_HOME:-~/.codex}/auth.json exists → AUTH_OK', () => {
    ...
    try {
      ...
      const r = runProbe({ snippet: '_gaganfoxwell_codex_auth_probe', home });
      expect(r.stdout.trim()).toBe('AUTH_OK');
      expect(r.status).toBe(0);
    } finally {
      fs.rmSync(home, { recursive: true, force: true });
  ...
  test('alternate $CODEX_HOME → checks the alternate path', () => {
    const home = tempHome();
    const altCodex = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-alt-codex-'));
    try {
...
function tempStubCodex(versionOutput: string, bool_command_fails = false): {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-codex-stub-'));
...
function runVersionCheck(versionOutput: string): string {
  ...
  try {
    const r = runProbe({
      snippet: '_gaganfoxwell_codex_version_check',
...
describe('gaganfoxwell-codex-probe: version check (anchored regex per Tension I)', () => {
  // Matches (should WARN)
...
describe('gaganfoxwell-codex-probe: timeout wrapper + namespace hygiene', () => {
  test('bin/gaganfoxwell-codex-probe is syntactically valid bash (bash -n)', () => {
  ...
  test('timeout wrapper executes command directly when neither binary present', () => {
    ...
    const r = runProbe({
      snippet: `_gaganfoxwell_codex_timeout_wrapper 5 echo hello_world`,
  ...
  test('timeout wrapper resolves gtimeout preferentially when on PATH', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-gto-stub-'));
    try {
      const stub = path.join(dir, 'gtimeout');
      fs.writeFileSync(stub, '#!/bin/bash\necho gtimeout_chosen_$1\n');
      fs.chmodSync(stub, 0o755);
      const r = runProbe({
        snippet: `_gaganfoxwell_codex_timeout_wrapper 5 echo nope`,
  ...
  test('bash-native watchdog kills a hung command at the deadline (exit 124, no timeout binary)', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-watchdog-'));
    try {
      ...
      const r = runProbe({
        snippet: `_gaganfoxwell_codex_timeout_wrapper 1 sleep 30; echo "rc=$?"`,
        env: { PATH: dir },
      });
...
describe('gaganfoxwell-codex-probe: telemetry event emission', () => {
  test('_gaganfoxwell_codex_log_event writes jsonl when _TEL != off', () => {
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: `_gaganfoxwell_codex_log_event "codex_test_event" "42"; cat "$HOME/.gaganfoxwell/analytics/skill-usage.jsonl"`,
        env: { _TEL: 'community' },
        home,
      });
  ...
  test('_gaganfoxwell_codex_log_event skips write when _TEL = off', () => {
    const home = tempHome();
    try {
      runProbe({
        snippet: `_gaganfoxwell_codex_log_event "codex_test_event" "99"`,
        env: { _TEL: 'off' },
        home,
      });
      const jsonl = path.join(home, '.gaganfoxwell/analytics/skill-usage.jsonl');
  ...
  test('payload never contains prompt content, env values, or auth tokens (schema check)', () => {
    ...
    try {
      const r = runProbe({
        snippet: `_gaganfoxwell_codex_log_event "codex_test_event" "1"; cat "$HOME/.gaganfoxwell/analytics/skill-usage.jsonl"`,
        env: {
          _TEL: 'community',
...
describe('codex timeout wrapper: /review + /ship diff passes', () => {
  ...
  for (const relPath of WRAPPED_SITES) {
    ...
    test(`${relPath}: both diff-review Codex calls run under the wrapper`, () => {
      const wrapped =
        read().match(/_gaganfoxwell_codex_timeout_wrapper\s+\d+\s+codex\s+(exec|review)\b/g) ?? [];
    ...
    test(`${relPath}: does not claim \`timeout\` is unavailable on macOS`, () => {
      // _gaganfoxwell_codex_timeout_wrapper resolves gtimeout -> timeout -> unwrapped,
    ...
    test(`${relPath}: wrapper budget stays under the outer Bash gate`, () => {
      const budgets = [...read().matchAll(/_gaganfoxwell_codex_timeout_wrapper\s+(\d+)\s+codex\b/g)].map(
        (m) => Number(m[1]) * 1000,
      );
  ...
  for (const relPath of ['codex/SKILL.md.tmpl', 'codex/SKILL.md']) {
    ...
    test(`${relPath}: (a) every scoped codex review invocation pins sandbox_mode="read-only"`, () => {
      const invocations = read()
        .split('\n')
        .filter((l) => /_gaganfoxwell_codex_timeout_wrapper\s+\d+\s+codex\s+review\b/.test(l));
    ...
    test(`${relPath}: (c) every Bash gate sits strictly above its section's wrapper budgets`, () => {
      ...
      // (`_gaganfoxwell_codex_timeout_wrapper S codex`), every gate must be strictly
      ...
      for (const section of sections) {
        ...
        const wrappers = [...section.matchAll(/_gaganfoxwell_codex_timeout_wrapper\s+(\d+)\s+codex\b/g)].map(
          (m) => Number(m[1]) * 1000,
        );
