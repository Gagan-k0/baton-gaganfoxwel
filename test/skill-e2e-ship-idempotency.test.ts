...
function buildShippedFixture(): ShipFixture {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-ship-fixture-'));
    ...
    async () => {
      ...
      try {
        ...
        while (Date.now() - start < budgetMs) {
          ...
          // (T9), Step 12 runs `gaganfoxwell-version-bump classify` which emits JSON
          ...
          //   - agent ran the CLI write path (gaganfoxwell-version-bump write) — a
          //     re-bump on an already-shipped branch
          if (
            /"state":\s*"FRESH"/.test(visible) ||
            /STATE:\s*FRESH(?![\w-])/i.test(visible) ||
            /gaganfoxwell-version-bump\s+write/i.test(visible) ||
