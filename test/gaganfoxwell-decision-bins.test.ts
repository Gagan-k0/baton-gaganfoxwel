/**
 * Subprocess tests for bin/gaganfoxwell-decision-log + bin/gaganfoxwell-decision-search.
 * Mirrors the learnings-bins test pattern (run the bin with GAGANFOXWELL_HOME=tmp).
 */
...
const LOG = path.join(ROOT, "bin", "gaganfoxwell-decision-log");
const SEARCH = path.join(ROOT, "bin", "gaganfoxwell-decision-search");

let tmpDir: string;

function opts(): ExecSyncOptionsWithStringEncoding {
  return { cwd: ROOT, env: { ...process.env, GAGANFOXWELL_HOME: tmpDir }, encoding: "utf-8", timeout: 20000 };
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-decision-"));
  fs.mkdirSync(path.join(tmpDir, "projects"), { recursive: true });
});
afterEach(() => fs.rmSync(tmpDir, { recursive: true, force: true }));

describe("gaganfoxwell-decision-log", () => {
...
describe("gaganfoxwell-decision-search", () => {
  ...
  test("redact + compact expunges everywhere", () => {
    ...
    const archive = path.join(tmpDir, "projects", "garrytan-gaganfoxwell", "decisions.archive.jsonl");
...
describe("gaganfoxwell-decision-search --semantic (optional gbrain enhancement)", () => {
  ...
  function searchWithPath(args: string, pathPrefix?: string): string {
    const env = { ...process.env, GAGANFOXWELL_HOME: tmpDir } as NodeJS.ProcessEnv;
  ...
  test("--semantic --query appends a related-memory block when gbrain returns hits", () => {
    ...
    const dir = shimDir(
      `#!/usr/bin/env bash
if [ "$1" = "sources" ]; then echo '{"sources":[{"id":"default","local_path":"/u/.gaganfoxwell-brain-worktree"}]}'; exit 0; fi
...
describe("gaganfoxwell-decision-search --recent / --scope / datamark", () => {
