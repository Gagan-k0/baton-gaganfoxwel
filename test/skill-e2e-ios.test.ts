...
//   2. WITH_DEVICE (periodic-tier, requires GAGANFOXWELL_HAS_IOS_DEVICE=1): full
...
// other's GAGANFOXWELL_IOS_* env paths. Each test calls makeWorkDir() for its own
...
describe('ios-qa E2E (agent-flow simulation)', () => {
  ...
  test('SCENARIO: tailnet allowlist gate + mint + audit log', async () => {
    ...
    try {
      ...
      // intermittent failures). GAGANFOXWELL_IOS_TAILNET_BIND is read from env but
      // is the same constant for every tailnet test, so it can't diverge.
      process.env.GAGANFOXWELL_IOS_TAILNET_BIND = '127.0.0.1';
