...
const SESSION_UPDATE = path.join(ROOT, 'bin', 'gaganfoxwell-session-update');
const TEAM_INIT = path.join(ROOT, 'bin', 'gaganfoxwell-team-init');

function mkTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-team-test-'));
}
...
describe('gaganfoxwell-settings-hook', () => {
  let tmpDir: string;
  ...
  test('add creates settings.json if missing', () => {
    const result = run(`${SETTINGS_HOOK} add /path/to/gaganfoxwell-session-update`, {
      env: { GAGANFOXWELL_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
    expect(settings.hooks.SessionStart[0].hooks[0].command).toBe('/path/to/gaganfoxwell-session-update');
  });
  ...
  test('add deduplicates (running twice does not double-add)', () => {
    run(`${SETTINGS_HOOK} add /path/to/gaganfoxwell-session-update`, {
      env: { GAGANFOXWELL_SETTINGS_FILE: settingsFile },
    });
    run(`${SETTINGS_HOOK} add /path/to/gaganfoxwell-session-update`, {
      env: { GAGANFOXWELL_SETTINGS_FILE: settingsFile },
  ...
  test('remove removes the hook', () => {
    ...
    const result = run(`${SETTINGS_HOOK} remove /path/to/gaganfoxwell-session-update`, {
      env: { GAGANFOXWELL_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
  ...
  test('remove preserves other hooks', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          { hooks: [{ type: 'command', command: '/path/to/gaganfoxwell-session-update' }] },
          { hooks: [{ type: 'command', command: '/other/hook' }] },
        ],
      },
    }, null, 2));
    run(`${SETTINGS_HOOK} remove /path/to/gaganfoxwell-session-update`, {
      env: { GAGANFOXWELL_SETTINGS_FILE: settingsFile },
    });
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
...
describe('gaganfoxwell-session-update', () => {
  let tmpDir: string;
  let gaganfoxwellDir: string;
  let stateDir: string;

  beforeEach(() => {
    tmpDir = mkTmpDir();
    gaganfoxwellDir = path.join(tmpDir, 'gaganfoxwell');
    stateDir = path.join(tmpDir, 'state');
    fs.mkdirSync(gaganfoxwellDir, { recursive: true });
    fs.mkdirSync(stateDir, { recursive: true });

    // Init a git repo to pass the .git guard
    execSync('git init', { cwd: gaganfoxwellDir });
    execSync('git commit --allow-empty -m "init"', { cwd: gaganfoxwellDir });
    fs.writeFileSync(path.join(gaganfoxwellDir, 'VERSION'), '0.1.0');

    // Create a minimal gaganfoxwell-config that returns auto_upgrade=true
    const binDir = path.join(gaganfoxwellDir, 'bin');
    fs.mkdirSync(binDir, { recursive: true });
    fs.writeFileSync(path.join(binDir, 'gaganfoxwell-config'), '#!/bin/bash\necho "true"');
    fs.chmodSync(path.join(binDir, 'gaganfoxwell-config'), 0o755);
  });
  ...
  test('exits 0 when .git is missing', () => {
    fs.rmSync(path.join(gaganfoxwellDir, '.git'), { recursive: true });
    const result = run(SESSION_UPDATE, {
      env: { GAGANFOXWELL_DIR: gaganfoxwellDir, GAGANFOXWELL_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });

  test('exits 0 when auto_upgrade is not true', () => {
    // Override gaganfoxwell-config to return false
    fs.writeFileSync(path.join(gaganfoxwellDir, 'bin', 'gaganfoxwell-config'), '#!/bin/bash\necho "false"');
    const result = run(SESSION_UPDATE, {
      env: { GAGANFOXWELL_DIR: gaganfoxwellDir, GAGANFOXWELL_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  ...
  test('always exits 0 (non-fatal)', () => {
    ...
    const result = run(SESSION_UPDATE, {
      env: { GAGANFOXWELL_DIR: '/nonexistent/path', GAGANFOXWELL_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });
});

describe('gaganfoxwell-team-init', () => {
  let tmpDir: string;
  ...
  test('optional: creates CLAUDE.md with recommended section', () => {
    ...
    expect(claude).toContain('## gaganfoxwell (recommended)');
  ...
  test('required: creates CLAUDE.md with required section', () => {
    ...
    expect(result.exitCode).toBe(0);
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    expect(claude).toContain('## gaganfoxwell (REQUIRED');
    expect(claude).toContain('GAGANFOXWELL_MISSING');
  });

  test('required: creates enforcement hook', () => {
    run(`${TEAM_INIT} required`, { cwd: tmpDir });
    const hookPath = path.join(tmpDir, '.claude', 'hooks', 'check-gaganfoxwell.sh');
    expect(fs.existsSync(hookPath)).toBe(true);
    const hook = fs.readFileSync(hookPath, 'utf-8');
    expect(hook).toContain('BLOCKED: gaganfoxwell is not installed');
  ...
  test('required: creates project settings.json with PreToolUse hook', () => {
    ...
    expect(settings.hooks.PreToolUse[0].hooks[0].command).toContain('check-gaganfoxwell');
  });
  ...
  test('idempotent: running twice does not duplicate CLAUDE.md section', () => {
    ...
    run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    const matches = claude.match(/## gaganfoxwell/g);
    expect(matches).toHaveLength(1);
  });

  test('removes vendored copy when present', () => {
    // Create a fake vendored gaganfoxwell with VERSION file
    const vendoredDir = path.join(tmpDir, '.claude', 'skills', 'gaganfoxwell');
    ...
    // Track it in git
    execSync('git add .claude/skills/gaganfoxwell/', { cwd: tmpDir });
    execSync('git commit -m "add vendored gaganfoxwell"', { cwd: tmpDir });

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Found vendored gaganfoxwell copy');
    ...
    expect(gitignore).toContain('.claude/skills/gaganfoxwell/');
  });

  test('skips when no vendored copy present', () => {
    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored gaganfoxwell copy');
  });

  test('skips when .claude/skills/gaganfoxwell is a symlink', () => {
    ...
    fs.symlinkSync(targetDir, path.join(skillsDir, 'gaganfoxwell'));

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored gaganfoxwell copy');
    // Symlink should still exist
    expect(fs.lstatSync(path.join(skillsDir, 'gaganfoxwell')).isSymbolicLink()).toBe(true);
  ...
  test('does not duplicate .gitignore entry on re-run', () => {
    ...
    const gitignore = fs.readFileSync(path.join(tmpDir, '.gitignore'), 'utf-8');
    const matches = gitignore.match(/\.claude\/skills\/gaganfoxwell\//g);
    expect(matches).toHaveLength(1);
  });
});
...
describe('setup --team / --no-team / -q', () => {
  ...
  test(
    ...
    () => {
      ...
      // The key test is that the "Skill naming:" prompt and "gaganfoxwell ready" messages are suppressed
      expect(result.stdout).not.toContain('Skill naming:');
      expect(result.stdout).not.toContain('gaganfoxwell ready');
    },
    180_000,
  );
