...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // gaganfoxwell-review-read reads from ~/.gaganfoxwell/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock gaganfoxwell-review-read script that returns our test data
    ...
    // Write a mock gaganfoxwell-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'gaganfoxwell-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'gaganfoxwell-review-read'), 0o755);
...
Instead of running ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-read, run this mock: ${mockBinDir}/gaganfoxwell-review-read
