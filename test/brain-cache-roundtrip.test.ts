 ...
 *
 * Uses tmp GAGANFOXWELL_HOME per-test to avoid polluting the real ~/.gaganfoxwell/.
 * Gate-tier, free, ~50ms.
 */
...
let TMP_HOME: string;
const ORIGINAL_HOME = process.env.GAGANFOXWELL_HOME;

beforeEach(() => {
  TMP_HOME = mkdtempSync(join(tmpdir(), 'gaganfoxwell-cache-test-'));
...
afterEach(() => {
  if (ORIGINAL_HOME) process.env.GAGANFOXWELL_HOME = ORIGINAL_HOME;
  else delete process.env.GAGANFOXWELL_HOME;
...
describe('brain-cache paths', () => {
  test('cross-project entity (user-profile) lives in ~/.gaganfoxwell/brain-cache/', async () => {
  ...
  test('per-project entity (product) lives in ~/.gaganfoxwell/projects/<slug>/brain-cache/', async () => {
    const mod = await importCache();
...
describe('brain-cache state machine', () => {
  test('warm: pre-seeded fresh cache returns warm without touching brain', async () => {
    ...
    writeFileSync(join(cacheDir, '_meta.json'), JSON.stringify({
      schema_version: '1.0.0', // matches GAGANFOXWELL_SCHEMA_PACK_VERSION
      endpoint_hash: mod.detectEndpointHash(),
