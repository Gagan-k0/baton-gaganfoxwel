...
const SCRIPT = path.join(ROOT, 'bin', 'gaganfoxwell-session-update');
...
function makeFixture() {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-supd-'));
  ...
  fs.writeFileSync(
    path.join(seed, 'bin', 'gaganfoxwell-config'),
    '#!/usr/bin/env bash\nif [ "$1" = "get" ]; then case "$2" in auto_upgrade) echo true;; skill_prefix) echo false;; *) echo "";; esac; fi\nexit 0\n',
    { mode: 0o755 },
  );
  fs.writeFileSync(path.join(seed, 'bin', 'gaganfoxwell-patch-names'), '#!/usr/bin/env bash\nexit 0\n', {
    mode: 0o755,
  });
  git(seed, 'init', '-q');
  git(seed, 'add', '-A');
...
function runScript(install: string, state: string) {
  return spawnSync('bash', [SCRIPT], {
    encoding: 'utf8',
    env: { ...process.env, GAGANFOXWELL_DIR: install, GAGANFOXWELL_STATE_DIR: state },
    timeout: 20000,
  });
}
...
describe('gaganfoxwell-session-update pull wedge (#2566)', () => {
...
describe('gaganfoxwell-session-update lock identity + TTL (#2613)', () => {
  ...
  function runScriptWithPath(install: string, state: string, shimDir: string) {
    return spawnSync('bash', [SCRIPT], {
      encoding: 'utf8',
      env: { ...process.env, GAGANFOXWELL_DIR: install, GAGANFOXWELL_STATE_DIR: state, PATH: `${shimDir}:${process.env.PATH}` },
      timeout: 20000,
    });
  }
