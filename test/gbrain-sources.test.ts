 ...
 * invocations. The same trick `test/gaganfoxwell-gbrain-source-wireup.test.ts` uses.
 */
...
describe("probeSource", () => {
  it("returns absent when source id is not in the list", () => {
    ...
    const state = probeSource("gaganfoxwell-code-foo", fake.env);
  ...
  it("returns match when source id is registered (path included)", () => {
    const fake = makeFakeGbrain({
      sources: [{ id: "gaganfoxwell-code-foo", local_path: "/Users/me/repo" }],
    });
    const state = probeSource("gaganfoxwell-code-foo", fake.env);
...
describe("ensureSourceRegistered", () => {
  it("adds source when absent, returns changed=true", async () => {
    ...
    const result = await ensureSourceRegistered("gaganfoxwell-code-foo", "/Users/me/repo", {
      federated: true,
      env: fake.env,
    });
    ...
    expect(log).toContain("sources add gaganfoxwell-code-foo --path /Users/me/repo --federated");
  ...
  it("is a no-op when source is already at the correct path, returns changed=false", async () => {
    ...
    const result = await ensureSourceRegistered("gaganfoxwell-code-foo", "/Users/me/repo", { env: fake.env });
  ...
  it("recreates source when path differs (gbrain has no `sources update`), returns changed=true", async () => {
    const fake = makeFakeGbrain({
      sources: [{ id: "gaganfoxwell-code-foo", local_path: "/old/path" }],
    });
    const result = await ensureSourceRegistered("gaganfoxwell-code-foo", "/new/path", {
      federated: true,
      env: fake.env,
    ...
    expect(log).toContain("sources remove gaganfoxwell-code-foo --yes --confirm-destructive");
    expect(log).toContain("sources add gaganfoxwell-code-foo --path /new/path --federated");
    fake.cleanup();
  });
  ...
  it("REFUSES the drift remove while autopilot is active (throws, no remove issued)", async () => {
    ...
    await expect(
      ensureSourceRegistered("gaganfoxwell-code-foo", "/new/path", {
        env: fake.env,
  ...
  it("propagates decideSourceRemove extraArgs (--keep-storage) to the drift remove", async () => {
    ...
    expect(log).toContain(
      "sources remove gaganfoxwell-code-foo --yes --confirm-destructive --keep-storage",
    );
    fake.cleanup();
  });
  ...
  it("does NOT declare drift when registered path is a symlink alias of the requested path", async () => {
    ...
    const fake = makeFakeGbrain({
      sources: [{ id: "gaganfoxwell-code-foo", local_path: realDir }],
    });
    const result = await ensureSourceRegistered("gaganfoxwell-code-foo", linkDir, {
      env: fake.env,
      ...guardsPinnedInactive,
    });
    expect(result.changed).toBe(false);
...
describe("sourcePageCount", () => {
  it("returns the page_count when the source is registered", () => {
    const fake = makeFakeGbrain({
      sources: [
        { id: "gaganfoxwell-code-foo", local_path: "/x", page_count: 1247 },
        { id: "other-source", local_path: "/y", page_count: 99 },
      ],
    });
    expect(sourcePageCount("gaganfoxwell-code-foo", fake.env)).toBe(1247);
