/**
 * gaganfoxwell-distill-apply — Layer 8 proposal application (plan-tune cathedral T11).
 *
 * Verifies the three apply paths:
 *   - memory-nugget → appended to ~/.gaganfoxwell/free-text-memory.json (local
 *     source-of-truth; gbrain is mirror when configured).
 *   - preference   → routed through gaganfoxwell-question-preference with
...
const BIN = path.join(ROOT, 'bin', 'gaganfoxwell-distill-apply');
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-apply-'));
...
describe('memory-nugget apply', () => {
  test('appends to ~/.gaganfoxwell/free-text-memory.json with full metadata', () => {
    writeProposals([
      {
        kind: 'memory-nugget',
        confidence: 0.9,
...
describe('preference apply', () => {
  test('routes through gaganfoxwell-question-preference with source=plan-tune', () => {
    writeProposals([
      {
        kind: 'preference',
        confidence: 0.9,
