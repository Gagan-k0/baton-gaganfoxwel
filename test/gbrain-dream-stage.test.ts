/**
 * Tests for the dream (call-graph build) stage of bin/gaganfoxwell-gbrain-sync.ts.
 *
...
describe("dream marker — concurrency guard", () => {
  const saved = process.env.GAGANFOXWELL_HOME;
  let tmp: string;

  afterEach(() => {
    if (tmp) rmSync(tmp, { recursive: true, force: true });
    if (saved === undefined) delete process.env.GAGANFOXWELL_HOME;
    else process.env.GAGANFOXWELL_HOME = saved;
  });

  function redirectHome(): void {
    tmp = mkdtempSync(join(tmpdir(), "gbrain-dream-marker-"));
    process.env.GAGANFOXWELL_HOME = tmp;
  }
