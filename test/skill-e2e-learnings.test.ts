...
describeIfSelected('Learnings E2E', ['learnings-show'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of ['gaganfoxwell-learnings-search', 'gaganfoxwell-learnings-log', 'gaganfoxwell-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // Seed learnings JSONL — slug must match what gaganfoxwell-slug computes.
    // With no git remote, gaganfoxwell-slug falls back to basename(workDir).
...
- Replace any references to ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug with ./bin/gaganfoxwell-slug.
