...
function copyIntoFakeInstall(workDir: string): { root: string; launcher: string } {
  const root = join(workDir, 'fake gaganfoxwell install');
  ...
  const launcher = join(binDir, 'gaganfoxwell-ios-qa-regen');
  copyFileSync(join(ROOT, 'bin', 'gaganfoxwell-ios-qa-regen'), launcher);
...
describe('gaganfoxwell-ios-qa-regen', () => {
  test('repository launcher is executable', () => {
    expect(statSync(join(ROOT, 'bin', 'gaganfoxwell-ios-qa-regen')).mode & 0o111).not.toBe(0);
  });
  ...
  test('leaves no completion marker when accessor generation fails', () => {
    ...
    writeFileSync(join(generatedDir, '.gaganfoxwell-version'), 'stale-complete-marker\n');
    ...
    expect(existsSync(join(generatedDir, '.gaganfoxwell-version'))).toBe(false);
  });
...
`);
    ...
    const env = {
      ...process.env,
      GAGANFOXWELL_IOS_CACHE_ROOT: cacheRoot,
      SWIFT_VERSION: '6.3.3',
    ...
    expect(readFileSync(join(generatedDir, '.gaganfoxwell-version'), 'utf8')).toBe(
