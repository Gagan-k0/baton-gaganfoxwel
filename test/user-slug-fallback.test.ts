 ...
 *
 * Verifies the gaganfoxwell-config resolve-user-slug subcommand walks the
 ...
 * Test isolation via GAGANFOXWELL_HOME and HOME env overrides.
 *
 * Gate-tier, free, ~50ms.
 */
...
const CONFIG_BIN = join(REPO_ROOT, 'bin', 'gaganfoxwell-config');
...
beforeEach(() => {
  TMP_HOME = mkdtempSync(join(tmpdir(), 'gaganfoxwell-user-slug-test-'));
  process.env.GAGANFOXWELL_HOME = TMP_HOME;
});
...
describe('endpoint-hash subcommand', () => {
  test('returns deterministic 8-char hex or literal "local"', () => {
    const result = runConfig(['endpoint-hash'], { GAGANFOXWELL_HOME: TMP_HOME });
...
describe('resolve-user-slug fallback chain', () => {
  test('uses $USER when set (layer 2)', () => {
    const result = runConfig(['resolve-user-slug'], { GAGANFOXWELL_HOME: TMP_HOME, USER: 'alice-test' });
    expect(result.status).toBe(0);
  ...
  test('falls through past empty $USER to git email or anonymous', () => {
    const result = runConfig(['resolve-user-slug'], { GAGANFOXWELL_HOME: TMP_HOME, USER: '' });
    expect(result.status).toBe(0);
  ...
  test('persists resolution to user_slug_at_<endpoint-id> on first call', () => {
    runConfig(['resolve-user-slug'], { GAGANFOXWELL_HOME: TMP_HOME, USER: 'persisttest' });
  ...
  test('subsequent calls return same slug (stable across sessions)', () => {
    const first = runConfig(['resolve-user-slug'], { GAGANFOXWELL_HOME: TMP_HOME, USER: 'stabletest' });
    const second = runConfig(['resolve-user-slug'], { GAGANFOXWELL_HOME: TMP_HOME, USER: 'changed-after' });
...
describe('brain_trust_policy@<endpoint-id> namespace', () => {
  test('default value is "unset"', () => {
    const result = runConfig(['get', 'brain_trust_policy@deadbeef'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(result.status).toBe(0);
    expect(result.stdout).toBe('unset');
  });

  test('set + get roundtrip works', () => {
    const setResult = runConfig(['set', 'brain_trust_policy@deadbeef', 'personal'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(setResult.status).toBe(0);
    const getResult = runConfig(['get', 'brain_trust_policy@deadbeef'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(getResult.stdout).toBe('personal');
  });

  test('invalid value falls back to unset with warning', () => {
    const result = runConfig(['set', 'brain_trust_policy@deadbeef', 'invalid-value'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(result.status).toBe(0);
  ...
  test('shared value accepted', () => {
    runConfig(['set', 'brain_trust_policy@deadbeef', 'shared'], { GAGANFOXWELL_HOME: TMP_HOME });
    const getResult = runConfig(['get', 'brain_trust_policy@deadbeef'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(getResult.stdout).toBe('shared');
  });

  test('per-endpoint policies dont collide', () => {
    runConfig(['set', 'brain_trust_policy@aaaaaaaa', 'personal'], { GAGANFOXWELL_HOME: TMP_HOME });
    runConfig(['set', 'brain_trust_policy@bbbbbbbb', 'shared'], { GAGANFOXWELL_HOME: TMP_HOME });
    const a = runConfig(['get', 'brain_trust_policy@aaaaaaaa'], { GAGANFOXWELL_HOME: TMP_HOME });
    const b = runConfig(['get', 'brain_trust_policy@bbbbbbbb'], { GAGANFOXWELL_HOME: TMP_HOME });
...
describe('key validation', () => {
  test('rejects keys with disallowed characters', () => {
    const result = runConfig(['get', 'bad-key'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('alphanumeric');
  });

  test('accepts plain alphanumeric/underscore keys', () => {
    const result = runConfig(['get', 'proactive'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(result.status).toBe(0);
  });

  test('accepts @<hex-hash> suffix on key', () => {
    const result = runConfig(['get', 'brain_trust_policy@abc123ff'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(result.status).toBe(0);
  });

  test('accepts @local suffix on key', () => {
    const result = runConfig(['get', 'brain_trust_policy@local'], { GAGANFOXWELL_HOME: TMP_HOME });
    expect(result.status).toBe(0);
    expect(result.stdout).toBe('unset');
  });
});
