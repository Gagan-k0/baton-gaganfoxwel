...
`);
    ...
    projectDir = path.join(os.homedir(), '.gaganfoxwell', 'projects', 'test-project');
...
// Verifies that plan-eng-review writes a "## GAGANFOXWELL REVIEW REPORT" section
...
`);
  ...
  test('/plan-eng-review writes GAGANFOXWELL REVIEW REPORT to plan file', async () => {
    const result = await runSkillTest({
      prompt: `Read plan-eng-review/SKILL.md for the review workflow.
...
CRITICAL REQUIREMENT: plan.md IS the plan file for this review session. After completing your review, you MUST write a "## GAGANFOXWELL REVIEW REPORT" section to the END of plan.md, exactly as described in the "Plan File Review Report" section of SKILL.md. If gaganfoxwell-review-read is not available or returns NO_REVIEWS, write the placeholder table with all five review rows (CEO, Codex, Eng, Design, DX). The report MUST end with the mandatory unresolved-decisions status as its final line — the exact unbolded line NO UNRESOLVED DECISIONS when nothing is open, or a "**UNRESOLVED DECISIONS:**" block of bullets when items remain. Nothing may follow it. Use the Edit tool to append to plan.md — do NOT overwrite the existing plan content.
...
This review report at the bottom of the plan is the MOST IMPORTANT deliverable of this test.`,
    ...
    expect(planContent).toContain('## GAGANFOXWELL REVIEW REPORT');

    // Report should be at the bottom of the file
    const reportIndex = planContent.lastIndexOf('## GAGANFOXWELL REVIEW REPORT');
