/**
 * Coverage for #1606 — `_gaganfoxwell_gbrain_validate_varname` LC_ALL=C pin.
 *
 ...
 * failures.
 *
 * Tests exercise the validator by sourcing bin/gaganfoxwell-gbrain-lib.sh and
 * calling _gaganfoxwell_gbrain_validate_varname directly. Asserts:
...
const LIB = path.join(ROOT, "bin", "gaganfoxwell-gbrain-lib.sh");
...
function runValidator(name: string): { status: number | null } {
  ...
  const result = spawnSync(
    "bash",
    ["-c", `. "${LIB}"; _gaganfoxwell_gbrain_validate_varname "$1"`, "bash", name],
    {
      encoding: "utf-8",
      timeout: 5000,
...
describe("#1606 _gaganfoxwell_gbrain_validate_varname — LC_ALL=C pin", () => {
  ...
  test("LC_ALL=C is local to the validator (does not leak to caller)", () => {
    ...
    const result = spawnSync(
      "bash",
      ["-c", `. "${LIB}"; LC_ALL=fr_FR.UTF-8; _gaganfoxwell_gbrain_validate_varname FOO; echo "$LC_ALL"`],
      {
        encoding: "utf-8",
        timeout: 5000,
