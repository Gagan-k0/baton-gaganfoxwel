/**
 * gaganfoxwell-artifacts-init — provider-selection + brain-admin-hookup tests.
 *
 * Mirrors the gaganfoxwell-brain-init-gh-mock.test.ts pattern: install fake gh /
 ...
 * ~/.gaganfoxwell-artifacts-remote.txt, (c) prints the "Send this to your brain
...
const INIT_BIN = path.join(ROOT, 'bin', 'gaganfoxwell-artifacts-init');
...
function makeFakeGh(opts: {
  ...
  const webUrl = opts.webUrl ?? `https://github.com/testuser/gaganfoxwell-artifacts-testuser`;
...
function makeFakeGlab(opts: {
  ...
  const authStatus = opts.authStatus ?? 'ok';
  const repoCreate = opts.repoCreate ?? 'success';
  const webUrl = opts.webUrl ?? 'https://gitlab.com/testuser/gaganfoxwell-artifacts-testuser';
  const gitProtocol = opts.gitProtocol ?? 'https';
  const script = `#!/bin/bash
...
describe('gaganfoxwell-artifacts-init provider selection', () => {
  test('--host github invokes gh repo create with gaganfoxwell-artifacts-$USER', () => {
    makeFakeGh({});
    ...
    expect(createCall).toContain('gaganfoxwell-artifacts-testuser');
  ...
  test('neither authed → falls through to manual URL paste', () => {
    // No gh, no glab fakes.
    const r = run([], { input: 'https://github.com/testuser/gaganfoxwell-artifacts-testuser\n' });
    expect(r.status).toBe(0);
    expect(r.stderr).toContain('Neither gh nor glab');
  });
});

describe('gaganfoxwell-artifacts-init canonical URL storage (codex Finding #10)', () => {
  test('stores HTTPS URL canonical in ~/.gaganfoxwell-artifacts-remote.txt', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/gaganfoxwell-artifacts-testuser' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const remoteFile = path.join(tmpHome, '.gaganfoxwell-artifacts-remote.txt');
    ...
    expect(stored).toBe('https://github.com/testuser/gaganfoxwell-artifacts-testuser');
  });

  test('strips trailing .git from gh repo view output', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/gaganfoxwell-artifacts-testuser.git' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const stored = fs.readFileSync(path.join(tmpHome, '.gaganfoxwell-artifacts-remote.txt'), 'utf-8').trim();
    expect(stored).toBe('https://github.com/testuser/gaganfoxwell-artifacts-testuser');
  });
  ...
  test('configures git origin with HTTPS when gh git_protocol is https', () => {
    ...
    expect(remote.stdout.trim()).toBe('https://github.com/testuser/gaganfoxwell-artifacts-testuser');
  });

  test('configures git origin with SSH when gh git_protocol is ssh', () => {
    makeFakeGh({
      webUrl: 'https://github.com/testuser/gaganfoxwell-artifacts-testuser',
      gitProtocol: 'ssh',
    });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const remote = spawnSync('git', ['-C', tmpHome, 'remote', 'get-url', 'origin'], { encoding: 'utf-8' });
    expect(remote.stdout.trim()).toBe('git@github.com:testuser/gaganfoxwell-artifacts-testuser.git');
  });
  ...
  test('honors glab git_protocol when configured', () => {
    ...
    expect(r.status).toBe(0);
    const remote = spawnSync('git', ['-C', tmpHome, 'remote', 'get-url', 'origin'], { encoding: 'utf-8' });
    expect(remote.stdout.trim()).toBe('git@gitlab.com:testuser/gaganfoxwell-artifacts-testuser.git');
  });
});

describe('gaganfoxwell-artifacts-init brain-admin hookup printout (codex Finding #3)', () => {
  ...
  test('--url-form-supported true prints the one-liner with --url', () => {
    ...
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('Send this to your brain admin');
    expect(r.stdout).toContain('gbrain sources add gaganfoxwell-artifacts-testuser --url');
  ...
  test('the gbrain command line uses canonical HTTPS, not SSH', () => {
    ...
    expect(gbrainLine).toContain('https://github.com/testuser/gaganfoxwell-artifacts-testuser');
    expect(gbrainLine).not.toContain('git@github.com');
    // The Push line follows the provider CLI preference independently.
  });
});

describe('gaganfoxwell-artifacts-init idempotency', () => {
  test('--remote <url> bypasses provider selection entirely', () => {
    makeFakeGh({});
    const r = run(['--remote', 'https://github.com/testuser/gaganfoxwell-artifacts-testuser']);
    expect(r.status).toBe(0);
  ...
  test('--push-protocol overrides the inferred protocol', () => {
    ...
    const r = run([
      '--remote',
      'https://github.com/testuser/gaganfoxwell-artifacts-testuser',
      '--push-protocol',
      'ssh',
    ]);
    expect(r.status).toBe(0);
  ...
  test('re-run with same --remote is safe (no conflict error)', () => {
    makeFakeGh({});
    const url = 'https://github.com/testuser/gaganfoxwell-artifacts-testuser';
    run(['--remote', url]);
  ...
  test('re-run with DIFFERENT --remote exits 1 with conflict message', () => {
    makeFakeGh({});
    run(['--remote', 'https://github.com/testuser/gaganfoxwell-artifacts-testuser']);
