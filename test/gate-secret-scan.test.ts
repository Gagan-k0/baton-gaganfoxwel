 ...
 * into bin/gaganfoxwell-redact and enforces: HIGH fails (exit 1), MEDIUM is an
...
describe("gate-secret-scan.mjs fail-closed legs", () => {
  test("oversize diff (report.oversize) fails the gate", () => {
    // The script pins --max-bytes 16000000; bin/gaganfoxwell-redact refuses to scan
  ...
  test("unexpected gaganfoxwell-redact exit code fails the gate even when the report is clean", () => {
    // Stub bin/gaganfoxwell-redact that emits a CLEAN JSON report but exits 1 —
    ...
    try {
      ...
      writeFileSync(
        join(dir, "bin", "gaganfoxwell-redact"),
        [
          "#!/usr/bin/env bun",
          'let input = "";',
  ...
  test("missing gaganfoxwell-redact (spawn crash, empty stdout) exits nonzero — never fail-open", () => {
    // cwd with no bin/gaganfoxwell-redact at all: bun exits module-not-found with
