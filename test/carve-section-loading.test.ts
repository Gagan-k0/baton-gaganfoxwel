 ...
 *   #3 / D-CODEX(A) — GAGANFOXWELL_CARVE_SKILL=<name> runs only that skill's case, so
...
const only = process.env.GAGANFOXWELL_CARVE_SKILL?.trim();
...
describeE2E('carve behavioral section-loading (periodic, SDK capture)', () => {
  for (const guard of Object.values(CARVE_GUARDS)) {
    ...
    // Cost-scoped selection: when GAGANFOXWELL_CARVE_SKILL is set, run only that skill.
    ...
    test(
      ...
      async () => {
        ...
        const planDir = setupSkillDir({
          ...
          sectionsFrom,
          fixtures,
          tmpPrefix: `gaganfoxwell-${guard.skill}-secload-`,
        });
