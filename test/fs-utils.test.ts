 ...
 * `gaganfoxwell-redact install-prepush-hook` on any repo whose .git/hooks already
...
describe("swept mkdirp sites under bun-on-Windows EEXIST semantics (#2635)", () => {
  const DECISION_LOG = path.resolve(import.meta.dir, "..", "bin", "gaganfoxwell-decision-log");
  ...
  test("decision-log still writes when its projects dir already exists", () => {
    ...
    // ~/.gaganfoxwell/projects/<slug>/, the second hits the emulated Windows EEXIST
...
describe("install-prepush-hook under bun-on-Windows EEXIST semantics (#2635)", () => {
  test("install succeeds when .git/hooks already exists, existing hook preserved", () => {
    ...
    try {
      ...
      expect(fs.readFileSync(hookPath, "utf8")).toContain("gaganfoxwell-redact pre-push (managed)");
