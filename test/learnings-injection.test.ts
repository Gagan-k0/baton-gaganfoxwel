...
const SCRIPT = path.join(import.meta.dir, "..", "bin", "gaganfoxwell-learnings-search");

describe("gaganfoxwell-learnings-search injection prevention", () => {
  ...
  test("uses process.env for all user-controlled values", () => {
    ...
    expect(bunBlock).toContain("process.env.GAGANFOXWELL_SEARCH_TYPE");
    expect(bunBlock).toContain("process.env.GAGANFOXWELL_SEARCH_QUERY");
    expect(bunBlock).toContain("process.env.GAGANFOXWELL_SEARCH_LIMIT");
  });
  ...
  test("env vars are set on the bun command line", () => {
    ...
    expect(script).toContain("GAGANFOXWELL_SEARCH_TYPE=");
    expect(script).toContain("GAGANFOXWELL_SEARCH_QUERY=");
    expect(script).toContain("GAGANFOXWELL_SEARCH_LIMIT=");
    expect(script).toContain("GAGANFOXWELL_SEARCH_CROSS=");
  });
