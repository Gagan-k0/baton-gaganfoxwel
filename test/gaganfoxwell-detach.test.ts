/**
 * gaganfoxwell-detach — the eval-infra robustness guard. Pins the four killer fixes:
...
const DETACH = path.join(ROOT, 'bin', 'gaganfoxwell-detach');
...
describe('gaganfoxwell-detach', () => {
  test('detaches (different pgid), returns immediately, completes, writes EXIT sentinel', () => {
    ...
    try {
      ...
      expect(r.stdout).toContain(`gaganfoxwell-detach LOG ${log}`);
      expect(elapsed).toBeLessThan(1500);                         // non-blocking
      expect(waitFor(() => logHas(log, '### gaganfoxwell-detach EXIT=0 ###'), 8000)).toBe(true);
  ...
  test('default log is run-scoped under ~/.gaganfoxwell-dev/eval-runs (no shared /tmp)', () => {
    const r = spawnSync(DETACH, ['--label', 'unittest', '--', 'true'], { encoding: 'utf-8', timeout: 10000 });
    const log = (r.stdout.match(/gaganfoxwell-detach LOG (\S+)/) || [])[1];
    try {
      expect(log).toContain('/.gaganfoxwell-dev/eval-runs/');
      expect(path.basename(log)).toContain('unittest-');
      expect(path.basename(log)).toMatch(/-\d+\.log$/);            // pid-unique
      waitFor(() => logHas(log, '### gaganfoxwell-detach EXIT=0 ###'), 6000);
  ...
  test('watchdog kills a stalled run and records EXIT=timeout (no silent hang)', () => {
    ...
    try {
      ...
      expect(waitFor(() => logHas(log, '### gaganfoxwell-detach EXIT=timeout ###'), 12000)).toBe(true);
  ...
  test('machine --lock serializes concurrent runs (second WAITS for the first)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gd-'));
    const lock = `gaganfoxwell-detach-test-${process.pid}`;
    ...
    try {
      ...
      expect(waitFor(() => logHas(logB, '### gaganfoxwell-detach EXIT=0 ###'), 12000)).toBe(true);
      expect(logHas(logB, 'second-ran')).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
      fs.rmSync(path.join(os.homedir(), '.gaganfoxwell', 'locks', `${lock}.lock`), { force: true });
    }
  }, 20000);
