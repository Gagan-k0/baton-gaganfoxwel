/**
 * gaganfoxwell-upgrade/migrations/v1.0.0.0.sh — writing style migration.
 *
 * Coverage:
...
const MIGRATION = path.join(ROOT, 'gaganfoxwell-upgrade', 'migrations', 'v1.0.0.0.sh');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-mig-test-'));
});
...
function run(): { stdout: string; stderr: string; status: number } {
  // Override HOME too — the migration reads `${HOME}/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config`,
