...
// Static tripwires for the C (machine-wide) render in `gaganfoxwell-config
...
const SRC = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-config'), 'utf-8');
...
describe('gaganfoxwell-config gbrain-refresh: machine-wide render guards', () => {
  const branch = okBranch();

  test('targets the global install', () => {
    expect(branch).toContain('$HOME/.claude/skills/gaganfoxwell');
  });
  ...
  test('verifies it is a real gaganfoxwell clone before mutating it', () => {
