 ...
 *
 * 2. gaganfoxwell-team-init's required-mode enforcement (the CLAUDE.md
 *    verification snippet and the generated check-gaganfoxwell.sh hook) must
 *    resolve the install root across GAGANFOXWELL_ROOT + every host's global
 *    install location, never hardcode ~/.claude/skills/gaganfoxwell. The drift
...
describe('team-init resolves GAGANFOXWELL_ROOT across every host (#2500)', () => {
  const teamInit = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-team-init'), 'utf-8');

  test('probe list covers GAGANFOXWELL_ROOT env + every registered host globalRoot + migrated repo', () => {
    expect(teamInit).toContain('"${GAGANFOXWELL_ROOT:-}"');
    for (const config of ALL_HOST_CONFIGS) {
      expect(teamInit).toContain(`$HOME/${config.globalRoot}`);
    }
    expect(teamInit).toContain('$HOME/.gaganfoxwell/repos/gaganfoxwell');
  });

  test('enforcement no longer hardcodes the Claude path as the only gate', () => {
    expect(teamInit).not.toContain('test -d ~/.claude/skills/gaganfoxwell/bin');
    expect(teamInit).not.toContain('if [ ! -d "$HOME/.claude/skills/gaganfoxwell/bin" ]');
  });
  ...
  test('generated hook blocks only when NO install root resolves', () => {
    ...
    expect(teamInit).toContain('if [ -z "$_GAGANFOXWELL_ROOT" ]; then');
  });
});
