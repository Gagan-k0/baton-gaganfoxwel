/**
 * gaganfoxwell-settings-hook schema-aware surface (T3 plan-tune cathedral).
 *
...
const SETTINGS_HOOK = path.join(ROOT, 'bin', 'gaganfoxwell-settings-hook');

let tmpDir: string;
let settingsFile: string;

beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-shsa-'));
...
function run(args: string[]): { stdout: string; stderr: string; exitCode: number } {
  try {
    const stdout = execSync([SETTINGS_HOOK, ...args].map((s) => `'${s}'`).join(' '), {
      env: { ...process.env, GAGANFOXWELL_SETTINGS_FILE: settingsFile },
      encoding: 'utf-8',
      timeout: 10000,
    });
...
describe('add-event', () => {
  test('registers a PreToolUse hook with matcher + source tag', () => {
    ...
    expect(s.hooks.PreToolUse[0]._gaganfoxwell_source).toBe('plan-tune-cathedral');
  ...
  test('dedup includes command: same (event, matcher, command) with different source updates in place', () => {
    ...
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(1);
    expect(s.hooks.PostToolUse[0]._gaganfoxwell_source).toBe('source-B');
  });
  ...
  test('dedup includes command: untagged entry with same command is updated not duplicated', () => {
    ...
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(1);
    expect(s.hooks.PostToolUse[0]._gaganfoxwell_source).toBe('plan-tune-cathedral');
  });
...
describe('ensure-event collapses duplicate (event, source) entries', () => {
  test('two same-source entries from the old matcher-keyed dedup collapse to ONE updated entry', () => {
    ...
    // (event, _gaganfoxwell_source) — the old dedup keyed on the matcher too, so a
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          { _gaganfoxwell_source: 'plan-tune-cathedral', matcher: 'OldMatcherA', hooks: [{ type: 'command', command: '/old-a', timeout: 5 }] },
          { matcher: 'Bash', hooks: [{ type: 'command', command: '/user-own-hook' }] },
          { _gaganfoxwell_source: 'plan-tune-cathedral', matcher: 'OldMatcherB', hooks: [{ type: 'command', command: '/old-b', timeout: 5 }] },
        ],
      },
    }, null, 2));
    ...
    const r = spawnSync('bash', [
      ...
      '--source', 'plan-tune-cathedral',
      '--timeout', '5',
    ], { env: { ...process.env, GAGANFOXWELL_SETTINGS_FILE: settingsFile }, encoding: 'utf-8', timeout: 15_000 });

    expect(r.status).toBe(0);
    // The collapse is reported on stderr, never silent.
    expect(r.stderr).toContain('collapsed 1 duplicate');
    const s = settings();
    const mine = s.hooks.PostToolUse.filter((e: any) => e._gaganfoxwell_source === 'plan-tune-cathedral');
  ...
  test('no duplicates → no collapse message, single entry updated as before', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          { _gaganfoxwell_source: 'plan-tune-cathedral', matcher: 'OldMatcher', hooks: [{ type: 'command', command: '/old', timeout: 5 }] },
        ],
      },
    }, null, 2));
...
// the fixed table) because Claude Code strips the _gaganfoxwell_source key when it
...
/** run() with hermetic gaganfoxwell-config state (prune-stale consults plan_tune_hooks). */
function runIso(args: string[], extraEnv: Record<string, string> = {}) {
  try {
    const stdout = execSync([SETTINGS_HOOK, ...args].map((s) => `'${s}'`).join(' '), {
      env: {
        ...process.env,
        GAGANFOXWELL_SETTINGS_FILE: settingsFile,
        GAGANFOXWELL_STATE_ROOT: tmpDir,
        ...extraEnv,
      },
      encoding: 'utf-8',
      timeout: 15000,
...
function mkCanon(base: string, name = 'canon'): string {
  ...
  const su = path.join(canon, 'bin', 'gaganfoxwell-session-update');
...
function hookEntry(cmd: string, matcher?: string, src?: string, extraItems: any[] = []) {
  ...
  if (src) e._gaganfoxwell_source = src;
  return e;
}
...
describe('add-event: per-item identity re-point', () => {
  ...
  test('foreign path with a gaganfoxwell basename is NOT claimed (wrong relpath suffix)', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
  ...
  test('mixed entry: only the gaganfoxwell item (index > 0) is replaced; the user item survives', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
...
describe('legacy remove: per-item (regression)', () => {
  test('mixed SessionStart entry: user item survives in place, gaganfoxwell item removed', () => {
    // REGRESSION pin: the pre-v1.67.2 legacy `remove` dropped the ENTIRE
    // entry when any item matched gaganfoxwell-session-update, destroying a user's
      ...
      hooks: {
        SessionStart: [{
          hooks: [
            ...
            { type: 'command', command: '/old/install/bin/gaganfoxwell-session-update' },
          ],
        }],
      },
    }, null, 2));
    const r = runIso(['remove', '/old/install/bin/gaganfoxwell-session-update']);
    expect(r.exitCode).toBe(0);
    const s = settings();
...
describe('review-army hardening (specialist findings)', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          ...
          { hooks: 'not-an-array' },
          { hooks: [] },
          { hooks: [{ type: 'command', command: '/x/bin/gaganfoxwell-session-update' }] },
        ],
      },
    }, null, 2));
    runIso(['remove', '/x/bin/gaganfoxwell-session-update']);
    const s = settings();
  ...
  test('add-event never tags a mixed entry (old-version ratchet guard)', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [{
          matcher: AUQ_MATCHER,
          _gaganfoxwell_source: 'plan-tune-cathedral',
          hooks: [
            { type: 'command', command: '/Users/me/my-own-hook' },
    ...
    expect(e._gaganfoxwell_source).toBeUndefined();
  });
  ...
  test('a 0600 settings.json keeps its mode across mutations (API keys stay private)', () => {
    ...
    runIso(['add-event', '--event', 'Stop', '--command', '/x/hosts/claude/hooks/timeline-stop-hook', '--source', 'gaganfoxwell-timeline-stop']);
  ...
  test('a canonical root containing $ is escaped in the registered command', () => {
    ...
    expect(r2.stdout).toMatch(/removed 0 gaganfoxwell hook entries \(repointed 0\)/);
    expect(fs.readFileSync(settingsFile, 'utf-8')).toBe(before);
  });

  test('backups rotate: at most 10 .bak files survive repeated mutations', () => {
    for (let i = 0; i < 13; i++) {
      runIso(['add-event', '--event', 'Stop', '--command', `/x/hosts/claude/hooks/timeline-stop-hook-${i}`, '--source', 'gaganfoxwell-timeline-stop']);
    }
  ...
  test('tag-stripped verify-gate entry is table-owned: healed by --repoint, swept by --all', () => {
    ...
    const canon = mkCanon(tmpDir);
    const vg = path.join(canon, 'bin', 'gaganfoxwell-verify-gate');
    fs.writeFileSync(vg, '#!/bin/sh\n');
    fs.chmodSync(vg, 0o755);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: { Stop: [hookEntry('/dead/install/bin/gaganfoxwell-verify-gate')] },   // tag STRIPPED
    }, null, 2));
    runIso(['prune-stale', '--repoint', canon]);
    let s = settings();
    expect(s.hooks.Stop[0].hooks[0].command).toBe(vg);
    expect(s.hooks.Stop[0]._gaganfoxwell_source).toBe('verify-gate');
  ...
  test('a foreign entry that STARTED empty survives prune-stale untouched', () => {
    ...
    expect(r.stdout).toMatch(/removed 0 gaganfoxwell hook entries \(repointed 0\)/);
    expect(fs.readFileSync(settingsFile, 'utf-8')).toBe(before);
  });
...
describe('remove-source: per-item', () => {
  test('mixed tagged entry: gaganfoxwell item removed, user item survives, tag dropped', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [hookEntry(
    ...
    expect(s.hooks.PostToolUse[0]._gaganfoxwell_source).toBeUndefined();
  });
});

describe('prune-stale', () => {
  test('prunes dead gaganfoxwell items; keeps live gaganfoxwell and dead non-gaganfoxwell', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          hookEntry(`${canon}/hosts/claude/hooks/question-log-hook`, AUQ_MATCHER),   // live gaganfoxwell
          hookEntry('/dead/wt/hosts/claude/hooks/auq-error-fallback-hook', AUQ_MATCHER), // dead gaganfoxwell
          hookEntry('/dead/user/own-hook', AUQ_MATCHER),                              // dead NON-gaganfoxwell
        ],
      },
    }, null, 2));
    const r = runIso(['prune-stale']);
    expect(r.stdout).toMatch(/removed 1 gaganfoxwell hook entries/);
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(2);
  ...
  test('--repoint re-points dead AND live items, preserves bash prefix, restores tags', () => {
    ...
    expect(s.hooks.Stop[0]._gaganfoxwell_source).toBe('gaganfoxwell-timeline-stop');
  ...
  test('Windows backslash path is classified as gaganfoxwell-owned and pruned when dead', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
  ...
  test('--all removes live untagged gaganfoxwell items, spares user hooks and mixed-entry user items', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [hookEntry(`${canon}/hosts/claude/hooks/question-log-hook`, AUQ_MATCHER)],
        Stop: [hookEntry(
          `${canon}/hosts/claude/hooks/timeline-stop-hook`, undefined, 'gaganfoxwell-timeline-stop',
    ...
    expect(s.hooks.Stop[0]._gaganfoxwell_source).toBeUndefined();
  ...
  test('--all removes tagged single-item legacy strays (no table match)', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: { Stop: [hookEntry('/old/install/bin/gaganfoxwell-verify-gate', undefined, 'gaganfoxwell-verify-gate')] },
    }, null, 2));
    const r = runIso(['prune-stale', '--all']);
    expect(r.stdout).toMatch(/removed 1/);
    expect(settings().hooks).toBeUndefined();
  ...
  test('explicit plan_tune_hooks:no — dead plan-tune pruned, live plan-tune NOT re-pointed, Stop still re-pointed', () => {
    const canon = mkCanon(tmpDir);
    const live = mkCanon(tmpDir, 'live-worktree');
    execSync(`'${path.join(ROOT, 'bin', 'gaganfoxwell-config')}' set plan_tune_hooks no`, {
      env: { ...process.env, GAGANFOXWELL_STATE_ROOT: tmpDir },
    });
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
  ...
  test('incident facsimile: the exact live-damage shape heals to canonical', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [hookEntry(`${canon}/bin/gaganfoxwell-session-update`)],
        PostToolUse: [
        ...
        Stop: [
          ...
          hookEntry(dead2('timeline-stop-hook'), undefined, 'gaganfoxwell-timeline-stop'),
        ],
        PreCompact: [hookEntry('/Users/me/my-own-hook')],
      },
    }, null, 2));
    ...
    for (const ev of ['SessionStart', 'PostToolUse', 'PreToolUse', 'Stop']) {
      for (const e of s.hooks[ev]) {
        expect(e._gaganfoxwell_source).toBeDefined();
        for (const it of e.hooks) expect(it.command.startsWith(canon)).toBe(true);
      }
    }
    const postSources = s.hooks.PostToolUse.map((e: any) => e._gaganfoxwell_source).sort();
...
describe('fail-closed parse (pre-existing data-loss fix)', () => {
  const MUTATORS: string[][] = [
    ['add', '/x/bin/gaganfoxwell-session-update'],
    ['remove', '/x/bin/gaganfoxwell-session-update'],
...
describe('mutation lock', () => {
  test('stale lock (old mtime) is taken over; mutation proceeds', () => {
    ...
    const r = runIso(['add-event', '--event', 'Stop', '--command', '/x/hosts/claude/hooks/timeline-stop-hook', '--source', 'gaganfoxwell-timeline-stop']);
    expect(r.exitCode).toBe(0);
  ...
  test('fresh foreign lock: mutation skipped loudly (exit 5), file untouched', () => {
    ...
    const r = runIso(
      ['add-event', '--event', 'Stop', '--command', '/x', '--source', 's'],
      { GAGANFOXWELL_SETTINGS_LOCK_TIMEOUT_MS: '300' },
    );
  ...
  test('two concurrent add-events both land (lock serializes; file stays valid JSON)', () => {
    ...
    execSync(`sh -c "${a} & ${b} & wait"`, {
      env: { ...process.env, GAGANFOXWELL_SETTINGS_FILE: settingsFile, GAGANFOXWELL_STATE_ROOT: tmpDir },
      encoding: 'utf-8',
      timeout: 20000,
    });
...
describe('gaganfoxwell-settings-hook adversarial hardening', () => {
  let tmpDir: string;
  let settingsFile: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-settings-adv-'));
    settingsFile = path.join(tmpDir, 'settings.json');
  });
  ...
  test('foreign hook whose basename collides with Object.prototype survives --all', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        Stop: [
          ...
          { _gaganfoxwell_source: 'gaganfoxwell-timeline-stop', hooks: [{ type: 'command', command: '/x/hosts/claude/hooks/timeline-stop-hook' }] },
        ],
      },
    }, null, 2) + '\n');
    const r = runAdv(['prune-stale', '--all']);
    expect(r.exitCode).toBe(0);
    const s = advSettings();
    expect(s.hooks.Stop).toHaveLength(1);   // gaganfoxwell entry swept...
    expect(s.hooks.Stop[0].hooks[0].command).toBe('/usr/local/bin/toString');  // ...foreign one kept
  });

  test('GAGANFOXWELL_SWEEP_EXCLUDE_SOURCES preserves verify-gate during an --all sweep', () => {
    // `setup --no-team` sweeps team hooks but must not delete the
    // user-registered verify-gate opt-in whose binary still exists.
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        Stop: [
          { _gaganfoxwell_source: 'verify-gate', hooks: [{ type: 'command', command: '/x/bin/gaganfoxwell-verify-gate' }] },
          { _gaganfoxwell_source: 'gaganfoxwell-timeline-stop', hooks: [{ type: 'command', command: '/x/hosts/claude/hooks/timeline-stop-hook' }] },
        ],
      },
    }, null, 2) + '\n');
    const r = runAdv(['prune-stale', '--all'], { GAGANFOXWELL_SWEEP_EXCLUDE_SOURCES: 'verify-gate' });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/removed 1 /);
    const s = advSettings();
    expect(s.hooks.Stop).toHaveLength(1);
