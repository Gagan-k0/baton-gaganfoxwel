/**
 * Shape regression test for bin/gaganfoxwell-gbrain-detect.
 *
 ...
 * callers across most gaganfoxwell skill preambles shell out to this script and
...
const DETECT_BIN = join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-detect");
...
interface DetectShape {
  ...
  gaganfoxwell_brain_sync_mode: string;
  gaganfoxwell_brain_git: boolean;
  gaganfoxwell_artifacts_remote: string;
  gbrain_local_status: string;
}

describe("bin/gaganfoxwell-gbrain-detect — shape regression", () => {
  it("emits valid JSON", () => {
  ...
  it("contains all 9 pre-existing keys + the new gbrain_local_status key", () => {
    ...
    try {
      ...
      expect(parsed).toHaveProperty("gaganfoxwell_brain_sync_mode");
      expect(parsed).toHaveProperty("gaganfoxwell_brain_git");
      expect(parsed).toHaveProperty("gaganfoxwell_artifacts_remote");
  ...
  it("preserves field types from the bash version", () => {
    ...
    try {
      ...
      expect(typeof parsed.gaganfoxwell_brain_git).toBe("boolean");
      ...
      expect(typeof parsed.gaganfoxwell_brain_sync_mode).toBe("string");
      expect(typeof parsed.gaganfoxwell_artifacts_remote).toBe("string");
  ...
  it("gaganfoxwell_brain_sync_mode is one of the three documented values", () => {
    const tmp = mkdtempSync(join(tmpdir(), "detect-shape-"));
    try {
      const out = runDetect({
        HOME: tmp,
      ...
      const parsed = JSON.parse(out) as DetectShape;
      expect(["off", "artifacts-only", "full"]).toContain(parsed.gaganfoxwell_brain_sync_mode);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  });
...
describe("bin/gaganfoxwell-gbrain-detect --is-ok — live gate", () => {
...
`;
  ...
  it("exit code agrees with the JSON gbrain_local_status (no skew)", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "detect-isok-"));
    try {
      const env = { HOME: tmp, PATH: "/usr/bin:/bin", GAGANFOXWELL_HOME: tmp, GAGANFOXWELL_DETECT_NO_CACHE: "1" };
