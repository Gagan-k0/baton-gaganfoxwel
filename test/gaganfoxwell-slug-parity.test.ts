/**
 * bin/gaganfoxwell-slug ↔ browse/bin/remote-slug parity.
 *
 * The bug this pins (2026-08-17, observed live in a Conductor worktree of
 * garrytan/gaganfoxwell): a stray marker-bearing ancestor above the repo — an empty
 * `~/.git` directory that is not even a valid git repo — captured
 * gaganfoxwell-slug's "outermost strong marker" walk-up as the project root. That
 ...
 * `garrytan-gaganfoxwell`. Every store keyed on the slug (decisions, timeline,
 * ceo-plans, learnings) filed into ~/.gaganfoxwell/projects/garrytan/ — one bucket
 * shared by every repo under $HOME.
 *
 * The fix makes the canonical remote authoritative: gaganfoxwell-slug now walks the
 ...
 *    reaches a canonical remote, gaganfoxwell-slug's SLUG equals remote-slug's
 ...
 *
 * Test pattern mirrors test/gaganfoxwell-slug-cwd-walk-up.test.ts: per-test
...
const SLUG_SCRIPT = path.join(ROOT, 'bin', 'gaganfoxwell-slug');
...
function baseEnv(tmpHome: string): Record<string, string | undefined> {
  // Drop any ambient override: a sibling test leaking GAGANFOXWELL_PROJECT_SLUG in
  // a shared-process shard would flip runs into override mode.
  const { GAGANFOXWELL_PROJECT_SLUG: _drop, ...ambient } = process.env;
  return { ...ambient, HOME: tmpHome, GAGANFOXWELL_HOME: path.join(tmpHome, '.gaganfoxwell') };
}
...
function expectParity(cwd: string, tmpHome: string, expected: string): void {
  const gaganfoxwell = runSlug(cwd, tmpHome);
  const remote = runRemoteSlug(cwd, tmpHome);
  expect(gaganfoxwell.status).toBe(0);
  expect(remote.status).toBe(0);
  const remoteOut = remote.stdout.trim();
  expect(slugOf(gaganfoxwell)).toBe(expected);
  expect(remoteOut).toBe(expected);
  expect(slugOf(gaganfoxwell)).toBe(remoteOut);
}

describe('gaganfoxwell-slug ↔ remote-slug parity', () => {
  let tmpHome: string;
  let fixtures: string;
  ...
  test('plain clone, https remote WITHOUT .git suffix (live-bug URL shape) — identical slug', () => {
    const repo = makeRepo(path.join(fixtures, 'proj'), 'https://github.com/garrytan/gaganfoxwell');
    expectParity(repo, tmpHome, 'garrytan-gaganfoxwell');
  });
  ...
  test('git-worktree of a clone (.git FILE, the Conductor shape) — identical slug', () => {
    const main = makeRepo(path.join(fixtures, 'main-clone'), 'https://github.com/garrytan/gaganfoxwell');
    ...
    expectParity(wt, tmpHome, 'garrytan-gaganfoxwell');
  });
  ...
  test('LIVE BUG SHAPE: stray empty .git on an ancestor "home" no longer degrades the slug', () => {
    ...
    const main = makeRepo(
      path.join(strayHome, 'conductor', 'workspaces', 'gaganfoxwell', 'main-clone'),
      'https://github.com/garrytan/gaganfoxwell',
    );
    git(['-C', main, '-c', 'user.email=t@t', '-c', 'user.name=t', 'commit', '-q', '--allow-empty', '-m', 'init']);
    const wt = path.join(strayHome, 'conductor', 'workspaces', 'gaganfoxwell', 'beirut-v4');
    git(['-C', main, 'worktree', 'add', '-q', wt, '-b', 'gaganfoxwell-fix-wave']);
    ...
    expectParity(main, tmpHome, 'garrytan-gaganfoxwell');
    expectParity(wt, tmpHome, 'garrytan-gaganfoxwell');
  ...
  test('walk-up preserved: nested inner repo (no remote) resolves to the OUTER repo slug', () => {
    ...
    const gaganfoxwell = runSlug(inner, tmpHome);
    expect(gaganfoxwell.status).toBe(0);
    ...
    expect(slugOf(gaganfoxwell)).toBe('acme-outer');
  ...
  test('fallback unchanged: no-remote repo resolves to its basename (and remote-slug agrees)', () => {
    ...
    const gaganfoxwell = runSlug(repo, tmpHome);
    expect(gaganfoxwell.status).toBe(0);
    expect(slugOf(gaganfoxwell)).toBe('lonely');
  ...
  test('cache self-heal: a pre-fix degraded cache entry is rewritten to the canonical slug', () => {
    ...
    const repo = makeRepo(path.join(strayHome, 'git', 'proj'), 'https://github.com/garrytan/gaganfoxwell');
    ...
    const cacheDir = path.join(tmpHome, '.gaganfoxwell', 'slug-cache');
    ...
    const gaganfoxwell = runSlug(repo, tmpHome);
    expect(gaganfoxwell.status).toBe(0);
    expect(slugOf(gaganfoxwell)).toBe('garrytan-gaganfoxwell');
    // The cache file itself must have been overwritten (self-healing).
    expect(fs.readFileSync(cacheFile, 'utf8').trim()).toBe('garrytan-gaganfoxwell');
  });
  ...
  test('hostile origin `url = ..` cannot become a ".." slug — basename fallback holds', () => {
    ...
    // ABOVE ~/.gaganfoxwell/projects/ (confined to ~/.gaganfoxwell, but still traversal).
    ...
    const cacheFile = path.join(tmpHome, '.gaganfoxwell', 'slug-cache', encodedCacheKey(repo));
  ...
  test('package.json wrapper root (no .git): sticky basename slug is PRESERVED — heal is stray-repo-shape only', () => {
    ...
    // gaganfoxwell before an inner dir grew a remote-bearing repo. The degraded-
  ...
  test('sticky identity preserved (#2212): repo that adopted a remote after first use is NOT healed', () => {
    ...
    const gaganfoxwell = runSlug(repo, tmpHome);
    expect(gaganfoxwell.status).toBe(0);
    expect(slugOf(gaganfoxwell)).toBe('stickyproj');
