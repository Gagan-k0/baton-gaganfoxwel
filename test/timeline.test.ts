...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/gaganfoxwell-timeline-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runRead(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/gaganfoxwell-timeline-read ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
function runReadArgs(args: string[] = []): string {
  ...
  try {
    return execFileSync(path.join(BIN, 'gaganfoxwell-timeline-read'), args, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-timeline-'));
...
describe('gaganfoxwell-timeline-log', () => {
...
describe('gaganfoxwell-timeline-read', () => {
