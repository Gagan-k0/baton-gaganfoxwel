...
function runProfile(): Record<string, string> {
  ...
  const stdout = execSync(`${BIN}/gaganfoxwell-builder-profile`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-profile-'));
});
...
describe('gaganfoxwell-builder-profile', () => {
