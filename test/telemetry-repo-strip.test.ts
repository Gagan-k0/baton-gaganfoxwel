 ...
 *   - gaganfoxwell-telemetry-log         → "_repo_slug", "_branch"
 *     (bin/gaganfoxwell-telemetry-log)
 *
 * gaganfoxwell-telemetry-sync MUST strip every one of those fields before the remote
 * POST (bin/gaganfoxwell-telemetry-sync). The script has TWO strip paths — jq del()
...
const SYNC = path.join(ROOT, 'bin', 'gaganfoxwell-telemetry-sync');
const PREAMBLE = path.join(ROOT, 'scripts', 'resolvers', 'preamble', 'generate-preamble-bash.ts');
const TEL_LOG = path.join(ROOT, 'bin', 'gaganfoxwell-telemetry-log');
 ...
 * file gaganfoxwell-telemetry-sync reads and uploads. Scoped to the emission lines
...
describe('telemetry no-repo-identity-egress invariant', () => {
  ...
  // preamble appends directly; gaganfoxwell-telemetry-log builds the synced event
  ...
  test('coverage: every repo/branch field the producers emit into skill-usage.jsonl is stripped (sed fallback path)', () => {
    ...
    for (const field of emitted) {
      expect(
        ...
        `producer emits repo-identity field "${field}" but gaganfoxwell-telemetry-sync's sed fallback does not strip it (would leak to remote)`,
      ).toBe(true);
    }
  });
