/**
 * Unit tests for lib/gaganfoxwell-memory-helpers.ts (Lane 0 foundation).
 *
...
describe("canonicalizeRemote", () => {
  it("strips https scheme and .git suffix", () => {
    expect(canonicalizeRemote("https://github.com/garrytan/gaganfoxwell.git")).toBe("github.com/garrytan/gaganfoxwell");
  });

  it("normalizes git@host:path scp-style remotes", () => {
    expect(canonicalizeRemote("git@github.com:garrytan/gaganfoxwell.git")).toBe("github.com/garrytan/gaganfoxwell");
  });
  ...
  it("produces the same key with or without a trailing slash", () => {
    expect(canonicalizeRemote("https://github.com/garrytan/gaganfoxwell.git/")).toBe(
      canonicalizeRemote("https://github.com/garrytan/gaganfoxwell.git")
    );
  });
...
describe("secretScanFile", () => {
  ...
  it("returns scanner=missing or runs gitleaks (env-dependent)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "gaganfoxwell-test-"));
...
`
    ...
    expect(m!.context_queries[1].glob).toBe("~/.gaganfoxwell/builder-profile.jsonl");
...
describe("withErrorContext", () => {
  ...
  beforeEach(() => {
    savedHome = process.env.GAGANFOXWELL_HOME;
    testHome = mkdtempSync(join(tmpdir(), "gaganfoxwell-test-home-"));
    process.env.GAGANFOXWELL_HOME = testHome;
  });
  ...
  // overwrote savedHome) and leak a gaganfoxwell-test-home-* dir into every test
  // file that runs after this one in the same bun process.
  afterEach(() => {
    if (savedHome === undefined) delete process.env.GAGANFOXWELL_HOME;
    else process.env.GAGANFOXWELL_HOME = savedHome;
  });
...
describe("detectEngineTier", () => {
  ...
  beforeEach(() => {
    ...
    testHome = mkdtempSync(join(tmpdir(), "gaganfoxwell-test-engine-"));
    testGbrainHome = mkdtempSync(join(tmpdir(), "gaganfoxwell-test-gbrain-"));
    process.env.GAGANFOXWELL_HOME = testHome;
  ...
  // afterEach, not afterAll: the save happens in beforeEach, so an afterAll
  // restore would put back the PREVIOUS test's gaganfoxwell-test-engine-* temp dir
  // (the last beforeEach overwrote the saved values). That leaked
  // HOME/GAGANFOXWELL_HOME/PATH into every test file that ran after this one in the
  // same bun process — child processes then looked for Playwright's Chromium
  // cache and ~/.gaganfoxwell config under a throwaway temp HOME.
  afterEach(() => {
    if (savedHome === undefined) delete process.env.GAGANFOXWELL_HOME;
    else process.env.GAGANFOXWELL_HOME = savedHome;
  ...
  it("writes a cache file at ~/.gaganfoxwell/.gbrain-engine-cache.json", () => {
    detectEngineTier();
    const cachePath = join(testHome, ".gbrain-engine-cache.json");
    expect(existsSync(cachePath)).toBe(true);
    const cached = JSON.parse(readFileSync(cachePath, "utf-8"));
    expect(cached.schema_version).toBe(1);
    expect(cached.last_writer).toBe("gaganfoxwell-memory-helpers.detectEngineTier");
  });
  ...
  it("parses schema_version:2 doctor JSON via the exec path (regression for #1418)", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "gaganfoxwell-gbrain-shim-"));
