/**
 * gaganfoxwell-gbrain-repo-policy — per-remote trust-tier policy store.
 *
 ...
 *
 * Each test uses a temp GAGANFOXWELL_HOME so nothing leaks into the user's real ~/.gaganfoxwell.
 */
...
function run(args: string[], opts: { env?: Record<string, string> } = {}) {
  const res = spawnSync(BIN, args, {
    env: { ...process.env, GAGANFOXWELL_HOME: tmpHome, ...(opts.env || {}) },
    encoding: 'utf-8',
  });
  return {
...
// gaganfoxwell-gbrain-sync invocation (skill prose bypassed — cron, curiosity,
...
describe('gaganfoxwell-gbrain-sync code stage honors the repo policy (#2140 sync path)', () => {
  const SYNC = path.join(ROOT, 'bin', 'gaganfoxwell-gbrain-sync.ts');
  ...
  function runSync(): { status: number; text: string; stages: any[] } {
    const res = spawnSync('bun', [SYNC, '--code-only', '--incremental'], {
      ...
      env: { ...process.env, GAGANFOXWELL_HOME: tmpHome, HOME: tmpHome },
    });
    let stages: any[] = [];
    try {
      stages = JSON.parse(
