/**
 * Regression test — bin/gaganfoxwell-slug must resolve to the OUTERMOST project root
 ...
 * `bin/gaganfoxwell-slug` derived its slug from the literal `pwd` with no walk-up.
 ...
 * (no manual `rm -rf ~/.gaganfoxwell/slug-cache/` required).
 *
...
function runSlug(
  ...
  // GAGANFOXWELL_HOME must be pinned to the temp home too: the cache dir is
  // GAGANFOXWELL_HOME-aware (matching lib/bin-context.ts's native port), and a
  // sibling test file leaking process.env.GAGANFOXWELL_HOME in a shared-process
  ...
  // script's cache writes land in tmpHome, never AJ's real ~/.gaganfoxwell.
  const env = { ...process.env, HOME: tmpHome, GAGANFOXWELL_HOME: path.join(tmpHome, '.gaganfoxwell'), ...extraEnv };
...
describe('gaganfoxwell-slug — outermost project-root resolution', () => {
  let tmpHome: string;
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-slug-test-')));
    projectsRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-slug-projects-')));
  });
  ...
  // Edge case: GAGANFOXWELL_PROJECT_SLUG env override wins over walk-up (documented escape hatch).
  test('GAGANFOXWELL_PROJECT_SLUG env override beats every other resolution path', () => {
    const projectRoot = path.join(projectsRoot, 'loadout');
    const siteSubdir = path.join(projectRoot, 'site');
    fs.mkdirSync(path.join(projectRoot, '.git'), { recursive: true });
    fs.mkdirSync(path.join(siteSubdir, '.vercel'), { recursive: true });
    ...
    const result = runSlug(siteSubdir, tmpHome, { GAGANFOXWELL_PROJECT_SLUG: 'custom-override' });
    expect(result.status).toBe(0);
    const { slug } = parseSlug(result.stdout);
...
describe('_outermost_project_root termination (windows-free-tests regression)', () => {
  ...
  const script = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-slug'), 'utf-8');
