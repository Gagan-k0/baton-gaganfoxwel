 ...
 * agent-executed instructions, not a gaganfoxwell binary, so it is covered by the
...
const POLARITY: Record<string, 'fail-closed' | 'fail-open'> = {
  // fail-closed: gaganfoxwell state leaving the machine unrecorded is worse than
...
const MODULE_SINKS = [
  'bin/gaganfoxwell-gbrain-sync.ts',
  'bin/gaganfoxwell-memory-ingest.ts',
  ...
  'lib/context-bill.ts',
  // supabase-provision engine (bin/gaganfoxwell-gbrain-supabase-provision is a thin
...
const SHELL_SINKS = [
  'bin/gaganfoxwell-telemetry-sync',
  'bin/gaganfoxwell-update-check',
  'bin/gaganfoxwell-brain-sync',
  'bin/gaganfoxwell-gbrain-mcp-verify',
  'bin/gaganfoxwell-security-dashboard',
  'bin/gaganfoxwell-community-dashboard',
  'bin/gaganfoxwell-artifacts-init',
  'bin/gaganfoxwell-brain-restore',
  'bin/gaganfoxwell-session-update',
];
...
const SCANNER_EXEMPT: Record<string, string> = {
  'bin/gaganfoxwell-team-init':
    'every git clone is inside an echoed instruction string (install docs); the script executes no network ops',
  'bin/gaganfoxwell-gbrain-install':
    'user-invoked installer: bodyless HEAD reachability probe to github.com + clone of the public gbrain repo (user-directed install; no gaganfoxwell state leaves the machine)',
  'bin/gaganfoxwell-next-version':
    'fetches the user\'s own repo\'s base branch for version-claim freshness — a user-repo dev-workflow op, not gaganfoxwell-state egress',
  'bin/gaganfoxwell-version-bump':
    'git fetch appears only in an error-message string',
  'bin/gaganfoxwell-redact-prepush':
  ...
  // not gaganfoxwell binaries. Includes the preamble-generated brain-sync block —
  // see the header.
  'scripts/resolvers':
    'skill prose templates — agent-executed instructions rendered into SKILL.md, not gaganfoxwell binaries',
};
...
const RECEIPT_MARKER =
  /_receipted_(curl|git|version_fetch)\b|gaganfoxwell-egress-receipt["']?\s+write\b|writeReceipt\(|receiptedFetch\(/;
...
describe('egress receipt wiring tripwire', () => {
  ...
  test('every shell sink sources gaganfoxwell-egress-lib.sh', () => {
    for (const rel of SHELL_SINKS) {
      const src = read(rel);
      expect(
        src.includes('gaganfoxwell-egress-lib.sh'),
        `${rel}: must source bin/gaganfoxwell-egress-lib.sh for _receipted_* helpers`,
      ).toBe(true);
    }
  });
  ...
  test('deprecated dead-endpoint brain consumer/reader scripts stay deleted', () => {
    ...
    for (const rel of ['bin/gaganfoxwell-brain-consumer', 'bin/gaganfoxwell-brain-reader']) {
      let present = true;
      try {
  ...
  test('polarity spot-checks: closed sinks refuse, open sinks warn', () => {
    ...
    expect(read('bin/gaganfoxwell-telemetry-sync')).toMatch(/_receipted_curl closed telemetry-sync/);
    // brain-sync (closed): refusal exits before the commit consumes the queue.
    expect(read('bin/gaganfoxwell-brain-sync')).toMatch(/gaganfoxwell-egress-receipt["']? write/);
    // update-check (open).
    expect(read('bin/gaganfoxwell-update-check')).toMatch(/_receipted_curl open update-check/);
    // dashboards (open).
    expect(read('bin/gaganfoxwell-security-dashboard')).toMatch(/_receipted_curl open security-dashboard/);
    expect(read('bin/gaganfoxwell-community-dashboard')).toMatch(/_receipted_curl open community-dashboard/);
    // mcp-verify (closed).
    expect(read('bin/gaganfoxwell-gbrain-mcp-verify')).toMatch(/_receipted_curl closed gbrain-mcp-verify/);
  ...
  test('shebang tripwire: no bin/gaganfoxwell-* file carries a node shebang (amendment 2A)', () => {
    const offenders: string[] = [];
    for (const entry of fs.readdirSync(path.join(ROOT, 'bin'))) {
      if (!entry.startsWith('gaganfoxwell-')) continue;
