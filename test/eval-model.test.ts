...
describe("resolveEvalModel", () => {
  test("explicit argument wins over everything", () => {
    expect(resolveEvalModel("capture", "my-model", { GAGANFOXWELL_EVAL_MODEL: "x" } as never)).toBe("my-model");
  });
  test("per-kind env beats the global env", () => {
    expect(resolveEvalModel("warmup", null, { GAGANFOXWELL_EVAL_MODEL_WARMUP: "w", GAGANFOXWELL_EVAL_MODEL: "g" } as never)).toBe("w");
  });
  test("global env beats the default", () => {
    expect(resolveEvalModel("distill", null, { GAGANFOXWELL_EVAL_MODEL: "g" } as never)).toBe("g");
  });
  test("defaults per kind", () => {
    // capture defaults to Sonnet per D1a (2026-08 review): Opus is opt-in via
    // explicit arg or GAGANFOXWELL_EVAL_MODEL_CAPTURE.
