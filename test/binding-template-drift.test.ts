...
describe('content-binding template drift', () => {
  test('ship Step 16 carries the evidence check (mechanized IRON LAW)', () => {
    ...
    expect(ship).toMatch(/gaganfoxwell-evidence check --label tests --expect-cmd '[^']+' --label vitest --expect-cmd '[^']+' --max-age 24 --allow-paths CHANGELOG\.md,VERSION,package\.json/);
  ...
  test('ship Step 5 lanes run wrapped with per-lane labels', () => {
    ...
    expect(tests).toContain('gaganfoxwell-evidence run --label tests');
    expect(tests).toContain('gaganfoxwell-evidence run --label vitest');
  });
  ...
  test('land-and-deploy grades staleness content-first (wtree rule) and checks evidence', () => {
    ...
    expect(land).toContain('---WTREE---');
    expect(land).toMatch(/gaganfoxwell-evidence check --label tests --expect-cmd '[^']+' --max-age 24/);
  ...
  test('release-body write side carries the banner tripwire (and it actually fires)', () => {
    ...
    expect(body).toContain('grep -c "UNTRUSTED TRACKER CONTENT" /tmp/gaganfoxwell-pr-body-$$.md');
    expect(body).toContain('grep -c "UNTRUSTED TRACKER CONTENT" /tmp/gaganfoxwell-pr-body-orig-$$.md');
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-banner-'));
    try {
      const scriptFor = (origContent: string, newContent: string) => {
        ...
        return block![0]
          .replaceAll('/tmp/gaganfoxwell-pr-body-orig-$$.md', path.join(dir, 'orig.md'))
          .replaceAll('/tmp/gaganfoxwell-pr-body-$$.md', path.join(dir, 'new.md'));
      };
  ...
  test('greptile triage reads bodies through the guard (metadata/body split)', () => {
    ...
    expect(triage).toContain('gaganfoxwell-issue-guard --stdin --source greptile-line');
    expect(triage).toContain('gaganfoxwell-issue-guard --stdin --source greptile-replies');
  });
});
