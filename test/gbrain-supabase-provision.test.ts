/**
 * gaganfoxwell-gbrain-supabase-provision — Supabase Management API wrapper.
 *
...
const BIN = path.join(ROOT, 'bin', 'gaganfoxwell-gbrain-supabase-provision');
...
// Per-test GAGANFOXWELL_HOME so egress receipts land in a throwaway ledger, never
// the operator's real ~/.gaganfoxwell/security/egress.jsonl.
...
async function runCmd(
  ...
  const status = await runProvision(args, {
    ...
    env: { GAGANFOXWELL_HOME: egressHome, ...env },
...
describe('pooler-url', () => {
  ...
  test('GAGANFOXWELL_SUPABASE_TRUST_API_PORT=1 disables the rewrite', async () => {
    mock = startMock({
      [`GET /v1/projects/${REF}/config/database/pooler`]: () =>
        jsonResp({ ...POOLER_OK, pool_mode: 'transaction', db_port: 6543 }),
    });
    const r = await runCmd(['pooler-url', REF, '--json'], {
      ...
      DB_PASS: 'pw',
      SUPABASE_API_BASE: mock.url,
      GAGANFOXWELL_SUPABASE_TRUST_API_PORT: '1',
    });
    expect(r.status).toBe(0);
    expect(JSON.parse(r.stdout).pooler_url).toContain(':6543/postgres');
    expect(r.stderr).not.toContain('rewriting');
