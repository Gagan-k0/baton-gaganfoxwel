/**
 * Regression pin: `gaganfoxwell-memory-ingest` must pass `--include-gitignored` to
 * `gbrain import`.
 *
 * gaganfoxwell-artifacts-init writes an ignore-everything `.gitignore` (a bare `*`,
 * headed "Do not edit") at the root of `~/.gaganfoxwell`. The memory ingest stages
 * pages into `~/.gaganfoxwell/.staging-ingest-<pid>-<ts>/`, which is INSIDE that
...
const SOURCE_PATH = join(import.meta.dir, "..", "bin", "gaganfoxwell-memory-ingest.ts");
...
describe("gaganfoxwell-memory-ingest: gbrain import must not be filtered by .gitignore", () => {
  ...
  it("proves the ceiling stops git discovery from the staging dir — including through a symlink", () => {
    const dir = mkdtempSync(join(tmpdir(), "gaganfoxwell-ingest-ceiling-"));
    try {
      ...
      // ~/.gaganfoxwell shape: a git repo whose root ignores everything, with the
      // staging dir as a direct child.
      const home = join(dir, "gaganfoxwell-home");
  ...
  it("demonstrates the collision: an ignore-everything root hides staged pages", () => {
    const dir = mkdtempSync(join(tmpdir(), "gaganfoxwell-ingest-gitignore-"));
    try {
      ...
      // Exactly what gaganfoxwell-artifacts-init writes at the root of ~/.gaganfoxwell.
