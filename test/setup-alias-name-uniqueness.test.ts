 ...
 *
 * setup installs two back-compat alias dirs — `_gaganfoxwell-command` (root router)
 * and `connect-chrome` (→ open-gaganfoxwell-browser). Both used to symlink the
 ...
 * /open-gaganfoxwell-browser (readdir-order roulette), and the `_gaganfoxwell-command`
...
const installDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-alias-install-'));

const sourceRootSkill = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
const sourceOgbSkill = fs.readFileSync(
  path.join(ROOT, 'open-gaganfoxwell-browser', 'SKILL.md'),
  'utf-8',
);
...
beforeAll(() => {
  const installOnce = [
    ...
    `_install_alias_skill_md "${ROOT}/open-gaganfoxwell-browser/SKILL.md" "${installDir}/connect-chrome" "connect-chrome"`,
  ].join('\n');
  const script = [
    'set -e',
    'IS_WINDOWS=0',
...
describe('alias installs are rewritten copies (#2511, #2201)', () => {
  test('_gaganfoxwell-command alias is NOT a symlink and carries its own name', () => {
    const aliasDir = path.join(installDir, '_gaganfoxwell-command');
    ...
    expect(frontmatterName(aliasSkill)).toBe('_gaganfoxwell-command');
  });
  ...
  test('alias body is the canonical content — only the name: line differs', () => {
    const alias = fs.readFileSync(
      path.join(installDir, '_gaganfoxwell-command', 'SKILL.md'),
      'utf-8',
    );
    expect(alias.replace(/^name:.*$/m, 'name: gaganfoxwell')).toBe(sourceRootSkill);

    const ogbAlias = fs.readFileSync(
      path.join(installDir, 'connect-chrome', 'SKILL.md'),
      'utf-8',
    );
    expect(ogbAlias.replace(/^name:.*$/m, 'name: open-gaganfoxwell-browser')).toBe(sourceOgbSkill);
  });
  ...
  test('the SOURCE files are byte-intact (E2: sed never wrote through a symlink)', () => {
    ...
    expect(
      fs.readFileSync(path.join(ROOT, 'open-gaganfoxwell-browser', 'SKILL.md'), 'utf-8'),
    ).toBe(sourceOgbSkill);
    expect(frontmatterName(path.join(ROOT, 'SKILL.md'))).toBe('gaganfoxwell');
    expect(frontmatterName(path.join(ROOT, 'open-gaganfoxwell-browser', 'SKILL.md'))).toBe(
  ...
  test('a legacy symlinked alias is replaced, not written through', () => {
    ...
    const legacyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-alias-legacy-'));
    try {
      const aliasDir = path.join(legacyDir, '_gaganfoxwell-command');
