 ...
 *
 * We invoke bin/gaganfoxwell-redact-prepush directly with the git pre-push stdin
...
const PREPUSH = path.resolve(import.meta.dir, "..", "bin", "gaganfoxwell-redact-prepush");
const REDACT = path.resolve(import.meta.dir, "..", "bin", "gaganfoxwell-redact");
 ...
 * gets wrong (mirrors `prependPath` in test/gaganfoxwell-brain-context-load.test.ts):
...
describe("fail closed on unscannable diffs (#1946)", () => {
  test("a diff git cannot compute BLOCKS the push and names the escape valve", () => {
    ...
    expect(stderr).toContain("GAGANFOXWELL_REDACT_PREPUSH=skip");
  });
...
describe("install UX surfaces (#1946 / eng review D3+D10)", () => {
  ...
  describe("one-time consent prompt in setup (#1946, decision 6)", () => {
    ...
    test("an explicit answer persists true/false; timeout persists NOTHING", () => {
      ...
      // the quoted "$GAGANFOXWELL_CONFIG" form.
      const timeoutBranch = block.slice(block.indexOf("*)"), block.indexOf("esac"));
      expect(timeoutBranch).not.toContain('"$GAGANFOXWELL_CONFIG" set redact_prepush_hook');
    });

    test("non-interactive setup keeps the hint-only posture (no prompt, no key write)", () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-consent-"));
      try {
        const script = [
          "QUIET=0",
          'log() { echo "$@"; }',
          `GAGANFOXWELL_CONFIG="${path.join(ROOT, "bin", "gaganfoxwell-config")}"`,
          block,
        ].join("\n");
    ...
    test("a recorded answer is never re-asked (key present → silent)", () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-consent-set-"));
      try {
...
describe("escape valve", () => {
  test("GAGANFOXWELL_REDACT_PREPUSH=skip bypasses + logs", () => {
    const base = git(["rev-parse", "HEAD"]);
    const head = commit("config.txt", "key " + FAKE_AWS_KEY + "\n", "add key");
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "ghome-"));
    const { code } = runHook(`refs/heads/main ${head} refs/heads/main ${base}\n`, {
      GAGANFOXWELL_REDACT_PREPUSH: "skip",
...
describe("install / chaining", () => {
  test("install creates a managed hook; existing hook preserved + chained", () => {
    ...
    expect(installed).toContain("gaganfoxwell-redact pre-push (managed)");
  ...
  test("chained pre-push.local receives the final ref line (trailing newline preserved)", () => {
    ...
    const r = spawnSync("bash", [path.join(hookDir, "pre-push")], {
      ...
      encoding: "utf8",
      env: { ...process.env, GAGANFOXWELL_REDACT_PREPUSH: "skip" },
    });
    expect(r.status).toBe(0);
