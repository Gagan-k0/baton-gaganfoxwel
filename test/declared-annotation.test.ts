...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-annot-'));
  prevStateRoot = process.env.GAGANFOXWELL_STATE_ROOT;
  prevHome = process.env.GAGANFOXWELL_HOME;
  process.env.GAGANFOXWELL_STATE_ROOT = stateRoot;
  delete process.env.GAGANFOXWELL_HOME;
});

afterEach(() => {
  if (prevStateRoot !== undefined) process.env.GAGANFOXWELL_STATE_ROOT = prevStateRoot;
  else delete process.env.GAGANFOXWELL_STATE_ROOT;
  if (prevHome !== undefined) process.env.GAGANFOXWELL_HOME = prevHome;
