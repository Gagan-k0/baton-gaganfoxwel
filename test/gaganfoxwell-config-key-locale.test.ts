/**
 * Locale-independent key validation tests for bin/gaganfoxwell-config.
 *
...
const CONFIG = path.join(ROOT, "bin", "gaganfoxwell-config");
...
function run(args: string[]) {
  const result = spawnSync(CONFIG, args, {
    encoding: "utf8",
    // GAGANFOXWELL_SETUP_RUNNING suppresses `set skill_prefix`'s auto-relink side
    // effect. Without it, this test invokes the REPO's gaganfoxwell-config, whose
    // auto-relink resolves the install dir from its own path — i.e. the repo —
    // and gaganfoxwell-patch-names rewrites all 52 tracked SKILL.md files to
    // gaganfoxwell-prefixed names, poisoning every downstream test that reads the
    ...
    env: { ...process.env, GAGANFOXWELL_STATE_ROOT: stateRoot, GAGANFOXWELL_SETUP_RUNNING: "1" },
  });
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-config-locale-"));
});
...
describe("gaganfoxwell-config key validation is locale-independent", () => {
