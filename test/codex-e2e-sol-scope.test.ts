...
describeSol('GPT-5.6 Sol full-artifact scope termination', () => {
  beforeAll(() => {
    ...
    const priorAgentsBackup = fs.existsSync(agentsDir)
      ? fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-agents-backup-'))
      : '';
    ...
    const generatedDir = path.join(agentsDir, 'skills', 'gaganfoxwell-investigate');
    ...
    skillDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-sol-skill-'));
    ...
    scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-sol-scope-'));
    run('git', ['init', '-b', 'main']);
...
`);
  ...
  testIfSelected('codex-sol-scope-termination', async () => {
    const result = await runCodexSkill({
      skillDir,
      skillName: 'gaganfoxwell-investigate',
      cwd: scratch,
      ...
      prompt: `Use the gaganfoxwell-investigate skill to diagnose and fix why parseLimit('0') returns 10.
