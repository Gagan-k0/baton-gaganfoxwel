/**
 * gaganfoxwell-redact-prepush — WHICH commits get scanned.
 *
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "gaganfoxwell-prepush-"));
  run(["init", "-q", "-b", "main"]);
...
function setUpRemoteWithForeignFixture(): void {
  const remote = mkdtempSync(join(tmpdir(), "gaganfoxwell-prepush-remote-"));
...
describe("S1: exclusion scoped to the push-target remote", () => {
  const PREPUSH = join(import.meta.dir, "..", "bin", "gaganfoxwell-redact-prepush");
  ...
  function buildSecretOnSecondRemote(): { originTip: string } {
    const origin = mkdtempSync(join(tmpdir(), "gaganfoxwell-prepush-origin-"));
    ...
    const other = mkdtempSync(join(tmpdir(), "gaganfoxwell-prepush-other-"));
