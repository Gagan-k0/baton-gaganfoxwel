/**
 * gaganfoxwell-codex-session-import — backfill question-log from Codex JSONL.
 *
 ...
 *   - Marker-first: <gaganfoxwell-qid:foo-bar> → source=codex-import-marker.
...
const BIN = path.join(ROOT, 'bin', 'gaganfoxwell-codex-session-import');
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-cdximp-'));
...
describe('marker-first import (source=codex-import-marker)', () => {
  test('extracts marker id from agent_message and pairs with next user_message', () => {
    const sessionPath = writeSessionFile([
      agentMessage(
        'D1 — Test\nELI10: blah\n<gaganfoxwell-qid:ship-test-failure-triage> Tests failed.\nRecommendation: A\nA) Fix now (recommended)\nB) Investigate\nC) Ack and ship',
      ),
      userMessage('A'),
    ]);
...
describe('edge cases', () => {
  ...
  test('agent_message with marker but no following user_message → skipped', () => {
    const sessionPath = writeSessionFile([
      agentMessage('<gaganfoxwell-qid:test-q> D1 — Q\nA) Foo\nB) Bar'),
      // no user_message
    ]);
    const r = runImport(sessionPath);
    expect(r.status).toBe(0);
  ...
  test('two D-briefs in sequence → both imported', () => {
    const sessionPath = writeSessionFile([
      agentMessage('D1 — First <gaganfoxwell-qid:q1>\nA) Foo (recommended)\nB) Bar'),
      userMessage('A'),
      agentMessage('D2 — Second <gaganfoxwell-qid:q2>\nA) Baz (recommended)\nB) Qux'),
      userMessage('B'),
    ]);
    const r = runImport(sessionPath);
    expect(r.status).toBe(0);
  ...
  test('numeric user response also resolves to letter index', () => {
    const sessionPath = writeSessionFile([
      agentMessage('D1 — Test <gaganfoxwell-qid:numeric-q>\nA) Foo\nB) Bar\nC) Baz'),
...
describe('default mode (no args → latest)', () => {
  test('returns NO_SESSIONS when sessions dir is empty', () => {
    const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-empty-cdx-'));
    try {
      const env: Record<string, string> = {};
      for (const [k, v] of Object.entries(process.env)) {
        if (v !== undefined) env[k] = v;
