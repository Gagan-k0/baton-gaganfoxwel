...
const CODEX_E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'codex-review-findings':   ['review/**', '.agents/skills/gaganfoxwell-review/**', 'codex/**', 'test/helpers/codex-session-runner.ts'],
};
...
describeCodex('Codex E2E', () => {
  ...
  testIfSelected('codex-discover-skill', async () => {
    // Install gaganfoxwell-review skill to a temp HOME and ask Codex to list skills.
    ...
    const skillDir = path.join(testWorktree, '.agents', 'skills', 'gaganfoxwell-review');
    ...
    expect(
      outputLower.includes('review') || outputLower.includes('gaganfoxwell') || outputLower.includes('skill'),
    ).toBe(true);
  }, 120_000);

  // Validates that Codex can invoke the gaganfoxwell-review skill, run a diff-based
  ...
  testIfSelected('codex-review-findings', async () => {
    // Install gaganfoxwell-review and ask Codex to review the worktree. The skill
    ...
    const result = await runCodexSkill({
      skillDir,
      prompt: 'Run the gaganfoxwell-review skill on this repository. Review the current branch diff and report your findings.',
      timeoutMs: 540_000,
      cwd: testWorktree,
      skillName: 'gaganfoxwell-review',
