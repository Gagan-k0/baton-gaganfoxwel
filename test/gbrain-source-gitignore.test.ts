 ...
 * `.gbrain-source` to gaganfoxwell's own `.gitignore`. Without the consumer-side
...
import { ensureGbrainSourceGitignored } from "../bin/gaganfoxwell-gbrain-sync";

describe("ensureGbrainSourceGitignored", () => {
  let root: string;

  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "gaganfoxwell-gbrain-gitignore-"));
  });
