 ...
 *
 * When gaganfoxwell-core@1.x.y bumps and the cached _meta.json records an older
...
import { GAGANFOXWELL_SCHEMA_PACK_VERSION } from '../scripts/brain-cache-spec';

let TMP_HOME: string;
const ORIGINAL_HOME = process.env.GAGANFOXWELL_HOME;

beforeEach(() => {
  TMP_HOME = mkdtempSync(join(tmpdir(), 'gaganfoxwell-schema-test-'));
...
describe('schema-version cache migration (D4 A4)', () => {
  test('cache file with mismatched schema_version triggers wipe-and-rebuild attempt', { timeout: SLOW_TIMEOUT }, async () => {
    ...
    expect(newMeta.schema_version).toBe(GAGANFOXWELL_SCHEMA_PACK_VERSION);
  });
  ...
  test('matching schema_version + fresh TTL is warm hit (no rebuild)', { timeout: SLOW_TIMEOUT }, async () => {
    ...
    writeFileSync(join(cacheDir, '_meta.json'), JSON.stringify({
      schema_version: GAGANFOXWELL_SCHEMA_PACK_VERSION,
