...
describe('validateHostConfig', () => {
  function makeValid(): HostConfig {
    return {
      ...
      cliCommand: 'testcli',
      defaultModel: 'claude',
      globalRoot: '.test/skills/gaganfoxwell',
      localSkillRoot: '.test/skills/gaganfoxwell',
      hostSubdir: '.test',
      usesEnvVars: true,
  ...
  test('paths with $ and ~ are valid', () => {
    const c = makeValid();
    c.globalRoot = '$HOME/.test/skills/gaganfoxwell';
    c.localSkillRoot = '~/.test/skills/gaganfoxwell';
    expect(validateHostConfig(c)).toEqual([]);
  });
...
describe('validateAllConfigs', () => {
  ...
  test('duplicate hostSubdir detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.claude', globalRoot: '.dup/skills/gaganfoxwell' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
    expect(errors.some(e => e.includes('Duplicate hostSubdir'))).toBe(true);
  });

  test('duplicate globalRoot detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.dup', globalRoot: '.claude/skills/gaganfoxwell' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
  ...
  test('unknown suppressedResolvers entry surfaces with host-name prefix', () => {
    const bad = { ...codex, name: 'bad-host', hostSubdir: '.bad', globalRoot: '.bad/skills/gaganfoxwell', suppressedResolvers: ['BOGUS_RESOLVER'] } as HostConfig;
...
describe('HOST_PATHS derivation from configs', () => {
  test('Claude uses literal home paths (no env vars)', () => {
    expect(HOST_PATHS.claude.skillRoot).toBe('~/.claude/skills/gaganfoxwell');
    expect(HOST_PATHS.claude.binDir).toBe('~/.claude/skills/gaganfoxwell/bin');
    expect(HOST_PATHS.claude.browseDir).toBe('~/.claude/skills/gaganfoxwell/browse/dist');
    expect(HOST_PATHS.claude.designDir).toBe('~/.claude/skills/gaganfoxwell/design/dist');
  });

  test('Codex uses $GAGANFOXWELL_ROOT env vars', () => {
    expect(HOST_PATHS.codex.skillRoot).toBe('$GAGANFOXWELL_ROOT');
    expect(HOST_PATHS.codex.binDir).toBe('$GAGANFOXWELL_BIN');
    expect(HOST_PATHS.codex.browseDir).toBe('$GAGANFOXWELL_BROWSE');
    expect(HOST_PATHS.codex.designDir).toBe('$GAGANFOXWELL_DESIGN');
  });

  test('every host with usesEnvVars=true gets env var paths', () => {
    for (const config of ALL_HOST_CONFIGS) {
      if (config.usesEnvVars) {
        expect(HOST_PATHS[config.name].skillRoot).toBe('$GAGANFOXWELL_ROOT');
        expect(HOST_PATHS[config.name].binDir).toBe('$GAGANFOXWELL_BIN');
      }
    }
  });
...
describe('host-config-export.ts CLI', () => {
  ...
  test('get returns string field', () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toBe('.codex/skills/gaganfoxwell');
  });
...
describe('golden-file regression', () => {
  ...
  beforeAll(() => {
    const hostArtifacts: Array<[string, string]> = [
      ['codex', path.join(ROOT, '.agents', 'skills', 'gaganfoxwell-ship', 'SKILL.md')],
      ['factory', path.join(ROOT, '.factory', 'skills', 'gaganfoxwell-ship', 'SKILL.md')],
    ];
  ...
  test('Codex ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'gaganfoxwell-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
  ...
  test('Factory ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.factory', 'skills', 'gaganfoxwell-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
});
