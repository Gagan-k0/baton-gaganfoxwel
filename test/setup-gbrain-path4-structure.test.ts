...
//     by unit tests for gaganfoxwell-gbrain-mcp-verify and gaganfoxwell-artifacts-init.
//   - The structural prose is the source of regressions for AUQ pacing
//     (the failure mode the gaganfoxwell repo has tracked since v1.26.x:
...
describe('setup-gbrain Path 4 (Remote MCP) — structural contract', () => {
  ...
  test('Step 4c invokes gaganfoxwell-gbrain-mcp-verify and STOPs on failure', () => {
    expect(tmpl).toContain('gaganfoxwell-gbrain-mcp-verify');
  ...
  test('Step 7 calls gaganfoxwell-artifacts-init with --url-form-supported flag', () => {
    expect(tmpl).toMatch(/gaganfoxwell-artifacts-init.*--url-form-supported/);
  });
