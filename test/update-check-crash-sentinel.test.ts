...
// #1974: gaganfoxwell-update-check runs under set -e, and its "up to date" signal
...
const SCRIPT = path.join(ROOT, 'bin', 'gaganfoxwell-update-check');
...
describe('gaganfoxwell-update-check crash sentinel (#1974)', () => {
  test('a mid-script crash emits CHECK_FAILED instead of silent up-to-date', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-upd-'));
    try {
      ...
      const r = run({ GAGANFOXWELL_STATE_DIR: asFile, GAGANFOXWELL_REMOTE_URL: 'file:///dev/null' });
  ...
  test('healthy up-to-date path stays silent (no sentinel noise)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-upd-'));
    try {
      const r = run({ GAGANFOXWELL_STATE_DIR: path.join(dir, 'state'), GAGANFOXWELL_REMOTE_URL: 'file:///dev/null' });
      expect(r.status).toBe(0);
