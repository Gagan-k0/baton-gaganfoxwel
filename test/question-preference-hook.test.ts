 ...
 *   - auto-decided event logged via gaganfoxwell-question-log (PostToolUse won't fire)
 *   - auto-decided marker written to ~/.gaganfoxwell/sessions/<id>/.auto-decided-<tool_use_id>
 */
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-prefhook-'));
  cwdSlug = 'fixture-slug';
  fs.mkdirSync(path.join(stateRoot, 'projects', cwdSlug), { recursive: true });
  // Real directory that the hook can chdir() into. gaganfoxwell-slug derives the
...
function runHook(stdin: object, cwd?: string, extraEnv?: Record<string, string>): {
  ...
  env.GAGANFOXWELL_STATE_ROOT = stateRoot;
  ...
  env.GAGANFOXWELL_QUESTION_LOG_NO_DERIVE = '1';
...
describe('passes through (no enforcement)', () => {
  test('no preference set → pass-through (empty stdout, no permissionDecision)', () => {
    const r = runHook({
      ...
      tool_input: {
        questions: [
          { question: '<gaganfoxwell-qid:test-q> Need approval?', options: ['A) Yes (recommended)', 'B) No'] },
        ],
      },
    });
    expectPassThrough(r);
  ...
  test('always-ask preference → pass-through', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          { question: '<gaganfoxwell-qid:test-q> Yes?', options: ['A) Yes (recommended)', 'B) No'] },
        ],
      },
    });
    expectPassThrough(r);
  ...
  test('pass-through stdout never contains "permissionDecision" (#2035)', () => {
    const paths = [
      runHook({
        ...
        tool_input: {
          questions: [
            { question: '<gaganfoxwell-qid:test-q> Approve?', options: ['A) Yes (recommended)', 'B) No'] },
          ],
        },
      }),
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question:
              '<gaganfoxwell-qid:ship-pre-landing-review-fix> Pre-landing review flagged issue.',
  ...
  test('one-way door → pass-through even with never-ask (safety override)', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:ship-test-failure-triage> Tests failed.',
  ...
  test('ambiguous recommendation (two labels) → pass-through (D2 refuse-on-ambiguous)', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:ship-pre-landing-review-fix> Ambiguous',
  ...
  test('no recommendation marker AND no prose match → pass-through', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:ship-pre-landing-review-fix> No rec',
            options: ['A) Foo', 'B) Bar'],
          },
        ],
      },
  ...
  test('unregistered id + never-ask + destructive text → pass-through (keyword net fires, #2024)', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:adhoc-credential-cleanup> Reset my secret and proceed?',
  ...
  test('unregistered id + never-ask + benign text → still deny (auto-decide unchanged)', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:adhoc-credential-cleanup> Reorganize the TODOs file?',
            options: ['A) Yes (recommended)', 'B) No'],
          },
        ],
      },
  ...
  test('project never-ask + global always-ask → enforce never-ask', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:ship-pre-landing-review-fix> P?',
...
describe('Conductor prose redirect', () => {
  ...
  test('CONDUCTOR_WORKSPACE_PATH alone also triggers the redirect', () => {
    const r = runHook({
      ...
      tool_use_id: 'tu-c4',
      tool_input: {
        questions: [{ question: '<gaganfoxwell-qid:test-q> Pick?', options: ['A) X (recommended)', 'B) Y'] }],
      },
  ...
  test('PRECEDENCE: full never-ask auto-decide still wins over Conductor prose', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:ship-pre-landing-review-fix> Pre-landing review flagged issue.',
            options: ['A) Fix now (recommended)', 'B) Skip'],
          },
        ],
      },
