...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-hooklog-'));
  // Pre-create slug-resolved project dir so the bin's gaganfoxwell-slug doesn't
...
describe('PostToolUse hook (native AskUserQuestion)', () => {
  ...
  test('marker-first question_id when <gaganfoxwell-qid:foo> present', () => {
    runHook({
      session_id: 'sess2',
      tool_name: 'AskUserQuestion',
      tool_use_id: 'tu-2',
      tool_input: {
        questions: [
          {
            question: 'D2 — Marker test <gaganfoxwell-qid:ship-test-failure-triage>\nRecommendation: A',
    ...
    expect((events[0].question_summary as string).includes('<gaganfoxwell-qid:')).toBe(false);
  });
});
