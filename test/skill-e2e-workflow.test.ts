...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-ship-work-'));
...
// --- gaganfoxwell-upgrade E2E ---

describeIfSelected('gaganfoxwell-upgrade E2E', ['gaganfoxwell-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-remote-'));
    ...
    // Create mock gaganfoxwell install directory (local-git type)
    const mockGaganfoxwell = path.join(upgradeDir, '.claude', 'skills', 'gaganfoxwell');
    fs.mkdirSync(mockGaganfoxwell, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockGaganfoxwell);
    run('git', ['config', 'user.email', 'test@test.com'], mockGaganfoxwell);
    run('git', ['config', 'user.name', 'Test'], mockGaganfoxwell);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockGaganfoxwell);

    // Write old version files
    fs.writeFileSync(path.join(mockGaganfoxwell, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockGaganfoxwell, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockGaganfoxwell, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockGaganfoxwell);
    run('git', ['commit', '-m', 'initial'], mockGaganfoxwell);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockGaganfoxwell);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockGaganfoxwell, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockGaganfoxwell, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockGaganfoxwell);
    run('git', ['commit', '-m', 'release 0.6.0'], mockGaganfoxwell);
    run('git', ['push', 'origin', 'HEAD:main'], mockGaganfoxwell);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockGaganfoxwell);

    // Copy gaganfoxwell-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'gaganfoxwell-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'gaganfoxwell-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'gaganfoxwell-upgrade', 'SKILL.md'),
    );
  ...
  testConcurrentIfSelected('gaganfoxwell-upgrade-happy-path', async () => {
    const mockGaganfoxwell = path.join(upgradeDir, '.claude', 'skills', 'gaganfoxwell');
    const result = await runSkillTest({
      prompt: `Read gaganfoxwell-upgrade/SKILL.md for the upgrade workflow.

You are running /gaganfoxwell-upgrade standalone. The gaganfoxwell installation is at ./.claude/skills/gaganfoxwell (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/gaganfoxwell — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'gaganfoxwell-upgrade-happy-path',
      runId,
    });

    logCost('/gaganfoxwell-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockGaganfoxwell, 'VERSION'), 'utf-8').trim();
    ...
    recordE2E(evalCollector, '/gaganfoxwell-upgrade happy path', 'gaganfoxwell-upgrade E2E', result, {
