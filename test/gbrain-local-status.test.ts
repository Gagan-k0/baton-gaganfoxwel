 ...
 *   5. timeout        — probe exceeds GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS with no recognized error (#1964)
 ...
 * setting GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS=300 against a fake gbrain that sleeps 2s.
 */
...
interface FakeEnv {
  ...
  bindir: string;
  home: string;
  gaganfoxwellHome: string;
  configPath: string;
  cleanup: () => void;
}

/**
 * Build a tmp HOME + GAGANFOXWELL_HOME + optional fake `gbrain` on PATH.
 *
...
function applyEnv(env: FakeEnv): () => void {
  const prev = {
    ...
    GAGANFOXWELL_HOME: process.env.GAGANFOXWELL_HOME,
    GBRAIN_HOME: process.env.GBRAIN_HOME,
    GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: process.env.GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS,
  };
  process.env.HOME = env.home;
  process.env.PATH = `${env.bindir}:/usr/bin:/bin`;
  process.env.GAGANFOXWELL_HOME = env.gaganfoxwellHome;
  delete process.env.GBRAIN_HOME;
  delete process.env.GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS;
  return () => {
...
describe("lib/gbrain-local-status — status classification", () => {
  ...
  it("returns 'timeout' (not broken-config) when the probe exceeds the deadline (#1964)", () => {
    ...
    restoreEnv = applyEnv(env);
    process.env.GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS = "300";
  ...
  it("does NOT read $GBRAIN_HOME/config.json directly — gbrain never reads that file (#2521)", () => {
    // A config placed at gaganfoxwell's OLD (wrong) resolution must be invisible:
    // gbrain itself would report "No brain configured" for this layout, so
    // gaganfoxwell classifying "ok" from it is the #2521 split-brain.
    env = makeEnv({ withGbrain: true, gbrainBehavior: "ok", withConfig: false });
    restoreEnv = applyEnv(env);
...
describe("gaganfoxwell-gbrain-detect --is-ok — timeout is usable (eng review D1)", () => {
  it("exits 0 when the engine probe times out (slow-but-healthy must not suppress brain features)", () => {
    ...
    try {
      const detect = join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-detect");
      const r = spawnSync(process.execPath, [detect, "--is-ok"], {
        ...
        env: {
          ...process.env,
          HOME: env.home,
          GAGANFOXWELL_HOME: env.gaganfoxwellHome,
          PATH: `${env.bindir}:/usr/bin:/bin`,
          GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: "300",
          GAGANFOXWELL_DETECT_NO_CACHE: "1",
          GBRAIN_HOME: "",
        },
      });
...
describe("probeTimeoutMs — env override parsing", () => {
  ...
  it("parses a numeric override", () => {
    expect(probeTimeoutMs({ GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: "300" })).toBe(300);
  });

  it("falls back to the default on non-numeric, empty, and non-positive values", () => {
    expect(probeTimeoutMs({ GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: "fast" })).toBe(DEFAULT_PROBE_TIMEOUT_MS);
    expect(probeTimeoutMs({ GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: "" })).toBe(DEFAULT_PROBE_TIMEOUT_MS);
    expect(probeTimeoutMs({ GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: "0" })).toBe(DEFAULT_PROBE_TIMEOUT_MS);
    expect(probeTimeoutMs({ GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: "-5" })).toBe(DEFAULT_PROBE_TIMEOUT_MS);
  });

  it("never returns 0 for fractional sub-millisecond values (0 = NO timeout in execFileSync)", () => {
    expect(probeTimeoutMs({ GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: "0.5" })).toBe(1);
    expect(probeTimeoutMs({ GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS: "0.0001" })).toBe(1);
  });
});
...
describe("lib/gbrain-local-status — cache behavior", () => {
  ...
  it("invalidates a cached 'timeout' when GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS changes (key invariant, codex D13)", () => {
    env = makeEnv({ withGbrain: true, gbrainBehavior: "slow", withConfig: true });
    restoreEnv = applyEnv(env);
    process.env.GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS = "300";
    expect(localEngineStatus({ noCache: false })).toBe("timeout");
    ...
    // re-probe succeeds.
    process.env.GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS = "5000";
    expect(localEngineStatus({ noCache: false })).toBe("ok");
  });
  ...
  it("invalidates cache when HOME changes (key invariant)", () => {
    ...
    // Switch to a new HOME (different user). Same gaganfoxwell home (shared cache file).
    const env2 = makeEnv({
      withGbrain: true,
    ...
    // GAGANFOXWELL_HOME stays pointing at env.gaganfoxwellHome (the original cache file).
...
describe("lib/gbrain-local-status — thin-client (#2051)", () => {
  ...
  // return. --is-ok drives setup:1299 and gaganfoxwell-config gbrain-refresh — this
  ...
  it("--is-ok exits 0 on a thin-client fixture (end-to-end gate)", () => {
    env = makeEnv({ withGbrain: true, gbrainBehavior: "ok", thinClientConfig: true });
    const detectBin = join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-detect");
