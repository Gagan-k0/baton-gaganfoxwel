/**
 * Regression: no stale `gaganfoxwell-brain-init`, `gbrain_sync_mode`, or
 * `~/.gaganfoxwell-brain-remote.txt` references survive the v1.27.0.0 rename.
 *
 ...
 *
 * Allowlist: the migration script (`gaganfoxwell-upgrade/migrations/v1.27.0.0.sh`)
...
const ALLOWLIST = [
  ...
  'gaganfoxwell-upgrade/migrations/v1.27.0.0.sh',
  // Older migration scripts — historical references; these document past state.
  'gaganfoxwell-upgrade/migrations/v1.17.0.0.sh',
  ...
  'test/gaganfoxwell-upgrade-migration-v1_17_0_0.test.ts',
  ...
  // The bin/gaganfoxwell-config comment explicitly preserves the rename note.
  ...
  'bin/gaganfoxwell-gbrain-source-wireup',
  'bin/gaganfoxwell-brain-uninstall',
  ...
  // gaganfoxwell-upgrade.test.ts may exercise old migration behavior.
  'test/gaganfoxwell-upgrade.test.ts',
  // This test itself references the patterns to grep for.
  'test/no-stale-gaganfoxwell-brain-refs.test.ts',
  ...
  'test/gaganfoxwell-artifacts-init.test.ts',
...
const SCAN_PATHS = [
  ...
  // a dead command (gaganfoxwell-brain-init) survive ~36 releases as a
  // command-not-found instruction. Docs are user-facing surface; scan them.
  'docs/',
  'README.md',
  'USING_GBRAIN_WITH_GAGANFOXWELL.md',
];
...
describe('no stale gaganfoxwell-brain refs (v1.27.0.0 rename)', () => {
