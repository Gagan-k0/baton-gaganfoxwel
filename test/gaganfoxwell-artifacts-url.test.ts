/**
 * gaganfoxwell-artifacts-url — URL canonicalization helper.
 *
...
const URL_BIN = path.join(ROOT, 'bin', 'gaganfoxwell-artifacts-url');
...
describe('gaganfoxwell-artifacts-url', () => {
  test('--to ssh from canonical https', () => {
    const r = run(['--to', 'ssh', 'https://github.com/garrytan/gaganfoxwell-artifacts-garrytan']);
    expect(r.code).toBe(0);
    expect(r.stdout).toBe('git@github.com:garrytan/gaganfoxwell-artifacts-garrytan.git');
  });

  test('--to ssh from https-with-.git', () => {
    const r = run(['--to', 'ssh', 'https://github.com/garrytan/gaganfoxwell-artifacts-garrytan.git']);
    expect(r.stdout).toBe('git@github.com:garrytan/gaganfoxwell-artifacts-garrytan.git');
  });

  test('--to https is idempotent on https input', () => {
    const r = run(['--to', 'https', 'https://github.com/garrytan/gaganfoxwell-artifacts-garrytan']);
    expect(r.stdout).toBe('https://github.com/garrytan/gaganfoxwell-artifacts-garrytan');
  });

  test('--to https from git@host:owner/repo.git', () => {
    const r = run(['--to', 'https', 'git@github.com:garrytan/gaganfoxwell-artifacts-garrytan.git']);
    expect(r.stdout).toBe('https://github.com/garrytan/gaganfoxwell-artifacts-garrytan');
  });

  test('--to https from ssh:// scheme (gitlab self-hosted style)', () => {
    const r = run(['--to', 'https', 'ssh://git@gitlab.example.org/team/gaganfoxwell-artifacts-team.git']);
    expect(r.stdout).toBe('https://gitlab.example.org/team/gaganfoxwell-artifacts-team');
  });
  ...
  test('--owner-repo extracts the path segment', () => {
    expect(run(['--owner-repo', 'https://github.com/garrytan/gaganfoxwell-artifacts-garrytan']).stdout)
      .toBe('garrytan/gaganfoxwell-artifacts-garrytan');
    expect(run(['--owner-repo', 'git@github.com:team/gaganfoxwell-artifacts-team.git']).stdout)
      .toBe('team/gaganfoxwell-artifacts-team');
  });
  ...
  test('round-trip: https → ssh → https is identity', () => {
    const original = 'https://github.com/garrytan/gaganfoxwell-artifacts-garrytan';
