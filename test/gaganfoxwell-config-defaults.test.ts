/**
 * gaganfoxwell-config default-table completeness (gate, free).
 *
 * Skill preambles read configuration with
 *
 *     VAR=$(gaganfoxwell-config get <key> 2>/dev/null || echo "<default>")
 *
 ...
 *      table. Add a `gaganfoxwell-config get some_new_key` to a preamble without
...
const CONFIG_BIN = path.join(ROOT, 'bin', 'gaganfoxwell-config');
const SELF = 'gaganfoxwell-config-defaults.test.ts';

// Isolated state dir, so a value the developer happens to have set in their own
// ~/.gaganfoxwell/config.yaml cannot mask a missing default.
const STATE = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-config-test-'));

function get(key: string): { out: string; code: number } {
  const r = spawnSync('bash', [CONFIG_BIN, 'get', key], {
    encoding: 'utf-8',
    env: { ...process.env, GAGANFOXWELL_STATE_ROOT: STATE },
  });
...
/** Every `gaganfoxwell-config get <key>` call site in the tree. */
function keysReadInTree(): string[] {
  const keys = new Set<string>();
  // [ \t]+ rather than \s+: \s crosses newlines and would pair a trailing
  // "gaganfoxwell-config get" with the first word of the next line.
  const re = /gaganfoxwell-config["']?[ \t]+get[ \t]+([a-zA-Z0-9_]+)/g;
  const stack = [ROOT];
  while (stack.length) {
    const cur = stack.pop()!;
    ...
    for (const ent of entries) {
      ...
      if (!/\.(md|ts|sh)$|^gaganfoxwell-[a-z-]+$/.test(ent.name)) continue;
      let text: string;
      try {
...
describe('gaganfoxwell-config defaults (gate, free)', () => {
  ...
  test('an unknown key exits non-zero, so the caller fallback fires', () => {
    const r = get('definitely_not_a_gaganfoxwell_key_9f3a');
    expect(r.code).not.toBe(0);
    expect(r.out).toBe('');
  });

  test('a known key whose default is intentionally empty still exits 0', () => {
    // repo_mode is in this class BY CONTRACT: gaganfoxwell-repo-mode treats any
    ...
    // (caught live by test/gaganfoxwell-repo-mode.test.ts during the wave).
