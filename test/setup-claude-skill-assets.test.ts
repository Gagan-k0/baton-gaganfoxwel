 ...
 * templates/ + references/, gaganfoxwell-upgrade's migrations/, careful/freeze's
 ...
 *   - Class 2 (repo-anchored): a `~/.claude/skills/gaganfoxwell/<relpath>`
 *     reference must exist in the source tree, EXCEPT built artifacts
 *     (browse/dist, design/dist, make-pdf/dist, the compiled
 *     bin/gaganfoxwell-global-discover) — the free suite never builds binaries, so
...
const BUILT_ARTIFACT_ALLOWLIST = [
  ...
  'design/dist/',
  'make-pdf/dist/',
  'bin/gaganfoxwell-global-discover', // compiled from bin/gaganfoxwell-global-discover.ts at build time
];
...
const installDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-claude-install-'));
...
describe('two-class referenced-paths (ENG-OV7)', () => {
  test('class 1: alias-relative references resolve under the install dir', () => {
    ...
    for (const { fromSkill, skillName, rel } of collectRefs()) {
      if (skillName === 'gaganfoxwell') continue; // class 2
      // Prefix-mode prose may reference gaganfoxwell-<name>; the flat install dir
      // is the unprefixed name.
      const candidates = [skillName, skillName.replace(/^gaganfoxwell-/, '')];
  ...
  test('class 2: repo-anchored references exist in the tree (modulo built artifacts)', () => {
    const missing: string[] = [];
    for (const { fromSkill, skillName, rel } of collectRefs()) {
      if (skillName !== 'gaganfoxwell') continue; // class 1
      ...
      if (!fs.existsSync(path.join(ROOT, rel))) {
        missing.push(`${fromSkill}/SKILL.md → ~/.claude/skills/gaganfoxwell/${rel}`);
      }
    }
    expect(missing).toEqual([]);
  });
  ...
  test('the referenced-path scan actually sees the review checklist refs (self-check)', () => {
    ...
    // installed gaganfoxwell root (class 2: gaganfoxwell/review/checklist.md), not the
    // alias-relative form — if the scanner stops seeing them, the class-2
    // assertion is vacuous.
    const refs = collectRefs();
    expect(refs.length).toBeGreaterThan(0);
    expect(refs.some((r) => r.skillName === 'gaganfoxwell' && r.rel === 'review/checklist.md')).toBe(true);
  });
