 ...
 *
 * Two compounding bugs, both in gaganfoxwell:
 *
 ...
 *
 *   2. Trailing slash in TMP_ROOT. `bin/gaganfoxwell-paths` emitted TMP_ROOT straight
...
const PATHS_BIN = path.join(ROOT, 'bin', 'gaganfoxwell-paths');
...
/** Directories that are not gaganfoxwell-authored skill/template sources. */
...
function collectScannedFiles(dir: string, out: string[]): void {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    // Symlinks (e.g. connect-chrome → open-gaganfoxwell-browser) would double-count
...
describe('#2091/#2370 bug 1: every mktemp template is BSD-safe (X placeholder at end)', () => {
  ...
  test('the offender-detector itself still detects the original bug shapes', () => {
    ...
    expect(
      offendingTemplatesOnLine('TMPOUT=$(mktemp "$GAGANFOXWELL_HOME/developer-profile.json.XXXXXX.tmp")'),
    ).toEqual(['$GAGANFOXWELL_HOME/developer-profile.json.XXXXXX.tmp']);
    expect(offendingTemplatesOnLine('RESP_FILE=$(mktemp /tmp/gaganfoxwell-claude-response-XXXXXX.json)')).toEqual([
      '/tmp/gaganfoxwell-claude-response-XXXXXX.json',
    ]);
    // Fixed shapes pass.
...
// ── Bug 2: gaganfoxwell-paths normalizes TMP_ROOT (no trailing slash) ──────────────
// Mirrors the invocation contract used by test/gaganfoxwell-paths.test.ts: the helper
...
function tmpRoot(env: Record<string, string | undefined>): string {
  ...
  throw new Error('gaganfoxwell-paths did not emit TMP_ROOT');
}

describe('#2091 bug 2: gaganfoxwell-paths strips the trailing slash from TMP_ROOT', () => {
