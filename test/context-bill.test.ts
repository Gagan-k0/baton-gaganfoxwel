/**
 * gaganfoxwell-context-bill — token bill-of-materials for an installed skills tree.
 *
...
describe("upstream fix a: root-as-container + walkMd exclusions", () => {
  ...
  it("totalMd: a container skill excludes nested child skills' bytes; the grand total sums per-skill with no overlap", () => {
    ...
    // CONTAINS other skill dirs (the gaganfoxwell root wraps the whole tree) used to
...
describe("upstream fix b: installed-tree layout (repo-checkout subdir skip)", () => {
  it("skips a subdir that is its own repo checkout (gaganfoxwell/ inside ~/.claude/skills)", () => {
    ...
    fs.mkdirSync(path.join(tmp, "gaganfoxwell", ".git"), { recursive: true });
    fs.writeFileSync(path.join(tmp, "gaganfoxwell", "SKILL.md"), "---\nname: _router\ndescription: d\n---\n# router\n");
    fs.mkdirSync(path.join(tmp, "gaganfoxwell", "review"));
    fs.writeFileSync(path.join(tmp, "gaganfoxwell", "review", "SKILL.md"), "---\nname: review\ndescription: d\n---\n# r\n");
...
describe("CLI plumbing", () => {
  ...
  it("bin/gaganfoxwell-context-bill runs standalone", () => {
    const result = Bun.spawnSync([path.join(ROOT, "bin", "gaganfoxwell-context-bill"), TREE_A]);
