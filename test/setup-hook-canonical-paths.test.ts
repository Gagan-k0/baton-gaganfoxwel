 ...
 *
 * Static tripwires over `setup` and `bin/gaganfoxwell-settings-hook`. The defect
 * class these pin against: hook commands baked from the SETUP-TIME tree
 * (`$SOURCE_GAGANFOXWELL_DIR` = `pwd -P` of the running tree) into the user's
 ...
 *     install: ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/gaganfoxwell) — an
...
const hookBinSrc = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-settings-hook'), 'utf-8');
const uninstallSrc = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-uninstall'), 'utf-8');

describe('setup: canonical-only hook paths', () => {
  test('CANONICAL_GAGANFOXWELL_ROOT honors CLAUDE_CONFIG_DIR with ~/.claude fallback', () => {
    expect(setupSrc).toContain(
      'CANONICAL_GAGANFOXWELL_ROOT="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/gaganfoxwell"',
    );
  });
  ...
  test('_hook_command_path body never references the running tree', () => {
    ...
    expect(body).not.toContain('SOURCE_GAGANFOXWELL_DIR');
    expect(body).toContain('CANONICAL_GAGANFOXWELL_ROOT');
  });

  test('every hook var routes through _hook_command_path; no raw SOURCE_GAGANFOXWELL_DIR hook assignment remains', () => {
    for (const rel of [
    ...
    // The bug: FOO_HOOK="$SOURCE_GAGANFOXWELL_DIR/hosts/claude/hooks/..."
    expect(setupSrc).not.toMatch(/="\$SOURCE_GAGANFOXWELL_DIR\/hosts\/claude\/hooks\//);
    expect(setupSrc).not.toMatch(/HOOK_CMD="(bash )?\$SOURCE_GAGANFOXWELL_DIR\/bin\/gaganfoxwell-session-update"/);
  });

  test('SessionStart registers via schema-aware add-event under its identity source', () => {
    expect(setupSrc).toMatch(
      /add-event --event SessionStart --command "\$HOOK_CMD" --source gaganfoxwell-session-update/,
    );
  });
...
describe('setup: heal-first ordering + visibility', () => {
  ...
  test('--no-team teardown includes the auq source and the identity sweep', () => {
    ...
    expect(slice).toContain('remove-source --source gaganfoxwell-timeline-stop');
    expect(slice).toContain('prune-stale --all');
  });
});

describe('gaganfoxwell-settings-hook: shared prelude (dedupe key == prune predicate)', () => {
  ...
  test('KNOWN_HOOKS table carries all five identities with source+event+relpath', () => {
    for (const [name, source, event] of [
      ...
      ['timeline-stop-hook', 'gaganfoxwell-timeline-stop', 'Stop'],
      ['gaganfoxwell-session-update', 'gaganfoxwell-session-update', 'SessionStart'],
    ]) {
...
describe('gaganfoxwell-uninstall: hook cleanup runs before install-root deletion', () => {
  test('the settings cleanup block precedes every install-root rm -rf', () => {
    // Pre-fix bug: SETTINGS_HOOK=$(dirname "$0")/gaganfoxwell-settings-hook resolved
    ...
    const cleanup = uninstallSrc.indexOf('Remove gaganfoxwell hooks from Claude Code settings');
    const rootDelete = uninstallSrc.indexOf('rm -rf "$CLAUDE_SKILLS/gaganfoxwell"');
  ...
  test('uninstall removes all three sources and sweeps untagged strays', () => {
    ...
    expect(uninstallSrc).toContain('remove-source --source gaganfoxwell-timeline-stop');
...
describe('the defect-class warning is written down where the next author will see it', () => {
  test('setup carries the never-register-tree-relative-paths warning', () => {
    expect(setupSrc).toMatch(/NEVER register .*SOURCE_GAGANFOXWELL_DIR.*hook paths/);
  });
});
