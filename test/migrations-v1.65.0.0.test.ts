 ...
 * (HOME / GAGANFOXWELL_HOME / PLAYWRIGHT_BROWSERS_PATH all redirected) with a
 ...
 *   - re-fetch runs from the gaganfoxwell install root (repo-pinned playwright)
...
const MIGRATION = path.join(ROOT, 'gaganfoxwell-upgrade', 'migrations', 'v1.65.0.0.sh');
...
const doneFile = () => path.join(tmpHome, '.gaganfoxwell', '.migrations', 'v1.65.0.0.done');
...
describe('v1.65.0.0 migration — poisoned bundle removal', () => {
  test('poisoned revision dir removed WHOLE (markers included); clean bundle untouched; verified re-fetch → .done', () => {
    ...
    // ...from the gaganfoxwell install root (repo-pinned playwright version), not
