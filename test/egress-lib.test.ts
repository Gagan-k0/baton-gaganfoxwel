/**
 * gaganfoxwell-egress-lib.sh — shared shell receipt helpers, tested end-to-end
...
const LIB = path.join(ROOT, 'bin', 'gaganfoxwell-egress-lib.sh');
...
beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-egress-lib-'));
  received.length = 0;
});
...
describe('_receipted_curl', () => {
  ...
  test('fail-closed refusal never hits the network; stderr is problem + cause + fix', async () => {
    ...
    expect(result.stderr).toContain('gaganfoxwell-egress');
  });
...
describe('_receipted_git', () => {
  ...
  test('fail-closed git refusal returns 3 without running the command', async () => {
    if (process.platform === 'win32' || process.getuid?.() === 0) return;
    fs.mkdirSync(path.join(home, 'security'), { recursive: true, mode: 0o500 });
    const marker = path.join(os.tmpdir(), `gaganfoxwell-egress-git-${process.pid}`);
