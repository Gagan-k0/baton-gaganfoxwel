 ...
 * auto-deciding a specific question via `gaganfoxwell-question-preference --write
 ...
 *
 * Set up:
 *   - tmpDir as GAGANFOXWELL_HOME (isolated state, doesn't touch the user's
 *     real ~/.gaganfoxwell)
...
describeE2E('AUTO_DECIDE opt-in preserved under Conductor flags (periodic)', () => {
  test('user-opted-in question still auto-decides when AskUserQuestion is --disallowedTools', async () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-auto-decide-'));
    try {
      // 1. Bootstrap the tmp GAGANFOXWELL_HOME with question_tuning=true.
      const configBin = path.join(ROOT, 'bin', 'gaganfoxwell-config');
      const setRes = spawnSync(configBin, ['set', 'question_tuning', 'true'], {
        env: { ...process.env, GAGANFOXWELL_HOME: tmpHome },
        encoding: 'utf-8',
      });
      if (setRes.status !== 0) {
        throw new Error(`gaganfoxwell-config set failed: ${setRes.stderr || setRes.stdout}`);
      }
      ...
      const slugBin = path.join(ROOT, 'bin', 'gaganfoxwell-slug');
      const slugRes = spawnSync(slugBin, [], {
        cwd: ROOT,
        env: { ...process.env, GAGANFOXWELL_HOME: tmpHome },
        encoding: 'utf-8',
      });
      // gaganfoxwell-slug emits `eval`-able shell exports like `SLUG=garrytan-gaganfoxwell`.
      ...
      if (writeRes.status !== 0) {
        throw new Error(`gaganfoxwell-question-preference --write failed: ${writeRes.stderr || writeRes.stdout}`);
      }
      ...
      //    GAGANFOXWELL_HOME=tmpHome is REQUIRED: the preference + question_tuning were
      //    seeded there. Without it the spawned claude reads the real ~/.gaganfoxwell,
      ...
      const obs = await runPlanSkillObservation({
        ...
        timeoutMs: 540_000,
        env: { GAGANFOXWELL_HOME: tmpHome, CONDUCTOR_WORKSPACE_PATH: tmpHome },
      });
