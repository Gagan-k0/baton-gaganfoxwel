...
// gaganfoxwell-learnings-log is the command from the original bug report: bin scripts
...
function extractKiroBlock(): string {
  const startAnchor = 'KIRO_GAGANFOXWELL="$KIRO_SKILLS/gaganfoxwell"';
  const endAnchor = '_link_or_copy "$SOURCE_GAGANFOXWELL_DIR/supabase/config.sh" "$KIRO_GAGANFOXWELL/supabase/config.sh"\n  fi';
...
// gaganfoxwell-learnings-log from the installed root and check the learning landed.
function buildRootAndRunCommand(
  isWindows: '0' | '1',
  buildScript: (sandbox: string) => { script: string; rootDir: string },
): CommandResult {
  const sandbox = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-runtime-lib-'));
  try {
    ...
    const run = spawnSync('bash', [path.join(rootDir, 'bin', 'gaganfoxwell-learnings-log'), PAYLOAD], {
      cwd: project,
      encoding: 'utf-8',
      timeout: 30000,
      env: { ...process.env, HOME: home, GAGANFOXWELL_HOME: path.join(home, '.gaganfoxwell') },
    });

    const projectsDir = path.join(home, '.gaganfoxwell', 'projects');
    ...
    return {
      ...
      // $GAGANFOXWELL_DIR/supabase/config.sh to resolve GAGANFOXWELL_SUPABASE_URL. The
...
const HOST_ROOTS: Record<string, (sandbox: string) => { script: string; rootDir: string }> = {
  'agents sidecar': (sandbox) => ({
    script: [
      `SOURCE_GAGANFOXWELL_DIR="${ROOT}"`,
      extractFunction('create_agents_sidecar'),
      `mkdir -p "${sandbox}/repo"`,
      `create_agents_sidecar "${sandbox}/repo"`,
    ].join('\n'),
    rootDir: path.join(sandbox, 'repo', '.agents', 'skills', 'gaganfoxwell'),
  }),
  codex: (sandbox) => ({
    script: [
      extractFunction('create_codex_runtime_root'),
      `create_codex_runtime_root "${ROOT}" "${sandbox}/home/.codex/skills/gaganfoxwell"`,
    ].join('\n'),
    rootDir: path.join(sandbox, 'home', '.codex', 'skills', 'gaganfoxwell'),
  }),
  factory: (sandbox) => ({
    script: [
      extractFunction('create_factory_runtime_root'),
      `create_factory_runtime_root "${ROOT}" "${sandbox}/home/.factory/skills/gaganfoxwell"`,
    ].join('\n'),
    rootDir: path.join(sandbox, 'home', '.factory', 'skills', 'gaganfoxwell'),
  }),
  opencode: (sandbox) => ({
    script: [
      extractFunction('create_opencode_runtime_root'),
      `create_opencode_runtime_root "${ROOT}" "${sandbox}/home/.opencode/skills/gaganfoxwell"`,
    ].join('\n'),
    rootDir: path.join(sandbox, 'home', '.opencode', 'skills', 'gaganfoxwell'),
  }),
  kiro: (sandbox) => ({
    script: [
      `HOME="${sandbox}/home"`,
    ...
    rootDir: path.join(sandbox, 'home', '.kiro', 'skills', 'gaganfoxwell'),
  }),
};
...
describe.skipIf(process.platform === 'win32')('setup: bin commands resolve sibling lib from every host root', () => {
  for (const [host, buildScript] of Object.entries(HOST_ROOTS)) {
    test(`${host} root (symlink install): gaganfoxwell-learnings-log imports ../lib and writes the learning`, () => {
    ...
    test(`${host} root (Windows copy install): gaganfoxwell-learnings-log imports ../lib and writes the learning`, () => {
