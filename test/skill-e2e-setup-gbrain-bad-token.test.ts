...
describeE2E('/setup-gbrain Path 4 — bad token STOPs cleanly', () => {
  test('AUTH classifier fires, no MCP registration, no CLAUDE.md mutation', async () => {
    ...
    const gaganfoxwellHome = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-bad-'));
    ...
    fs.writeFileSync(path.join(gaganfoxwellHome, 'CLAUDE.md'), ORIGINAL_CLAUDE_MD);
    ...
    const orig = {
      gaganfoxwellHome: process.env.GAGANFOXWELL_HOME,
      pathEnv: process.env.PATH,
      mcpToken: process.env.GBRAIN_MCP_TOKEN,
    };
    process.env.GAGANFOXWELL_HOME = gaganfoxwellHome;
    ...
    try {
      ...
      const finalClaudeMd = fs.readFileSync(path.join(gaganfoxwellHome, 'CLAUDE.md'), 'utf-8');
      ...
      if (orig.gaganfoxwellHome === undefined) delete process.env.GAGANFOXWELL_HOME; else process.env.GAGANFOXWELL_HOME = orig.gaganfoxwellHome;
