...
describe('preamble — QUESTION_TUNING injection', () => {
  test('tier 2+ skills include the Question Tuning section', async () => {
    ...
    const ctx = {
      ...
      paths: {
        skillRoot: '~/.claude/skills/gaganfoxwell',
        localSkillRoot: '.claude/skills/gaganfoxwell',
        binDir: '~/.claude/skills/gaganfoxwell/bin',
        browseDir: '~/.claude/skills/gaganfoxwell/browse/dist',
        designDir: '~/.claude/skills/gaganfoxwell/design/dist',
      },
      preambleTier: 2,
    };
    ...
    expect(out).toContain('gaganfoxwell-question-preference --check');
    expect(out).toContain('gaganfoxwell-question-log');
  ...
  test('codex host produces different paths', async () => {
    ...
    const codexCtx = {
      ...
      host: 'codex' as const,
      paths: {
        skillRoot: '$GAGANFOXWELL_ROOT',
        localSkillRoot: '.agents/skills/gaganfoxwell',
        binDir: '$GAGANFOXWELL_BIN',
        browseDir: '$GAGANFOXWELL_BROWSE',
        designDir: '$GAGANFOXWELL_DESIGN',
      },
    };
    const out = generateQuestionTuning(codexCtx);
    expect(out).toContain('$GAGANFOXWELL_BIN/gaganfoxwell-question-preference');
    expect(out).toContain('$GAGANFOXWELL_BIN/gaganfoxwell-question-log');
  });
});
...
describe('end-to-end pipeline (binaries working together)', () => {
  test('log many expand choices → derive pushes scope_appetite up', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'gaganfoxwell-e2e-'));
    try {
      // GAGANFOXWELL_QUESTION_LOG_NO_DERIVE=1 suppresses gaganfoxwell-question-log's
      ...
      const env = { ...process.env, GAGANFOXWELL_HOME: tmpHome, GAGANFOXWELL_QUESTION_LOG_NO_DERIVE: '1' };
      const { spawnSync } = require('child_process');
      const logBin = path.join(ROOT, 'bin', 'gaganfoxwell-question-log');
      const devBin = path.join(ROOT, 'bin', 'gaganfoxwell-developer-profile');
  ...
  test('preference blocks tune: write from inline-tool-output in full pipeline', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'gaganfoxwell-e2e-'));
    try {
      const env = { ...process.env, GAGANFOXWELL_HOME: tmpHome };
      const { spawnSync } = require('child_process');
      const prefBin = path.join(ROOT, 'bin', 'gaganfoxwell-question-preference');
  ...
  test('migration preserves sessions, builder-profile shim still works', () => {
    ...
    try {
      ...
      const { spawnSync } = require('child_process');
      const devBin = path.join(ROOT, 'bin', 'gaganfoxwell-developer-profile');
      const shimBin = path.join(ROOT, 'bin', 'gaganfoxwell-builder-profile');
