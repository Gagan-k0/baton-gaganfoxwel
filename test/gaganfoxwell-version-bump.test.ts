/**
 * Tests for the gaganfoxwell-version-bump CLI (v2 plan T9 hybrid extraction). Covers
...
import { classifyState, VERSION_RE } from '../bin/gaganfoxwell-version-bump';

const BIN = path.join(import.meta.dir, '..', 'bin', 'gaganfoxwell-version-bump');
 ...
 * holding plain 3-digit semver — the shape gaganfoxwell's native VERSION-file
 ...
 *
 * These cases pass --version-path explicitly; the .gaganfoxwell/version-path pin
...
/**
 * #2462: cmdClassify's current-version read resolved the .gaganfoxwell/version-path
 ...
 */
describe('.gaganfoxwell/version-path pin, no --version-path flag (#2462)', () => {
  const mkPinned = (pinRel: string): string => {
    ...
    fs.mkdirSync(path.join(d, '.gaganfoxwell'), { recursive: true });
    fs.writeFileSync(path.join(d, '.gaganfoxwell', 'version-path'), pinRel + '\n');
    return d;
  };
...
describe('subdirectory manifest (no root package.json, #2531)', () => {
  ...
  const mk = (): string => {
    ...
    fs.mkdirSync(path.join(d, '.gaganfoxwell'));
    fs.writeFileSync(path.join(d, '.gaganfoxwell', 'package-json-path'), 'web/package.json\n');
...
describe('path containment: pins and flags cannot escape the repo', () => {
  // .gaganfoxwell/version-path and .gaganfoxwell/package-json-path are repo-controlled
  ...
  function resetRepo() {
    ...
    fs.mkdirSync(path.join(dir, '.gaganfoxwell'), { recursive: true });
    fs.writeFileSync(path.join(dir, 'VERSION'), '1.0.0.0\n');
    fs.writeFileSync(victim, JSON.stringify({ version: '9.9.9' }, null, 2) + '\n');
  }

  test('a ../ escape in .gaganfoxwell/version-path fails exit 2 and writes nothing', () => {
    resetRepo();
    fs.writeFileSync(path.join(dir, '.gaganfoxwell', 'version-path'), '../victim.json\n');
  ...
  test('an absolute path in .gaganfoxwell/package-json-path fails exit 2', () => {
    resetRepo();
    fs.writeFileSync(path.join(dir, '.gaganfoxwell', 'package-json-path'), victim + '\n');
    const r = runFail(['write', '--version', '1.1.0.0']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('outside the repository');
    expect(JSON.parse(fs.readFileSync(victim, 'utf-8')).version).toBe('9.9.9');
  ...
  test('an in-repo symlink pointing outside fails exit 2 and never follows', () => {
    ...
    fs.writeFileSync(path.join(dir, '.gaganfoxwell', 'version-path'), 'link.json\n');
    const r = runFail(['write', '--version', '1.1.0.0']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('outside the repository');
    expect(JSON.parse(fs.readFileSync(victim, 'utf-8')).version).toBe('9.9.9');
  ...
  test('legitimate subdirectory pins still work (containment is not over-broad)', () => {
    ...
    fs.writeFileSync(path.join(dir, '.gaganfoxwell', 'version-path'), 'frontend/package.json\n');
