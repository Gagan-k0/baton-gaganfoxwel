...
const SRC = "gaganfoxwell-code-gaganfoxwell-c5994d95";
...
describe("cycleCompleted", () => {
  ...
  it("matches the source id as a LITERAL substring (regex metachars are inert)", () => {
    ...
    const metaId = "gaganfoxwell-code-a.b+c";
    const fake = makeFakeGbrain({
      doctorJson: doctor({
        name: "cycle_freshness",
        status: "warn",
    ...
    expect(cycleCompleted("gaganfoxwell-code-aXbc", fake.env)).toBe("unknown");
    fake.cleanup();
  });
});
