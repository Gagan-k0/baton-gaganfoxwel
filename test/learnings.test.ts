...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/gaganfoxwell-learnings-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runSearch(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/gaganfoxwell-learnings-search ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-learn-'));
...
describe('gaganfoxwell-learnings-log', () => {
...
describe('gaganfoxwell-learnings-search', () => {
...
describe('gaganfoxwell-learnings-log edge cases', () => {
...
describe('gaganfoxwell-learnings-search edge cases', () => {
