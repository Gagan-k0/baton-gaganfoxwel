...
describe('dev-setup: worktree stays canonical', () => {
  ...
  test('passes GAGANFOXWELL_SKIP_GBRAIN_REGEN inline on the nested setup call', () => {
    expect(devSetup).toContain('GAGANFOXWELL_SKIP_GBRAIN_REGEN=1 "$GAGANFOXWELL_LINK/setup"');
  });

  test('never exports GAGANFOXWELL_SKIP_GBRAIN_REGEN (would leak into other setup paths)', () => {
    expect(devSetup).not.toMatch(/export\s+GAGANFOXWELL_SKIP_GBRAIN_REGEN/);
  });

  test('renders the :user variant into an out-dir, not in place', () => {
    expect(devSetup).toContain('--out-dir');
    expect(devSetup).toContain('.claude/gaganfoxwell-rendered');
  });

  test('gates the render on gaganfoxwell-gbrain-detect --is-ok', () => {
    expect(devSetup).toContain('--is-ok');
  });
});

describe('setup: honors GAGANFOXWELL_SKIP_GBRAIN_REGEN', () => {
  const setup = read('setup');

  test('skips the in-place :user regen when the guard is set', () => {
    expect(setup).toContain('${GAGANFOXWELL_SKIP_GBRAIN_REGEN:-}');
    // The guard must wrap the in-place render, not the detection persist.
    const idx = setup.indexOf('GAGANFOXWELL_SKIP_GBRAIN_REGEN');
...
describe('dev-teardown: removes the untracked render', () => {
  ...
  test('rm -rf the gaganfoxwell-rendered dir', () => {
    expect(teardown).toContain('gaganfoxwell-rendered');
    expect(teardown).toMatch(/rm -rf .*RENDER_DIR/);
  });
});

describe('.gitignore: render dir is declared untracked', () => {
  test('.claude/gaganfoxwell-rendered/ is ignored', () => {
    expect(read('.gitignore')).toContain('.claude/gaganfoxwell-rendered/');
  });
});
...
describe('dev-skill: refreshes the render on template change', () => {
  ...
  test('re-renders the :user variant into the workspace render dir', () => {
    expect(devSkill).toContain('gaganfoxwell-rendered');
