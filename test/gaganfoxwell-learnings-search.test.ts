...
const BIN = path.join(ROOT, 'bin', 'gaganfoxwell-learnings-search');

const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-search-test-'));
const tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-search-cwd-'));
// gaganfoxwell-slug derives slug from git remote (none here) → falls back to basename of cwd.
...
describe('gaganfoxwell-learnings-search token-OR query semantics', () => {
...
describe('gaganfoxwell-learnings-search cross-project trust gating', () => {
