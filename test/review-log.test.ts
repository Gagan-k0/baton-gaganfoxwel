...
function run(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  const execOpts: ExecSyncOptionsWithStringEncoding = {
    cwd: ROOT,
    env: { ...process.env, GAGANFOXWELL_HOME: tmpDir },
    encoding: 'utf-8',
    timeout: 10000,
  };
  try {
    const stdout = execSync(`${BIN}/gaganfoxwell-review-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-revlog-'));
  // gaganfoxwell-review-log uses gaganfoxwell-slug which needs a git repo — create the projects dir
...
describe('gaganfoxwell-review-log', () => {
  ...
  test('append still succeeds outside a git repo (binding fields omitted)', () => {
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-nongit-'));
    try {
      const execOpts: ExecSyncOptionsWithStringEncoding = {
        cwd: nonGit,
        env: { ...process.env, GAGANFOXWELL_HOME: tmpDir },
      ...
      execSync(`${BIN}/gaganfoxwell-review-log '{"skill":"review","status":"clean"}'`, execOpts);
...
describe('gaganfoxwell-wtree', () => {
  function withScratchRepo(fn: (repoDir: string, wtree: () => string) => void) {
    const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-wtree-'));
    try {
      ...
      git('commit -q -m init');
      const wtree = () => execSync(`${BIN}/gaganfoxwell-wtree`, { cwd: repoDir, encoding: 'utf-8', timeout: 10000 }).trim();
      fn(repoDir, wtree);
    } finally {
  ...
  test('exits non-zero outside a git repo', () => {
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-wtree-nongit-'));
    try {
      expect(() => execSync(`${BIN}/gaganfoxwell-wtree`, { cwd: nonGit, timeout: 10000, stdio: 'pipe' })).toThrow();
    } finally {
      fs.rmSync(nonGit, { recursive: true, force: true });
    }
  });
...
describe('gaganfoxwell-review-read', () => {
  test('emits ---WTREE---, ---TREE--- and ---DIRTY--- sections', () => {
    const out = execSync(`${BIN}/gaganfoxwell-review-read`, {
      cwd: ROOT,
      env: { ...process.env, GAGANFOXWELL_HOME: tmpDir },
      encoding: 'utf-8',
      timeout: 10000,
