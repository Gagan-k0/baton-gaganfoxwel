/**
 * gaganfoxwell-upgrade/migrations/v1.17.0.0.sh — migration script unit tests.
 *
 * The migration runs on /gaganfoxwell-upgrade for users with brain-sync configured but
 * never wired up to gbrain. It has 4 skip conditions and one happy path.
 *
 * Strategy: stub gaganfoxwell-config and gaganfoxwell-gbrain-source-wireup binaries on PATH
...
const MIGRATION = path.join(ROOT, 'gaganfoxwell-upgrade', 'migrations', 'v1.17.0.0.sh');
...
function makeFakeStubs(opts: {
  configValue?: string; // value gaganfoxwell-config returns for gbrain_sync_mode
  configMissing?: boolean; // gaganfoxwell-config binary itself missing (test edge)
  wireupMissing?: boolean; // wireup binary missing
  wireupExitCode?: number;
}) {
  const skillsBin = path.join(tmpHome, '.claude', 'skills', 'gaganfoxwell', 'bin');
  fs.mkdirSync(skillsBin, { recursive: true });

  if (!opts.configMissing) {
    const cfg = `#!/bin/bash
echo "gaganfoxwell-config $@" >> "${stubLog}"
[ "$1" = "get" ] && [ "$2" = "gbrain_sync_mode" ] && echo "${opts.configValue ?? ''}"
exit 0
`;
    fs.writeFileSync(path.join(skillsBin, 'gaganfoxwell-config'), cfg, { mode: 0o755 });
  }

  if (!opts.wireupMissing) {
    const wu = `#!/bin/bash
echo "gaganfoxwell-gbrain-source-wireup $@" >> "${stubLog}"
exit ${opts.wireupExitCode ?? 0}
`;
    fs.writeFileSync(path.join(skillsBin, 'gaganfoxwell-gbrain-source-wireup'), wu, { mode: 0o755 });
  }
}

function makeBrainGitRepo() {
  const gaganfoxwellHome = path.join(tmpHome, '.gaganfoxwell');
  fs.mkdirSync(path.join(gaganfoxwellHome, '.git'), { recursive: true });
}
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-migration-test-'));
...
describe('migrations/v1.17.0.0.sh', () => {
  ...
  test('gbrain_sync_mode = off: exit 0 silently (no helper invoked)', () => {
    ...
    expect(calls.some((c) => c.startsWith('gaganfoxwell-gbrain-source-wireup'))).toBe(false);
  });
  ...
  test('no ~/.gaganfoxwell/.git: exit 0 silently (no brain-sync configured)', () => {
    makeFakeStubs({ configValue: 'full' });
    // Do NOT call makeBrainGitRepo() — no .gaganfoxwell/.git directory exists
    const r = run();
    expect(r.status).toBe(0);
    const calls = stubCalls();
    expect(calls.some((c) => c.startsWith('gaganfoxwell-gbrain-source-wireup'))).toBe(false);
  ...
  test('happy path: invokes the helper', () => {
    ...
    expect(r.status).toBe(0);
    const calls = stubCalls();
    expect(calls.some((c) => c.startsWith('gaganfoxwell-gbrain-source-wireup'))).toBe(true);
    // Note: migration invokes WITHOUT --strict (benign-skip semantics for batch upgrade)
    const helperCall = calls.find((c) => c.startsWith('gaganfoxwell-gbrain-source-wireup'));
