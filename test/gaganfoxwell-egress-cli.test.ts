/**
 * gaganfoxwell-egress CLI — list | verify | grants smoke tests. Free tier.
 *
 * Spawns the real bin against a temp GAGANFOXWELL_HOME: list filters, verify
...
const BIN = path.join(ROOT, 'bin', 'gaganfoxwell-egress');

let home: string;

beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-egress-cli-'));
});
...
describe('gaganfoxwell-egress list', () => {
...
describe('gaganfoxwell-egress verify', () => {
  ...
  test('prints the sizeWarning when the ledger exceeds the threshold', () => {
    ...
    expect(parsed.sizeWarning).toContain('gaganfoxwell-egress list');
    const human = run(['verify']);
    expect(human.stdout).toContain('egress ledger is large');
  });
});

describe('gaganfoxwell-egress grants', () => {
  ...
  test('--json flips granted=true when telemetry and sync mode are enabled', () => {
    const config = spawnSync(path.join(ROOT, 'bin', 'gaganfoxwell-config'), ['set', 'telemetry', 'community'], {
      encoding: 'utf-8',
      env: { ...process.env, GAGANFOXWELL_HOME: home },
    });
    expect(config.status).toBe(0);
    spawnSync(path.join(ROOT, 'bin', 'gaganfoxwell-config'), ['set', 'artifacts_sync_mode', 'full'], {
      encoding: 'utf-8',
      env: { ...process.env, GAGANFOXWELL_HOME: home },
    });
...
describe('gaganfoxwell-egress usage', () => {
