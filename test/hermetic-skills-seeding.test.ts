 ...
 * 3. connect-chrome (dir symlink) collapses into open-gaganfoxwell-browser — no
...
describe('hermeticSkillsConfigDir', () => {
  ...
  test('connect-chrome collapses into a single open-gaganfoxwell-browser entry', () => {
    const seeded = fs.readdirSync(skillsDir);
    expect(seeded.filter((n) => n === 'open-gaganfoxwell-browser')).toHaveLength(1);
    expect(seeded).not.toContain('connect-chrome');
  });

  test('root router registered as _gaganfoxwell-command pointing at the root SKILL.md', () => {
    const link = path.join(skillsDir, '_gaganfoxwell-command', 'SKILL.md');
