...
function setupWorkdir(suffix: string): { workDir: string; gaganfoxwellHome: string; slug: string } {
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), `skill-e2e-ctx-${suffix}-`));
  const gaganfoxwellHome = path.join(workDir, '.gaganfoxwell-home');
  ...
  for (const script of [
    'gaganfoxwell-timeline-log', 'gaganfoxwell-timeline-read', 'gaganfoxwell-slug',
    'gaganfoxwell-learnings-log', 'gaganfoxwell-learnings-search',
    'gaganfoxwell-update-check', 'gaganfoxwell-config', 'gaganfoxwell-repo-mode',
  ]) {
...
Environment:
- Use GAGANFOXWELL_HOME="${gaganfoxwellHome}" for all gaganfoxwell bin scripts.
...
`);
  ...
  return { workDir, gaganfoxwellHome, slug };
}

// Helper: seed a saved-context file into the storage dir.
function seedSave(gaganfoxwellHome: string, slug: string, filename: string, frontmatter: Record<string, string>, body: string) {
  const dir = path.join(gaganfoxwellHome, 'projects', slug, 'checkpoints');
...
describeIfSelected('Context Skills E2E (live-fire)', [
  ...
  testConcurrentIfSelected('context-save-routing', async () => {
    const { workDir, gaganfoxwellHome, slug } = setupWorkdir('routing');

    // Prompt pattern: the slash command + explicit "invoke via Skill tool"
    // instruction. The GAGANFOXWELL_HOME / ./bin bash setup that used to be in
    ...
    const result = await runSkillTest({
      ...
      env: { GAGANFOXWELL_HOME: gaganfoxwellHome },
      maxTurns: 12,
    ...
    const checkpointDir = path.join(gaganfoxwellHome, 'projects', slug, 'checkpoints');
  ...
  testConcurrentIfSelected('context-save-then-restore-roundtrip', async () => {
    const { workDir, gaganfoxwellHome, slug } = setupWorkdir('roundtrip');
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('context-restore-fragment-match', async () => {
    const { workDir, gaganfoxwellHome, slug } = setupWorkdir('fragment');

    // Seed three saves with distinct titles.
    seedSave(gaganfoxwellHome, slug, '20260101-120000-alpha-feature.md',
    ...
    seedSave(gaganfoxwellHome, slug, '20260202-120000-middle-payments.md',
    ...
    seedSave(gaganfoxwellHome, slug, '20260303-120000-omega-release.md',
  ...
  testConcurrentIfSelected('context-restore-empty-state', async () => {
    const { workDir, gaganfoxwellHome, slug } = setupWorkdir('empty');
  ...
  testConcurrentIfSelected('context-restore-list-delegates', async () => {
    const { workDir, gaganfoxwellHome, slug } = setupWorkdir('delegates');
    seedSave(gaganfoxwellHome, slug, '20260101-120000-seed.md',
  ...
  testConcurrentIfSelected('context-restore-legacy-compat', async () => {
    const { workDir, gaganfoxwellHome, slug } = setupWorkdir('legacy');
    ...
    // exactly this reason.
    seedSave(gaganfoxwellHome, slug, '20260301-120000-legacy-pre-rename-work.md',
      {
        status: 'in-progress',
  ...
  testConcurrentIfSelected('context-save-list-current-branch', async () => {
    const { workDir, gaganfoxwellHome, slug } = setupWorkdir('list-current');

    // Seed 3 files on 3 different branches. Current branch is "main".
    seedSave(gaganfoxwellHome, slug, '20260101-120000-main-work.md',
      { status: 'in-progress', branch: 'main', timestamp: '2026-01-01T12:00:00Z' },
      '## Working on: main work LISTCURR_MAIN_TOKEN\n');
    seedSave(gaganfoxwellHome, slug, '20260202-120000-feat-alpha.md',
      { status: 'in-progress', branch: 'feat/alpha', timestamp: '2026-02-02T12:00:00Z' },
      '## Working on: alpha LISTCURR_ALPHA_TOKEN\n');
    seedSave(gaganfoxwellHome, slug, '20260303-120000-feat-beta.md',
  ...
  testConcurrentIfSelected('context-save-list-all-branches', async () => {
    const { workDir, gaganfoxwellHome, slug } = setupWorkdir('list-all');
