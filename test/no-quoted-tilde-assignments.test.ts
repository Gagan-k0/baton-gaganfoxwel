...
describe('quoted-tilde assignments (#1656/#1715)', () => {
  ...
  test('detector self-check: the pre-fix shape is caught', () => {
    expect(QUOTED_TILDE_ASSIGN.test('_BRAIN_SYNC_BIN="~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-sync"')).toBe(true);
    expect(QUOTED_TILDE_ASSIGN.test('_BIN="$HOME/.claude/skills/gaganfoxwell/bin/x"')).toBe(false);
  });
});
