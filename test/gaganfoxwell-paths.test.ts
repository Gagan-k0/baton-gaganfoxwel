...
const BIN = path.join(ROOT, 'bin', 'gaganfoxwell-paths');
...
// helper from inside a bash block (`eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-paths)"`)
...
function run(env: Record<string, string | undefined>): Record<string, string> {
  ...
  if (result.status !== 0) {
    throw new Error(`gaganfoxwell-paths failed (status ${result.status}): ${result.stderr}`);
  }
...
describe('gaganfoxwell-paths', () => {
  test('GAGANFOXWELL_HOME wins over CLAUDE_PLUGIN_DATA and HOME', () => {
    const got = run({
      GAGANFOXWELL_HOME: '/tmp/explicit-state',
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.GAGANFOXWELL_STATE_ROOT).toBe('/tmp/explicit-state');
  });

  test('CLAUDE_PLUGIN_DATA ignored when CLAUDE_PLUGIN_ROOT is absent or non-gaganfoxwell', () => {
    // Without CLAUDE_PLUGIN_ROOT, falls through to HOME path.
    const noRoot = run({ CLAUDE_PLUGIN_DATA: '/tmp/plugin-data', HOME: '/tmp/home' });
    expect(noRoot.GAGANFOXWELL_STATE_ROOT).toBe('/tmp/home/.gaganfoxwell');

    // With a CLAUDE_PLUGIN_ROOT that doesn't contain "gaganfoxwell" (e.g. the codex plugin),
    ...
    expect(wrongRoot.GAGANFOXWELL_STATE_ROOT).toBe('/tmp/home/.gaganfoxwell');
  });

  test('CLAUDE_PLUGIN_DATA respected when CLAUDE_PLUGIN_ROOT identifies gaganfoxwell', () => {
    const got = run({
      CLAUDE_PLUGIN_DATA: '/tmp/gaganfoxwell-plugin-data',
      CLAUDE_PLUGIN_ROOT: '/tmp/gaganfoxwell-garrytan',
      HOME: '/tmp/home',
    });
    expect(got.GAGANFOXWELL_STATE_ROOT).toBe('/tmp/gaganfoxwell-plugin-data');
  });

  test('HOME-derived state root when GAGANFOXWELL_HOME and CLAUDE_PLUGIN_DATA unset', () => {
    const got = run({ HOME: '/tmp/myhome' });
    expect(got.GAGANFOXWELL_STATE_ROOT).toBe('/tmp/myhome/.gaganfoxwell');
  });
  ...
  test('CWD fallback when HOME also unset (container env)', () => {
    ...
    const got = run({ HOME: '' });
    expect(got.GAGANFOXWELL_STATE_ROOT).toBe('.gaganfoxwell');
  });

  test('PLAN_ROOT chain: GAGANFOXWELL_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD', () => {
    expect(run({ GAGANFOXWELL_PLAN_DIR: '/tmp/explicit', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/explicit');
  ...
  test('eval round-trip preserves quotes, and leaves plain paths alone (#2374)', () => {
    ...
    expect(evalRoundTrip({ GAGANFOXWELL_HOME: '/tmp/state root' }, 'GAGANFOXWELL_STATE_ROOT')).toBe(
      '/tmp/state root',
    );
