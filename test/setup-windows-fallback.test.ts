...
describe('setup: _link_or_copy invariant (D7)', () => {
  ...
  test('SessionStart HOOK_CMD is prefixed with bash on Windows (D7-session-hook)', () => {
    ...
    // via _hook_command_path (never $SOURCE_GAGANFOXWELL_DIR — ephemeral trees were
    ...
    expect(hookSection).toContain('_hook_command_path bin/gaganfoxwell-session-update');
  });
});
...
describe.skipIf(process.platform === 'win32')('setup: _link_or_copy helper — behavior matrix', () => {
  ...
  function runHelper(
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-helper-'));
    try {
