/**
 * gaganfoxwell-verify-gate — Stop-hook enforcement tier.
 *
 * Pins the behaviours the gate exists for:
 *   trust    — a declared command NEVER runs until the user records it via
 *              `gaganfoxwell-verify-gate --trust` (per-repo command trust store).
...
const GATE = path.join(ROOT, 'bin', 'gaganfoxwell-verify-gate');

let dir: string;
let gaganfoxwellHome: string;

beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-verify-gate-'));
  gaganfoxwellHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-verify-gate-home-'));
});

afterEach(() => {
  fs.rmSync(dir, { recursive: true, force: true });
  fs.rmSync(gaganfoxwellHome, { recursive: true, force: true });
});

/** Declare a verification command in the project's CLAUDE.md (comment form). */
function declareCheck(command: string): void {
  fs.writeFileSync(path.join(dir, 'CLAUDE.md'), `# Fixture\n\n<!-- gaganfoxwell:verify: ${command} -->\n`);
}
...
function gateEnv(projectDirEnv: boolean): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...process.env, GAGANFOXWELL_HOME: gaganfoxwellHome };
...
describe('gaganfoxwell-verify-gate trust store', () => {
  ...
  test('non-comment declaration form (gaganfoxwell:verify: cmd without <!-- -->) is honored', () => {
    fs.writeFileSync(path.join(dir, 'CLAUDE.md'), '# Fixture\n\ngaganfoxwell:verify: ./check.sh\n');
    check(0, 'all good');
  ...
  test('the trust store file is created 0600 under GAGANFOXWELL_HOME', () => {
    declareCheck('./check.sh');
    check(0, 'all good');
    expect(trust().code).toBe(0);

    const store = path.join(gaganfoxwellHome, 'verify-gate-trust');
...
describe('gaganfoxwell-verify-gate', () => {
  ...
  test('fails open when CLAUDE.md declares no check', () => {
    ...
    expect(r.code).toBe(0);
    expect(r.stdout).toContain("declares no 'gaganfoxwell:verify:' command");
  });
  ...
  test('never invents a command: an empty declaration fails open', () => {
    fs.writeFileSync(path.join(dir, 'CLAUDE.md'), '<!-- gaganfoxwell:verify: -->\n');
...
describe('gaganfoxwell-verify-gate re-entry enforcement (no one-shot bypass)', () => {
...
describe('gaganfoxwell-verify-gate trust-grant audit trail', () => {
  test('every --trust grant appends a JSON audit line (right sha256, 0600, verbatim cmd)', () => {
    ...
    const log = path.join(gaganfoxwellHome, 'security', 'verify-gate-trust-grants.jsonl');
...
describe('opt-in contract (adapted from the fork: NOT registered by default)', () => {
  ...
  test('./setup does NOT register the gate — a Stop hook running the verify command after every turn is opt-in', () => {
    ...
    for (const line of mentions) {
      const t = line.trim();
      const allowed = t.startsWith('#') || t.includes('GAGANFOXWELL_SWEEP_EXCLUDE_SOURCES="verify-gate"');
  ...
  test('the bin documents its own registration and removal commands', () => {
    ...
    expect(gate).toContain('gaganfoxwell:verify:');
  });
});
