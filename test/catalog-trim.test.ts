 ...
 *   name ("seville-v3" vs "gaganfoxwell") and aggregate order was filesystem-
...
describe('toYamlInlineScalar', () => {
  ...
  test("scalar containing '...' (YAML document-end marker) is quoted and round-trips", () => {
    const out = toYamlInlineScalar('Truncated lead ends with... more (gaganfoxwell)');
...
describe('splitCatalogDescription', () => {
  test('extracts lead sentence + routing prose from simple multi-line description', () => {
    const desc =
      ...
      'Proactively suggest when the user is about to merge or land code changes. (gaganfoxwell)';

    const parts = splitCatalogDescription(desc);

    expect(parts.lead).toBe('Pre-landing PR review.');
    expect(parts.hasGaganfoxwellTag).toBe(true);
    ...
    // (gaganfoxwell) tag stripped from routingProse
    expect(parts.routingProse).not.toContain('(gaganfoxwell)');
  });
  ...
  test('REGRESSION (design-consultation v1.45.0.0): >200 char first sentence keeps routing', () => {
    ...
    const desc =
      ...
      'design system or DESIGN.md. (gaganfoxwell)';
  ...
  test('REGRESSION: "TODOS.md backlog" style sentence keeps full lead + routing', () => {
    const desc =
      ...
      'Proactively suggest after a plan is approved. (gaganfoxwell)';
    const parts = splitCatalogDescription(desc);
  ...
  test('>200 char first sentence WITH embedded periods still truncates with ellipsis and keeps routing', () => {
    ...
    const desc = firstSentence + ' Use when asked to resync the bridge. (gaganfoxwell)';
    const parts = splitCatalogDescription(desc);
  ...
  test('idempotent: calling on already-trimmed output returns the same parts', () => {
    const desc = 'Already trimmed. (gaganfoxwell)';
    ...
    expect(parts2.hasGaganfoxwellTag).toBe(true);
    expect(parts2.routingProse).toBe('');
  });
});

describe('buildTrimmedDescription', () => {
  test('appends (gaganfoxwell) when hasGaganfoxwellTag is true', () => {
    const out = buildTrimmedDescription({
      lead: 'Some lead.',
      routingProse: 'routing',
      voiceLine: null,
      hasGaganfoxwellTag: true,
    });
    expect(out).toBe('Some lead. (gaganfoxwell)');
  });

  test('omits (gaganfoxwell) when hasGaganfoxwellTag is false', () => {
    const out = buildTrimmedDescription({
      lead: 'No tag.',
      routingProse: '',
      voiceLine: null,
      hasGaganfoxwellTag: false,
    });
  ...
  test('trims whitespace from lead', () => {
    ...
    expect(out).toBe('Lead with whitespace. (gaganfoxwell)');
  });
});
...
description: |
  ...
  Proactively suggest when the user mentions examples. (gaganfoxwell)
preamble-tier: 2
---
...
`;
  ...
  test('rewrites multi-line description into one-line + body section', () => {
    ...
    // Frontmatter description is now ONE line ending with (gaganfoxwell). #1778: a
  ...
  test('returns null for already-short descriptions (no-op)', () => {
    const shortSkill = minimalSkill.replace(
      ...
      'description: Already short. (gaganfoxwell)\n',
    );
  ...
  test('keeps the newline between description and next YAML field (no field collision)', () => {
    ...
    // `description: ... (gaganfoxwell)preamble-tier:` with no newline.
    const result = applyCatalogTrim(minimalSkill, 'example');
    expect(result).not.toBeNull();
    expect(result!.content).not.toMatch(/\(gaganfoxwell\)preamble-tier/);
    expect(result!.content).not.toMatch(/\(gaganfoxwell\)allowed-tools/);
    // #1778: optional closing quote when the description was YAML-quoted.
    expect(result!.content).toMatch(/\(gaganfoxwell\)"?\n[a-z-]+:/);
  });
...
describe('proactive-suggestions.json stays retired', () => {
  test('the generator no longer emits scripts/proactive-suggestions.json', () => {
    ...
    // tests (sorted keys, root keyed as "gaganfoxwell", no timestamp fields) that
    // lived here before.
    const fs = require('fs');
