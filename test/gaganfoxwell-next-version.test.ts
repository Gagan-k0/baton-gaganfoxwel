// Pure-function tests for bin/gaganfoxwell-next-version.
...
import {
  ...
  resolveVersionPath,
  fetchGitClaimed,
} from "../bin/gaganfoxwell-next-version";
...
describe("resolveVersionPath (monorepo VERSION-path support)", () => {
  test("CLI flag wins over everything", () => {
    ...
    try {
      mkdirSync(join(dir, ".gaganfoxwell"));
      writeFileSync(join(dir, ".gaganfoxwell", "version-path"), "config/VERSION\n");
  ...
  test(".gaganfoxwell/version-path config is picked up", () => {
    const dir = mkdtempSync(join(tmpdir(), "nextver-"));
    try {
      mkdirSync(join(dir, ".gaganfoxwell"));
      writeFileSync(join(dir, ".gaganfoxwell", "version-path"), "Tinas Second Brain/health-tracker/VERSION\n");
  ...
  test("trims whitespace and ignores blank lines after the first", () => {
    ...
    try {
      mkdirSync(join(dir, ".gaganfoxwell"));
      writeFileSync(join(dir, ".gaganfoxwell", "version-path"), "  apps/web/VERSION  \n\n# comment-ish line\n");
  ...
  test("empty config file falls back to default VERSION", () => {
    ...
    try {
      mkdirSync(join(dir, ".gaganfoxwell"));
      writeFileSync(join(dir, ".gaganfoxwell", "version-path"), "\n");
  ...
  test("empty override string falls back to config/default", () => {
    ...
    try {
      mkdirSync(join(dir, ".gaganfoxwell"));
      writeFileSync(join(dir, ".gaganfoxwell", "version-path"), "subproj/VERSION\n");
...
describe("default-base detection (no --base)", () => {
  const SCRIPT = join(import.meta.dir, "..", "bin", "gaganfoxwell-next-version");
...
describe("offline output contract (what /ship branches on, #2545)", () => {
  ...
  const NEXTVER = join(import.meta.dir, "..", "bin", "gaganfoxwell-next-version");
