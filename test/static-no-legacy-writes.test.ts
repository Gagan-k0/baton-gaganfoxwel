 ...
 * append directly to ~/.gaganfoxwell/builder-profile.jsonl. All session writes
 * must go through `gaganfoxwell-developer-profile --log-session`. The legacy
 * file is now read-only — populated only by the pre-existing migration
 * and reconcile paths in bin/gaganfoxwell-developer-profile.
 *
...
const ALLOWED_FILES = new Set<string>([
  ...
  'bin/gaganfoxwell-developer-profile',
  // The legacy-shim binary that delegates reads.
  'bin/gaganfoxwell-builder-profile',
...
describe('#1671 invariant: no production code writes to builder-profile.jsonl', () => {
    ...
    if (offending.length > 0) {
      ...
      throw new Error(
        `Found production writes to builder-profile.jsonl outside the allowlist.\n` +
          ...
          `Use \`gaganfoxwell-developer-profile --log-session\` instead.\n${msg}`,
      );
    }
  ...
  test('office-hours/SKILL.md uses --log-session, not raw echo append', () => {
    ...
    expect(skill).toContain('gaganfoxwell-developer-profile --log-session');
  ...
  test('office-hours/SKILL.md.tmpl uses --log-session, not raw echo append', () => {
    ...
    expect(tmpl).toContain('gaganfoxwell-developer-profile --log-session');
