 ...
 * PR after main merged GAGANFOXWELL_PLAN_MODE + /spec, pushing the v1.44.1
 ...
 *
 * Override:
 * - GAGANFOXWELL_SIZE_BUDGET_RATIO=<n> changes the per-skill regression ratio.
 ...
 *   doesn't offset).
 * - GAGANFOXWELL_SIZE_BUDGET_OVERRIDE_REASON="text" allows a regression to
 *   pass and logs the reason to ~/.gaganfoxwell/analytics/spend-overrides.jsonl
...
const RATIO = Number(process.env.GAGANFOXWELL_SIZE_BUDGET_RATIO) || DEFAULT_RATIO;
...
describe('SKILL.md size budget regression (gate, free)', () => {
  ...
  test('no skill exceeds v1.47.0.0 baseline size × ratio', () => {
    ...
    const overrideReason = process.env.GAGANFOXWELL_SIZE_BUDGET_OVERRIDE_REASON?.trim();
    if (overrideReason) {
      logBudgetOverride({
    ...
    throw new Error(
      ...
      `Override: set GAGANFOXWELL_SIZE_BUDGET_OVERRIDE_REASON="why this is OK" to allow and audit-log.`,
    );
  });
  ...
  test('total corpus byte count does not regress past baseline × ratio', () => {
    ...
    throw new Error(
      ...
      `Override: set GAGANFOXWELL_SIZE_BUDGET_OVERRIDE_REASON to allow.`,
    );
  });
  ...
  test('no skill shrinks past 80% of v1.47.0.0 baseline (catches accidental body strip)', () => {
    ...
    // - scrape/diagram/open-gaganfoxwell-browser/landing-report/pair-agent/skillify:
    ...
    const INTENTIONAL_SHRINKS = new Set<string>([
      'spec',
      'scrape', 'diagram', 'open-gaganfoxwell-browser',
    ...
    throw new Error(
      ...
      `GAGANFOXWELL_SIZE_BUDGET_OVERRIDE_REASON="why" allows + audit-logs.`,
    );
  });
  ...
  test('catalog token estimate stays compressed (v1.45 target ≤ 7000)', () => {
    ...
    throw new Error(
      ...
      `T4 catalog trim should keep this under control. Override: set GAGANFOXWELL_SIZE_BUDGET_OVERRIDE_REASON to allow.`,
    );
  });
});
