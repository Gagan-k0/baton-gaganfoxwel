...
beforeEach(() => {
  ...
  tmp = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-slug-")));
  // An ambient GAGANFOXWELL_PROJECT_SLUG (leaked from an operator shell or a sibling
  // test in a shared-process shard) would override every derivation under test.
  savedEnvSlug = process.env.GAGANFOXWELL_PROJECT_SLUG;
  delete process.env.GAGANFOXWELL_PROJECT_SLUG;
});
afterEach(() => {
  try { fs.rmSync(tmp, { recursive: true, force: true }); } catch {}
  if (savedEnvSlug === undefined) delete process.env.GAGANFOXWELL_PROJECT_SLUG;
  else process.env.GAGANFOXWELL_PROJECT_SLUG = savedEnvSlug;
});

/**
 * Windows cannot exec bin/gaganfoxwell-slug -- a `#!/usr/bin/env bash` script with no file
 ...
 * ~/.gaganfoxwell/projects/unknown/ bucket, while the bash-side Context Recovery preamble
 ...
 */
describe("native slug fallback mirrors bin/gaganfoxwell-slug", () => {
  test("toMsysPath reproduces the git-bash cache key", () => {
    // gaganfoxwell-slug does: CACHE_KEY=$(printf '%s' "$(pwd)" | tr '/' '_')
  ...
  test("the resolved slug is cached back, as the shell script does", () => {
    ...
    // no trailing newline: gaganfoxwell-slug writes with printf '%s'
 ...
 * root as bin/gaganfoxwell-slug's `_outermost_project_root` (see
 * test/gaganfoxwell-slug-cwd-walk-up.test.ts for the bash-side pins). Before this
 ...
 * (separate GAGANFOXWELL_HOMEs so neither reads the other's cache) and pinned to the
 ...
 */
describe("walk-up parity with bin/gaganfoxwell-slug (outermost project root)", () => {
  const SCRIPT = path.join(ROOT, "bin", "gaganfoxwell-slug");
  ...
  function bashSlug(cwd: string, extraEnv: Record<string, string> = {}): string {
    const env: Record<string, string | undefined> = {
      ...
      GAGANFOXWELL_HOME: path.join(tmp, "bash-home", ".gaganfoxwell"),
    };
    delete env.GAGANFOXWELL_PROJECT_SLUG; // only set when a scenario passes it explicitly
  ...
  test("nested repo: outermost .git wins — nested/vendored repos don't split stores", () => {
    ...
    // remote. bin/gaganfoxwell-slug resolves the OUTERMOST strong marker instead.
  ...
  test("LIVE BUG SHAPE: a stray empty .git ancestor no longer degrades the slug (remote-first)", () => {
    ...
    // every repo under it into one shared ~/.gaganfoxwell/projects/<basename>/.
    ...
    spawnSync("git", ["-C", repo, "remote", "add", "origin", "https://github.com/garrytan/gaganfoxwell"]);
    expectBoth(repo, "garrytan-gaganfoxwell");
  ...
  test("nested repos under a stray marker: a no-remote outer cannot shadow an inner remote", () => {
    ...
    // repo carries identity (parity with bin/gaganfoxwell-slug's _outermost_remote_repo).
  ...
  test("degraded-ancestor cache self-heals: the pre-remote-first cached value is rewritten", () => {
    ...
    expect(slugFromEnvironment(nativeHome(), repo)).toBe("garrytan-gaganfoxwell");
    // The cache file itself must have been overwritten (self-healing).
    expect(fs.readFileSync(cacheFile, "utf-8")).toBe("garrytan-gaganfoxwell");
  });
  ...
  test("package.json wrapper root (no .git): sticky basename slug is PRESERVED — heal is stray-repo-shape only", () => {
    ...
    if (HAS_BASH) {
      const bashCacheDir = path.join(tmp, "bash-home", ".gaganfoxwell", "slug-cache");
  ...
  test('hostile origin `url = ..` never becomes a dot slug — basename fallback (dot-only guard)', () => {
    ...
    // ".." — path traversal one level above ~/.gaganfoxwell/projects/. Both
  ...
  test("GAGANFOXWELL_PROJECT_SLUG env override beats every other resolution path, never cached", () => {
    const projectRoot = path.join(tmp, "loadout");
    const siteSubdir = path.join(projectRoot, "site");
    fs.mkdirSync(path.join(projectRoot, ".git"), { recursive: true });
    fs.mkdirSync(siteSubdir, { recursive: true });

    process.env.GAGANFOXWELL_PROJECT_SLUG = "custom-override";
    try {
      expect(slugFromEnvironment(nativeHome(), siteSubdir)).toBe("custom-override");
      if (HAS_BASH) {
        expect(bashSlug(siteSubdir, { GAGANFOXWELL_PROJECT_SLUG: "custom-override" })).toBe("custom-override");
      }
    } finally {
      delete process.env.GAGANFOXWELL_PROJECT_SLUG;
    }
