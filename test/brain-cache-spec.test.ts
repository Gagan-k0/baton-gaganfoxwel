...
describe('brain-cache-spec internal consistency', () => {
  ...
  test('salience allowlist has sane defaults (no personal/family/therapy)', () => {
    ...
    // Must contain at least projects/ + gaganfoxwell/ (work-flow surfaces)
    expect(SALIENCE_DEFAULT_ALLOWLIST).toContain('projects/');
    expect(SALIENCE_DEFAULT_ALLOWLIST).toContain('gaganfoxwell/');
  });
  ...
  test('schema pack identity is stable strings', () => {
    expect(GAGANFOXWELL_SCHEMA_PACK_NAME).toBe('gaganfoxwell-core');
    expect(GAGANFOXWELL_SCHEMA_PACK_VERSION).toMatch(/^\d+\.\d+\.\d+$/);
  });
...
describe('brain-cache MCP scope precedence (C15 pin)', () => {
  ...
  const cache = () => import('../bin/gaganfoxwell-brain-cache');
