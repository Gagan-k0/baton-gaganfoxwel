...
// careful/freeze skills (#1871). The old standalone `gaganfoxwell-freeze` sibling
// fallback was part of the never-resolving path — prefix installs keep the
// payload at ~/.claude/skills/gaganfoxwell/, so the $HOME anchor covers them.
describe('investigate freeze path resolution', () => {
  for (const rel of FILES) {
    const content = fs.readFileSync(path.join(ROOT, rel), 'utf-8');

    test(`${rel} hook resolves check-freeze via the $HOME anchor`, () => {
      expect(content).toContain('S="$HOME/.claude/skills/gaganfoxwell/freeze/bin/check-freeze.sh"');
    ...
    test(`${rel} scope lock availability probe uses the $HOME anchor`, () => {
      expect(content).toContain('_FREEZE_SCRIPT="$HOME/.claude/skills/gaganfoxwell/freeze/bin/check-freeze.sh"');
