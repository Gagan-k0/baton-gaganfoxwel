 ...
 *
 * Covers the `get --batch` verb of bin/gaganfoxwell-gbrain-repo-policy (bash level:
 ...
 *
 * Each test uses a temp GAGANFOXWELL_HOME so nothing leaks into the user's real
 * ~/.gaganfoxwell. The client is exercised against the REAL bash script — the
...
import { canonicalizeRemote } from "../lib/gaganfoxwell-memory-helpers";

const ROOT = path.resolve(import.meta.dir, "..");
const BIN = path.join(ROOT, "bin", "gaganfoxwell-gbrain-repo-policy");

let tmpHome: string;

function env(): NodeJS.ProcessEnv {
  return { ...process.env, GAGANFOXWELL_HOME: tmpHome };
}
...
describe("bin/gaganfoxwell-gbrain-repo-policy get --batch (bash level)", () => {
...
//
// bin/gaganfoxwell-memory-ingest.ts produces page.git_remote via canonicalizeRemote
// (lib/gaganfoxwell-memory-helpers) and then looks the policy up through
