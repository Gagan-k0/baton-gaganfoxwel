...
// via a crafted GAGANFOXWELL_HOME so the test is deterministic regardless of whether
...
//   (c) its section refs point at the out-dir, not ~/.claude/skills/gaganfoxwell,
...
describe('gen-skill-docs --out-dir (B2 render isolation)', () => {
  ...
  test('renders :user to out-dir, rewrites section paths, leaves worktree canonical', () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-home-'));
    const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-out-'));
    ...
    try {
      ...
      const res = spawnSync(
        ...
        { cwd: ROOT, encoding: 'utf-8', timeout: 120_000, env: { ...process.env, GAGANFOXWELL_HOME: tmpHome } },
      );
      ...
      expect(skillContent).not.toContain('~/.claude/skills/gaganfoxwell/ship/sections/');

      // (d) bin refs are NOT rewritten — they still resolve to the global install
      expect(skillContent).toContain('~/.claude/skills/gaganfoxwell/bin/');
