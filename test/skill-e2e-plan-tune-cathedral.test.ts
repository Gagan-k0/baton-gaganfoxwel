 ...
 *
 * Each scenario uses GAGANFOXWELL_STATE_ROOT to isolate from the user's real
 * ~/.gaganfoxwell (per cathedral T1 + Codex D16 fix). Cost budget ~$3-4/scenario.
 */
...
function scaffoldFixture(prefix: string): { workDir: string; stateRoot: string; slug: string } {
  ...
  const stateRoot = path.join(workDir, '.gaganfoxwell-state');
  fs.mkdirSync(stateRoot, { recursive: true });

  // git init so gaganfoxwell-slug resolves a deterministic slug.
  ...
  for (const script of [
    ...
    'gaganfoxwell-config',
    'gaganfoxwell-paths',
    'gaganfoxwell-question-log',
    'gaganfoxwell-question-preference',
    'gaganfoxwell-developer-profile',
    'gaganfoxwell-codex-session-import',
    'gaganfoxwell-distill-free-text',
    'gaganfoxwell-distill-apply',
  ]) {
...
describeIfSelected('PlanTune cathedral E2E: hook capture', ['plan-tune-hook-capture'], () => {
  ...
  testConcurrentIfSelected('hook directly invoked → log fills', async () => {
    ...
    const payload = {
      ...
      tool_input: {
        questions: [
          {
            question:
              'D1 — Cathedral E2E capture <gaganfoxwell-qid:ship-test-failure-triage>\nRecommendation: A',
    ...
    const res = spawnSync(hookPath, [], {
      env: {
        ...process.env,
        GAGANFOXWELL_STATE_ROOT: fixture.stateRoot,
        GAGANFOXWELL_QUESTION_LOG_NO_DERIVE: '1',
      },
    ...
    const payload = {
      ...
      tool_input: {
        questions: [
          {
            question:
              '<gaganfoxwell-qid:ship-changelog-voice-polish> Polish CHANGELOG entry?',
  ...
  testConcurrentIfSelected('PreToolUse hook surfaces memory nugget on defer', async () => {
    ...
    const payload = {
      ...
      tool_input: {
        questions: [
          {
            question: '<gaganfoxwell-qid:ship-todos-reorganize> Reorganize TODOs?',
            options: ['A) Accept (recommended)', 'B) Skip'],
          },
        ],
      },
...
describeIfSelected('PlanTune cathedral E2E: codex import', ['plan-tune-codex-import'], () => {
    ...
    const lines = [
      ...
      JSON.stringify({
        ...
        payload: {
          type: 'agent_message',
          message:
            'D1 — Cathedral import <gaganfoxwell-qid:plan-eng-review-scope-reduce>\nRecommendation: A\nA) Reduce (recommended)\nB) Keep',
        },
      }),
      JSON.stringify({
        timestamp: new Date().toISOString(),
  ...
  testConcurrentIfSelected('importer extracts events with codex-import-marker source', async () => {
    const bin = path.join(fixture.workDir, 'bin', 'gaganfoxwell-codex-session-import');
...
describeIfSelected('PlanTune cathedral E2E: dream cycle', ['plan-tune-dream-cycle'], () => {
  ...
  testConcurrentIfSelected('apply → re-fire → memory injected via additionalContext', async () => {
    // 1. Apply the proposal via gaganfoxwell-distill-apply.
    const applyBin = path.join(fixture.workDir, 'bin', 'gaganfoxwell-distill-apply');
    const applyRes = spawnSync(applyBin, ['--proposal', '0'], {
      env: { ...process.env, GAGANFOXWELL_STATE_ROOT: fixture.stateRoot },
      encoding: 'utf-8',
      cwd: fixture.workDir,
    });
    ...
    const payload = {
      ...
      tool_input: {
        questions: [
          {
            question:
              '<gaganfoxwell-qid:plan-eng-review-test-gap> Add tests for this gap?',
