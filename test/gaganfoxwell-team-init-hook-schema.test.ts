...
// Regression guard for #2413: gaganfoxwell-team-init required generated a
...
describe('gaganfoxwell-team-init required: PreToolUse hook schema (#2413)', () => {
  beforeEach(() => {
    repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-team-init-repo-'));
    execFileSync('git', ['init', '-q'], { cwd: repoDir });
    execFileSync(TEAM_INIT, ['required'], { cwd: repoDir, encoding: 'utf-8' });

    fakeHomeAbsent = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-team-init-home-absent-'));

    fakeHomePresent = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-team-init-home-present-'));
    fs.mkdirSync(path.join(fakeHomePresent, '.claude', 'skills', 'gaganfoxwell', 'bin'), { recursive: true });
  });
  ...
  function runHook(home: string): { status: number; stdout: string; stderr: string } {
    const hookPath = path.join(repoDir, '.claude', 'hooks', 'check-gaganfoxwell.sh');
    try {
  ...
  test('generates the hook and registers it in settings.json', () => {
    expect(fs.existsSync(path.join(repoDir, '.claude', 'hooks', 'check-gaganfoxwell.sh'))).toBe(true);
    const settings = JSON.parse(
      fs.readFileSync(path.join(repoDir, '.claude', 'settings.json'), 'utf-8'),
    );
    expect(JSON.stringify(settings)).toContain('check-gaganfoxwell.sh');
  });

  test('gaganfoxwell absent: exits 2 (blocking) with schema-valid deny JSON', () => {
    const { status, stdout, stderr } = runHook(fakeHomeAbsent);
    expect(status).toBe(2);
    expect(stderr).toContain('BLOCKED: gaganfoxwell is not installed globally.');
  ...
  test('gaganfoxwell present: exits 0 with an empty (no-opinion) payload', () => {
