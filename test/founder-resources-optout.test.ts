 ...
 * the real bin/gaganfoxwell-config subprocess; (2) the generated section gates on
...
let tmpHome: string;
beforeEach(() => { tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-538-")); });
...
describe("office-hours section gates on the key (#538)", () => {
  ...
  test("the opt-out check precedes any resource content", () => {
    const gate = src.indexOf("gaganfoxwell-config get founder_resources");
  ...
  test("skip is silent and permanent — never means never", () => {
    ...
    expect(src).toContain("gaganfoxwell-config set founder_resources true");
  });
