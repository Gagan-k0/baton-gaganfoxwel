...
describe('setup: ensure_emoji_font static invariants', () => {
  ...
  test('honors the GAGANFOXWELL_SKIP_FONTS escape hatch', () => {
    expect(helper).toContain('GAGANFOXWELL_SKIP_FONTS');
  });
...
describe.skipIf(process.platform === 'win32')('setup: ensure_emoji_font behavior', () => {
  function runHelper(fcMatchOutput: string): {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-emoji-'));
    try {
