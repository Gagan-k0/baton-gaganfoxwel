...
//   2. invoked gaganfoxwell-gbrain-install (Step 4.5 Yes branch)
...
/**
 * Fake gaganfoxwell-gbrain-install so we don't actually clone the gbrain repo +
...
exit 0
`;
  fs.writeFileSync(path.join(binDir, 'gaganfoxwell-gbrain-install'), script, {
    mode: 0o755,
  });
  return callLog;
}
...
describeE2E('/setup-gbrain Path 4 + Step 4.5 Yes → local PGLite for code', () => {
  test('opt-in flow invokes install + gbrain init + remote MCP register', async () => {
    ...
    try {
      ...
      //   - gaganfoxwell-gbrain-install (install step)
