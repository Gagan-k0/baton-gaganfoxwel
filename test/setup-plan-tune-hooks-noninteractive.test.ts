...
const GAGANFOXWELL_CONFIG = path.join(ROOT, 'bin', 'gaganfoxwell-config');
...
describe('setup: plan-tune hooks are non-interactive-safe', () => {
  ...
  test('resolution falls through env then saved config', () => {
    expect(setupSrc).toContain('GAGANFOXWELL_PLAN_TUNE_HOOKS');
...
describe('setup: PT_EXPLICIT provenance (Conductor auto-opt-in respects explicit decisions)', () => {
  ...
  test('config provenance uses gaganfoxwell-config has (env-resolution-safe), never a hardcoded config grep', () => {
    // `gaganfoxwell-config get` returns the default "prompt" for absent keys, so
    // key PRESENCE must come from `has`, which resolves GAGANFOXWELL_STATE_ROOT /
    // GAGANFOXWELL_HOME / GAGANFOXWELL_STATE_DIR the same way `get` does. A hardcoded
    // grep of ~/.gaganfoxwell/config.yaml misclassifies under env overrides.
    expect(setupSrc).toMatch(/"\$GAGANFOXWELL_CONFIG" has plan_tune_hooks/);
    expect(setupSrc).not.toMatch(/grep -q ["']\^plan_tune_hooks:/);
  });
});

describe('gaganfoxwell-config: has subcommand (key-presence provenance)', () => {
  let tmpHome2: string;
  let env2: NodeJS.ProcessEnv;

  beforeAll(() => {
    tmpHome2 = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-cfg-has-'));
    env2 = { ...process.env, GAGANFOXWELL_STATE_ROOT: tmpHome2 };
  });
  ...
  function has(key: string): number {
    try {
      execSync(`${GAGANFOXWELL_CONFIG} has '${key}'`, { encoding: 'utf-8', env: env2 });
      return 0;
    } catch (e: any) {
      return e.status ?? 1;
    }
  ...
  test('absent key exits nonzero even though get returns the default', () => {
    ...
    const got = execSync(`${GAGANFOXWELL_CONFIG} get plan_tune_hooks`, { encoding: 'utf-8', env: env2 }).trim();
  ...
  test('present key exits 0 through the same STATE_DIR resolution as get', () => {
    execSync(`${GAGANFOXWELL_CONFIG} set plan_tune_hooks no`, { encoding: 'utf-8', env: env2 });
    expect(has('plan_tune_hooks')).toBe(0);
    // GAGANFOXWELL_STATE_ROOT was the writer — a hardcoded ~/.gaganfoxwell grep would miss it.
  });
...
describe('gaganfoxwell-config: plan_tune_hooks key', () => {
  // Isolate state: gaganfoxwell-config reads $GAGANFOXWELL_HOME/config.yaml. Point it at a
  // fresh temp dir so `get` returns the built-in default rather than whatever
  // the host machine has in ~/.gaganfoxwell/config.yaml (which would make the
  // default-value assertion non-deterministic).
  let tmpHome: string;
  let env: NodeJS.ProcessEnv;

  beforeAll(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-cfg-test-'));
  ...
  test('default is "prompt"', () => {
    const out = execSync(`${GAGANFOXWELL_CONFIG} get plan_tune_hooks`, {
      encoding: 'utf-8',
      env,
    }).trim();
  ...
  test('appears in defaults and list output', () => {
    const defaults = execSync(`${GAGANFOXWELL_CONFIG} defaults`, { encoding: 'utf-8', env });
    expect(defaults).toContain('plan_tune_hooks');
    const list = execSync(`${GAGANFOXWELL_CONFIG} list`, { encoding: 'utf-8', env });
    expect(list).toContain('plan_tune_hooks');
  });

  test('accepts valid values (round-trips yes/no/prompt)', () => {
    for (const v of ['yes', 'no', 'prompt']) {
      execSync(`${GAGANFOXWELL_CONFIG} set plan_tune_hooks ${v}`, { encoding: 'utf-8', env });
      const got = execSync(`${GAGANFOXWELL_CONFIG} get plan_tune_hooks`, { encoding: 'utf-8', env }).trim();
      expect(got).toBe(v);
    }
  });

  test('rejects out-of-domain values (warns + falls back to prompt)', () => {
    const res = execSync(`${GAGANFOXWELL_CONFIG} set plan_tune_hooks maybe 2>&1`, { encoding: 'utf-8', env });
