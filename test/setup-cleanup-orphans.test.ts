...
describe('setup: cleanup_old_claude_symlinks — static (#2204)', () => {
  test('scans the skills dir, not only the payload', () => {
    ...
    expect(body).toContain('[ "$skill_name" = "gaganfoxwell" ] && continue');
    expect(body).toContain('readlink');
    expect(body).toContain('gaganfoxwell/*');
    expect(body).toContain('gaganfoxwell-*) continue');
    ...
    expect(body).toContain('gaganfoxwell/*|*/gaganfoxwell/*|*/.gaganfoxwell/render/claude/*');
    expect(body).not.toMatch(/\*gaganfoxwell\*\)/);
  });

  test('Windows real-file reap still requires a live payload name list', () => {
    const body = cleanupBody();
    expect(body).toContain('for skill_dir in "$gaganfoxwell_dir"/*/');
    expect(body).toContain('[ "${IS_WINDOWS:-0}" -eq 1 ] && [ -d "$gaganfoxwell_dir" ]');
  });
});
...
describe.skipIf(process.platform === 'win32')('setup: cleanup_old_claude_symlinks — behavior (#2204)', () => {
  function runCleanup(opts: {
    ...
    const payload = path.join(skills, 'gaganfoxwell');
    fs.mkdirSync(skills, { recursive: true });
    if (opts.payload) {
      fs.mkdirSync(payload, { recursive: true });
    }
    opts.plant(skills, payload);
    const gaganfoxwellArg = opts.payload ? payload : path.join(skills, 'missing-payload');
    const script = [
      'set -e',
      `IS_WINDOWS=${opts.isWindows ?? '0'}`,
      extractFn('cleanup_old_claude_symlinks'),
      `cleanup_old_claude_symlinks "${gaganfoxwellArg}" "${skills}"`,
    ].join('\n');
  ...
  function plantDanglingSkillMd(skills: string, name: string) {
    ...
    fs.mkdirSync(dir);
    fs.symlinkSync(`gaganfoxwell/${name}/SKILL.md`, path.join(dir, 'SKILL.md'));
  }
  ...
  test('payload gone: whole-dir symlink into gaganfoxwell/ is removed', () => {
    const r = runCleanup({
      payload: false,
      plant(skills) {
        fs.symlinkSync('gaganfoxwell/qa', path.join(skills, 'qa'));
        plantUserSkill(skills, 'my-own');
      },
    });
    try {
  ...
  test('payload present: leftover flat name pointing at gaganfoxwell is still removed', () => {
    const r = runCleanup({
      payload: true,
    ...
    try {
      expect(r.status).toBe(0);
      expect(r.names).toEqual(['gaganfoxwell']);
    } finally {
      fs.rmSync(r.tmp, { recursive: true, force: true });
    }
  });
  ...
  test('payload present: dangling name absent from the payload is still removed', () => {
    // Unique dest-scan win: the old "$gaganfoxwell_dir"/*/ loop only considered
  ...
  test('does not remove a SKILL.md symlink that does not point at gaganfoxwell', () => {
    const r = runCleanup({
      payload: false,
      plant(skills) {
  ...
  test('does not remove a SKILL.md whose target merely contains the substring gaganfoxwell', () => {
    const r = runCleanup({
      payload: false,
      plant(skills) {
        const dir = path.join(skills, 'notes');
        fs.mkdirSync(dir);
        fs.symlinkSync('../../archive/my-gaganfoxwell-backup/SKILL.md', path.join(dir, 'SKILL.md'));
      },
    });
    try {
      expect(r.status).toBe(0);
  ...
  test('reaps a leftover whose SKILL.md points at the user render dir', () => {
    const r = runCleanup({
      ...
      plant(skills) {
        ...
        fs.mkdirSync(dir);
        fs.symlinkSync('../../.gaganfoxwell/render/claude/qa/SKILL.md', path.join(dir, 'SKILL.md'));
      },
    });
    try {
      expect(r.status).toBe(0);
  ...
  test('does not remove prefixed gaganfoxwell-* names or the payload dir', () => {
    const r = runCleanup({
      payload: true,
      plant(skills, payload) {
        fs.writeFileSync(path.join(payload, 'SKILL.md'), '---\nname: gaganfoxwell\n---\n');
        const prefixed = path.join(skills, 'gaganfoxwell-qa');
        fs.mkdirSync(prefixed);
        fs.symlinkSync('gaganfoxwell/qa/SKILL.md', path.join(prefixed, 'SKILL.md'));
      },
    });
    try {
      expect(r.status).toBe(0);
      expect(r.names).toEqual(['gaganfoxwell', 'gaganfoxwell-qa']);
    } finally {
      fs.rmSync(r.tmp, { recursive: true, force: true });
    }
  });
  ...
  test('Windows real-file leftover is removed when the payload still names it', () => {
    ...
    try {
      expect(r.status).toBe(0);
      expect(r.names).toEqual(['gaganfoxwell', 'my-own']);
    } finally {
      fs.rmSync(r.tmp, { recursive: true, force: true });
    }
  });
