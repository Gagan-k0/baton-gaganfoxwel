 ...
 *   - bin/gaganfoxwell-config gbrain keys (validation, isolation)
 *   - bin/gaganfoxwell-brain-enqueue (atomicity, skip list, no-op gates)
 *   - bin/gaganfoxwell-jsonl-merge (3-way, ts-sort, hash-fallback)
 *   - bin/gaganfoxwell-brain-sync --once (drain, commit, push, secret-scan, skip-file)
 *   - bin/gaganfoxwell-artifacts-init + --restore round-trip
 *   - bin/gaganfoxwell-brain-uninstall preserves user data
 *   - env isolation (GAGANFOXWELL_HOME never bleeds into real ~/.gaganfoxwell/config.yaml)
 *
 * Runs each test against a temp GAGANFOXWELL_HOME and a local bare git repo as
...
function run(argv: string[], opts: { env?: Record<string, string>; input?: string } = {}) {
  ...
  const res = spawnSync(full, argv.slice(1), {
    // HOME is overridden too: gaganfoxwell-artifacts-init writes
    // $HOME/.gaganfoxwell-artifacts-remote.txt (plain $HOME, not GAGANFOXWELL_HOME), so
    ...
    env: { ...process.env, HOME: tmpHome, GAGANFOXWELL_HOME: tmpHome, ...(opts.env || {}) },
    encoding: 'utf-8',
    input: opts.input,
    cwd: ROOT,
  });
...
afterEach(() => {
  ...
  for (const name of ['.gaganfoxwell-brain-remote.txt', '.gaganfoxwell-artifacts-remote.txt']) {
...
// ---------------------------------------------------------------
describe('gaganfoxwell-config gbrain keys', () => {
  test('default artifacts_sync_mode is off', () => {
    const r = run(['gaganfoxwell-config', 'get', 'artifacts_sync_mode']);
    expect(r.status).toBe(0);
    expect(r.stdout.trim()).toBe('off');
  });

  test('default artifacts_sync_mode_prompted is false', () => {
    const r = run(['gaganfoxwell-config', 'get', 'artifacts_sync_mode_prompted']);
    expect(r.stdout.trim()).toBe('false');
  });

  test('accepts full / artifacts-only / off', () => {
    for (const val of ['full', 'artifacts-only', 'off']) {
      const set = run(['gaganfoxwell-config', 'set', 'artifacts_sync_mode', val]);
      expect(set.status).toBe(0);
      const get = run(['gaganfoxwell-config', 'get', 'artifacts_sync_mode']);
      expect(get.stdout.trim()).toBe(val);
    }
  });

  test('invalid artifacts_sync_mode value warns + defaults', () => {
    const r = run(['gaganfoxwell-config', 'set', 'artifacts_sync_mode', 'bogus']);
    expect(r.stderr).toContain('not recognized');
    const get = run(['gaganfoxwell-config', 'get', 'artifacts_sync_mode']);
    expect(get.stdout.trim()).toBe('off');
  });

  test('GAGANFOXWELL_HOME overrides real config dir', () => {
    // Real ~/.gaganfoxwell/config.yaml must not change, regardless of what it
    // already contains on the developer's machine.
    const realConfig = path.join(os.homedir(), '.gaganfoxwell', 'config.yaml');
    const before = fs.existsSync(realConfig) ? fs.readFileSync(realConfig, 'utf-8') : null;

    run(['gaganfoxwell-config', 'set', 'artifacts_sync_mode', 'full']);
    ...
    // Real ~/.gaganfoxwell/config.yaml must not be touched.
...
// Enqueue behavior
// ---------------------------------------------------------------
describe('gaganfoxwell-brain-enqueue', () => {
  test('no-op when feature not initialized', () => {
    const r = run(['gaganfoxwell-brain-enqueue', 'projects/foo/learnings.jsonl']);
    expect(r.status).toBe(0);
  ...
  test('enqueues one spool file when mode is full and .git exists', () => {
    fs.mkdirSync(path.join(tmpHome, '.git'), { recursive: true });
    run(['gaganfoxwell-config', 'set', 'artifacts_sync_mode', 'full']);
    run(['gaganfoxwell-brain-enqueue', 'projects/foo/learnings.jsonl']);
  ...
  test('skip list honored', () => {
    ...
    run(['gaganfoxwell-brain-enqueue', 'projects/foo/secret.jsonl']);
    run(['gaganfoxwell-brain-enqueue', 'projects/foo/ok.jsonl']);
  ...
  test('concurrent enqueues all land (one spool file per record)', async () => {
    ...
    for (let i = 0; i < 10; i++) {
      procs.push(new Promise<void>((resolve) => {
        const r = spawnSync(path.join(BIN, 'gaganfoxwell-brain-enqueue'), [`file-${i}.jsonl`], {
          env: { ...process.env, GAGANFOXWELL_HOME: tmpHome },
          encoding: 'utf-8',
        });
        resolve();
      }));
  ...
  test('no args does not crash', () => {
    const r = run(['gaganfoxwell-brain-enqueue']);
    expect(r.status).toBe(0);
  });
});
...
// JSONL merge driver
// ---------------------------------------------------------------
describe('gaganfoxwell-jsonl-merge', () => {
  test('3-way merge dedups + sorts by ts', () => {
    ...
    const r = run([path.join(BIN, 'gaganfoxwell-jsonl-merge'), base, ours, theirs]);
    expect(r.status).toBe(0);
  ...
  test('falls back to hash order for lines without ts', () => {
    ...
    run([path.join(BIN, 'gaganfoxwell-jsonl-merge'), base, ours, theirs]);
    const lines = fs.readFileSync(ours, 'utf-8').trim().split('\n');
    expect(lines.length).toBe(3);
    // Order is deterministic (sha256 of each line).
    const again = spawnSync(path.join(BIN, 'gaganfoxwell-jsonl-merge'), [base, ours, theirs]);
...
describe('init + sync + restore round-trip', () => {
  test('init creates canonical files + registers drivers', () => {
    const r = run(['gaganfoxwell-artifacts-init', '--remote', bareRemote]);
    expect(r.status).toBe(0);
    ...
    expect(cfg.stdout).toContain('gaganfoxwell-jsonl-merge');
  });

  test('refuses init on different remote', () => {
    run(['gaganfoxwell-artifacts-init', '--remote', bareRemote]);
    const otherRemote = fs.mkdtempSync(path.join(os.tmpdir(), 'brain-other-'));
    spawnSync('git', ['init', '--bare', '-q', '-b', 'main', otherRemote]);
    const r = run(['gaganfoxwell-artifacts-init', '--remote', otherRemote]);
  ...
  test('full sync: init → enqueue → --once → commit pushed', () => {
    ...
    run(['gaganfoxwell-brain-enqueue', 'projects/p/learnings.jsonl']);
    const r = run(['gaganfoxwell-brain-sync', '--once']);
    expect(r.status).toBe(0);
  ...
  test('restore round-trip: writes on machine A visible on machine B', () => {
    ...
    run(['gaganfoxwell-brain-enqueue', 'projects/myproj/learnings.jsonl']);
    run(['gaganfoxwell-brain-sync', '--once']);

    // Machine B (new temp home).
    const machineB = fs.mkdtempSync(path.join(os.tmpdir(), 'brain-machineB-'));
    const r = run(['gaganfoxwell-brain-restore', bareRemote], {
      env: { GAGANFOXWELL_HOME: machineB },
    });
    expect(r.status).toBe(0);
...
// ---------------------------------------------------------------
describe('gaganfoxwell-brain-sync secret scan', () => {
  ...
  test('--skip-file unblocks specific file', () => {
    ...
    run(['gaganfoxwell-brain-enqueue', leakPath]);
    run(['gaganfoxwell-brain-sync', '--once']);  // blocked
    run(['gaganfoxwell-brain-sync', '--skip-file', leakPath]);
...
// ---------------------------------------------------------------
describe('gaganfoxwell-brain-sync egress receipt gate', () => {
  test('refused receipt leaves the queue intact, makes no commit, and next run retries', () => {
    ...
    try {
      const refused = run(['gaganfoxwell-brain-sync', '--once']);
    ...
    const retry = run(['gaganfoxwell-brain-sync', '--once']);
...
// ---------------------------------------------------------------
describe('gaganfoxwell-brain-uninstall', () => {
  test('removes sync config but preserves learnings/project data', () => {
    ...
    const r = run(['gaganfoxwell-brain-uninstall', '--yes']);
    expect(r.status).toBe(0);
    ...
    // Config key reset.
    const mode = run(['gaganfoxwell-config', 'get', 'artifacts_sync_mode']);
...
// ---------------------------------------------------------------
describe('gaganfoxwell-brain-sync --discover-new', () => {
  test('enqueues new allowlisted files as spool records; idempotent on re-run', () => {
    ...
    run(['gaganfoxwell-brain-sync', '--discover-new']);
...
describe('enqueue tmp janitor', () => {
  test('an orphaned .tmp-* older than 1h is reaped on --once; a fresh one survives', () => {
    ...
    expect(run(['gaganfoxwell-brain-sync', '--once']).status).toBe(0);
...
describe('#2549 queue integrity', () => {
  function initWithMode(mode: string) {
    run(['gaganfoxwell-artifacts-init', '--remote', bareRemote]);
    run(['gaganfoxwell-config', 'set', 'artifacts_sync_mode', mode]);
  }
  ...
  test('privacy-held entries are RETAINED and classified, not wiped as "no allowlisted changes"', () => {
    ...
    run(['gaganfoxwell-brain-enqueue', 'projects/p/timeline.jsonl']);
    const r = run(['gaganfoxwell-brain-sync', '--once']);
    expect(r.status).toBe(0);
  ...
  test('push failure retains the commit locally and the run-start detector re-pushes it', () => {
    ...
    run(['gaganfoxwell-brain-enqueue', 'projects/p/learnings.jsonl']);
    const fail = run(['gaganfoxwell-brain-sync', '--once']);
  ...
  test('an interleaved user commit disables the detector push (exclusive author gate)', () => {
    ...
    // A user manually commits in ~/.gaganfoxwell on top of the stranded bot commit.
...
describe('C12 spool queue', () => {
  ...
  test('two rapid enqueues of different paths create two spool files; one drain syncs both', () => {
    ...
    fs.writeFileSync(path.join(tmpHome, 'retros/week-1.md'), '# retro\n');
    run(['gaganfoxwell-brain-enqueue', 'projects/p/learnings.jsonl']);
    run(['gaganfoxwell-brain-enqueue', 'retros/week-1.md']);
  ...
  test('--status queue_depth counts spool records plus unmigrated legacy lines', () => {
    ...
    const r = run(['gaganfoxwell-brain-sync', '--status']);
    expect(r.status).toBe(0);
  ...
  test('G1: finalize is explicit-delete-only and the fast path is .migrating-aware (static pins)', () => {
    const src = fs.readFileSync(path.join(BIN, 'gaganfoxwell-brain-sync'), 'utf-8');
  ...
  test('--drop-queue keeps the --yes gate and counts spool + legacy entries', () => {
    ...
    seedSpool('{"file":"projects/p/b.jsonl","ts":"t"}');
    fs.writeFileSync(path.join(tmpHome, '.brain-queue.jsonl'), '{"file":"projects/p/c.jsonl","ts":"t"}\n');
    const refused = run(['gaganfoxwell-brain-sync', '--drop-queue']);
    expect(refused.status).toBe(1);
    expect(refused.stderr).toContain('--yes');
    expect(spoolFiles().length).toBe(2);
    const dropped = run(['gaganfoxwell-brain-sync', '--drop-queue', '--yes']);
    ...
    const again = run(['gaganfoxwell-brain-sync', '--drop-queue', '--yes']);
