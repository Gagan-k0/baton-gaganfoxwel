 ...
 * (~/.gaganfoxwell/render/claude) and makes the Claude installers — setup's
 * link_claude_skill_dirs AND bin/gaganfoxwell-relink — prefer rendered files when
...
const CONFIG_SRC = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-config'), 'utf-8');
const RELINK_SRC = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-relink'), 'utf-8');
const MIGRATION = path.join(ROOT, 'gaganfoxwell-upgrade', 'migrations', 'v1.67.0.0.sh');
...
describe(':user render targets the out-dir, never the checkout (#2569)', () => {
  ...
  test('setup renders into a TMP dir, swaps on success, repoints after (never wipes first)', () => {
    ...
    expect(block).toContain('--out-dir "$_GAGANFOXWELL_RENDER_TMP"');
    expect(block).not.toContain('--out-dir "$_GAGANFOXWELL_RENDER_DIR"');
    expect(block).toContain('_swap_in_render "$_GAGANFOXWELL_RENDER_DIR" "$_GAGANFOXWELL_RENDER_TMP"');
    expect(block).toContain('link_claude_skill_dirs "$SOURCE_GAGANFOXWELL_DIR" "$INSTALL_SKILLS_DIR"');
    // Stale-render cleanup on the gbrain-gone path (a deliberate wipe).
    expect(block).toContain('gbrain not detected');
    expect(block).toContain('rm -rf "$_GAGANFOXWELL_RENDER_DIR"');
  });

  test('gaganfoxwell-config gbrain-refresh renders to a TMP out-dir, swaps on success', () => {
    ...
    expect(CONFIG_SRC).toContain('gaganfoxwell-relink');
  ...
  test('_swap_in_render behavior: success replaces, and the shape means failure never touches the live dir', () => {
    ...
    for (const src of [SETUP_SRC, CONFIG_SRC]) {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-render-swap-'));
      try {
  ...
  test('a FAILED render leaves the previous render dir fully intact (gaganfoxwell-config path, end-to-end shape)', () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-render-fail-'));
    try {
      const live = path.join(tmp, 'claude');
      fs.mkdirSync(path.join(live, 'ship'), { recursive: true });
  ...
  test('gaganfoxwell-relink prefers the render dir when a rendered SKILL.md exists', () => {
...
describe('link_claude_skill_dirs prefers rendered SKILL.md (behavior)', () => {
  test('a rendered variant is served; skills without one fall back to source', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-render-pref-'));
    try {
      const src = path.join(tmp, 'src');
      const skills = path.join(tmp, 'skills');
      const home = path.join(tmp, 'gaganfoxwell-home');
...
describe('migration v1.67.0.0 — legacy in-place render cleanup (F12)', () => {
  ...
  function runMigration(install: string): { status: number | null; stdout: string } {
    const r = spawnSync('bash', [MIGRATION], {
      encoding: 'utf-8',
      env: { ...process.env, GAGANFOXWELL_INSTALL_DIR: install },
      timeout: 15_000,
    });
  ...
  test('restores render-class dirt, leaves user changes alone, idempotent', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-migration-'));
    try {
  ...
  test('clean checkout, missing dir, and symlinked install are all silent no-ops', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-migration-noop-'));
    try {
      const install = makeLegacyInstall(tmp);
