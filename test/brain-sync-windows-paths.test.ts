...
// Static invariants guarding Windows artifact-sync (bin/gaganfoxwell-brain-sync).
//
...
const SRC = fs.readFileSync(path.join(ROOT, 'bin', 'gaganfoxwell-brain-sync'), 'utf-8');

describe('gaganfoxwell-brain-sync — Windows path/exec invariants', () => {
  test('discover-new normalizes relpath separators before fnmatch (bug 1)', () => {
    expect(SRC).toContain('os.path.relpath(full, gaganfoxwell_home).replace(os.sep, "/")');
  });
