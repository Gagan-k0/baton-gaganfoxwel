/**
 * Unit tests for gaganfoxwell-upgrade/migrations/v1.37.0.0.sh — split-engine notice.
 *
...
function makeEnv(opts: {
  ...
  const claudeSkillsBin = join(home, ".claude", "skills", "gaganfoxwell", "bin");
  const claudeJson = join(home, ".claude.json");
  const gbrainConfig = join(gbrainDir, "config.json");
  const configBin = join(claudeSkillsBin, "gaganfoxwell-config");
  ...
  // Fake gaganfoxwell-config: returns "true" iff opted-out (matches the real bin's
...
`,
  ...
  return {
    ...
    home,
    gaganfoxwellHome,
    doneTouch: join(gaganfoxwellHome, ".migrations", "v1.37.0.0.done"),
    claudeJson,
    gbrainConfig,
    configBin,
...
function runMigration(env: MigEnv): { stdout: string; stderr: string; exitCode: number } {
  const result = spawnSync("bash", [MIGRATION], {
    ...
    env: {
      ...
      // The script looks for gaganfoxwell-config at $HOME/.claude/skills/gaganfoxwell/bin
...
describe("gaganfoxwell-upgrade/migrations/v1.37.0.0.sh", () => {
