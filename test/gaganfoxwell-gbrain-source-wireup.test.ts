/**
 * gaganfoxwell-gbrain-source-wireup — unit tests with mocked gbrain CLI.
 *
 * The helper registers the gaganfoxwell brain repo as a gbrain federated source
...
const WIREUP_BIN = path.join(BIN_DIR, 'gaganfoxwell-gbrain-source-wireup');
...
function run(
  ...
  const env = {
    ...
    GAGANFOXWELL_BRAIN_WORKTREE: worktreeDir,
    GAGANFOXWELL_BRAIN_NO_SYNC: '0',
    ...(opts.env || {}),
  };
...
function setupGaganfoxwellRepo(remoteUrl: string) {
  // Real git repo at gaganfoxwellHome with at least one commit + an origin remote.
  fs.mkdirSync(gaganfoxwellHome, { recursive: true });
  spawnSync('git', ['-C', gaganfoxwellHome, 'init', '-q', '-b', 'main'], { stdio: 'pipe' });
  spawnSync('git', ['-C', gaganfoxwellHome, 'config', 'user.email', 'test@example.com'], { stdio: 'pipe' });
  spawnSync('git', ['-C', gaganfoxwellHome, 'config', 'user.name', 'test'], { stdio: 'pipe' });
  fs.writeFileSync(path.join(gaganfoxwellHome, '.brain-allowlist'), '# allowlist\n');
  spawnSync('git', ['-C', gaganfoxwellHome, 'add', '.'], { stdio: 'pipe' });
  spawnSync('git', ['-C', gaganfoxwellHome, 'commit', '-q', '-m', 'init'], { stdio: 'pipe' });
  spawnSync('git', ['-C', gaganfoxwellHome, 'remote', 'add', 'origin', remoteUrl], { stdio: 'pipe' });
}

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-wireup-test-'));
  gaganfoxwellHome = path.join(tmpHome, '.gaganfoxwell');
  worktreeDir = path.join(tmpHome, '.gaganfoxwell-brain-worktree');
...
describe('gaganfoxwell-gbrain-source-wireup — wireup mode', () => {
  test('fresh state: registers source + creates worktree + syncs', () => {
    setupGaganfoxwellRepo('git@github.com:user/gaganfoxwell-brain-user.git');
    makeFakeGbrain({});
    const r = run([], { env: { GAGANFOXWELL_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(fs.existsSync(worktreeDir)).toBe(true);
    const state = readState();
    expect(state.sources).toHaveLength(1);
    expect(state.sources[0].id).toBe('gaganfoxwell-brain-user');
  ...
  test('the real sync targets the REGISTERED source, never --repo (#2662)', () => {
    ...
    // WITHOUT GAGANFOXWELL_BRAIN_NO_SYNC — the skip-mode cases never reach the sync,
    ...
    expect(calls.some((c) => c.startsWith('gbrain sync --source gaganfoxwell-brain-user'))).toBe(true);
  ...
  test('idempotent re-run after success: no new sources add call', () => {
    setupGaganfoxwellRepo('git@github.com:user/gaganfoxwell-brain-user.git');
    makeFakeGbrain({});
    run([], { env: { GAGANFOXWELL_BRAIN_NO_SYNC: '1' } });
  ...
  test('drift recovery: existing source with different path triggers remove + add', () => {
    ...
    fs.writeFileSync(
      gbrainStateFile,
      JSON.stringify({
        sources: [{ id: 'gaganfoxwell-brain-user', local_path: '/old/stale/path', federated: true }],
      })
    );
    const r = run([], { env: { GAGANFOXWELL_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
    expect(calls.some((c) => c.startsWith('gbrain sources remove gaganfoxwell-brain-user'))).toBe(true);
    expect(calls.some((c) => c.includes(`gbrain sources add gaganfoxwell-brain-user --path ${worktreeDir}`))).toBe(true);
    const state = readState();
    expect(state.sources[0].local_path).toBe(worktreeDir);
  });
  ...
  test('--strict + gbrain present in controlled dir: exits 0 (positive control)', () => {
    ...
    makeFakeGbrain({});
    const r = run(['--strict'], { env: { GAGANFOXWELL_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
  ...
  test('source-id derived from origin URL', () => {
    setupGaganfoxwellRepo('git@github.com:user/gaganfoxwell-brain-alice.git');
    makeFakeGbrain({});
    const r = run([], { env: { GAGANFOXWELL_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(readState().sources[0].id).toBe('gaganfoxwell-brain-alice');
  });

  test('source-id fallback to ~/.gaganfoxwell-brain-remote.txt when .git is gone', () => {
    // No git repo at gaganfoxwellHome; just the remote-file
    fs.mkdirSync(tmpHome, { recursive: true });
    fs.writeFileSync(
      path.join(tmpHome, '.gaganfoxwell-brain-remote.txt'),
      'git@github.com:user/gaganfoxwell-brain-bob.git\n'
    );
    makeFakeGbrain({});
    // No --strict: helper should benign-skip because .gaganfoxwell/.git is missing
    const r = run([]);
  ...
  test('source-id from --source-id flag overrides everything', () => {
    setupGaganfoxwellRepo('git@github.com:user/gaganfoxwell-brain-different.git');
    makeFakeGbrain({});
    run(['--source-id', 'custom-id'], { env: { GAGANFOXWELL_BRAIN_NO_SYNC: '1' } });
    const state = readState();
  ...
  test('--probe: read-only, prints state without mutating', () => {
    ...
    const r = run(['--probe']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('source_id=gaganfoxwell-brain-user');
...
describe('gaganfoxwell-gbrain-source-wireup — ZeroEntropy sunset advisory (#2365)', () => {
  ...
  test('advisory docs entry exists (USING_GBRAIN_WITH_GAGANFOXWELL.md content pin)', () => {
    const doc = fs.readFileSync(path.join(ROOT, 'USING_GBRAIN_WITH_GAGANFOXWELL.md'), 'utf-8');
    expect(doc).toContain('September 4, 2026');
    expect(doc).toContain('#2365');
  });
});

describe('gaganfoxwell-gbrain-source-wireup — --database-url lock (defends against external config rewrites)', () => {
  test('--database-url flag is exported as GBRAIN_DATABASE_URL to child gbrain calls', () => {
    ...
    const r = run(['--database-url', TARGET], { env: { GAGANFOXWELL_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
...
describe('gaganfoxwell-gbrain-source-wireup — uninstall mode', () => {
...
describe('gaganfoxwell-gbrain-source-wireup — defensive paths', () => {
  test('--no-pull skips HEAD advance on existing worktree', () => {
    ...
    fs.writeFileSync(path.join(gaganfoxwellHome, 'newfile.md'), 'new');
    spawnSync('git', ['-C', gaganfoxwellHome, 'add', '.'], { stdio: 'pipe' });
    spawnSync('git', ['-C', gaganfoxwellHome, 'commit', '-q', '-m', 'second commit'], { stdio: 'pipe' });
    const parentHeadAfter = spawnSync('git', ['-C', gaganfoxwellHome, 'rev-parse', 'HEAD'], {
      encoding: 'utf-8',
    }).stdout.trim();
    ...
    const r = run(['--no-pull'], { env: { GAGANFOXWELL_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
