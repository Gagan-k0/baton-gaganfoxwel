/**
 * gaganfoxwell-uninstall: real-directory installs are removed, gated on provenance
 * (#2563, F8, ENG-OV10).
 *
 * On Windows, setup installs skills as REAL directory copies (no symlinks).
 * gaganfoxwell-uninstall's per-skill loop filtered on `[ -L ]`, so every copy was
 * skipped: the tool exited 0, printed "gaganfoxwell uninstalled.", and left ~52
 * gaganfoxwell-* directories behind. The same filter also missed the standard Unix
 * shape (real dir + symlinked SKILL.md).
 *
 * Deletion gate for real-file installs (F8): the directory name must be in
 * gaganfoxwell's skill inventory AND its SKILL.md must carry the existing generated
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-uninstall-copies-'));
  mockHome = path.join(tmpDir, 'home');
  skillsDir = path.join(mockHome, '.claude', 'skills');
  installRoot = path.join(skillsDir, 'gaganfoxwell');
  ...
  fs.writeFileSync(path.join(installRoot, 'SKILL.md'), skillMd('gaganfoxwell'));
...
describe('gaganfoxwell-uninstall removes Windows real-dir copies (#2563)', () => {
  test('inventory name + banner → removed (flat, prefixed, and alias forms)', () => {
    const review = realDirEntry('review', skillMd('review'));
    const prefixedShip = realDirEntry('gaganfoxwell-ship', skillMd('gaganfoxwell-ship'));
    const alias = realDirEntry('_gaganfoxwell-command', skillMd('_gaganfoxwell-command'));
  ...
  test('no banner → kept and listed, even when the name collides with a gaganfoxwell skill', () => {
  ...
  test('a clean sweep reports the removed entries', () => {
    ...
    expect(r.stdout).toContain('gaganfoxwell uninstalled.');
  });
});
...
describe.skipIf(process.platform === 'win32')(
  'gaganfoxwell-uninstall removes the Unix real-dir + symlinked-SKILL.md shape',
  () => {
    test('SKILL.md symlink pointing into gaganfoxwell → removed', () => {
    ...
    test('SKILL.md symlink into a gaganfoxwell-SUBSTRING path (gaganfoxwell-fork) → kept and listed', () => {
      // DM5: the shape-2 gate must match "gaganfoxwell" as an anchored path
      // segment, not a substring — a user's own skill whose SKILL.md links
      // into ~/tools/gaganfoxwell-fork/ is NOT ours, even when the dir name
      // collides with a real gaganfoxwell skill (here: review, in the inventory).
      // The anchored gate only matches a literal /gaganfoxwell/ path segment, so
      // the tmpdir must not carry one (shared-process shard runs can leave
      // $TMPDIR pointing into a gaganfoxwell worktree — same hazard as the
      // "pointing elsewhere" test below). Fall back to a fixed neutral root
      // and ASSERT the precondition.
      let neutralRoot = os.tmpdir();
      if (neutralRoot.split(path.sep).includes('gaganfoxwell')) neutralRoot = '/private' + path.sep + 'tmp';
      const forkRoot = fs.mkdtempSync(path.join(neutralRoot, 'tools-'));
      expect(forkRoot.split(path.sep).includes('gaganfoxwell')).toBe(false);
      const forkSrc = path.join(forkRoot, 'gaganfoxwell-fork', 'review');
    ...
    test('SKILL.md symlink into gaganfoxwell but name NOT in inventory → kept and listed', () => {
    ...
    test('SKILL.md symlink pointing elsewhere → kept and listed', () => {
      // Target path must not contain a gaganfoxwell path segment (the provenance
      // match is anchored: gaganfoxwell/*|*/gaganfoxwell/*; keeping the stricter
      ...
      // neighboring test can leave it pointing at a gaganfoxwell-containing path —
      ...
      // shard runner's TMPDIR is the gaganfoxwell-containing path that forces this
      // branch. (Never taken on Windows — its TMPDIR carries no 'gaganfoxwell'.)
      if (neutralRoot.includes('gaganfoxwell')) neutralRoot = fs.realpathSync('/tmp');
      const neutral = fs.mkdtempSync(path.join(neutralRoot, 'other-skill-src-'));
      expect(neutral.includes('gaganfoxwell')).toBe(false);
