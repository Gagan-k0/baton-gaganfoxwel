...
describe('gaganfoxwell-upgrade template: ff-only precedes the gated reset (#2517)', () => {
  const tmpl = read('gaganfoxwell-upgrade/SKILL.md.tmpl');
...
describe('brain-uninstall removes the spool queue', () => {
  test('uninstall cleans .brain-queue.d alongside the legacy queue file', () => {
    const src = read('bin/gaganfoxwell-brain-uninstall');
