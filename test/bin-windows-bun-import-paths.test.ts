 ...
 *   2. Behavioral — gaganfoxwell-learnings-log, invoked the way Windows CI
...
describe("bin/ — Windows bun-import path guard (#1950)", () => {
  ...
  it("known-affected bins carry the guard explicitly", () => {
    for (const name of ["gaganfoxwell-learnings-log", "gaganfoxwell-question-log"]) {
      const content = readFileSync(join(BIN_DIR, name), "utf-8");
...
describe("gaganfoxwell-learnings-log — behavioral (runs on Windows CI via git-bash)", () => {
  function runViaBash(input: string, gaganfoxwellHome: string) {
    // spawnSync("bash", [path]) mirrors how git-bash users (and Windows CI)
    // execute the bin — Windows CreateProcess cannot parse shebangs.
    return spawnSync("bash", [join(BIN_DIR, "gaganfoxwell-learnings-log"), input], {
      encoding: "utf-8",
      timeout: 20_000,
      cwd: ROOT,
  ...
  it("writes a learning end-to-end (proves the bun import resolves on this platform)", () => {
    const tmp = mkdtempSync(join(tmpdir(), "gaganfoxwell-win-learn-"));
    try {
      const r = runViaBash(
        JSON.stringify({
          skill: "test",
