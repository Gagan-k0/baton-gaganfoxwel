 ...
 * re-run: `./setup --host codex` reported "gaganfoxwell ready" but never refreshed
...
describe('setup: Windows re-run refresh — static guard sites (#2444)', () => {
  ...
  // skill-dir installer must gate the replacement on provable gaganfoxwell
...
describe('setup: Windows refresh ownership gate — behavior fixture (#2142)', () => {
  test("IS_WINDOWS=1: a user's own real dir on a gaganfoxwell* name survives; a bannered install refreshes", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-owned-'));
    try {
      const fake = path.join(tmp, 'gaganfoxwell');
      ...
      for (const name of ['gaganfoxwell-demo', 'gaganfoxwell-notes']) {
        const d = path.join(fake, '.agents', 'skills', name);
        fs.mkdirSync(d, { recursive: true });
        fs.writeFileSync(path.join(d, 'SKILL.md'), `${banner}upstream-v2\n`);
      }
      fs.mkdirSync(skills, { recursive: true });
      // gaganfoxwell-demo: a prior gaganfoxwell install (bannered) — must refresh.
      fs.mkdirSync(path.join(skills, 'gaganfoxwell-demo'), { recursive: true });
      fs.writeFileSync(path.join(skills, 'gaganfoxwell-demo', 'SKILL.md'), `${banner}installed-v1\n`);
      // gaganfoxwell-notes: the USER'S own hand-written skill — must survive.
      fs.mkdirSync(path.join(skills, 'gaganfoxwell-notes'), { recursive: true });
      fs.writeFileSync(path.join(skills, 'gaganfoxwell-notes', 'SKILL.md'), '# my own notes\n');

      const r = runInstaller(
        '1',
        ['_owned_for_windows_refresh', 'link_codex_skill_dirs'],
        `link_codex_skill_dirs "${tmp}/gaganfoxwell" "${skills}"`,
      );
      expect(r.status).toBe(0);
      expect(fs.readFileSync(path.join(skills, 'gaganfoxwell-demo', 'SKILL.md'), 'utf-8')).toContain('upstream-v2');
      expect(fs.readFileSync(path.join(skills, 'gaganfoxwell-notes', 'SKILL.md'), 'utf-8')).toBe('# my own notes\n');
      expect(r.stderr).toContain('left in place');
      expect(r.stderr).toContain('gaganfoxwell-notes');
    } finally {
  ...
  test('IS_WINDOWS=1: create_agents_sidecar refuses a user-owned root and writes nothing into it', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-owned-sidecar-'));
    try {
      const fake = path.join(tmp, 'gaganfoxwell');
      fs.mkdirSync(path.join(fake, 'bin'), { recursive: true });
      fs.writeFileSync(path.join(fake, 'bin', 'tool.sh'), 'v1\n');
      // The user's own skill squats on .agents/skills/gaganfoxwell.
      const root = path.join(fake, '.agents', 'skills', 'gaganfoxwell');
      fs.mkdirSync(root, { recursive: true });
      fs.writeFileSync(path.join(root, 'SKILL.md'), '# hand-written\n');

      const vars = `SOURCE_GAGANFOXWELL_DIR="${fake}"`;
      const r = runInstaller(
        '1',
...
describe('setup: Windows re-run refresh — behavior fixture (#2444)', () => {
  test('IS_WINDOWS=1: link_codex_skill_dirs refreshes an already-installed skill', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-rerun-'));
    try {
      const fake = path.join(tmp, 'gaganfoxwell');
      const skills = path.join(tmp, 'skills');
      const demo = path.join(fake, '.agents', 'skills', 'gaganfoxwell-demo');
      ...
      expect(r.status).toBe(0);
      const installed = path.join(skills, 'gaganfoxwell-demo', 'SKILL.md');
      expect(fs.readFileSync(installed, 'utf-8')).toBe(`${banner}v1-original\n`);
      expect(fs.lstatSync(path.join(skills, 'gaganfoxwell-demo')).isSymbolicLink()).toBe(false);
  ...
  test('IS_WINDOWS=1: create_agents_sidecar refreshes copied runtime assets', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-rerun-sidecar-'));
    try {
      const fake = path.join(tmp, 'gaganfoxwell');
      fs.mkdirSync(path.join(fake, 'bin'), { recursive: true });
      fs.writeFileSync(path.join(fake, 'bin', 'tool.sh'), 'v1\n');
      ...
      expect(r.status).toBe(0);
      const sidecarBin = path.join(fake, '.agents', 'skills', 'gaganfoxwell', 'bin', 'tool.sh');
      const sidecarEthos = path.join(fake, '.agents', 'skills', 'gaganfoxwell', 'ETHOS.md');
  ...
  test('IS_WINDOWS=1: nested gitignored build output does NOT survive the runtime-asset copy (P5)', () => {
    ...
    // survive.
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-rerun-prune-'));
    try {
  ...
  test('IS_WINDOWS=0: the Unix symlink path never prunes through into the source', () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-rerun-prune-unix-'));
    try {
      const src = path.join(tmp, 'skill-src');
      const dst = path.join(tmp, 'skill-dst');
      fs.mkdirSync(path.join(src, 'scripts', 'gen-tool', '.build'), { recursive: true });
  ...
  test('IS_WINDOWS=1: the gaganfoxwell sidecar dir is still skipped by the skill loop', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-rerun-skip-'));
    try {
      const fake = path.join(tmp, 'gaganfoxwell');
      const skills = path.join(tmp, 'skills');
      const sidecar = path.join(fake, '.agents', 'skills', 'gaganfoxwell');
      ...
      expect(r.status).toBe(0);
      expect(fs.existsSync(path.join(skills, 'gaganfoxwell'))).toBe(false);
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true });
    }
  });
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    test('IS_WINDOWS=0: installs a symlink and re-runs still refresh through it', () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-rerun-unix-'));
      try {
        const fake = path.join(tmp, 'gaganfoxwell');
        const skills = path.join(tmp, 'skills');
        const demo = path.join(fake, '.agents', 'skills', 'gaganfoxwell-demo');
        ...
        expect(r.status).toBe(0);
        const target = path.join(skills, 'gaganfoxwell-demo');
