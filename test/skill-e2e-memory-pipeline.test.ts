 ...
 *      ~/.gaganfoxwell/ artifacts (eureka, learning, ceo-plan, design-doc, retro,
 *      builder-profile)
 *   2. Run gaganfoxwell-memory-ingest --probe → verify stage counts match disk
 *      (post-attribution headline + unattributed skip line, #2394)
 *   3. Run gaganfoxwell-memory-ingest --bulk → verify state file gets written +
 *      session_id dedup works on re-run (idempotency)
 *   4. Run gaganfoxwell-gbrain-sync --dry-run → verify all 3 stages preview
 *   5. Run gaganfoxwell-brain-context-load against a real V1 skill manifest
 ...
 * may or may not be available in CI. We assert on side effects gaganfoxwell
...
const INGEST = join(REPO_ROOT, "bin", "gaganfoxwell-memory-ingest.ts");
const SYNC = join(REPO_ROOT, "bin", "gaganfoxwell-gbrain-sync.ts");
const CONTEXT = join(REPO_ROOT, "bin", "gaganfoxwell-brain-context-load.ts");

function makeFixtureHome(): string {
  return mkdtempSync(join(tmpdir(), "gaganfoxwell-e2e-pipeline-"));
}

function setupFixture(home: string): { gaganfoxwellHome: string; counts: Record<string, number> } {
  ...
  mkdirSync(join(gaganfoxwellHome, "projects", "test-repo", "ceo-plans"), { recursive: true });
  mkdirSync(join(gaganfoxwellHome, "projects", "test-repo", "retros"), { recursive: true });
  ...
  // gaganfoxwell artifacts
  writeFileSync(join(gaganfoxwellHome, "analytics", "eureka.jsonl"), '{"insight":"boil the lake"}\n', "utf-8");
  writeFileSync(join(gaganfoxwellHome, "builder-profile.jsonl"), '{"date":"2026-05-01","mode":"startup"}\n', "utf-8");
  writeFileSync(join(gaganfoxwellHome, "projects", "test-repo", "learnings.jsonl"), '{"key":"a","insight":"b","confidence":8}\n', "utf-8");
  writeFileSync(join(gaganfoxwellHome, "projects", "test-repo", "timeline.jsonl"), '{"skill":"office-hours","event":"completed"}\n', "utf-8");
  writeFileSync(join(gaganfoxwellHome, "projects", "test-repo", "ceo-plans", "2026-05-01-test.md"), "# CEO Plan: Test\n\nbody\n", "utf-8");
  writeFileSync(join(gaganfoxwellHome, "projects", "test-repo", "garrytan-main-design-20260501-090000.md"), "# Design: Test\n", "utf-8");
  writeFileSync(join(gaganfoxwellHome, "projects", "test-repo", "retros", "2026-05-01-week.md"), "# Retro\n", "utf-8");
...
describe("V1 memory ingest pipeline E2E", () => {
  it("--probe accounts for all 9 fixture files: 7 attributable + 2 unattributed transcripts skipped (#2394)", () => {
    ...
    const { gaganfoxwellHome, counts } = setupFixture(home);
    const env = { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome, GAGANFOXWELL_MEMORY_INGEST_NO_WRITE: "1" };
    ...
    // both; the gaganfoxwell artifacts are store-local and always attributable.
  ...
  it("--incremental writes a state file with schema_version: 1 + last_writer", () => {
    const home = makeFixtureHome();
    const { gaganfoxwellHome } = setupFixture(home);
    const env = { HOME: home, GAGANFOXWELL_HOME: gaganfoxwellHome, GAGANFOXWELL_MEMORY_INGEST_NO_WRITE: "1" };
  ...
  it("--incremental is idempotent — re-run reports 0 changes", () => {
    ...
    // First run
    runBun(INGEST, ["--incremental", "--quiet"], env);
    const stateAfterFirst = readFileSync(join(gaganfoxwellHome, ".transcript-ingest-state.json"), "utf-8");
