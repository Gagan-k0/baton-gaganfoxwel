 ...
 * ~/.gaganfoxwell/projects/<slug>/brain-cache/.refresh.lock. Stale locks (process
 * dead, or older than CACHE_REFRESH_LOCK_TIMEOUT_MS) are taken over.
 *
 * Gate-tier, free, pure file-IO. Uses tmp GAGANFOXWELL_HOME.
 */
...
beforeEach(() => {
  TMP_HOME = mkdtempSync(join(tmpdir(), 'gaganfoxwell-lock-test-'));
...
describe('concurrent-refresh lockfile dedup', () => {
  ...
  test('cross-project lock uses ~/.gaganfoxwell/brain-cache/.refresh.lock', async () => {
    const mod = await importCache();
