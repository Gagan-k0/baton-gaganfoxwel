...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-eval-list-'));
  const evalDir = path.join(tmpHome, '.gaganfoxwell-dev', 'evals');
...
function runEvalList(...args: string[]): { stdout: string; stderr: string; status: number } {
  ...
  // cwd-relative .claude/skills/gaganfoxwell/bin/gaganfoxwell-slug, and on dev machines
  // with the self-symlink that probe succeeds, routing reads to an (empty)
  // project-scoped dir instead of the legacy ~/.gaganfoxwell-dev/evals this test
