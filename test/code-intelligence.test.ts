 ...
 * selection store the `gaganfoxwell-code-intelligence` CLI drives.
 *
...
describe("selection store + provider-OFF", () => {
  ...
  beforeEach(() => {
    ...
    env = { ...process.env, GAGANFOXWELL_HOME: home };
  });
...
// gaganfoxwell-gbrain-repo-policy (per-remote trust tiers) is the single authority.
describe("consent unification — deny tier wins (R1)", () => {
  ...
  const POLICY_BIN = path.join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-repo-policy");
  ...
  test("recorded consent survives when no policy store exists", () => {
    ...
    try {
      const env = { ...process.env, GAGANFOXWELL_HOME: home };
  ...
  // gaganfoxwell-gbrain-sync semantics: "search allowed, page writes never". The
...
describe("read-only repo policy blocks write-class CLI index (R2)", () => {
  const CLI = path.join(import.meta.dir, "..", "bin", "gaganfoxwell-code-intelligence");
  const POLICY_BIN = path.join(import.meta.dir, "..", "bin", "gaganfoxwell-gbrain-repo-policy");
  ...
  test("index refuses on read-only even with recorded consent (gbrain provider)", () => {
    ...
    try {
      ...
      const env = { ...process.env, GAGANFOXWELL_HOME: home, PATH: `${shimDir}:${process.env.PATH}` };
...
describe("Sourcebot egress receipts record the TRUE consent state", () => {
  ...
  test("status probe: allowed without consent, receipt says consent=unchecked (never consented=true)", async () => {
    ...
    const s = await sb.status(undefined, { env: { GAGANFOXWELL_HOME: home } });
    expect(s.state).toBe("ready");
  ...
  test("non-loopback search without consent is refused BEFORE any bytes leave (fail-closed)", async () => {
    ...
    await expect(sb.search("internalSecretFn", { env: { GAGANFOXWELL_HOME: home } })).rejects.toMatchObject({
      code: "PROVIDER_NOT_CONSENTED",
    });
  ...
  test("non-loopback refresh without consent → PROVIDER_NOT_CONSENTED (write-class)", async () => {
    const sb = new SourcebotProvider({ baseUrl: "http://sb.example.com:3000", fetch: okFetch });
    await expect(sb.refresh({ id: "r" }, { env: { GAGANFOXWELL_HOME: home } })).rejects.toMatchObject({
      code: "PROVIDER_NOT_CONSENTED",
    });
  ...
  test("consented non-loopback search sends, and the receipt truthfully records consented=true", async () => {
    const sb = new SourcebotProvider({ baseUrl: "http://sb.example.com:3000", fetch: okFetch });
    const hits = await sb.search("foo", { consented: true, env: { GAGANFOXWELL_HOME: home } });
    expect(hits).toEqual([]);
    const lines = ledgerLines();
    expect(lines.length).toBe(1);
  ...
  test("loopback search stays consent-free and writes no receipt (no egress)", async () => {
    ...
    await sb.search("foo", { env: { GAGANFOXWELL_HOME: home } });
    expect(ledgerLines()).toEqual([]);
  });
});
...
describe("GBrain search/export consent gate + egress receipts", () => {
  ...
  let marker: string;
  function env(): NodeJS.ProcessEnv {
    return { PATH: `${binDir}:${process.env.PATH}`, HOME: home, GAGANFOXWELL_HOME: home };
  }
  function ledgerLines(): Array<Record<string, unknown>> {
    const p = path.join(home, "security", "egress.jsonl");
    if (!fs.existsSync(p)) return [];
...
describe("CLI search consent gate (gbrain provider, honest refusal message)", () => {
  ...
  beforeEach(() => {
    ...
    execFileSync("git", ["init", "-q", "."], { cwd: repo });
    execFileSync("git", ["remote", "add", "origin", URL], { cwd: repo });
    env = { ...process.env, GAGANFOXWELL_HOME: home, PATH: `${shimDir}:${process.env.PATH}` };
    setProvider("gbrain", env);
...
`,
  ...
  test("add/delete/export each degrade to PROVIDER_UNAVAILABLE when the CLI is absent", async () => {
    const missing = { PATH: os.tmpdir(), HOME: home, GAGANFOXWELL_HOME: home };
    const g = new GbrainProvider();
...
describe("detectAvailable probes fast even when Sourcebot is a dead non-loopback host", () => {
  test("rows come back in recommendation order, well under the old 30s stall", async () => {
    ...
    // localEngineStatus writes its probe cache via process.env.GAGANFOXWELL_HOME —
    // point it at the temp home for the duration so nothing touches ~/.gaganfoxwell.
    const prevGaganfoxwellHome = process.env.GAGANFOXWELL_HOME;
    process.env.GAGANFOXWELL_HOME = homeDir;
    try {
...
`,
      ...
      const env: NodeJS.ProcessEnv = { PATH: binDir, HOME: homeDir, GAGANFOXWELL_HOME: homeDir };
      ...
      if (prevGaganfoxwellHome === undefined) delete process.env.GAGANFOXWELL_HOME;
      else process.env.GAGANFOXWELL_HOME = prevGaganfoxwellHome;
      fs.rmSync(binDir, { recursive: true, force: true });
      fs.rmSync(homeDir, { recursive: true, force: true });
    }
  }, 20_000);
