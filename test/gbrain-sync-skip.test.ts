/**
 * Tests the split-engine SKIP semantics in bin/gaganfoxwell-gbrain-sync.ts (plan D12).
 *
...
describe("gaganfoxwell-gbrain-sync — split-engine SKIP (plan D12)", () => {
  it("PROCEEDS (with warning) when the engine probe times out — slow is not broken (#1964)", () => {
    ...
    try {
      ...
      expect(out).toContain("GAGANFOXWELL_GBRAIN_PROBE_TIMEOUT_MS");
    } finally {
      env.cleanup();
    }
