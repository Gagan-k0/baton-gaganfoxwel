/**
 * Project-slug consistency for gaganfoxwell-repo-mode.
 *
 * These are end-to-end reproductions of #2212. Each case is a real local Git
 * workspace which predates its public GitHub origin: gaganfoxwell-slug caches the
...
const REPO_MODE_BIN = path.join(ROOT, 'bin', 'gaganfoxwell-repo-mode');
...
describe('gaganfoxwell-repo-mode cached slug consistency (#2212)', () => {
  for (const workspace of publicWorkspaces) {
    test(`${workspace.origin} keeps its pre-origin project cache`, () => {
      ...
      try {
        ...
        git(['config', 'user.name', 'gaganfoxwell regression test'], project, home);
        git(['config', 'user.email', 'test@gaganfoxwell.dev'], project, home);

        // This workspace existed and used gaganfoxwell before it adopted GitHub.
  ...
  test('still treats a workspace without origin as unknown', () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(home, '.gaganfoxwell', 'projects'))).toBe(false);
    } finally {
      fs.rmSync(home, { recursive: true, force: true });
  ...
  test('sanitizes unusual origin URLs via gaganfoxwell-slug before mkdir', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'grepo-home-'));
    ...
    try {
      ...
      const projects = fs.readdirSync(path.join(home, '.gaganfoxwell', 'projects'));
