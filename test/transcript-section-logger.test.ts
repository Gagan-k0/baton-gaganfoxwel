...
describe('extractSectionReads', () => {
  test('picks up section reads via the /sections/<file>.md segment', () => {
    const result = {
      toolCalls: [
        read('/Users/x/.claude/skills/gaganfoxwell-ship/sections/version-bump.md'),
        read('ship/sections/changelog.md'),
        read('/abs/.factory/skills/gaganfoxwell-ship/sections/review-army.md'),
      ],
    };
  ...
  test('detects the full action fingerprint from bash + writes', () => {
    const result = {
      toolCalls: [
        ...
        bash('bun test'),
        bash('gaganfoxwell-version-bump --bump minor'),
