 ...
 * gbrain see when spawned from gaganfoxwell." The bug it prevents: gbrain's
...
describe("buildGbrainEnv", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "gaganfoxwell-build-env-"));
  ...
  it("leaves DATABASE_URL untouched when GAGANFOXWELL_RESPECT_ENV_DATABASE_URL=1", () => {
    writeFileSync(join(gbrainHome, "config.json"), JSON.stringify({ database_url: "postgresql://gbrain/db" }));
    const baseEnv = {
      HOME: home,
      DATABASE_URL: "postgresql://intentional/app-db",
      GAGANFOXWELL_RESPECT_ENV_DATABASE_URL: "1",
    };
    const result = buildGbrainEnv({ baseEnv });
