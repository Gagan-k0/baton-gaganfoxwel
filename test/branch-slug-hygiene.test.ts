 ...
 *
 * gaganfoxwell-review-log WRITES `<canonical-branch>-reviews.jsonl` where the
 * canonical form comes from bin/gaganfoxwell-slug (tr '/' '-' then
 ...
 *     gaganfoxwell-slug eval that opens Context Recovery).
...
describe('branch slug hygiene (#2550, #1851)', () => {
  ...
  test('Context Recovery probes reviews.jsonl with the slug-canonical $BRANCH', () => {
    ...
    // The gaganfoxwell-slug eval that defines $BRANCH must render BEFORE the probe.
    const evalIdx = out.indexOf('gaganfoxwell-slug');
  ...
  test('plan content-search BRANCH uses the full gaganfoxwell-slug canonical pipeline', () => {
  ...
  test('live round-trip: gaganfoxwell-review-log writes, Context Recovery probe finds it (slash branch)', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-home-'));
    const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-repo-'));
    try {
      ...
      // Writer: the real gaganfoxwell-review-log (canonicalizes via gaganfoxwell-slug).
      execSync(
        `"${path.join(ROOT, 'bin', 'gaganfoxwell-review-log')}" '{"skill":"ship","status":"ok"}'`,
        { cwd: repo, env, encoding: 'utf-8' },
      );

      // The slug-canonical filename must exist; the raw form must not.
      const slugVars = execSync(`"${path.join(ROOT, 'bin', 'gaganfoxwell-slug')}"`, {
        cwd: repo, env, encoding: 'utf-8',
      });
      ...
      // Reader: execute the rendered probe line with $BRANCH from gaganfoxwell-slug.
      const ctx: TemplateContext = {
