...
describeE2E('/setup-gbrain Path 4 (Remote MCP) — happy path', () => {
  test('verifies, registers HTTP MCP, never writes token to CLAUDE.md', async () => {
    ...
    const gaganfoxwellHome = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-remote-'));
    const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-remote-bin-'));
    const callLog = makeFakeClaude(fakeBinDir);

    // The skill writes CLAUDE.md in cwd. Use gaganfoxwellHome as cwd so we
    // can inspect it after the run.
    fs.writeFileSync(path.join(gaganfoxwellHome, 'CLAUDE.md'), '# Test project\n');
    ...
    // hermetic-env scrubs GBRAIN_*/GAGANFOXWELL_* by allowlist — this test's token
    ...
    try {
      ...
      const claudeMd = fs.readFileSync(path.join(gaganfoxwellHome, 'CLAUDE.md'), 'utf-8');
      ...
      // footer contains the literal "GAGANFOXWELL REVIEW REPORT" — a guaranteed
      // false positive on both trees.
      const wroteBefore = /## GAGANFOXWELL REVIEW REPORT|critical_gaps/i.test(result.output);
