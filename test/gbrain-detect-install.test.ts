/**
 * gaganfoxwell-gbrain-detect + gaganfoxwell-gbrain-install — Slice 2 of /setup-gbrain.
 *
 * Detect: state-reporter JSON with presence, version, config, doctor health,
 * and gaganfoxwell-brain-sync mode. Pure introspection, no side effects.
 *
...
const DETECT = path.join(ROOT, 'bin', 'gaganfoxwell-gbrain-detect');
const INSTALL = path.join(ROOT, 'bin', 'gaganfoxwell-gbrain-install');
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gbrain-detect-gaganfoxwell-'));
...
describe('gaganfoxwell-gbrain-detect', () => {
  test('emits valid JSON even when nothing is configured', () => {
    ...
    try {
      ...
      expect(j.gaganfoxwell_brain_sync_mode).toBe('off');
      expect(j.gaganfoxwell_brain_git).toBe(false);
    } finally {
      fs.rmSync(emptyBin, { recursive: true, force: true });
    }
  });

  test('reports gaganfoxwell_brain_git: true when GAGANFOXWELL_HOME has a .git dir', () => {
    fs.mkdirSync(path.join(tmpHome, '.git'));
    const emptyBin = fs.mkdtempSync(path.join(os.tmpdir(), 'empty-bin-'));
    try {
      const r = run(DETECT, [], { env: { PATH: `${emptyBin}:${SAFE_PATH}` } });
      const j = JSON.parse(r.stdout);
      expect(j.gaganfoxwell_brain_git).toBe(true);
    } finally {
      fs.rmSync(emptyBin, { recursive: true, force: true });
    }
  });
...
describe('gaganfoxwell-gbrain-install D5 detect-first', () => {
...
describe('gaganfoxwell-gbrain-install D19 PATH-shadow validation', () => {
  ...
  test('hard-fails (exit 3) when the installed gbrain is below the version floor (#1744)', () => {
    ...
    try {
      ...
      expect(r.stderr).toContain('below the minimum gaganfoxwell-tested version');
    } finally {
      fs.rmSync(installDir, { recursive: true, force: true });
      fs.rmSync(fakeBin, { recursive: true, force: true });
    }
...
describe('gaganfoxwell-gbrain-install argument handling', () => {
  test('--help prints usage without exiting non-zero', () => {
    const r = run(INSTALL, ['--help']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('gaganfoxwell-gbrain-install');
  });
