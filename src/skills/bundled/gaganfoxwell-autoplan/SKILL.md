---
name: gaganfoxwell-autoplan
preamble-tier: 3
version: 1.0.0
description: Auto-review pipeline — reads the full CEO, design, eng, and DX review skills from disk and runs them sequentially with auto-decisions using 6 decision principles. (gaganfoxwell)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"autoplan","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"autoplan","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Read the `/office-hours` skill file at `~/.claude/skills/gaganfoxwell/office-hours/SKILL.md` using the Read tool.
...
SLUG=$(~/.claude/skills/gaganfoxwell/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
_LOCALDOC=$(ls -t ~/.gaganfoxwell/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$_LOCALDOC" ] && _LOCALDOC=$(ls -t ~/.gaganfoxwell/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
# Repo-local docs win when at least as fresh (#703): office-hours dual-writes
# docs/designs/ alongside ~/.gaganfoxwell, and the committed copy is what teammates
...
> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/gaganfoxwell). These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents Codex from discovering gaganfoxwell skill files on disk and following their
...
```bash
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)" && mkdir -p ~/.gaganfoxwell/projects/$SLUG
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-')
DATETIME=$(date +%Y%m%d-%H%M%S)
echo "RESTORE_PATH=$HOME/.gaganfoxwell/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t ~/.gaganfoxwell/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- `~/.claude/skills/gaganfoxwell/plan-ceo-review/SKILL.md`
- `~/.claude/skills/gaganfoxwell/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/.claude/skills/gaganfoxwell/plan-eng-review/SKILL.md`
- `~/.claude/skills/gaganfoxwell/plan-devex-review/SKILL.md` (only if DX scope detected)
...
source ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-codex-probe
...
if [ "$_CODEX_CFG" = "disabled" ]; then
  echo "[codex disabled by config — Claude-only voices] Re-enable: gaganfoxwell-config set codex_reviews enabled"
...
# with Claude subagent only (autoplan's existing degradation fallback).
  ...
  _gaganfoxwell_codex_log_event "codex_cli_missing"
  echo "[codex-unavailable: binary not found] — proceeding with Claude subagent only"
  _CODEX_AVAILABLE=false
elif ! _gaganfoxwell_codex_auth_probe >/dev/null; then
  _gaganfoxwell_codex_log_event "codex_auth_failed"
...
# ~10s on first run, cached 1h; timeouts fail open (probe returns 0).
  ...
  _gaganfoxwell_codex_version_check   # non-blocking warn if known-bad
...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  _gaganfoxwell_codex_timeout_wrapper 600 codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/gaganfoxwell). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.
  ...
  if [ "$_CODEX_EXIT" = "124" ]; then
    _gaganfoxwell_codex_log_event "codex_timeout" "600"
    _gaganfoxwell_codex_log_hang "autoplan" "0"
...
- Test plan: generate artifact at `~/.gaganfoxwell/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at ~/.gaganfoxwell/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
```bash
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)"
TASKS_DIR="${HOME}/.gaganfoxwell/projects/${SLUG:-unknown}"
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
