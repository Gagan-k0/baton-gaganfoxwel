---
name: gaganfoxwell-spec
preamble-tier: 3
version: 0.1.0
description: Turn vague intent into a precise, executable spec in five phases. (gaganfoxwell)
allowed-tools:
  - Bash
  - Read
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"spec","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
gh issue list --search "<keywords>" --state open --limit 10 --json number,title,url 2>/dev/null \
  | jq -r '.[] | "#\(.number) \(.title)"' \
  | ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-issue-guard --stdin --source issue-dedupe 2>/dev/null || true
```
...
bun ~/.claude/skills/gaganfoxwell/lib/redact-audit-log.ts \
...
# Resolve visibility once; cache + reuse. Order: local config (~/.gaganfoxwell, never
# committed) → gh → glab → unknown(=public-strict).
REDACT_VIS=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get redact_repo_visibility 2>/dev/null)
...
REDACT_BODY_EOF
REDACT_JSON=$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
Read `GAGANFOXWELL_PLAN_MODE` from the environment (emitted by the preamble bash at
the top of this skill). Then:

1. **`--file-only` or `--no-execute` flag present** → file-only path.
2. **`--execute` flag present** → file + spawn path.
3. **No flag, `GAGANFOXWELL_PLAN_MODE=active`** → file-only path. Also load the spec
   into the active plan file (specified by `--plan-file <path>` or inferred from
   harness context as the work-to-do).
4. **No flag, `GAGANFOXWELL_PLAN_MODE=inactive`** → file + spawn path. The default in
...
reuse it; write the exact bytes to `$REDACT_FILE`; `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-redact --from-file "$REDACT_FILE"
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>/dev/null || true
```
...
Resolve the archive path via the existing `gaganfoxwell-paths` helper (handles
`GAGANFOXWELL_HOME`, `CLAUDE_PLUGIN_DATA`, Windows fallback):

```bash
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-paths)"
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug)"
ARCHIVE_DIR="$GAGANFOXWELL_STATE_ROOT/projects/$SLUG/specs"
mkdir -p "$ARCHIVE_DIR"
...
spec_plan_mode: ${GAGANFOXWELL_PLAN_MODE:-unset}
