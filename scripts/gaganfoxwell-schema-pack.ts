/**
 * gaganfoxwell-core@1.0.0 schema pack (T1 / Phase 0).
 *
 * Defines the 7 typed page kinds gaganfoxwell writes into a personal gbrain:
 *   gaganfoxwell/user-profile, gaganfoxwell/product, gaganfoxwell/goal, gaganfoxwell/developer-persona,
 *   gaganfoxwell/brand, gaganfoxwell/competitive-intel, gaganfoxwell/skill-run
 *
 * Plus the typed take kind gaganfoxwell writes for Phase 2 calibration:
 *   gaganfoxwell/take (kind=bet, holder=<user>, with expected_resolution_date)
 *
 ...
 * `mcp__gbrain__schema_graph` to render the gaganfoxwell subgraph correctly.
 */

import {
  GAGANFOXWELL_SCHEMA_PACK_NAME,
  GAGANFOXWELL_SCHEMA_PACK_VERSION,
...
export interface SchemaTypeDefinition {
  /** Page type slug, e.g. `gaganfoxwell/product`. */
  type: string;
...
const USER_PROFILE: SchemaTypeDefinition = {
  type: 'gaganfoxwell/user-profile',
  description:
    'Cross-project profile of the gaganfoxwell user: tone/conviction patterns, ' +
  ...
  fields: [
    { name: 'type', type: 'string', required: true, description: 'gaganfoxwell/user-profile' },
    { name: 'slug', type: 'string', required: true, description: 'gaganfoxwell/user-profile/<user-slug>' },
  ...
  emits_links: [
    { verb: 'has_calibration', target_type: 'gaganfoxwell/take' },
  ],
};

const PRODUCT: SchemaTypeDefinition = {
  type: 'gaganfoxwell/product',
  description:
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'gaganfoxwell/product' },
    { name: 'slug', type: 'string', required: true, description: 'gaganfoxwell/product/<project-slug>' },
  ...
  emits_links: [
    { verb: 'targets', target_type: 'gaganfoxwell/goal' },
    { verb: 'observed_by', target_type: 'gaganfoxwell/developer-persona' },
    { verb: 'has_brand', target_type: 'gaganfoxwell/brand' },
    { verb: 'competes_with', target_type: 'gaganfoxwell/competitive-intel' },
    { verb: 'history', target_type: 'gaganfoxwell/skill-run' },
  ],
};

const GOAL: SchemaTypeDefinition = {
  type: 'gaganfoxwell/goal',
  description:
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'gaganfoxwell/goal' },
    { name: 'slug', type: 'string', required: true, description: 'gaganfoxwell/goal/<project-slug>/<goal-id>' },
  ...
  emits_links: [
    { verb: 'belongs_to', target_type: 'gaganfoxwell/product' },
  ],
};

const DEVELOPER_PERSONA: SchemaTypeDefinition = {
  type: 'gaganfoxwell/developer-persona',
  description:
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'gaganfoxwell/developer-persona' },
    { name: 'slug', type: 'string', required: true, description: 'gaganfoxwell/developer-persona/<project-slug>' },
...
const BRAND: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'gaganfoxwell/brand' },
    { name: 'slug', type: 'string', required: true, description: 'gaganfoxwell/brand/<project-slug>' },
...
const COMPETITIVE_INTEL: SchemaTypeDefinition = {
  type: 'gaganfoxwell/competitive-intel',
  description:
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'gaganfoxwell/competitive-intel' },
    { name: 'slug', type: 'string', required: true, description: 'gaganfoxwell/competitive-intel/<project-slug>' },
...
const SKILL_RUN: SchemaTypeDefinition = {
  type: 'gaganfoxwell/skill-run',
  description:
    'Every gaganfoxwell skill invocation that produces output writes one of these on completion. ' +
    'Time-series log of decisions, modes, mode-selected, outcomes. Powers /retro ' +
    'and (deferred) /gaganfoxwell-reflect. Auto-archives to summary-only after 90 days.',
  retention: 'archive-after-90d',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'gaganfoxwell/skill-run' },
    { name: 'slug', type: 'string', required: true, description: 'gaganfoxwell/skill-run/<project>/<skill>/<timestamp>' },
  ...
  emits_links: [
    { verb: 'related_to', target_type: 'gaganfoxwell/product' },
    { verb: 'related_to', target_type: 'gaganfoxwell/goal' },
    { verb: 'writes_bet', target_type: 'gaganfoxwell/take' },
  ],
};

const TAKE: SchemaTypeDefinition = {
  type: 'gaganfoxwell/take',
  description:
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'gaganfoxwell/take' },
    { name: 'slug', type: 'string', required: true, description: 'gaganfoxwell/take/<project>/<date>/<id>' },
  ...
  emits_links: [
    { verb: 'belongs_to', target_type: 'gaganfoxwell/user-profile' },
    { verb: 'origin', target_type: 'gaganfoxwell/skill-run' },
  ],
};
...
export const GAGANFOXWELL_CORE_SCHEMA_PACK: SchemaPackJSON = {
  name: GAGANFOXWELL_SCHEMA_PACK_NAME,
  version: GAGANFOXWELL_SCHEMA_PACK_VERSION,
  page_types: [
    USER_PROFILE,
    PRODUCT,
    GOAL,
...
export function getSchemaPackMutationPayload(): {
  ...
  return {
    schema_pack: GAGANFOXWELL_CORE_SCHEMA_PACK,
...
export function getSchemaPackTypeNames(): ReadonlyArray<string> {
  return GAGANFOXWELL_CORE_SCHEMA_PACK.page_types.map((t) => t.type);
}
...
export function getRetentionPolicy(pageType: string): SchemaTypeDefinition['retention'] {
  const def = GAGANFOXWELL_CORE_SCHEMA_PACK.page_types.find((t) => t.type === pageType);
