...
// When --respect-detection is passed, read ~/.gaganfoxwell/gbrain-detection.json
...
// bin/gaganfoxwell-gbrain-detect and persisted by `gaganfoxwell-config gbrain-refresh`
// or by ./setup.
//
...
function loadGbrainOverride(): { detected: boolean } {
  ...
  const stateDir = process.env.GAGANFOXWELL_HOME || path.join(process.env.HOME || '', '.gaganfoxwell');
  ...
  try {
    ...
    // "ok", matching gaganfoxwell-gbrain-detect --is-ok.
...
// section-base path (`~/.claude/skills/gaganfoxwell/<skill>/sections/`) inside the
 ...
 * docs/ references keep pointing at `~/.claude/skills/gaganfoxwell` (the global
...
function rewriteSectionBase(content: string): string {
  ...
  return content.replace(
    /~\/\.claude\/skills\/gaganfoxwell\/([^\s)`"'*]+\/sections\/)/g,
    `${OUT_DIR}/$1`,
  );
}
...
function externalSkillName(skillDir: string, frontmatterName?: string): string {
  // Root skill (skillDir === '' or '.') always maps to 'gaganfoxwell' regardless of frontmatter
  if (skillDir === '.' || skillDir === '') return 'gaganfoxwell';
  ...
  // Don't double-prefix: gaganfoxwell-upgrade → gaganfoxwell-upgrade (not gaganfoxwell-gaganfoxwell-upgrade)
  if (baseName.startsWith('gaganfoxwell-')) return baseName;
  return `gaganfoxwell-${baseName}`;
}
...
// "(gaganfoxwell)" tag. This pile is the always-loaded catalog surface — every
// session pays for the full text. The catalog trim splits the description
// into a one-line catalog entry (lead sentence + "(gaganfoxwell)") that stays in
...
export function splitCatalogDescription(description: string): CatalogParts {
  ...
  const hasGaganfoxwellTag = /\(gaganfoxwell\)/.test(working);
  if (hasGaganfoxwellTag) working = working.replace(/\(gaganfoxwell\)/, '').trim();
  ...
  return { lead, routingProse, voiceLine, hasGaganfoxwellTag };
}
...
export function buildTrimmedDescription(parts: CatalogParts): string {
  ...
  const suffix = parts.hasGaganfoxwellTag ? ' (gaganfoxwell)' : '';
  return `${lead}${suffix}`;
}
 ...
 *  - shorten frontmatter `description:` to lead + (gaganfoxwell)
...
export function applyCatalogTrim(content: string, skillName: string): { content: string; parts: CatalogParts } | null {
  ...
  // If lead + (gaganfoxwell) is already most of the text, no trim needed.
...
for (const currentHost of hostsToRun) {
  ...
  try {
    ...
    // Generate the OpenClaw orchestrator-injection docs (gaganfoxwell-lite / gaganfoxwell-full /
    // gaganfoxwell-plan CLAUDE.md snippets). Sources live in openclaw/templates/ —
    ...
    if (currentHost === 'openclaw' && !DRY_RUN) {
      ...
      for (const variant of ['lite', 'full', 'plan'] as const) {
        const fileName = `gaganfoxwell-${variant}-CLAUDE.md`;
...
if (!DRY_RUN) {
  try {
    const configPath = path.join(process.env.HOME || '', '.gaganfoxwell', 'config.yaml');
    if (fs.existsSync(configPath)) {
      const config = fs.readFileSync(configPath, 'utf-8');
      if (/^skill_prefix:\s*true/m.test(config)) {
        console.log('\nNote: skill_prefix is true. Run gaganfoxwell-relink to re-apply name: patches.');
      }
    }
  } catch { /* non-fatal */ }
}

// Regenerate gaganfoxwell/llms.txt — single-file capability index for AI agents.
  ...
  void (async () => {
    try {
      ...
      if (result.warnings.length > 0) {
        ...
        console.log(`[gen-llms-txt] gaganfoxwell/llms.txt: ${result.skills.length} skills, ${result.browseCommands.length} browse commands`);
      }
    } catch (err) {
