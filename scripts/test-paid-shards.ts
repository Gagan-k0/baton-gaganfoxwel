 ...
 *   3. No per-shard env / eval dir. Each shard needs its own GAGANFOXWELL_EVAL_DIR
...
export interface RunShardsOptions {
  ...
  /** When set, each shard child gets GAGANFOXWELL_EVAL_DIR=<evalDirBase>/shards/<slug>/. */
  evalDirBase?: string;
...
export async function runPaidShard(
  ...
  if (options.evalDirBase) {
    env.GAGANFOXWELL_EVAL_DIR = path.join(options.evalDirBase, 'shards', shardSlug(files));
  }
...
async function main(): Promise<number> {
  ...
  const runSummary = await runPaidShards(runnable, {
    ...
    evalDirBase: process.env.GAGANFOXWELL_EVAL_DIR || getProjectEvalDir(),
  });
