#!/bin/bash
# Build Gaganfoxwell Browser.app — macOS application bundle
#
...
#
# Output: dist/Gaganfoxwell Browser.app and dist/Gaganfoxwell-Browser.dmg
#
# Usage:
...
APP_NAME="Gaganfoxwell Browser"
BUNDLE_ID="com.gaganfoxwell.browser"
...
# Launcher script
cp "$ROOT/scripts/app/gaganfoxwell-browser" "$APP_DIR/Contents/MacOS/gaganfoxwell-browser"
chmod +x "$APP_DIR/Contents/MacOS/gaganfoxwell-browser"
...
# Patch the bundled Chromium's Info.plist so macOS shows "Gaganfoxwell Browser"
...
if [ -f "$CHROMIUM_PLIST" ]; then
  ...
  # Replace Chromium's icon with ours so the Dock shows the Gaganfoxwell icon
...
<dict>
  ...
  <string>gaganfoxwell-browser</string>
...
echo "    Contents/MacOS/gaganfoxwell-browser     (launcher)"
...
DMG_PATH="$BUILD_DIR/Gaganfoxwell-Browser.dmg"
echo ""
echo "  Creating DMG..."
rm -f "$DMG_PATH"
