#!/usr/bin/env bun
/**
 * analytics — CLI for viewing gaganfoxwell skill usage statistics.
 *
 * Reads ~/.gaganfoxwell/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.gaganfoxwell', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('gaganfoxwell skill usage analytics');
