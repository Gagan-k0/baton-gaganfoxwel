...
function regenerateAndValidate() {
  ...
  // (.claude/gaganfoxwell-rendered), refresh it too so live template edits reflect at
  // this workspace's runtime. Only runs when the render dir already exists — we
  // never create it during plain template dev.
  const RENDER_DIR = path.join(ROOT, '.claude', 'gaganfoxwell-rendered');
  if (fs.existsSync(RENDER_DIR)) {
    try {
      ...
      console.log('  [render] refreshed .claude/gaganfoxwell-rendered (brain-aware workspace copy)');
    } catch (err: any) {
