...
// writes ~/.gaganfoxwell-dev/e2e-live.json regardless of project ("heartbeat stays
// global"). The PARTIAL file is per-project: EvalCollector writes it into
// getProjectEvalDir() (or GAGANFOXWELL_EVAL_DIR), so watching the legacy global
// path missed it whenever slug detection succeeded — i.e. the normal case.
const HEARTBEAT_PATH = path.join(GAGANFOXWELL_DEV_DIR, 'e2e-live.json');
const PARTIAL_PATH = path.join(
  process.env.GAGANFOXWELL_EVAL_DIR || getProjectEvalDir(),
  '_partial-e2e.json',
);
...
export function renderDashboard(heartbeat: HeartbeatData | null, partial: PartialData | null): string {
  ...
  if (heartbeat?.runId) {
    const logPath = path.join(GAGANFOXWELL_DEV_DIR, 'e2e-runs', heartbeat.runId, 'progress.log');
