...
/**
 * gaganfoxwell's AI slop anti-patterns — shared between DESIGN_METHODOLOGY and DESIGN_HARD_RULES.
 *
 ...
 * it. Deliberate: gaganfoxwell wants deterministic cached search for review
 ...
 *   2. sources `gaganfoxwell-codex-probe`,
 *   3. runs `command -v codex` (literal — keeps the e2e substring assertion),
 *      then `_gaganfoxwell_codex_auth_probe`, then `_gaganfoxwell_codex_version_check`,
 *   4. logs the relevant `_gaganfoxwell_codex_log_event` for each non-ready outcome,
 ...
 * literal `~/.claude/skills/gaganfoxwell` is correct here and becomes `$GAGANFOXWELL_ROOT`
...
export function codexPreflight(opts: { modeVar?: string; disabledBehavior: 'skip-all' | 'codex-only' }): string {
  ...
  const disabledLine = opts.disabledBehavior === 'codex-only'
    ...
    : 'Skip this section entirely; do NOT fall back to a Claude subagent — disabled means no extra review step. Print: "Codex review skipped (codex_reviews disabled). Re-enable: `gaganfoxwell-config set codex_reviews enabled`."';
  return `\`\`\`bash
...
# the nested passes anyway.
  ...
  ${m}="not_installed"; _gaganfoxwell_codex_log_event "codex_cli_missing" 2>/dev/null || true
elif ! _gaganfoxwell_codex_auth_probe >/dev/null 2>&1; then
  ${m}="not_authed"; _gaganfoxwell_codex_log_event "codex_auth_failed" 2>/dev/null || true
elif ! _gaganfoxwell_codex_model_probe; then
  ${m}="model_unusable"
else
  ${m}="ready"; _gaganfoxwell_codex_version_check 2>/dev/null || true
fi
echo "CODEX_MODE: $${m}"
\`\`\`
