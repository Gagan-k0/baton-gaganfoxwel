 ...
 *
 * GBrain is a "mod" for gaganfoxwell. When installed, coding skills become brain-aware:
 ...
 * the bin/gaganfoxwell-brain-cache CLI and scripts/brain-cache-spec.ts. The new
 ...
 * bash script that:
 *   1. Reads each digest file from gaganfoxwell-brain-cache get (one call per digest)
...
export function generateBrainPreflight(ctx: TemplateContext): string {
  ...
  const loadLines = subset.map((entityName) => {
    ...
    return `  printf '\\n### %s\\n\\n' "${entityName}"\n  ${binDir}/gaganfoxwell-brain-cache get ${entityName} ${projectFlag} 2>/dev/null || printf '_(no ${entityName} digest available yet)_\\n'`;
  }).join('\n');
...
\`\`\`bash
eval "$(${binDir}/gaganfoxwell-slug 2>/dev/null)" 2>/dev/null || true
{
...
\`\`\`bash
eval "$(${binDir}/gaganfoxwell-slug 2>/dev/null)" 2>/dev/null || true
(${binDir}/gaganfoxwell-brain-cache refresh --project "$SLUG" 2>/dev/null &) || true
\`\`\`
`;
}
...
export function generateBrainWriteBack(ctx: TemplateContext): string {
  ...
  const invalidateBash = invalidatesEntities
    .map((e) => `  ${ctx.paths.binDir}/gaganfoxwell-brain-cache invalidate ${e} --project "$SLUG" 2>/dev/null || true`)
    .join('\n');
...
1. Brain trust policy for the active endpoint is \`personal\` (check via
   \`${ctx.paths.binDir}/gaganfoxwell-config get brain_trust_policy@<endpoint-hash>\`).
