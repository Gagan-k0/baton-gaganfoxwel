 ...
 *
 * Review logs are stored locally at ~/.gaganfoxwell/reviews/review-log.jsonl.
...
echo '{"skill":"${_ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.gaganfoxwell/analytics/spec-review.jsonl 2>/dev/null || true
\`\`\`
...
_gaganfoxwell_codex_timeout_wrapper 540 codex exec "${CODEX_BOUNDARY}Review the changes on this branch against the base branch. Run DIFF_BASE=$(git merge-base origin/<base> HEAD) && git diff "$DIFF_BASE" to see the diff. Your job is to find ways this code will fail in production. Think like an attacker and a chaos engineer. Find edge cases, race conditions, security holes, resource leaks, failure modes, and silent data corruption paths. Be adversarial. Be thorough. No compliments — just the problems. End your output with ONE line in the canonical format \`Recommendation: <action> because <one-line reason naming the most exploitable finding>\`. Generic reasons like 'because it's safer' do not qualify; the reason must point to a specific finding or no-fix rationale." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' ${CODEX_WEB_SEARCH_FLAG} < /dev/null 2>"$TMPERR_ADV"
\`\`\`
...
# unwrapped fallback), added in #1056 but never wired into this call site.
source ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-codex-probe 2>/dev/null || true
_gaganfoxwell_codex_timeout_wrapper 540 codex review --base <base> -c 'model_reasoning_effort="high"' ${CODEX_WEB_SEARCH_FLAG} < /dev/null 2>"$TMPERR"
\`\`\`
...
When the mode is \`ready\`, \`not_installed\`, or \`not_authed\`, print one line so the off-switch
stays discoverable: "Running the Codex doc review automatically (standard step). Disable: \`gaganfoxwell-config set codex_reviews disabled\`."
...
**Persist the result:**
\`\`\`bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
