 ...
 *   1. If ~/.gaganfoxwell-artifacts-remote.txt (or legacy ~/.gaganfoxwell-brain-remote.txt
 *      during the v1.27.0.0 migration window) exists AND ~/.gaganfoxwell/.git is
 *      missing, surface a restore-available hint (does NOT auto-run restore).
 *   2. If sync is on, run `gaganfoxwell-brain-sync --once` (drain + push). The
 ...
 * to `gaganfoxwell-brain-sync --discover-new` + `--once`.
 */
...
_BRAIN_SYNC_BIN="${quoteSafePath(ctx.paths.binDir)}/gaganfoxwell-brain-sync"
_BRAIN_CONFIG_BIN="${quoteSafePath(ctx.paths.binDir)}/gaganfoxwell-config"
...
${isBrainHost ? `If output shows \`ARTIFACTS_SYNC: artifacts repo detected\`, offer \`gaganfoxwell-brain-restore\` via AskUserQuestion; otherwise continue.` : ''}
...
\`\`\`bash
"${quoteSafePath(ctx.paths.binDir)}/gaganfoxwell-brain-sync" --discover-new 2>/dev/null || true
"${quoteSafePath(ctx.paths.binDir)}/gaganfoxwell-brain-sync" --once 2>/dev/null || true
\`\`\`
`;
}
