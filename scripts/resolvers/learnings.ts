 ...
 *
 * Learnings are stored per-project at ~/.gaganfoxwell/projects/{slug}/learnings.jsonl.
 ...
 * by gaganfoxwell-learnings-search ("latest winner" per key+type).
 *
 * Cross-project discovery is opt-in. The resolver asks the user once via
 * AskUserQuestion and persists the preference via gaganfoxwell-config.
 */
...
// queries hand-written in gaganfoxwell are safe, but the resolver API must defend against
...
export function generateLearningsSearch(ctx: TemplateContext, args?: string[]): string {
  ...
  if (getHostConfig(ctx.host).learningsMode === 'basic') {
    ...
    // uses $GAGANFOXWELL_BIN (all basic hosts are env-var hosts)
    return `## Prior Learnings

Search for relevant learnings from previous sessions on this project:

\`\`\`bash
$GAGANFOXWELL_BIN/gaganfoxwell-learnings-search --limit 10${queryFlag} 2>/dev/null || true
\`\`\`
...
\`\`\`bash
_CROSS_PROJ=$(${ctx.paths.binDir}/gaganfoxwell-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ${ctx.paths.binDir}/gaganfoxwell-learnings-search --limit 10${queryFlag} --cross-project 2>/dev/null || true
else
  ${ctx.paths.binDir}/gaganfoxwell-learnings-search --limit 10${queryFlag} 2>/dev/null || true
fi
\`\`\`
...
If A: run \`${ctx.paths.binDir}/gaganfoxwell-config set cross_project_learnings true\`
If B: run \`${ctx.paths.binDir}/gaganfoxwell-config set cross_project_learnings false\`
...
export function generateLearningsLog(ctx: TemplateContext): string {
  const binDir = ctx.paths.binDir; // env-var hosts already resolve to $GAGANFOXWELL_BIN via types.ts
...
\`\`\`bash
${binDir}/gaganfoxwell-learnings-log '{"skill":"${ctx.skillName}","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
\`\`\`
