 ...
 * `~/.claude/skills/gaganfoxwell/bin`. Bash only performs tilde expansion when the
 ...
 *
 * Env-var hosts already use `$GAGANFOXWELL_BIN`, which expands correctly when
 * quoted, so they pass through untouched.
 *
 * Use this ONLY where the path lands inside double quotes. Unquoted
 * interpolations (`${ctx.paths.binDir}/gaganfoxwell-slug`) expand fine as-is and are
 ...
 * Non-Claude hosts use $GAGANFOXWELL_ROOT env vars (set by preamble).
 */
  ...
  for (const config of ALL_HOST_CONFIGS) {
    if (config.usesEnvVars) {
      paths[config.name] = {
        ...
        makePdfDir: '$GAGANFOXWELL_MAKE_PDF',
      };
    } else {
 ...
 * Claude-style dirs are `~`-rooted (e.g. `~/.claude/skills/gaganfoxwell/browse/dist`)
 * and expand via `$HOME`; env-var hosts already carry an absolute `$GAGANFOXWELL_*`
