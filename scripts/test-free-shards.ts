 ...
 *
 * Adapted from the McGluut/gaganfoxwell fork's test-free-shards.ts (190 LOC). The
...
export const KNOWN_WINDOWS_INCOMPATIBLE: Array<{ file: string; reason: string }> = [
  ...
  {
    file: 'test/gaganfoxwell-decision-semantic.test.ts',
...
const KNOWN_WINDOWS_SAFE: Array<{ file: string; reason: string }> = [
  ...
  },
  {
    ...
    // path.join(ROOT, 'bin', 'gaganfoxwell-uninstall') constant, but the script is
 ...
 * Deliberately NOT GAGANFOXWELL_HOME: injecting one shared scratch home for a whole
 ...
 * GAGANFOXWELL_HOME isolation mkdtemp their own per test — the repo convention —
...
export async function runFreeShard(
  ...
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-free-shard-'));
...
function nextDefaultLogPath(): string {
  ...
  logPathSequence += 1;
  return path.join(os.tmpdir(), `gaganfoxwell-free-test-${stamp}-${process.pid}-${logPathSequence}.log`);
}
