#!/usr/bin/env bun
/**
 * Generate gaganfoxwell/llms.txt — a single discoverable index of every gaganfoxwell
 ...
 *
 * Output: gaganfoxwell/llms.txt at repo root.
 *
...
const OUTPUT = path.join(ROOT, 'gaganfoxwell', 'llms.txt');
 ...
 * the gaganfoxwell convention; we collapse those into a single paragraph.
 */
 ...
 * in a full gaganfoxwell checkout; absent on minimal installs. Returns {} if the
...
export async function generateLlmsTxt(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  const lines: string[] = [];
  lines.push('# gaganfoxwell');
  lines.push('');
  lines.push("> gaganfoxwell is Gaganfoxwell Suite: AI coding skills + a fast headless browser binary + a design CLI. This file indexes every capability so agents can discover and invoke them without crawling individual SKILL.md files.");
  lines.push('');
  ...
  lines.push('## More');
  lines.push('');
  lines.push('- Repository: https://github.com/Gagan-k0/baton-gaganfoxwel.git');
  ...
  void (async () => {
    ...
    if (dryRun) {
      ...
      if (existing !== result.content) {
        console.error('[gen-llms-txt] OUT OF DATE — run `bun run gen:skill-docs` to regenerate gaganfoxwell/llms.txt');
        process.exit(1);
      }
