 ...
 * gaganfoxwell daemon over loopback HTTP.
 *
 * Distribution model:
 ...
 *
 * Auth resolution:
 *   1. GAGANFOXWELL_PORT + GAGANFOXWELL_SKILL_TOKEN env vars (set by `$B skill run` when
 ...
 *   2. State file fallback: read `BROWSE_STATE_FILE` env or `<git-root>/.gaganfoxwell/browse.json`
...
export interface BrowseClientOptions {
  /** Override port. Default: GAGANFOXWELL_PORT env or state file. */
  port?: number;
  /** Override token. Default: GAGANFOXWELL_SKILL_TOKEN env, then state file root token. */
  token?: string;
  /** Tab id to target (every command can scope to a tab). Default: BROWSE_TAB env or undefined (active tab). */
  tabId?: number;
  /** Per-request timeout in milliseconds. Default: 30_000. */
  timeoutMs?: number;
  /** Override state-file path. Default: BROWSE_STATE_FILE env or <git-root>/.gaganfoxwell/browse.json. */
  stateFile?: string;
}
...
export function resolveBrowseAuth(opts: BrowseClientOptions = {}): ResolvedAuth {
  ...
  const envPort = process.env.GAGANFOXWELL_PORT;
  const envToken = process.env.GAGANFOXWELL_SKILL_TOKEN;
  if (envPort && envToken) {
  ...
  throw new Error(
    ...
    '(sets GAGANFOXWELL_PORT + GAGANFOXWELL_SKILL_TOKEN) or run from a project with a live daemon ' +
    '(.gaganfoxwell/browse.json must exist).'
  );
}
...
function defaultStateFile(): string | null {
  try {
    ...
    return path.join(base, '.gaganfoxwell', 'browse.json');
  } catch {
    return path.join(process.cwd(), '.gaganfoxwell', 'browse.json');
  }
}
