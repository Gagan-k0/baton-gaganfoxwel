// gaganfoxwell telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/gaganfoxwell-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.gaganfoxwell_version || !event.os || !event.outcome) {
      ...
      // scaffold), handoff (P1 office-hours → next skill), route (gaganfoxwell router).
      const validTypes = [
        "skill_run",
        "upgrade_prompted",
        "upgrade_completed",
      ...
      rows.push({
        ...
        gaganfoxwell_version: String(event.gaganfoxwell_version).slice(0, 20),
      ...
      if (event.installation_id) {
        installationUpserts.set(event.installation_id, {
          version: event.gaganfoxwell_version,
          os: event.os,
        });
      }
    }
        ...
        .upsert(
          {
            ...
            gaganfoxwell_version: data.version,
            os: data.os,
          },
