// gaganfoxwell community-pulse edge function
...
Deno.serve(async () => {
  ...
  try {
    ...
    const { data: crashes, error: crashErr } = await supabase
      .from("crash_clusters")
      .select("error_class, gaganfoxwell_version, total_occurrences, identified_users")
      .limit(5);
    ...
    for (const row of versionRows ?? []) {
      if (row.gaganfoxwell_version) {
        versionCounts[row.gaganfoxwell_version] = (versionCounts[row.gaganfoxwell_version] ?? 0) + 1;
      }
    }
    ...
    // Fields emitted by gaganfoxwell-telemetry-log --event-type attack_attempt:
    ...
    // gaganfoxwell user who polls /community-pulse. With it, the dashboard shows
