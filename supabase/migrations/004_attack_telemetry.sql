-- gaganfoxwell attack telemetry — schema extension for prompt injection events.
--
-- Ships alongside the gaganfoxwell-telemetry-log `--event-type attack_attempt`
-- flag (bin/gaganfoxwell-telemetry-log, commits 28ce883c + f68fa4a9). These
-- columns are nullable so the existing skill_run events continue inserting
-- unchanged.
--
-- Fields (1:1 with gaganfoxwell-telemetry-log flags):
...
--   * salt is per-device local file (~/.gaganfoxwell/security/device-salt) —
