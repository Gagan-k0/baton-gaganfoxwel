...
// ~/.gaganfoxwell/cache/gen-accessors/<hash>/StateAccessor.swift.
...
struct GenAccessors {
    static func main() async {
        ...
        let cacheDir = getEnv("GAGANFOXWELL_IOS_CACHE_ROOT")
            ?? ("~/.gaganfoxwell/cache/gen-accessors" as NSString).expandingTildeInPath
    ...
    static func render(specs: [AccessorSpec], buildId: String, accessorHash: String) -> String {
        ...
        if !specs.isEmpty {
            ...
            out += "private enum _GaganfoxwellDebugBridgeSnapshotJSON {\n"
        ...
        for spec in specs {
            ...
            for (index, field) in spec.fields.enumerated() {
                ...
                out += "                }\n"
                out += "                guard let restored\(index): \(typeText) = _GaganfoxwellDebugBridgeSnapshotJSON.decode(raw\(index), as: \(typeText).self) else {\n"
            ...
            for (name, typeText) in spec.fields {
                ...
                out += "            write: { value in\n"
                out += "                guard let typed: \(typeText) = _GaganfoxwellDebugBridgeSnapshotJSON.decode(value, as: \(typeText).self) else { return false }\n"
