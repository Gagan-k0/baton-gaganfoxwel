...
//
// Invoked from bin/gaganfoxwell-ios-qa-mint.
...
function printHelp() {
  const help = `gaganfoxwell-ios-qa-mint — manage the tailnet allowlist for remote iOS QA agents

USAGE
  gaganfoxwell-ios-qa-mint grant  --remote <identity> [--capability <tier>] [--ttl <seconds>] [--note <text>]
  gaganfoxwell-ios-qa-mint revoke --remote <identity>
...
EXAMPLES
  ...
  gaganfoxwell-ios-qa-mint grant --remote 'tag:ci' --capability mutate --ttl 86400 --note 'nightly run'
  gaganfoxwell-ios-qa-mint revoke --remote 'alice@example.com'
  gaganfoxwell-ios-qa-mint list

The allowlist lives at ~/.gaganfoxwell/ios-qa-allowlist.json (mode 0600). The daemon's
...
if (import.meta.main) {
  main().catch((err) => {
    process.stderr.write(`gaganfoxwell-ios-qa-mint: ${(err as Error).message}\n`);
    process.exit(1);
  });
}
