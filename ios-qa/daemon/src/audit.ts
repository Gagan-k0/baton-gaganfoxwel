...
export function defaultAuditPath(): string {
  return process.env.GAGANFOXWELL_IOS_AUDIT_PATH
    ?? join(homedir(), '.gaganfoxwell', 'security', 'ios-qa-audit.jsonl');
}

export function defaultAttemptsPath(): string {
  return process.env.GAGANFOXWELL_IOS_ATTEMPTS_PATH
    ?? join(homedir(), '.gaganfoxwell', 'security', 'attempts.jsonl');
}
...
async function loadDeviceSalt(): Promise<string> {
  ...
  const path = join(homedir(), '.gaganfoxwell', 'security', 'device-salt');
  try {
