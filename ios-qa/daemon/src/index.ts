// gaganfoxwell-ios-qa-daemon entrypoint.
//
// Two listeners:
...
export async function startDaemon(opts: DaemonOptions): Promise<RunningDaemon | { error: string; reason?: string }> {
  ...
  if (opts.tailnetEnabled && probe?.ok) {
    ...
    const tailnetBindAddr = process.env.GAGANFOXWELL_IOS_TAILNET_BIND ?? '127.0.0.1';
...
if (import.meta.main) {
  const port = parseInt(process.env.GAGANFOXWELL_IOS_DAEMON_PORT ?? '9099', 10);
  const tailnet = process.argv.includes('--tailnet');
  const targetUDID = process.env.GAGANFOXWELL_IOS_TARGET_UDID;
  const bundleId = process.env.GAGANFOXWELL_IOS_TARGET_BUNDLE_ID ?? 'com.gaganfoxwell.iosqa.fixture';

  // Default tunnelProvider: when GAGANFOXWELL_IOS_TARGET_UDID (or a default with
