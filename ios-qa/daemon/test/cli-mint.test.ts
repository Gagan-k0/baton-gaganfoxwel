// CLI tests for gaganfoxwell-ios-qa-mint. Invokes the bash launcher end-to-end
...
// so the user's real ~/.gaganfoxwell/ios-qa-allowlist.json is untouched.
...
const MINT_BIN = join(ROOT, 'bin', 'gaganfoxwell-ios-qa-mint');
const DAEMON_BIN = join(ROOT, 'bin', 'gaganfoxwell-ios-qa-daemon');

function runMint(args: string[]) {
  return spawnSync(MINT_BIN, args, { stdio: 'pipe', encoding: 'utf-8' });
}

describe('bin/gaganfoxwell-ios-qa-mint launcher', () => {
  let tmpDir: string;
  let listPath: string;
  ...
  test('--help prints usage without touching allowlist', () => {
    ...
    expect(r.stdout).toContain('gaganfoxwell-ios-qa-mint');
...
describe('bin/gaganfoxwell-ios-qa-daemon launcher', () => {
