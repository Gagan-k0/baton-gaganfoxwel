...
describe('daemon — tailnet listener (mocked tailscaled)', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.GAGANFOXWELL_IOS_ALLOWLIST_PATH = listPath;
    process.env.GAGANFOXWELL_IOS_AUDIT_PATH = join(workDir, 'audit.jsonl');
    process.env.GAGANFOXWELL_IOS_ATTEMPTS_PATH = join(workDir, 'attempts.jsonl');
    process.env.GAGANFOXWELL_IOS_TAILNET_BIND = '127.0.0.1'; // safe test bind
  ...
  afterEach(async () => {
    ...
    delete process.env.GAGANFOXWELL_IOS_ALLOWLIST_PATH;
    delete process.env.GAGANFOXWELL_IOS_AUDIT_PATH;
    delete process.env.GAGANFOXWELL_IOS_ATTEMPTS_PATH;
    delete process.env.GAGANFOXWELL_IOS_TAILNET_BIND;
  ...
  test('audit log records mutating tailnet requests', async () => {
    ...
    const auditPath = process.env.GAGANFOXWELL_IOS_AUDIT_PATH!;
