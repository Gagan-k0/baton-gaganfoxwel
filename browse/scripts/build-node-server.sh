...
GAGANFOXWELL_DIR="$(cd "$(dirname "$0")/../.." && pwd)"
...
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*) GAGANFOXWELL_DIR="$(cygpath -m "$GAGANFOXWELL_DIR")" ;;
esac
SRC_DIR="$GAGANFOXWELL_DIR/browse/src"
DIST_DIR="$GAGANFOXWELL_DIR/browse/dist"
mkdir -p "$DIST_DIR"
