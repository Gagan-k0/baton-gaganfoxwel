...
preamble-tier: 1
version: 1.1.0
description: Fast headless browser for QA testing and site dogfooding. (gaganfoxwell)
triggers:
  - browse a page
  - headless browser
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"browse","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gaganfoxwell/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-timeline-log '{"skill":"browse","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```
Scale must be 1-3 (gaganfoxwell policy cap). Changing `--scale` recreates the browser context; refs from `snapshot` are invalidated (rerun `snapshot`), but `load-html` content is replayed automatically. Not supported in headed mode.
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/gaganfoxwell/browse/dist/browse` or `~/.claude/skills/gaganfoxwell/browse/dist/browse`).
