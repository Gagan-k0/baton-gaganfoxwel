/**
 * find-browse — locate the gaganfoxwell browse binary.
 *
...
export function locateBinary(): string | null {
  ...
  if (root) {
    for (const m of markers) {
      const local = join(root, m, 'skills', 'gaganfoxwell', 'browse', 'dist', 'browse');
    ...
    // - gaganfoxwell repo dev workflow before `./setup` runs
  ...
  // Global fallback
  for (const m of markers) {
    const global = join(home, m, 'skills', 'gaganfoxwell', 'browse', 'dist', 'browse');
