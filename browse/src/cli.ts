/**
 * gaganfoxwell CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .gaganfoxwell/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and gaganfoxwell
...
function chromiumProfileDir(): string {
  return path.join(process.env.HOME || '/tmp', '.gaganfoxwell', 'chromium-profile');
}
...
async function ensureServer(flags?: GlobalFlags): Promise<ServerState> {
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    ...
    console.error('[browse] The headed browser may have been closed. Run /open-gaganfoxwell-browser to restart.');
    process.exit(1);
  }
  ...
  if (state && state.mode === 'headed' && isProcessAlive(state.pid)) {
    ...
    console.error(`[browse] Run '/open-gaganfoxwell-browser' to restart.`);
    process.exit(1);
  }
...
/** Check if ngrok is installed and authenticated (native config or gaganfoxwell env). */
function isNgrokAvailable(): boolean {
  // Check gaganfoxwell's own ngrok env
  const ngrokEnvPath = path.join(process.env.HOME || '/tmp', '.gaganfoxwell', 'ngrok.env');
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (pairData.tunnel_url) {
    ...
    if (ngrokAvailable) {
      ...
      console.warn('[browse] For remote agents: enable once with `gaganfoxwell-config set pair_agent on` — or run /pair-agent, which asks for consent and sets it.\n');
      serverUrl = pairData.server_url;
    } else {
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/gaganfoxwell`;
      try {
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`gaganfoxwell browse — Fast headless browser for AI coding agents
...
Refs:           After 'snapshot', use @e1, @e2... as selectors:
  ...
  if (command === 'connect') {
    ...
    const SUPERVISOR_TICK_MS = parseInt(
      process.env.GAGANFOXWELL_SUPERVISOR_TICK_MS || '30000',
      10,
    );
    const SUPERVISOR_GUARD_WINDOW_MS = 5 * 60_000;
    const SUPERVISOR_GUARD_MAX = 5;
    const SUPERVISOR_BACKOFF_MS = (process.env.GAGANFOXWELL_SUPERVISOR_BACKOFF || '1000,2000,4000,8000,30000')
  ...
  if (command === 'stop') {
    ...
    // gaganfoxwell-upgrade's Step 4.8 sends users down exactly this path when a
  ...
  if (command === 'pair-agent') {
    ...
    if (state.mode !== 'headed' && !hasFlag(commandArgs, '--headless')) {
      if (pairAgentPreexistingDaemonAlive && !globalFlags.forceRestart) {
        ...
        console.log('[browse] Opening Gaganfoxwell Browser so you can see what the remote agent does...');
