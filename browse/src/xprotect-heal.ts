 ...
 *      a GAGANFOXWELL_CHROMIUM_PATH bundle belongs to the wrapper/embedder and is
 *      never touched (same scope contract as probePoisonedChromiumBundle).
 *   3. Force-reinstall Chromium FROM THE GAGANFOXWELL INSTALL ROOT (ENG-OV3: the
...
/**
 * Find the gaganfoxwell install root whose node_modules pins the SAME
 ...
 */
export function findGaganfoxwellInstallRoot(
  ...
  const roots = candidates ?? [
    ...
    // path '.claude/skills/gaganfoxwell' under the daemon's cwd — often an
    ...
    path.join(os.homedir(), '.claude', 'skills', 'gaganfoxwell'),
  ];
 ...
 * on a GAGANFOXWELL_CHROMIUM_PATH bundle — that belongs to the wrapper/embedder.
...
/**
 * Run `bunx playwright install --force chromium` from the gaganfoxwell install
...
export async function maybeHealXProtectKill(
  ...
  if (opts.usesCustomExecutable) {
    // A GAGANFOXWELL_CHROMIUM_PATH bundle belongs to the wrapper/embedder — never
  ...
  const root = (deps.installRoot ?? findGaganfoxwellInstallRoot)(revision);
  if (!root) {
...
export function buildXProtectGuidance(originalMessage: string): string {
  return (
    ...
    'Fix manually: run `bunx playwright install chromium` from your gaganfoxwell install ' +
    '(the directory whose node_modules pins playwright — ~/.claude/skills/gaganfoxwell for ' +
