 ...
 *   project:  <project>/.gaganfoxwell/browser-skills/<name>/
 *   global:   ~/.gaganfoxwell/browser-skills/<name>/
 *   bundled:  <gaganfoxwell-install>/browser-skills/<name>/   (read-only, ships with gaganfoxwell)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.gaganfoxwell', 'browser-skills') : null,
    global: path.join(home, '.gaganfoxwell', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <gaganfoxwell-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <gaganfoxwell-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the gaganfoxwell root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with gaganfoxwell and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the gaganfoxwell source tree.
 */
