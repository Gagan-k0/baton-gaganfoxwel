 ...
 *
 * Scope:
 *   - per-project: ~/.gaganfoxwell/projects/<slug>/learnings.jsonl
 *   - global:      ~/.gaganfoxwell/global-domain-skills.jsonl
 *
...
function globalFile(): string {
  return path.join(gaganfoxwellHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(gaganfoxwellHome(), 'projects', slug, 'learnings.jsonl');
}
