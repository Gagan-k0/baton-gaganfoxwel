/**
 * Cross-platform file permission restriction for sensitive gaganfoxwell state.
 *
 * Why this exists
 * ----------------
 * POSIX mode bits (`0o600` for files, `0o700` for dirs) are how gaganfoxwell marks
 ...
 * install, sensitive gaganfoxwell state files inherit whatever ACL the parent
...
function warnIcaclsFailure(fsPath: string, err: unknown): void {
  ...
  console.warn(
    `[gaganfoxwell] Failed to restrict Windows ACL on ${fsPath}: ${msg}\n` +
