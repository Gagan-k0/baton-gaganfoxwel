 ...
 * GAGANFOXWELL_STEALTH=extended, off by default) DOES fake plugins — that mode
 ...
 *      detectors flag.
 *   4. Report per-install hardware values via GAGANFOXWELL_HW_CONCURRENCY /
 *      GAGANFOXWELL_DEVICE_MEMORY env vars (set by gbd at startup via
 ...
 * on macOS), passed through the GAGANFOXWELL_* env vars. Each field falls
...
// the UA-CH cmdline switch in buildGaganfoxwellLaunchArgs (which reads GAGANFOXWELL_PLATFORM
...
export function readHostProfile(): HostProfile {
  ...
  const concurrency = Number(env.GAGANFOXWELL_HW_CONCURRENCY);
  const memory = Number(env.GAGANFOXWELL_DEVICE_MEMORY);
  return {
...
export function buildStealthScript(hw: HostProfile): string {
  ...
  // ──── Per-install hardware values from GAGANFOXWELL_* env (T2) ────
...
function extendedModeEnabled(): boolean {
  const v = process.env.GAGANFOXWELL_STEALTH;
 ...
 *   3. EXTENDED_STEALTH_SCRIPT — only when GAGANFOXWELL_STEALTH=extended (off by
 ...
 * disagree with the env-driven GPU spoof in buildGaganfoxwellLaunchArgs on non-Apple
...
/**
 * Build the `--gaganfoxwell-*=` cmdline switches that the Pack 1 Chromium
 * patches read (webgl-vendor-spoof, ua-client-hints-stealth, worker-
 * navigator-stealth). Values come from the GAGANFOXWELL_* env vars that
 ...
 *
 * TRUSTED-SOURCE ONLY. These GAGANFOXWELL_* values are populated by gbd from
 ...
 *   GAGANFOXWELL_GPU_VENDOR        → --gaganfoxwell-gpu-vendor        → webgl-vendor-spoof.patch
 *   GAGANFOXWELL_GPU_RENDERER      → --gaganfoxwell-gpu-renderer      → webgl-vendor-spoof.patch
 *   GAGANFOXWELL_PLATFORM          → --gaganfoxwell-ua-platform       → ua-client-hints-stealth.patch
 *                              (maps MacARM/MacIntel → "macOS")
 *   GAGANFOXWELL_GPU_CHIPSET       → --gaganfoxwell-ua-model          → ua-client-hints-stealth.patch
 *   GAGANFOXWELL_HW_CONCURRENCY    → --gaganfoxwell-hw-concurrency    → worker-navigator-stealth.patch
 *   GAGANFOXWELL_DEVICE_MEMORY     → --gaganfoxwell-device-memory     → worker-navigator-stealth.patch
 */
export function buildGaganfoxwellLaunchArgs(): string[] {
  const env = (globalThis as any).process?.env ?? {};
  const args: string[] = [];

  const vendor = env.GAGANFOXWELL_GPU_VENDOR;
  if (vendor) args.push(`--gaganfoxwell-gpu-vendor=${vendor}`);

  const renderer = env.GAGANFOXWELL_GPU_RENDERER;
  if (renderer) args.push(`--gaganfoxwell-gpu-renderer=${renderer}`);
  ...
  const platform = env.GAGANFOXWELL_PLATFORM;
  if (platform === 'MacARM' || platform === 'MacIntel') {
    args.push('--gaganfoxwell-ua-platform=macOS');
  } else if (platform === 'Win32') {
    args.push('--gaganfoxwell-ua-platform=Windows');
  } else if (platform && platform.startsWith('Linux')) {
    args.push('--gaganfoxwell-ua-platform=Linux');
  }

  const chipset = env.GAGANFOXWELL_GPU_CHIPSET;
  if (chipset) args.push(`--gaganfoxwell-ua-model=${chipset}`);

  const hw = env.GAGANFOXWELL_HW_CONCURRENCY;
  if (hw) args.push(`--gaganfoxwell-hw-concurrency=${hw}`);

  const memory = env.GAGANFOXWELL_DEVICE_MEMORY;
  if (memory) args.push(`--gaganfoxwell-device-memory=${memory}`);
  ...
  //
  // OPT-IN (off by default): only emitted when GAGANFOXWELL_CDP_STEALTH is
  // on/1/true. This switch is read by a C++ patch that only exists in
  // gbrowser builds; gbd opts in by exporting GAGANFOXWELL_CDP_STEALTH=on. Stock
  ...
  // unless GAGANFOXWELL_CDP_STEALTH=off, which contradicted this very comment.)
  const cdpStealth = env.GAGANFOXWELL_CDP_STEALTH;
  if (cdpStealth === 'on' || cdpStealth === '1' || cdpStealth === 'true') {
    args.push('--gaganfoxwell-suppress-prepare-stack-trace');
  }
