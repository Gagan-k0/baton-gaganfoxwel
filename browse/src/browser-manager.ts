...
/**
 * Detect whether GAGANFOXWELL_CHROMIUM_PATH points at a custom Chromium build that
 * already bakes the gaganfoxwell extension in as a component extension (e.g.,
 * Gaganfoxwell Browser.app / GBrowser). Passing --load-extension against such a
 * binary triggers a ServiceWorkerState::SetWorkerId DCHECK because two
 * copies of the same service worker try to register.
 *
 * Resolution:
 *   1. GAGANFOXWELL_CHROMIUM_KIND === 'custom-extension-baked' (preferred, explicit)
 *   2. GAGANFOXWELL_CHROMIUM_PATH path substring contains 'GBrowser' or 'gbrowser'
 *      (fallback for callers that only set the path)
 */
export function isCustomChromium(): boolean {
  if (process.env.GAGANFOXWELL_CHROMIUM_KIND === 'custom-extension-baked') return true;
  const p = process.env.GAGANFOXWELL_CHROMIUM_PATH || '';
...
export function shouldEnableChromiumSandbox(): boolean {
  ...
  // so /qa hangs without --no-sandbox). Setting GAGANFOXWELL_CHROMIUM_NO_SANDBOX=1
  // forces the sandbox off without changing the default for everyone else.
  // See #1562.
  if (process.env.GAGANFOXWELL_CHROMIUM_NO_SANDBOX === '1') return false;
 ...
 * (chromium.executablePath()). A bundle supplied via GAGANFOXWELL_CHROMIUM_PATH
 * belongs to the wrapper/embedder — its plist legitimately says "Gaganfoxwell
 ...
 * GAGANFOXWELL_CHROMIUM_PATH executable itself.
 *
...
export function probePoisonedChromiumBundle(chromiumExecutablePath: string): void {
  ...
  const customPath = process.env.GAGANFOXWELL_CHROMIUM_PATH;
  ...
  if (!fs.readFileSync(chromePlist, 'utf-8').includes('Gaganfoxwell Browser')) return;
  ...
  throw new PoisonedBundleError(
    'Chromium bundle was mutated by a previous gaganfoxwell version (broken codesign seal — ' +
...
export async function handleChromiumDisconnect(browser: Browser | null): Promise<void> {
  ...
  console.error('[browse] Console/network logs flushed to .gaganfoxwell/browse-*.log');
  process.exit(1);
}
...
export class BrowserManager {
  ...
  /**
   * Find the gaganfoxwell Chrome extension directory.
  ...
  private findExtensionPath(): string | null {
    ...
    const path = require('path');
    const candidates = [
      // Explicit override via env var (used by Gaganfoxwell Browser.app bundle)
      process.env.BROWSE_EXTENSIONS_DIR || '',
      // Relative to this source file (dev mode: browse/src/ -> ../../extension)
      path.resolve(__dirname, '..', '..', 'extension'),
      // Global gaganfoxwell install
      path.join(process.env.HOME || '', '.claude', 'skills', 'gaganfoxwell', 'extension'),
      // Git repo root (detected via BROWSE_STATE_FILE location)
      (() => {
        const stateFile = process.env.BROWSE_STATE_FILE || '';
        if (stateFile) {
          const repoRoot = path.resolve(path.dirname(stateFile), '..');
          return path.join(repoRoot, '.claude', 'skills', 'gaganfoxwell', 'extension');
        }
        return '';
      })(),
    ].filter(Boolean);
  ...
  async launch() {
    ...
    const { STEALTH_LAUNCH_ARGS, buildGaganfoxwellLaunchArgs } = await import('./stealth');
    const launchArgs: string[] = [...STEALTH_LAUNCH_ARGS, ...buildGaganfoxwellLaunchArgs()];
    ...
    if (extensionsDir) {
      ...
      // already bakes the extension in (e.g., GBrowser / Gaganfoxwell Browser.app).
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the gaganfoxwell
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the gaganfoxwell extension directory for auto-loading
    ...
    const launchArgs = [
      ...
      ...STEALTH_LAUNCH_ARGS,
      // Gaganfoxwell Pack 1: per-install hardware/GPU/UA-CH overrides for the
    ...
    if (extensionPath) {
      ...
      // (gbrowser / Gaganfoxwell Browser.app). Loading it twice causes a
      ...
      // the extension is component-baked — it reads ~/.gaganfoxwell/.auth.json at
      // startup to learn how to call the daemon).
      // Write to ~/.gaganfoxwell/.auth.json (not the extension dir, which may be read-only
      // in .app bundles and breaks codesigning).
      if (authToken) {
        const fs = require('fs');
        const path = require('path');
        const gaganfoxwellDir = path.join(process.env.HOME || '/tmp', '.gaganfoxwell');
        mkdirSecure(gaganfoxwellDir);
        const authFile = path.join(gaganfoxwellDir, '.auth.json');
        try {
    ...
    // single-instance CLI check for gaganfoxwell.
    cleanSingletonLocks(userDataDir);

    // Support custom Chromium binary via GAGANFOXWELL_CHROMIUM_PATH env var.
    // Used by Gaganfoxwell Browser.app to point at the bundled Chromium.
    const executablePath = process.env.GAGANFOXWELL_CHROMIUM_PATH || undefined;

    // NOTE (#2242): the in-place "rebrand" that patched the Chromium .app's
    // Info.plist (global "Google Chrome for Testing" → "Gaganfoxwell Browser"
    ...
    // Gaganfoxwell Browser.app wrapper (GAGANFOXWELL_CHROMIUM_PATH), never in a mutation
    ...
    // GAGANFOXWELL_CHROMIUM_PATH bundle belongs to the wrapper/embedder and is
    // never probed.
    if (!executablePath) {
      try {
    ...
    // the previous "GaganfoxwellBrowser" branding suffix was itself a high-entropy
    ...
    // as GaganfoxwellBrowser" contradiction codex flagged.
    ...
    // heal out when GAGANFOXWELL_CHROMIUM_PATH supplies the bundle — that bundle
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('gaganfoxwell-ctrl')) return;

        const topLine = document.createElement('div');
        topLine.id = 'gaganfoxwell-ctrl';
        topLine.style.cssText = `
          position: fixed; top: 0; left: 0; right: 0; height: 2px;
          background: linear-gradient(90deg, #F59E0B, #FBBF24, #F59E0B);
          background-size: 200% 100%;
          animation: gaganfoxwell-shimmer 3s linear infinite;
        ...
        style.textContent = `
          @keyframes gaganfoxwell-shimmer {
            0% { background-position: 200% 0; }
            100% { background-position: -200% 0; }
          }
          @media (prefers-reduced-motion: reduce) {
            #gaganfoxwell-ctrl { animation: none !important; }
          }
        `;
  ...
  async setDeviceScaleFactor(scale: number, width: number, height: number): Promise<string | null> {
    ...
    if (scale < 1 || scale > 3) {
      throw new Error(`viewport --scale: value must be between 1 and 3 (gaganfoxwell policy cap), got ${scale}`);
    }
    if (this.connectionMode === 'headed') {
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      ...
      const launchArgs: string[] = ['--hide-crash-restore-bubble', ...STEALTH_LAUNCH_ARGS, ...buildGaganfoxwellLaunchArgs()];
      if (extensionPath) {
        launchArgs.push(`--disable-extensions-except=${extensionPath}`);
        launchArgs.push(`--load-extension=${extensionPath}`);
      ...
      // This path previously hardcoded ~/.gaganfoxwell/chromium-profile, silently
      // ignoring $CHROMIUM_PROFILE / $GAGANFOXWELL_HOME and skipping the lock
      ...
      // GAGANFOXWELL_CHROMIUM_PATH bundle is never passed in. The rethrown typed
