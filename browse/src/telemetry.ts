 ...
 *
 * Piggybacks on ~/.gaganfoxwell/analytics/skill-usage.jsonl pattern (existing
 * gaganfoxwell telemetry). Hostname + aggregate counters only; no body content,
 * no agent text, no command args. Respects the user's telemetry tier
 * setting (off | anonymous | community) via gaganfoxwell-config.
 *
...
import { readGaganfoxwellConfigYamlKey } from './config';
...
function analyticsDir(): string {
  return path.join(gaganfoxwellHome(), 'analytics');
}
 ...
 * ~/.gaganfoxwell/config.yaml, and only a granted tier enables emission. Tiers,
 * checked in order:
 *
 *   1. Env hint GAGANFOXWELL_TELEMETRY_OFF=1 (set by preambles and test
 ...
 *      means consent was never granted — matching bin/gaganfoxwell-config's
 *      DEFAULTS table, which reports 'off' for an unset telemetry key.
 *      Anything else would be a split-brain where `gaganfoxwell-config get
 *      telemetry` tells the user 'off' while a direct-$B daemon emits.
 *      One escape hatch: GAGANFOXWELL_TELEMETRY_OFF=0 is a harness-side consent
 *      assertion that flips this DEFAULT only (test harnesses exercising the
 *      write path against a scratch GAGANFOXWELL_HOME) — it never overrides an
...
export function isTelemetryDisabled(): boolean {
  ...
  if (process.env.GAGANFOXWELL_TELEMETRY_OFF === '1') {
    telemetryDisabled = true;
    return true;
  }
  // Persistent tier: an explicit user-written value always wins next.
  const tier = readGaganfoxwellConfigYamlKey('telemetry');
  if (tier === 'off') {
    telemetryDisabled = true;
    return true;
  }
  ...
  telemetryDisabled = process.env.GAGANFOXWELL_TELEMETRY_OFF !== '0';
