/**
 * Terminal Agent — PTY-backed Claude Code terminal for the gaganfoxwell browser
...
const STATE_FILE = process.env.BROWSE_STATE_FILE || path.join(process.env.HOME || '/tmp', '.gaganfoxwell', 'browse.json');
...
const OWNER_WATCHDOG_MS = parseInt(
  process.env.GAGANFOXWELL_TERMINAL_OWNER_WATCHDOG_MS || '15000',
  10,
);
...
const KEEPALIVE_INTERVAL_MS = parseInt(
  process.env.GAGANFOXWELL_PTY_KEEPALIVE_INTERVAL_MS || '25000',
  10,
);
...
const RING_BUFFER_MAX_BYTES = parseInt(
  process.env.GAGANFOXWELL_PTY_RING_BUFFER_BYTES || `${1024 * 1024}`,
  10,
);
...
const DETACH_WINDOW_MS = parseInt(
  process.env.GAGANFOXWELL_PTY_DETACH_WINDOW_MS || '60000',
  10,
);
 ...
 *      (updated continuously by the gaganfoxwell browser extension).
...
function buildTabAwarenessHint(stateDir: string): string {
  ...
  return [
    'You are running inside the gaganfoxwell browser sidebar with live access to the user\'s browser tabs.',
    '',
...
function spawnClaude(cols: number, rows: number, onData: (chunk: Buffer) => void) {
  ...
  // headed-mode browser; BROWSE_NO_AUTOSTART prevents claude's gaganfoxwell
...
function buildServer(port: number) {
  return Bun.serve({
    ...
    fetch(req, server) {
      ...
      //         - Cookie gaganfoxwell_pty (works for non-browser callers and
      ...
      if (url.pathname === '/ws') {
        ...
        // with a `gaganfoxwell-pty.` prefix (which we strip). Browsers send a
        ...
        for (const raw of protoHeader.split(',').map(s => s.trim()).filter(Boolean)) {
          const candidate = raw.startsWith('gaganfoxwell-pty.') ? raw.slice('gaganfoxwell-pty.'.length) : raw;
        ...
        // Fallback: Cookie gaganfoxwell_pty (legacy / non-browser callers).
...
async function main() {
  ...
  // sibling gaganfoxwell sessions. Callers (cli.ts spawn site, server.ts
