/**
 * Tests for bin/gaganfoxwell-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * GAGANFOXWELL_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'gaganfoxwell-config');

let stateDir: string;

function run(args: string[] = [], extraEnv: Record<string, string> = {}) {
  // The script resolves its state dir as GAGANFOXWELL_STATE_ROOT > GAGANFOXWELL_HOME >
  // GAGANFOXWELL_STATE_DIR > $HOME/.gaganfoxwell. Strip the higher-precedence vars so a
  // stray value in the harness env (another test file's leftovers, operator
  // shell) can never outrank the per-test GAGANFOXWELL_STATE_DIR isolation.
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'gaganfoxwell-config-test-'));
});
...
describe('gaganfoxwell-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { GAGANFOXWELL_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
  ...
  test('first set writes annotated header with docs', () => {
    ...
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    expect(content).toContain('# gaganfoxwell configuration');
  ...
  test('header written only once, not duplicated on second set', () => {
    ...
    run(['set', 'baz', 'qux']);
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    const headerCount = (content.match(/# gaganfoxwell configuration/g) || []).length;
  ...
  test('existing config file is not overwritten with header', () => {
    ...
    expect(content).not.toContain('# gaganfoxwell configuration');
  });
