...
const savedEnv = { GAGANFOXWELL_HOME: process.env.GAGANFOXWELL_HOME, GAGANFOXWELL_PAIR_AGENT: process.env.GAGANFOXWELL_PAIR_AGENT };
const tmpHomes: string[] = [];

function tmpHomeWith(config: Record<string, string> | null): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gaganfoxwell-pair-'));
  tmpHomes.push(dir);
  if (config !== null) {
    // Canonical store: flat YAML lines, the shape bin/gaganfoxwell-config writes.
  ...
  delete process.env.GAGANFOXWELL_PAIR_AGENT;
  return dir;
}

afterEach(() => {
  for (const k of ['GAGANFOXWELL_HOME', 'GAGANFOXWELL_PAIR_AGENT'] as const) {
...
describe('isPairAgentEnabled — fail-closed default', () => {
  ...
  test('env override wins: GAGANFOXWELL_PAIR_AGENT=on forces ON even with config off', () => {
    tmpHomeWith({ pair_agent: 'off' });
    process.env.GAGANFOXWELL_PAIR_AGENT = 'on';
    expect(isPairAgentEnabled()).toBe(true);
  });

  test('env override wins: GAGANFOXWELL_PAIR_AGENT=off forces OFF even with config on', () => {
    tmpHomeWith({ pair_agent: 'on' });
    process.env.GAGANFOXWELL_PAIR_AGENT = 'off';
    expect(isPairAgentEnabled()).toBe(false);
  });
});
...
describe('gate wiring — every tunnel activation point consults the guard', () => {
  ...
  test('CLI consent-off branch names the real remedy, never ngrok reinstall', () => {
    ...
    expect(branch).toContain('gaganfoxwell-config set pair_agent on');
  ...
  test('/tunnel/start refuses with the enable hint when disabled', () => {
    ...
    expect(block).toContain('gaganfoxwell-config set pair_agent on');
  });
