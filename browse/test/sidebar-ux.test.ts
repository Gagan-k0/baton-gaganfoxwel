...
describe('cleanup and screenshot buttons', () => {
  ...
  test('cleanup button injects smart prompt into the live PTY (not just deterministic selectors)', () => {
    ...
    // gaganfoxwellInjectToTerminal (the chat-queue POST to /sidebar-command was
    ...
    expect(cleanupFn).toContain('gaganfoxwellInjectToTerminal');
...
describe('cleanup heuristics (write-commands.ts)', () => {
  ...
  test('sticky cleanup skips gaganfoxwell control indicator', () => {
    expect(wcSrc).toContain("gaganfoxwell-ctrl");
  });
...
describe('welcome page', () => {
  ...
  test('welcome page has Gaganfoxwell Browser branding', () => {
    expect(welcomeSrc).toContain('Gaganfoxwell Browser');
  });

  test('welcome page has extension-ready listener to hide prompt', () => {
    expect(welcomeSrc).toContain('gaganfoxwell-extension-ready');
...
describe('server /welcome endpoint', () => {
  ...
  test('/welcome serves fallback HTML if no welcome file found', () => {
    ...
    expect(welcomeSection).toContain('Gaganfoxwell Browser ready');
...
describe('sidebar arrow hint hide flow (4-step signal chain)', () => {
  ...
  //   3. content.js receives 'sidebarOpened' → dispatches 'gaganfoxwell-extension-ready'
  //   4. welcome.html listens for 'gaganfoxwell-extension-ready' → hides arrow
  //
  const contentSrc = fs.readFileSync(path.join(ROOT, '..', 'extension', 'content.js'), 'utf-8');
  const bgSrc = fs.readFileSync(path.join(ROOT, '..', 'extension', 'background.js'), 'utf-8');
  ...
  test('step 1: sidepanel sends sidebarOpened message on connect', () => {
    ...
    // globals (gaganfoxwellServerPort/gaganfoxwellAuthToken) before the connected branch.
  ...
  // Step 3: content.js fires gaganfoxwell-extension-ready ONLY on sidebarOpened
  test('step 3: content.js dispatches extension-ready on sidebarOpened message', () => {
    ...
    expect(contentSrc).toContain("new CustomEvent('gaganfoxwell-extension-ready')");
  });
  ...
  test('step 3: content.js does NOT auto-fire extension-ready on load', () => {
    ...
    expect(beforeListener).not.toContain("dispatchEvent(new CustomEvent('gaganfoxwell-extension-ready'))");
  });

  // Step 4: welcome page hides arrow on gaganfoxwell-extension-ready
  test('step 4: welcome page hides arrow on gaganfoxwell-extension-ready event', () => {
    expect(welcomeSrc).toContain("'gaganfoxwell-extension-ready'");
    expect(welcomeSrc).toContain("classList.add('hidden')");
  });

  test('step 4: welcome page does NOT auto-hide via status pill polling', () => {
    // The old fallback (checkPill/gaganfoxwell-status-pill) would hide the arrow
    // as soon as the content script injected the pill, even without sidebar open.
    expect(welcomeSrc).not.toContain('checkPill');
    expect(welcomeSrc).not.toContain('gaganfoxwell-status-pill');
  });
});
...
describe('BROWSE_NO_AUTOSTART (sidebar headless prevention)', () => {
  ...
  test('cli.ts shows actionable error message when BROWSE_NO_AUTOSTART blocks', () => {
    expect(cliSrc).toContain('/open-gaganfoxwell-browser');
...
describe('shutdown cleanup (server.ts)', () => {
  ...
  test('shutdown kills the terminal-agent via identity-based kill (no pkill)', () => {
    ...
    // regex killed sibling gaganfoxwell sessions on the same host (also pinned
