...
describe('sidepanel.js: chat helpers ripped, terminal-injection helper survives', () => {
  ...
  test('Cleanup runs through the live PTY (no /sidebar-command POST)', () => {
    ...
    // PTY via gaganfoxwellInjectToTerminal. The dead code path was a POST to
    // /sidebar-command which kicked off a fresh claude -p subprocess.
    const cleanup = JS.slice(JS.indexOf('async function runCleanup'));
    expect(cleanup).toContain('window.gaganfoxwellInjectToTerminal');
  ...
  test('Inspector "Send to Code" routes through the live PTY', () => {
    ...
    expect(sendBtn).toContain('window.gaganfoxwellInjectToTerminal');
  ...
  test('updateConnection no longer kicks off chat / tab polling', () => {
    ...
    expect(update).toContain('window.gaganfoxwellServerPort');
    expect(update).toContain('window.gaganfoxwellAuthToken');
  });
});

describe('sidepanel-terminal.js: eager auto-connect + injection API', () => {
  test('Exposes window.gaganfoxwellInjectToTerminal for cross-pane use', () => {
    expect(TERM_JS).toContain('window.gaganfoxwellInjectToTerminal');
    // Returns false when no live session, true when bytes go out.
    const inject = TERM_JS.slice(TERM_JS.indexOf('window.gaganfoxwellInjectToTerminal'));
  ...
  test('Repaint hook fires when Terminal pane becomes visible', () => {
    // The chat-tab rip removed gaganfoxwell:primary-tab-changed; we use a
