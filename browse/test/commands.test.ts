...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/gaganfoxwell-cli-');
    const execPath = path.join(root, '.claude/skills/gaganfoxwell/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/gaganfoxwell/browse/src/server.ts');
