 ...
 * were registered. The subprocess uses HOME=tmp + GAGANFOXWELL_HOME=tmp so any
...
describe('server.ts module import has no auto-start side effects', () => {
  test('importing server.ts does not bind Bun.serve, register signal handlers, or write state', async () => {
    ...
    const tmpGaganfoxwell = path.join(tmpHome, '.gaganfoxwell');
...
// Check that the gaganfoxwell home directory wasn't populated as a side effect.
...
let gaganfoxwellPopulated = false;
try {
  const entries = fs.readdirSync(${JSON.stringify(tmpGaganfoxwell)}).filter(e => e !== '.gitignore');
  gaganfoxwellPopulated = entries.length > 0;
} catch {
...
`;
    ...
    const proc = Bun.spawn(['bun', '-e', childScript], {
      env: {
        ...process.env,
        HOME: tmpHome,
        GAGANFOXWELL_HOME: tmpGaganfoxwell,
        ...
        // happens) won't crawl the host's real .gaganfoxwell/.
        BROWSE_STATE_FILE: path.join(tmpGaganfoxwell, 'browse.json'),
      },
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    // gaganfoxwell home should remain empty — initRegistry/initAuditLog/etc. side
    ...
    expect(snapshot.gaganfoxwellPopulated).toBe(false);
