...
describe('buildStealthScript — T3 Layer C', () => {
  ...
  test('script does not include "GaganfoxwellBrowser" branding string', () => {
    const s = buildStealthScript(hw);
    // D6: dropped from UA, must not leak in via stealth payload either.
    expect(s).not.toContain('GaganfoxwellBrowser');
  });
});

describe('buildGaganfoxwellLaunchArgs — Pack 1 cmdline-switch construction', () => {
  // Helper: clear all GAGANFOXWELL_* env, run test body, restore env.
  function withEnv(env: Record<string, string | undefined>, body: () => void) {
    const saved: Record<string, string | undefined> = {};
    for (const k of Object.keys(process.env)) {
      if (k.startsWith('GAGANFOXWELL_')) {
    ...
    try {
      ...
      for (const k of Object.keys(process.env)) {
        if (k.startsWith('GAGANFOXWELL_')) delete process.env[k];
      }
  ...
  test('empty env produces no switches (suppress-stack is opt-in)', () => {
    withEnv({}, () => {
      ...
      // emitted when GAGANFOXWELL_CDP_STEALTH is on/1/true. Empty env → [].
      expect(buildGaganfoxwellLaunchArgs()).toEqual([]);
    });
  });

  test('all env values populated (incl. CDP stealth opt-in) → all 7 switches emitted', () => {
    withEnv({
      GAGANFOXWELL_GPU_VENDOR: 'Apple Inc.',
      GAGANFOXWELL_GPU_RENDERER: 'ANGLE (Apple, ANGLE Metal Renderer: Apple M4 Max, Unspecified Version)',
      GAGANFOXWELL_PLATFORM: 'MacARM',
      GAGANFOXWELL_GPU_CHIPSET: 'Apple M4 Max',
      GAGANFOXWELL_HW_CONCURRENCY: '16',
      GAGANFOXWELL_DEVICE_MEMORY: '8',
      GAGANFOXWELL_CDP_STEALTH: 'on',
    }, () => {
      const args = buildGaganfoxwellLaunchArgs();
      expect(args).toContain('--gaganfoxwell-gpu-vendor=Apple Inc.');
      expect(args).toContain('--gaganfoxwell-gpu-renderer=ANGLE (Apple, ANGLE Metal Renderer: Apple M4 Max, Unspecified Version)');
      expect(args).toContain('--gaganfoxwell-ua-platform=macOS');
      expect(args).toContain('--gaganfoxwell-ua-model=Apple M4 Max');
      expect(args).toContain('--gaganfoxwell-hw-concurrency=16');
      expect(args).toContain('--gaganfoxwell-device-memory=8');
      expect(args).toContain('--gaganfoxwell-suppress-prepare-stack-trace');
      expect(args.length).toBe(7);
    });
  });

  test('platform mapping: MacARM and MacIntel both → macOS', () => {
    withEnv({ GAGANFOXWELL_PLATFORM: 'MacARM' }, () => {
      expect(buildGaganfoxwellLaunchArgs()).toContain('--gaganfoxwell-ua-platform=macOS');
    });
    withEnv({ GAGANFOXWELL_PLATFORM: 'MacIntel' }, () => {
      expect(buildGaganfoxwellLaunchArgs()).toContain('--gaganfoxwell-ua-platform=macOS');
    });
  });

  test('platform mapping: Win32 → Windows, Linux x86_64 → Linux', () => {
    withEnv({ GAGANFOXWELL_PLATFORM: 'Win32' }, () => {
      expect(buildGaganfoxwellLaunchArgs()).toContain('--gaganfoxwell-ua-platform=Windows');
    });
    withEnv({ GAGANFOXWELL_PLATFORM: 'Linux x86_64' }, () => {
      expect(buildGaganfoxwellLaunchArgs()).toContain('--gaganfoxwell-ua-platform=Linux');
    });
  });

  test('partial env: only set switches that have values', () => {
    withEnv({ GAGANFOXWELL_HW_CONCURRENCY: '12' }, () => {
      const args = buildGaganfoxwellLaunchArgs();
      // hw only — suppress-stack is opt-in and GAGANFOXWELL_CDP_STEALTH is unset.
      expect(args).toContain('--gaganfoxwell-hw-concurrency=12');
      expect(args).not.toContain('--gaganfoxwell-suppress-prepare-stack-trace');
      expect(args.length).toBe(1);
    });
  });

  test('prepare-stack-trace suppression is opt-in via GAGANFOXWELL_CDP_STEALTH', () => {
    // on/1/true enable it; off and unset omit it, so stock Playwright
    // Chromium (no GAGANFOXWELL_CDP_STEALTH) never receives the unknown switch.
    for (const v of ['on', '1', 'true']) {
      withEnv({ GAGANFOXWELL_CDP_STEALTH: v }, () => {
        expect(buildGaganfoxwellLaunchArgs()).toContain('--gaganfoxwell-suppress-prepare-stack-trace');
      });
    }
    withEnv({ GAGANFOXWELL_CDP_STEALTH: 'off' }, () => {
      expect(buildGaganfoxwellLaunchArgs()).not.toContain('--gaganfoxwell-suppress-prepare-stack-trace');
    });
    withEnv({}, () => {
      expect(buildGaganfoxwellLaunchArgs()).not.toContain('--gaganfoxwell-suppress-prepare-stack-trace');
    });
  ...
  test('unrecognized platform falls through without --gaganfoxwell-ua-platform', () => {
    withEnv({ GAGANFOXWELL_PLATFORM: 'OS/2' }, () => {
      const args = buildGaganfoxwellLaunchArgs();
      expect(args.some(a => a.startsWith('--gaganfoxwell-ua-platform='))).toBe(false);
    });
  });

  test('GPU vendor with spaces survives intact (no quote/escape regression)', () => {
    withEnv({ GAGANFOXWELL_GPU_VENDOR: 'NVIDIA Corporation' }, () => {
      const args = buildGaganfoxwellLaunchArgs();
      expect(args).toContain('--gaganfoxwell-gpu-vendor=NVIDIA Corporation');
    });
  });
});
...
describe('readHostProfile — clamp/fallback', () => {
  ...
  afterEach(() => {
    if (savedHw === undefined) delete process.env.GAGANFOXWELL_HW_CONCURRENCY;
    else process.env.GAGANFOXWELL_HW_CONCURRENCY = savedHw;
    if (savedMem === undefined) delete process.env.GAGANFOXWELL_DEVICE_MEMORY;
    else process.env.GAGANFOXWELL_DEVICE_MEMORY = savedMem;
  });
  function withHw(hw: string | undefined, mem: string | undefined): ReturnType<typeof readHostProfile> {
    savedHw = process.env.GAGANFOXWELL_HW_CONCURRENCY;
    savedMem = process.env.GAGANFOXWELL_DEVICE_MEMORY;
    if (hw === undefined) delete process.env.GAGANFOXWELL_HW_CONCURRENCY; else process.env.GAGANFOXWELL_HW_CONCURRENCY = hw;
    if (mem === undefined) delete process.env.GAGANFOXWELL_DEVICE_MEMORY; else process.env.GAGANFOXWELL_DEVICE_MEMORY = mem;
