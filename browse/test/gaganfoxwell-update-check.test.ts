/**
 * Tests for bin/gaganfoxwell-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * GAGANFOXWELL_DIR / GAGANFOXWELL_STATE_DIR / GAGANFOXWELL_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'gaganfoxwell-update-check');

let gaganfoxwellDir: string;
let stateDir: string;

function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  // gaganfoxwell-config (which this script shells out to for update_check) resolves
  // state as GAGANFOXWELL_STATE_ROOT > GAGANFOXWELL_HOME > GAGANFOXWELL_STATE_DIR > ~/.gaganfoxwell.
  // Strip the higher-precedence vars so harness-env leftovers can never
  // outrank the per-test GAGANFOXWELL_STATE_DIR isolation.
  const env: Record<string, string | undefined> = {
    ...process.env,
    GAGANFOXWELL_DIR: gaganfoxwellDir,
    GAGANFOXWELL_STATE_DIR: stateDir,
    GAGANFOXWELL_REMOTE_URL: `file://${join(gaganfoxwellDir, 'REMOTE_VERSION')}`,
  };
  delete env.GAGANFOXWELL_STATE_ROOT;
  delete env.GAGANFOXWELL_HOME;
...
beforeEach(() => {
  gaganfoxwellDir = mkdtempSync(join(tmpdir(), 'gaganfoxwell-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'gaganfoxwell-state-test-'));
  // Link real gaganfoxwell-config so update_check config check works
  const binDir = join(gaganfoxwellDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'gaganfoxwell-config'), join(binDir, 'gaganfoxwell-config'));
  // v1.63+: the script sources bin/gaganfoxwell-egress-lib.sh unconditionally
  // (receipted fetch helpers). A real install always has it beside
  // gaganfoxwell-config; without this link every test failed at the source line —
  // masked until the suite-truncation fix because the runner died first.
  symlinkSync(
    join(import.meta.dir, '..', '..', 'bin', 'gaganfoxwell-egress-lib.sh'),
    join(binDir, 'gaganfoxwell-egress-lib.sh'),
  );
});

afterEach(() => {
  rmSync(gaganfoxwellDir, { recursive: true, force: true });
...
describe('gaganfoxwell-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '0.4.0\n');
  ...
  test('just-upgraded marker does not mask newer remote version', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(gaganfoxwellDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('just-upgraded with no further updates writes UP_TO_DATE cache', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(gaganfoxwellDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '0.3.3\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(gaganfoxwellDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(gaganfoxwellDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      GAGANFOXWELL_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ GAGANFOXWELL_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), version + '\n');
  ...
  test('remote older than local (stale CDN) → silent, cache UP_TO_DATE', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '1.34.0.0\n');
    writeFileSync(join(gaganfoxwellDir, 'REMOTE_VERSION'), '1.33.2.0\n');
  ...
  test('multi-segment sort: 1.9.0.0 < 1.10.0.0', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '1.9.0.0\n');
    writeFileSync(join(gaganfoxwellDir, 'REMOTE_VERSION'), '1.10.0.0\n');
  ...
  test('multi-segment reverse sort: 1.10.0.0 > 1.9.0.0 → no rewind', () => {
    writeFileSync(join(gaganfoxwellDir, 'VERSION'), '1.10.0.0\n');
    writeFileSync(join(gaganfoxwellDir, 'REMOTE_VERSION'), '1.9.0.0\n');
