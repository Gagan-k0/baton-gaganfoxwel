 ...
 * 2. GAGANFOXWELL_STEALTH=extended adds EXTENDED_STEALTH_SCRIPT on top of Layer C.
...
beforeEach(() => {
  originalEnv = process.env.GAGANFOXWELL_STEALTH;
});

afterEach(() => {
  if (originalEnv === undefined) delete process.env.GAGANFOXWELL_STEALTH;
  else process.env.GAGANFOXWELL_STEALTH = originalEnv;
});

describe('extended stealth — opt-in mode flag', () => {
  test('default mode is OFF (consistency-first contract)', () => {
    delete process.env.GAGANFOXWELL_STEALTH;
    expect(isExtendedStealthEnabled()).toBe(false);
  });

  test('GAGANFOXWELL_STEALTH=extended enables extended mode', () => {
    process.env.GAGANFOXWELL_STEALTH = 'extended';
    expect(isExtendedStealthEnabled()).toBe(true);
  });

  test('GAGANFOXWELL_STEALTH=1 also enables (env-style boolean)', () => {
    process.env.GAGANFOXWELL_STEALTH = '1';
    expect(isExtendedStealthEnabled()).toBe(true);
  });

  test('GAGANFOXWELL_STEALTH=anything-else does NOT enable', () => {
    process.env.GAGANFOXWELL_STEALTH = 'verbose';
    expect(isExtendedStealthEnabled()).toBe(false);
  });
});
