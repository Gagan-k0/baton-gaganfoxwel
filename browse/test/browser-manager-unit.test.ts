...
describe('shouldEnableChromiumSandbox', () => {
  ...
  const origNoSandbox = process.env.GAGANFOXWELL_CHROMIUM_NO_SANDBOX;
  const origGetuid = process.getuid;

  beforeEach(() => {
    delete process.env.CI;
    delete process.env.CONTAINER;
    delete process.env.GAGANFOXWELL_CHROMIUM_NO_SANDBOX;
  });
  ...
  afterEach(() => {
    ...
    if (origNoSandbox === undefined) delete process.env.GAGANFOXWELL_CHROMIUM_NO_SANDBOX; else process.env.GAGANFOXWELL_CHROMIUM_NO_SANDBOX = origNoSandbox;
  ...
  it('linux + GAGANFOXWELL_CHROMIUM_NO_SANDBOX=1 → false (Ubuntu/AppArmor opt-out)', async () => {
    setPlatform('linux');
    process.env.GAGANFOXWELL_CHROMIUM_NO_SANDBOX = '1';
    process.getuid = (() => 1000) as typeof process.getuid;
    const { shouldEnableChromiumSandbox } = await import('../src/browser-manager');
    expect(shouldEnableChromiumSandbox()).toBe(false);
  });

  it('darwin + GAGANFOXWELL_CHROMIUM_NO_SANDBOX=1 → false (env override wins on any platform)', async () => {
    setPlatform('darwin');
    process.env.GAGANFOXWELL_CHROMIUM_NO_SANDBOX = '1';
    process.getuid = (() => 501) as typeof process.getuid;
    const { shouldEnableChromiumSandbox } = await import('../src/browser-manager');
  ...
  it('GAGANFOXWELL_CHROMIUM_NO_SANDBOX=0 → does NOT trigger override (must be exactly "1")', async () => {
    setPlatform('linux');
    process.env.GAGANFOXWELL_CHROMIUM_NO_SANDBOX = '0';
    process.getuid = (() => 1000) as typeof process.getuid;
    const { shouldEnableChromiumSandbox } = await import('../src/browser-manager');
    expect(shouldEnableChromiumSandbox()).toBe(true);
  });
...
describe('stealth injected on every context-creation path', () => {
  ...
  it('buildGaganfoxwellLaunchArgs() is spread into all 3 launch sites', async () => {
    // Same silent-drop regression class as applyStealth: a launch path that
    // omits buildGaganfoxwellLaunchArgs() loses the per-install GPU/UA/hardware
    // cmdline spoof. launch(), launchHeaded(), and handoff() must all call it.
    const { readFileSync } = await import('node:fs');
    const { join } = await import('node:path');
    const src = readFileSync(join(import.meta.dir, '..', 'src', 'browser-manager.ts'), 'utf-8');
    const callSites = src.match(/buildGaganfoxwellLaunchArgs\(\)/g) || [];
