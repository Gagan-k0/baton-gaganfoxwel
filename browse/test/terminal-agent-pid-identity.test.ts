...
//
//   * sibling gaganfoxwell sessions on the same host
//   * editor processes (vim, code, less) that had the file open
//   * any second gaganfoxwell run on the host
//
...
describe('terminal-agent PID identity (v1.44+)', () => {
  ...
  test('3. readAgentRecord round-trips writeAgentRecord', () => {
    const tmpDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'gaganfoxwell-pid-id-'));
    try {
