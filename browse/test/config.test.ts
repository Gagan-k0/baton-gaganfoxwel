...
import { resolveConfig, ensureStateDir, readVersionHash, getGitRoot, getRemoteSlug, resolveGaganfoxwellHome, resolveChromiumProfile, cleanSingletonLocks } from '../src/config';
...
describe('config', () => {
  ...
  describe('resolveConfig', () => {
    test('uses git root by default', () => {
      ...
      expect(config.stateDir).toBe(path.join(gitRoot!, '.gaganfoxwell'));
      expect(config.stateFile).toBe(path.join(gitRoot!, '.gaganfoxwell', 'browse.json'));
    });

    test('derives paths from BROWSE_STATE_FILE when set', () => {
      const stateFile = '/tmp/test-config/.gaganfoxwell/browse.json';
      const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
      expect(config.stateFile).toBe(stateFile);
      expect(config.stateDir).toBe('/tmp/test-config/.gaganfoxwell');
  ...
  describe('ensureStateDir', () => {
    test('creates directory if it does not exist', () => {
      ...
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.gaganfoxwell', 'browse.json') });
    ...
    test('is a no-op if directory already exists', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
      const stateDir = path.join(tmpDir, '.gaganfoxwell');
    ...
    test('writes a self-contained .gaganfoxwell/.gitignore with * unconditionally', () => {
      ...
      // No nesting: the ignore is directly inside the state dir, not .gaganfoxwell/.gaganfoxwell/.
      expect(fs.existsSync(path.join(config.stateDir, '.gaganfoxwell'))).toBe(false);
      fs.rmSync(tmpDir, { recursive: true, force: true });
    });

    test('writes the self-contained .gitignore even when git already ignores .gaganfoxwell/ (before the early return)', () => {
      ...
      // A git repo whose root .gitignore already lists .gaganfoxwell/ makes
      ...
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), '.gaganfoxwell/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.gaganfoxwell', 'browse.json') });
      ensureStateDir(config);
      const selfIgnore = path.join(config.stateDir, '.gitignore');
      expect(fs.existsSync(selfIgnore)).toBe(true);
    ...
    test('adds .gaganfoxwell/ to .gitignore if not present', () => {
      ...
      expect(content).toContain('.gaganfoxwell/');
      expect(content).toBe('node_modules/\n.gaganfoxwell/\n');
      fs.rmSync(tmpDir, { recursive: true, force: true });
    });

    test('does not duplicate .gaganfoxwell/ in .gitignore', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n.gaganfoxwell/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.gaganfoxwell', 'browse.json') });
    ...
    test('logs warning to browse-server.log on non-ENOENT gitignore error', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      // Create a read-only .gitignore (no .gaganfoxwell/ entry → would try to append)
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n');
    ...
    test('leaves .gitignore alone when git already ignores .gaganfoxwell/ globally', () => {
      ...
      // Write a global excludes file that ignores .gaganfoxwell/
      const excludesFile = path.join(tmpDir, 'global-gitignore');
      fs.writeFileSync(excludesFile, '.gaganfoxwell/\n');
      spawnSync('git', ['config', 'core.excludesFile', excludesFile], { cwd: tmpDir });

      // .gitignore exists but does NOT contain .gaganfoxwell/
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n');
      spawnSync('git', ['add', '.gitignore'], { cwd: tmpDir });
      spawnSync('git', ['commit', '-qm', 'init'], { cwd: tmpDir });

      // Verify git knows .gaganfoxwell/ is ignored
      const check = spawnSync('git', ['check-ignore', '-q', '.gaganfoxwell/'], { cwd: tmpDir });
      ...
      const content = fs.readFileSync(path.join(tmpDir, '.gitignore'), 'utf-8');
      expect(content).toBe('node_modules/\n');
      expect(fs.existsSync(path.join(tmpDir, '.gaganfoxwell'))).toBe(true);
  ...
  describe('getRemoteSlug', () => {
    ...
    test('parses SSH remote URLs', () => {
      ...
      const url = 'git@github.com:Gagan-k0/baton-gaganfoxwel.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('Gagan-k0-gaganfoxwell');
    });

    test('parses HTTPS remote URLs', () => {
      const url = 'https://github.com/Gagan-k0/baton-gaganfoxwel.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('Gagan-k0-gaganfoxwell');
    });
...
describe('resolveGaganfoxwellHome', () => {
  test('honors GAGANFOXWELL_HOME env var when set', () => {
    const orig = process.env.GAGANFOXWELL_HOME;
    process.env.GAGANFOXWELL_HOME = '/tmp/custom-gaganfoxwell-home';
    try {
      expect(resolveGaganfoxwellHome()).toBe('/tmp/custom-gaganfoxwell-home');
    } finally {
      if (orig === undefined) delete process.env.GAGANFOXWELL_HOME;
      else process.env.GAGANFOXWELL_HOME = orig;
    }
  });

  test('falls back to os.homedir() + /.gaganfoxwell when env unset', () => {
    const orig = process.env.GAGANFOXWELL_HOME;
    delete process.env.GAGANFOXWELL_HOME;
    try {
      expect(resolveGaganfoxwellHome()).toBe(path.join(os.homedir(), '.gaganfoxwell'));
    } finally {
      if (orig !== undefined) process.env.GAGANFOXWELL_HOME = orig;
    }
  });
});
...
describe('resolveChromiumProfile', () => {
  ...
  test('falls back to resolveGaganfoxwellHome()/chromium-profile when nothing set', () => {
    const origEnv = process.env.CHROMIUM_PROFILE;
    const origHome = process.env.GAGANFOXWELL_HOME;
    delete process.env.CHROMIUM_PROFILE;
    process.env.GAGANFOXWELL_HOME = '/tmp/fallback-gaganfoxwell';
    try {
      expect(resolveChromiumProfile()).toBe('/tmp/fallback-gaganfoxwell/chromium-profile');
    } finally {
      if (origEnv !== undefined) process.env.CHROMIUM_PROFILE = origEnv;
      if (origHome === undefined) delete process.env.GAGANFOXWELL_HOME;
      else process.env.GAGANFOXWELL_HOME = origHome;
    }
  });
