...
describe('applyStealth — per-install hardware from env', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.GAGANFOXWELL_HW_CONCURRENCY = '12';
    process.env.GAGANFOXWELL_DEVICE_MEMORY = '4';
...
describe('applyStealth — extended mode layered on Layer C (GAGANFOXWELL_STEALTH=extended)', () => {
  let ctx: BrowserContext;
  let savedStealth: string | undefined;

  beforeAll(async () => {
    savedStealth = process.env.GAGANFOXWELL_STEALTH;
    process.env.GAGANFOXWELL_STEALTH = 'extended';
    ctx = await browser.newContext();
  ...
  afterAll(async () => {
    await ctx.close();
    if (savedStealth === undefined) delete process.env.GAGANFOXWELL_STEALTH;
    else process.env.GAGANFOXWELL_STEALTH = savedStealth;
  });
