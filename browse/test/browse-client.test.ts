...
describe('browse-client', () => {
  ...
  beforeEach(async () => {
    ...
    for (const k of ['GAGANFOXWELL_PORT', 'GAGANFOXWELL_SKILL_TOKEN', 'BROWSE_STATE_FILE', 'BROWSE_TAB']) {
  ...
  describe('resolveBrowseAuth', () => {
    it('uses GAGANFOXWELL_PORT + GAGANFOXWELL_SKILL_TOKEN env when present', () => {
      process.env.GAGANFOXWELL_PORT = String(server.port);
      process.env.GAGANFOXWELL_SKILL_TOKEN = 'scoped-token';
    ...
    it('rejects GAGANFOXWELL_PORT env values with trailing characters', () => {
      process.env.GAGANFOXWELL_PORT = `${server.port}abc`;
      process.env.GAGANFOXWELL_SKILL_TOKEN = 'scoped-token';
