 ...
 * never touch the user's real ~/.gaganfoxwell/ tree. The contract under test:
 *
 *   stageSkill    → writes files into ~/.gaganfoxwell/.tmp/skillify-<spawnId>/<name>/
...
beforeEach(() => {
  ...
  // Synthetic tmp root keeps tests off the real ~/.gaganfoxwell/.tmp/.
  stagingTmpRoot = path.join(tmpRoot, 'home', '.gaganfoxwell', '.tmp');
});
