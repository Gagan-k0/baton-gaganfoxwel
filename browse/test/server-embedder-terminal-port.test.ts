...
// getting those files clobbered by gaganfoxwell's shutdown(). The flag (default true)
...
// False = embedder owns them, gaganfoxwell stays hands-off.
//
// Pre-v1.44 used `pkill -f terminal-agent\.ts` which matched sibling gaganfoxwell
...
describe('buildFetchHandler ownsTerminalAgent gate', () => {
  ...
  // not a per-test temp dir. If a real gaganfoxwell daemon is running on this host,
