# Remote Browser Access — How to Pair With a Gaganfoxwell Browser

A Gaganfoxwell Browser server can be shared with any AI agent that can make HTTP requests.
...
─────────────                         ────────────
Gaganfoxwell Browser Server                 Any AI agent
...
SSE endpoints (`/activity/stream`, `/inspector/events`) accept either a Bearer token or the HttpOnly `gaganfoxwell_sse` cookie (minted via `POST /sse-session`, 30-minute TTL, stream-scope only — cannot be used against `/command`). As of v1.6.0.0 the `?token=<ROOT>` query-string auth is no longer accepted.
...
- **Path traversal guarded** on `/welcome` — `GAGANFOXWELL_SLUG` must match `^[a-z0-9_-]+$` or falls back to the built-in template.
- **SSRF guards** on `goto`, `download`, and scrape paths — validates URL target against a localhost/private-range blocklist.
- **Tunnel surface denial logging.** Every rejection on the tunnel listener (`path_not_on_tunnel`, `root_token_on_tunnel`, `missing_scoped_token`, `disallowed_command:*`) is appended to `~/.gaganfoxwell/security/attempts.jsonl` with timestamp, source IP, path, method. Rate-capped at 60 writes/min.
- **Egress receipt on tunnel start (v1.63+).** Every tunnel session open writes a hash-chained receipt (sink `browse-tunnel`) to `~/.gaganfoxwell/security/egress.jsonl` BEFORE ngrok forwards anything. Fail-closed: if the receipt can't be written, the tunnel refuses to start. Audit with `bin/gaganfoxwell-egress list` / `bin/gaganfoxwell-egress verify`.
...
```bash
$B pair-agent --local openclaw    # writes to ~/.openclaw/skills/gaganfoxwell/browse-remote.json
$B pair-agent --local codex       # writes to ~/.codex/skills/gaganfoxwell/browse-remote.json
$B pair-agent --local cursor      # writes to ~/.cursor/skills/gaganfoxwell/browse-remote.json
```
...
3. Save it: `echo 'NGROK_AUTHTOKEN=your_token' > ~/.gaganfoxwell/ngrok.env`
4. Optionally claim a stable domain: `echo 'NGROK_DOMAIN=your-name.ngrok-free.dev' >> ~/.gaganfoxwell/ngrok.env`
