...
This is gaganfoxwell's borrow from [browser-use/browser-harness](https://github.com/browser-use/browser-harness).
gaganfoxwell copies the per-site-notes pattern, NOT the self-modifying-runtime
...
on every gaganfoxwell project I work on"), explicitly run
...
- **Per-project**: `~/.gaganfoxwell/projects/<slug>/learnings.jsonl` — same JSONL
  file the `/learn` skill uses. Domain skills are `type:"domain"` rows.
- **Global**: `~/.gaganfoxwell/global-domain-skills.jsonl` — only `state:"global"`
  rows.
...
following events are written to `~/.gaganfoxwell/analytics/browse-telemetry.jsonl`:
...
`gaganfoxwell-config set telemetry off` or `GAGANFOXWELL_TELEMETRY_OFF=1`.
