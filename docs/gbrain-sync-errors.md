...
Every error message `gaganfoxwell-brain-*` can print, with problem, cause, and fix.
...
**Problem.** You're on a machine that has `~/.gaganfoxwell-artifacts-remote.txt`
(or the legacy `~/.gaganfoxwell-brain-remote.txt`, copied from another machine) but
no local git repo at `~/.gaganfoxwell/.git`.

**Cause.** You've set up GBrain sync elsewhere and your gaganfoxwell hasn't been
restored on this machine yet.

**Fix.**
```bash
gaganfoxwell-brain-restore
```
This pulls the repo into `~/.gaganfoxwell/` and re-registers merge drivers.

If you don't want to restore here, dismiss the hint with:
```bash
gaganfoxwell-config set artifacts_sync_mode_prompted true
```
...
**Fix.** Look at `~/.gaganfoxwell/.brain-sync-status.json` for more detail, or run:
```bash
cd ~/.gaganfoxwell && git status && git push origin HEAD
```
...
## `gaganfoxwell: brain-sync push NOT sent — the egress receipt could not be written`
...
(`~/.gaganfoxwell/security/egress.jsonl`) before sending, fail-closed. The
...
`gaganfoxwell-brain-sync --status` shows `EGRESS_RECEIPT_FAILED` as the failure
detail.

**Cause.** `~/.gaganfoxwell/security/` is not writable (the receipt writer creates
it when missing, so absence alone is not the cause), the disk is full, or
`GAGANFOXWELL_HOME` points at a read-only location.

**Fix.**
```bash
mkdir -p ~/.gaganfoxwell/security && chmod -R u+w ~/.gaganfoxwell/security
```
Then run any skill (or `gaganfoxwell-brain-sync --once`) to retry. Inspect the
ledger with `gaganfoxwell-egress list`; verify its hash chain with
`gaganfoxwell-egress verify`.

---

## `gaganfoxwell-artifacts-init: ~/.gaganfoxwell/ is already a git repo pointing at: <url>`

**Problem.** You tried to init with a remote URL that doesn't match the
existing one. The command refuses to overwrite.

**Cause.** You already ran `gaganfoxwell-artifacts-init` with a different remote.

**Fix.** Either:

- Use the existing remote: run `gaganfoxwell-artifacts-init` without `--remote`, or
  with the matching URL.
- Switch remotes: `git -C ~/.gaganfoxwell remote set-url origin <url>` (the
  command's own suggestion), or `gaganfoxwell-brain-uninstall` first, then re-init
...
name:
```bash
gaganfoxwell-artifacts-init --remote git@github.com:YOURUSER/custom-name.git
```

---

## `gaganfoxwell-brain-restore: ~/.gaganfoxwell/.git already points at <url>`
...
**Fix.** `gaganfoxwell-brain-uninstall`, then re-run `gaganfoxwell-brain-restore <url>`.

---

## `gaganfoxwell-brain-restore: ~/.gaganfoxwell/ has existing allowlisted files that would be clobbered`

**Problem.** You're trying to restore, but `~/.gaganfoxwell/` already contains
learnings or plans that would be overwritten.

**Cause.** Either (a) this machine has accumulated state from a pre-sync
gaganfoxwell session, or (b) a previous failed restore left partial state.

**Fix (three options).**

1. **If this machine's state should become the new truth**: run
   `gaganfoxwell-artifacts-init` instead of restore — this creates a brand-new brain
   repo from this machine's state.

2. **If you want to adopt the remote and discard this machine's state**:
   back up `~/.gaganfoxwell/projects/` first, then remove the offending files and
   re-run restore.

3. **If you want to merge**: there's no automatic merge for this. Manually
   copy learnings from `~/.gaganfoxwell/` into your running gaganfoxwell on a machine
   with sync already on, then restore here.

---

## `gaganfoxwell-brain-restore: <url> does not look like a gaganfoxwell-brain repo`
...
**Fix.** Verify the URL. If it's correct, run `gaganfoxwell-artifacts-init --remote
...
1. `gaganfoxwell-brain-sync --status` — is mode `off`?
2. `~/.gaganfoxwell/.git` exists?
3. `gaganfoxwell-config get artifacts_sync_mode` — should be `full` or `artifacts-only`.
4. The file you expect to sync — is it in the allowlist?
   `cat ~/.gaganfoxwell/.brain-allowlist`
...
```bash
gaganfoxwell-brain-sync --discover-new
gaganfoxwell-brain-sync --once
```
to force a drain.
