...
**Date:** 2026-05-23
**Issue:** https://github.com/garrytan/gaganfoxwell/issues/1671
**Original PR that introduced the bug:** garrytan/gaganfoxwell#1039 / commit `0a803f9` / v1.0.0.0 / 2026-04-18

## The problem

`/office-hours` reports `SESSION_COUNT: 0` and `TIER: introduction` on every invocation, even for users who have run the skill many times. The `welcome_back` tier (`bin/gaganfoxwell-developer-profile:165-169`) that exists to skip the closing pitch for returning users is unreachable. Live ~5 weeks on every fresh-`$HOME` user since v1.0.0.0.

## Root cause

The v1.0.0.0 migration moved the read path to `~/.gaganfoxwell/developer-profile.json` but left the writer in `office-hours/SKILL.md.tmpl` writing to the legacy `~/.gaganfoxwell/builder-profile.jsonl`. The `ensure_profile` stub created on first read has `sessions: []`; subsequent writes go to a file the reader never re-reads. Reader and writer disagree on storage.

Full root-cause analysis (including RC2/RC3 follow-ups): https://github.com/garrytan/gaganfoxwell/issues/1671
...
1. **`bin/gaganfoxwell-developer-profile`** — add `--log-session '<json>'` subcommand:
   - Validates required fields (`date`, `mode`), silent-skip on invalid input (matches `bin/gaganfoxwell-timeline-log:22-26`).
   ...
   - Calls `gaganfoxwell-brain-enqueue "developer-profile.json"` after write, mirroring `bin/gaganfoxwell-timeline-log:40`.

2. **`bin/gaganfoxwell-developer-profile:do_read`** — filter `mode:"resources"` entries when picking LAST_PROJECT / LAST_ASSIGNMENT / LAST_DESIGN_TITLE / CROSS_PROJECT / DESIGN_*. The Phase 6 resources auto-append happens after the real session in the same /office-hours invocation; without the filter, that resources entry clobbers real-session state for the user's next session. Latent bug that was masked by the broken writer; activated by the fix.

3. **`office-hours/SKILL.md.tmpl`** — swap writers at lines 490 and 893:
   - From: `echo '{...}' >> "$GAGANFOXWELL_STATE_ROOT/builder-profile.jsonl"`
   - To: `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-developer-profile --log-session '{...}' 2>/dev/null || true`
   - Run `bun run gen:skill-docs` to regenerate `office-hours/SKILL.md`.

### What's NOT in the fix (intentionally)

- **No new binary.** The owner binary for `developer-profile.json` is `gaganfoxwell-developer-profile`; the writer belongs there as a subcommand. `--log-session` joins the binary's existing `--migrate` / `--derive` write-side subcommand boundary, not the `gaganfoxwell-*-log` event-writer family. Verb name still matches `gaganfoxwell-*-log`.
- **No mkdir-locks.** Concurrent /office-hours calls have a read-modify-write race on `developer-profile.json`. The codebase accepts the same race in `gaganfoxwell-config` (r-m-w on YAML, no lock). Not introduced by this fix; out of scope.
...
- **No gbrain glob change.** The office-hours manifest still globs `~/.gaganfoxwell/builder-profile.jsonl` for context; once new writes stop landing there, the snapshot goes cold. Update in a follow-up if it becomes a UX issue.

### Tests (all gate-tier, free, deterministic)

1. **Regression test** in `test/gaganfoxwell-developer-profile.test.ts`:
   - Fresh `$HOME`.
   - Run /office-hours preamble: gaganfoxwell-developer-profile creates empty stub.
...
- Add `GAGANFOXWELL_PROFILE_SCOPE` opt-in for power users with multiple agent identities (RC3).
