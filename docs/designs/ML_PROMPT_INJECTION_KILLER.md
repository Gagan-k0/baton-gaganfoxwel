...
**Date:** 2026-03-28
**CEO Plan:** ~/.gaganfoxwell/projects/garrytan-gaganfoxwell/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The gaganfoxwell Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any gaganfoxwell component can call these
...
```json
// ~/.gaganfoxwell/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "gaganfoxwell just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all gaganfoxwell users. Can we
...
2. Cache to ~/.gaganfoxwell/models/browsesafe-bench/ (not re-downloaded in CI)
...
- gaganfoxwell becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But gaganfoxwell already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.gaganfoxwell/models/
