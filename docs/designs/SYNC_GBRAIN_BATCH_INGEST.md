...
**Estimated effort:** human ~3 days / CC+gaganfoxwell ~2 hr
...
exfiltration boundary is `gaganfoxwell-brain-sync` (bin/gaganfoxwell-brain-sync:78-110,
...
for 5031). That's a gbrain-side perf issue, not gaganfoxwell architecture.
...
1. **gbrain import perf on large dirs** — investigate why 5031 files
   ...
   FTS index updates without batching. Lives in gbrain, not gaganfoxwell.
...
Two compounding bugs in `bin/gaganfoxwell-memory-ingest.ts`:

1. **Subprocess-per-file architecture.** The ingest loop at line 911 walks
   1,841 files in `~/.gaganfoxwell/projects/` and spawns two subprocesses per file:
   - `gitleaks detect --no-git --source <path>` — 46ms cold start (`lib/gaganfoxwell-memory-helpers.ts:157`)
   - `gbrain put <slug>` — 329ms cold start (`bin/gaganfoxwell-memory-ingest.ts:823`)
   - Per-file floor: 375ms × 1841 = 690s (11.5 min) of pure subprocess startup
     before any actual work happens.

2. **Kill-no-save timeout.** Orchestrator at `bin/gaganfoxwell-gbrain-sync.ts:442`
   enforces a 35-min timeout. When it fires, `spawnSync` returns
   `result.status === null`, the child gets SIGTERM, and the in-memory
   ingest state never flushes to `~/.gaganfoxwell/.transcript-ingest-state.json`.
...
| Metric | Value | Source |
|---|---|---|
| Files in walkAllSources | 1,841 | `find ~/.gaganfoxwell/projects -type f \( -name "*.md" -o -name "*.jsonl" \)` |
...
Take everything in `ingestPass` (lines 899-988 of `bin/gaganfoxwell-memory-ingest.ts`)
...
- Created at `~/.gaganfoxwell/.staging-ingest-${pid}-${ts}/`
...
redaction policy in `lib/gaganfoxwell-memory-helpers.ts`).
...
### Step 6: SIGTERM handler in `gaganfoxwell-memory-ingest.ts`
...
In `bin/gaganfoxwell-gbrain-sync.ts:444`:
...
`~/.gaganfoxwell/.ingest-skip-list.json` so the next prepare stage skips that file
entirely. Avoids re-staging a file that will always fail. User can review
the skip list with a new `gaganfoxwell-memory-ingest --skip-list` flag.
...
3. **Benchmark gate (periodic, paid):**
   ...
   - Test fixture: copy of `~/.gaganfoxwell/projects/` snapshot for repeatable timing.

## Rollback strategy

- New `--legacy-ingest` flag on `gaganfoxwell-memory-ingest` keeps the old
  per-file path callable for one release cycle.
- If batch path regresses on a real corpus, set
  `gaganfoxwell-config set memory_ingest_path legacy` to revert without redeploy.
...
4. **Cross-worktree concurrency.** `~/.gaganfoxwell/.staging-ingest-${pid}-${ts}/`
   is pid-namespaced so two concurrent /sync-gbrain runs don't collide.
   But the orchestrator already holds a lock at `~/.gaganfoxwell/.sync-gbrain.lock`
...
6. **Should we deprecate `gbrain put` for memory entirely?** The legacy
   ...
   ingestion? Probably yes (for `~/.gaganfoxwell/.transcript-ingest-state.json`
   updates triggered outside the orchestrator), but worth confirming.

## What this isn't

- Not a gbrain CLI change. All work is in gaganfoxwell.
