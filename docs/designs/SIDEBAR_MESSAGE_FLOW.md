...
How the Gaganfoxwell Browser sidebar actually works. Read this before touching
...
T+1-3s    Extension loads, sidebar opens
            ...
            └── tryAutoConnect() polls until window.gaganfoxwellServerPort + token are set
...
2. Extension calls `new WebSocket(url, ['gaganfoxwell-pty.<token>'])`.
3. Agent reads `Sec-WebSocket-Protocol`, strips `gaganfoxwell-pty.`, validates
...
A `Set-Cookie: gaganfoxwell_pty=...` header is also returned for non-browser
...
| `gaganfoxwell-pty.<...>` (Sec-WebSocket-Protocol) | Browser memory only; agent `validTokens` Set | `/ws` upgrade auth | 30 min, auto-revoked on WS close |
...
| Button | Behavior |
|--------|----------|
| 🧹 Cleanup | `window.gaganfoxwellInjectToTerminal(prompt)` — pipes a "remove ads/banners" instruction into the live PTY. claude in the terminal sees it and acts. |
...
The Inspector's "Send to Code" button uses the same `gaganfoxwellInjectToTerminal`
