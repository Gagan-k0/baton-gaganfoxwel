# Design: gaganfoxwell Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.gaganfoxwell/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/gaganfoxwell-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/gaganfoxwell-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `GAGANFOXWELL_DESIGN` env var, similar to how `GAGANFOXWELL_BROWSE` is set.
...
[ -z "$D" ] && D=~/.claude/skills/gaganfoxwell/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `GAGANFOXWELL_DESIGN` env var export for Codex host |
...
1. Read `~/.gaganfoxwell/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.gaganfoxwell/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/gaganfoxwell/` install path
...
- Auth module (read ~/.gaganfoxwell/openai.json, fallback to env var, guided setup flow)
...
- Add GAGANFOXWELL_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
