...
gaganfoxwell already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has gaganfoxwell's artifact architecture.

## The Insight

gaganfoxwell already writes structured artifacts to `~/.gaganfoxwell/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.gaganfoxwell/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.gaganfoxwell/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle gaganfoxwell artifacts)
