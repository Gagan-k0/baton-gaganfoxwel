...
`scripts/gaganfoxwell2/browser-provider-contract.ts` (the existing provider-contract idiom),
`lib/gaganfoxwell-decision-semantic.ts` (degrade-to-null reliability contract)

## Problem

gaganfoxwell carries ~17k LOC of home-grown code-intelligence glue: transcript
ingestion (`bin/gaganfoxwell-memory-ingest.ts`, ~1.9k), a unified sync verb
(`bin/gaganfoxwell-gbrain-sync.ts`, ~1.6k), context loading
(`bin/gaganfoxwell-brain-context-load.ts`), a three-tier planning cache
(`bin/gaganfoxwell-brain-cache`), source reconciliation, engine-status classification,
destructive-op guards, plus ~15 `bin/gaganfoxwell-gbrain-*` and `bin/gaganfoxwell-brain-*`
entrypoints and ~40 tests. All of it is bespoke wiring around one external tool
(GBrain) reached by direct CLI shell-out.

We do not want to keep maintaining a home-grown indexer. We want gaganfoxwell to
define a **small optional contract** that external providers implement, so the
indexing/search/graph work lives in the provider, not in gaganfoxwell.

Hard requirement, non-negotiable: **gaganfoxwell must remain fully functional with the
...
decision-store philosophy (`lib/gaganfoxwell-decision.ts` has zero gbrain imports;
`lib/gaganfoxwell-decision-semantic.ts` degrades to `null`). The contract is an
...
2. **Install consent (Graphify only).** Graphify is never auto-installed. The
   ...
   and nothing in gaganfoxwell runs a Graphify installer. Install is a user action
...
- **GBrain** (`garrytan/gbrain`, the gaganfoxwell-ecosystem tool): full contract fit,
...
Each provider exposes a runtime-drivable surface, so gaganfoxwell drives them with a
...
with `gaganfoxwell-code-intelligence select <provider>`, persisted to
`$GAGANFOXWELL_HOME/code-intelligence.json`. `resolveSelectedProvider()` constructs the
...
|-----------------|--------------------|
| `bin/gaganfoxwell-gbrain-sync.ts` (`sync`/`reindex-code`/`sources`) | `provider.registerSource` / `provider.refresh` |
| `lib/gaganfoxwell-decision-semantic.ts` `semanticRecall` | `provider.search` (scoped) → same degrade-to-null |
| `bin/gaganfoxwell-brain-context-load.ts` (`query`/`list_pages`) | `provider.search` / `provider.status` |
| `bin/gaganfoxwell-memory-ingest.ts` (`import`, put) | `provider.add` (optional cap; GBrain-only) |
...
| `bin/gaganfoxwell-gbrain-detect` / `-install` / `-source-wireup` / `-repo-policy` | provider setup + picker + consent (thinner) |
...
`gen:gaganfoxwell2` / parity re-baseline cycle.
...
- **Phase 1 (this slice): the contract, three real adapters, and a usable CLI.**
  ...
  (HTTP + config) adapters + the selection store + the `gaganfoxwell-code-intelligence`
  ...
  generated-file changes yet, so no `gen:gaganfoxwell2` / parity re-baseline.
- **Phase 2: route internal consumers through the contract.** Point
  `gaganfoxwell-decision-semantic` and `gaganfoxwell-brain-context-load` at
...
- **Phase 3: surface selection in the skills.** Offer the picker at the moments a
  ...
  just-in-time consent prompt. Regenerate skills (`bun run gen:gaganfoxwell2`), re-run
  `bun run test:gaganfoxwell2`, re-baseline parity intentionally.
...
Per the Gaganfoxwell 2 canonical contract and CLAUDE.md boundaries: no cloud browsers,
...
is absent. The `gaganfoxwell-code-intelligence` CLI was smoke-tested end-to-end:
