...
Catalog trim (Codex high-leverage win)  Lighter-touch migration
  ...
  drop voice triggers/proactive blocks    on /gaganfoxwell-upgrade
...
| `gaganfoxwell-upgrade/migrations/` | Pattern exists for state-format migrations; reuse for v2 auto-regenerate |
| `~/.gaganfoxwell/analytics/skill-usage.jsonl` | Already collected; powers deferred `gaganfoxwell budget` CLI |
...
**Goal:** every skill in gaganfoxwell ships with at least one gate-tier eval AND one periodic-tier eval that asserts a must-have behavior. The eval suite becomes the design spec. This is the load-bearing claim of the plan — must come first.
...
| gaganfoxwell-upgrade | detects install type, runs upgrade | full upgrade + migration round-trip | $0.20 / $1.00 |
...
| open-gaganfoxwell-browser | launches browser without error | sidebar attaches and shows activity | $0.20 / $0.80 |
...
- `/gaganfoxwell-upgrade` auto-regenerates on next invocation. No interactive migration prompts.
...
- `gaganfoxwell-upgrade --explain-v2` flag for users who want the full explanation on demand.
...
| 8. Observability | budget-regression test already exists; canary cohort transcript logging in Phase B; migration outcome logged to ~/.gaganfoxwell/analytics/migrations.jsonl | Findings addressed |
...
| 11. Design/UX | README v2 banner + CHANGELOG numbers table land in v2.0.0.0; concrete numbers, gaganfoxwell voice, no AI slop | OK |

## NOT in scope

- **Skill removals.** User said "keep all functions." qa-only, design-shotgun, pair-agent, open-gaganfoxwell-browser all stay. They get evals + catalog trim like everyone else.
- **Skill renames.** No `qa` → `qa-fix` collapses. Keep CLI surface stable.
- **gaganfoxwell lite/pro install profiles.** Deferred to TODOS for post-v2.
- **gaganfoxwell budget CLI.** Deferred to TODOS for post-v2.
...
- **gaganfoxwell diff PR comment.** Deferred to TODOS.

## TODOS.md updates (deferred items, recommend bulk-add post-merge)

| TODO | Priority | Effort (human / CC) | Depends on |
|---|---|---|---|
| `gaganfoxwell lite` install profile (5-skill core) | P2 | 2 days / 3-4 hrs | v2.0.0.0 |
| `gaganfoxwell pro` opt-in upgrade path | P2 | 1 day / 1 hr | gaganfoxwell lite |
| `gaganfoxwell budget` CLI (per-skill token usage telemetry) | P2 | 1 day / 1 hr | v1.45.0.0 |
| Per-skill eval coverage badge in `gaganfoxwell-skills list` + README | P3 | 1 day / 1 hr | Phase 0 |
| Cross-tool portability test/demo (Codex CLI, Cursor) | P3 | 2 days / 2 hrs | v2.0.0.0 |
| Token-cost preview on skill invocation | P3 | 1 day / 1 hr | gaganfoxwell budget CLI |
| Skill autoload telemetry (dead-weight detection) | P3 | 2 days / 2 hrs | v1.45.0.0 |
| `gaganfoxwell diff` PR comment (per-PR budget delta) | P3 | 1 day / 1 hr | budget-regression extended |
...
| `gaganfoxwell-upgrade/migrations/v2.0.0.0.sh` (new) | Auto-regenerate + vendored-install warning | B |
...
6. Migration: `gaganfoxwell-upgrade` on a v1.45 install successfully regenerates without prompts; vendored-install warning appears once
...
- [ ] **T3 (P1, human: ~half day / CC: ~30 min)** — A.2 + A.3 jargon dedup + terse-mode gen-time compression
  ...
  - Verify: jargon-list no longer appears inlined in generated SKILL.md; `gaganfoxwell-config set explain_level terse && bun run gen:skill-docs` produces shorter files
...
- [ ] **T14 (P1, human: ~half day / CC: ~30 min)** — `gaganfoxwell-upgrade/migrations/v2.0.0.0.sh` lighter-touch auto-regenerate
  - Surfaced by: Migration approach section
  - Files: `gaganfoxwell-upgrade/migrations/v2.0.0.0.sh`
...
- [ ] **T16 (P2, human: ~1 day / CC: ~1 hour)** — Bulk-add 9 deferred TODOS to TODOS.md (gaganfoxwell lite, gaganfoxwell budget, etc.)
...
| Migration script v2.0.0.0.sh | regenerate fails on damaged install | Y — script aborts, prints repair steps | Y (migration test) | clear error + repair steps | ~/.gaganfoxwell/analytics/migrations.jsonl |
...
| Canary section-Read logger | logger missing for a heavyweight skill | Y — silently skipped, gap visible in dashboard | Y (transcript-logger test) | none directly; surfaced in canary dashboard | ~/.gaganfoxwell/analytics/section-reads.jsonl |
...
```
  CONFIG (~/.gaganfoxwell/config.yaml)
     |
     v
  +-----------------+      +--------------------+
...
Test plan written to `~/.gaganfoxwell/projects/garrytan-gaganfoxwell/garrytan-garrytan-slim-skill-tokens-eng-review-test-plan-<timestamp>.md`. `/qa` and `/qa-only` consume this as primary test input. Covers: per-phase test coverage targets, fixture design for section-loading tests, CI budget enforcement check, migration round-trip test.
...
| T14 migration script | `gaganfoxwell-upgrade/migrations/v2.0.0.0.sh` | T13 |
...
- **Public benchmark page:** `gaganfoxwell.benchmarks.md` (new), continuously updated. Quotable: "v2 average parity score: 9.2/10, average token reduction: 67%."
...
Triggered by `gaganfoxwell-update-check` showing `JUST_UPGRADED v1.x v2.0.0.0`. Replaces the generic v1 "Running gaganfoxwell v{to} (just updated!)" with persona-A-aware copy that names the perceived speed win AND signals "your muscle memory still works."

```
Running gaganfoxwell v2.0.0.0 (just updated!) — your sessions are now ~67% lighter.
...
/review commands haven't changed. Run `/gaganfoxwell-upgrade --explain-v2` for the
...
Implementation: update `~/.claude/skills/gaganfoxwell/gaganfoxwell-upgrade/SKILL.md.tmpl` Inline upgrade flow with v2-aware message; existing `JUST_UPGRADED <from> <to>` detection in skill preamble fires it.
...
Below the table, one paragraph in gaganfoxwell voice: "v1 was the heaviest opinionated skill pack. v2 is the lightest. The compression isn't free — every skill ships with both gate-tier and periodic-tier E2E evals, and a continuous parity-monitor catches silent quality regressions. The numbers above are measured against `test/helpers/parity-baseline-v1.44.1/` and reproduced by `bun run eval:parity`."
...
```markdown
> **gaganfoxwell v2.0.0.0 — the lightest opinionated skill pack (May 2026)**
>
...
> the migration story. Existing users: `/gaganfoxwell-upgrade` auto-regenerates.
```
...
- The JUST_UPGRADED notice fires automatically via existing `gaganfoxwell-upgrade` detection — no new mechanism required.
- The README banner placement above the existing Karpathy quote is intentional: persona B (new evaluator) sees the v2 win BEFORE the Karpathy framing, anchoring "this is May 2026's most-current gaganfoxwell."

## GAGANFOXWELL REVIEW REPORT

| Review | Trigger | Why | Runs | Status | Findings |
|---|---|---|---|---|---|
| CEO Review | `/plan-ceo-review` | Scope & strategy | 1 | CLEAR | SCOPE_EXPANSION mode; 3 expansion proposals (1 accepted: v2 launch positioning; 2 deferred: gaganfoxwell lite, gaganfoxwell budget); 11/11 sections reviewed; 0 critical gaps |
