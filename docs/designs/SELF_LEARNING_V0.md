# Design: Gaganfoxwell Self-Learning Infrastructure
...
Gaganfoxwell runs 30+ skills across sessions but learns nothing between them. A /review
...
Structured, typed, confidence-scored learnings that every gaganfoxwell skill can read and
write. The goal: after 20 sessions on the same codebase, gaganfoxwell knows every
...
YC founders building with AI. The people who run gaganfoxwell on real codebases 20+ times
...
| **Gaganfoxwell** | **Per-project JSONL** | **Cross-session, cross-skill** | **Typed, scored, decaying** |

---

## State Systems

gaganfoxwell has four distinct persistence layers. They share storage patterns
(JSONL in `~/.gaganfoxwell/projects/$SLUG/`) but serve different purposes:
...
### Release 1: "Gaganfoxwell Learns" (v0.13-0.14) — SHIPPED

**Headline:** Every session makes the next one smarter.

What shipped:
- Learnings persistence at `~/.gaganfoxwell/projects/{slug}/learnings.jsonl`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via gaganfoxwell-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read gaganfoxwell-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via gaganfoxwell-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.gaganfoxwell/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), gaganfoxwell suggests what you probably want next.
...
- Timeline binaries: `bin/gaganfoxwell-timeline-log` and `bin/gaganfoxwell-timeline-read`.
...
**Headline:** Gaganfoxwell respects your time without compromising your safety.
...
The self-learning roadmap was inspired by ideas from the [Compound Engineering](https://github.com/nicobailon/compound-engineering) project by Nico Bailon. Their exploration of learnings persistence, parallel review agents, and autonomous pipelines catalyzed the design of Gaganfoxwell's approach. We adapted every concept to fit Gaganfoxwell's template system, voice, and architecture rather than porting directly.
