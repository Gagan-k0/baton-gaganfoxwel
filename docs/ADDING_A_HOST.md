# Adding a New Host to gaganfoxwell

gaganfoxwell uses a declarative host config system. Each supported AI coding agent
...
- `globalRoot` / `localSkillRoot`: `.myhost/skills/gaganfoxwell`, `hostSubdir`: `.myhost`
...
- `runtimeRoot`: the shared asset list (`bin`, `browse/dist`, `browse/bin`,
  `gaganfoxwell-upgrade`, `ETHOS.md` + review checklist files)
...
- `pathRewrites`: replaces the derived list entirely. Only for non-mechanical
  cases — codex and factory rewrite the global path to `$GAGANFOXWELL_ROOT` and add
...
ls .myhost/skills/gaganfoxwell-*/SKILL.md
