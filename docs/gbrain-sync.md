...
gaganfoxwell writes a lot of useful state to `~/.gaganfoxwell/` — learnings, retros, CEO
...
- A clean off-ramp (`gaganfoxwell-brain-uninstall`) that never touches your data.
...
The exact allowlist lives in `~/.gaganfoxwell/.brain-allowlist`. The CLI manages
...
1. Turns `~/.gaganfoxwell/` into a git repo.
2. Asks for a remote URL (default: `gh repo create --private
   gaganfoxwell-artifacts-$USER`). Any git remote works — GitHub, GitLab, Gitea,
   self-hosted.
3. Pushes an initial commit with just the config.
4. Writes `~/.gaganfoxwell-artifacts-remote.txt` (URL-only, no secrets —
...
5. Prints the `gbrain sources add` hookup command for the brain host
   ...
   `bin/gaganfoxwell-gbrain-source-wireup` does the same wiring) so
   `gbrain search` can index your synced learnings, plans, and designs.
   The old `gaganfoxwell-brain-reader add --ingest-url ...` HTTP path was
...
- **Decline**: keep everything local. You can turn sync on later with
  `gaganfoxwell-config set artifacts_sync_mode full`.

Your answer is persisted. You won't be asked again.

## Cross-machine workflow

On machine A: run `gaganfoxwell-artifacts-init` once. That's it — every skill
invocation now drains the sync queue at its start and end boundaries
(~200–800 ms network pause per skill).

On machine B:

1. Copy `~/.gaganfoxwell-artifacts-remote.txt` from machine A to machine B
   (password manager, dotfile repo, USB stick — your call; the legacy
   `~/.gaganfoxwell-brain-remote.txt` name is still recognized).
2. Run any gaganfoxwell skill. The preamble sees the URL file and prints:
   ```
   BRAIN_SYNC: brain repo detected: <url>
   BRAIN_SYNC: run 'gaganfoxwell-brain-restore' to pull your cross-machine memory
   ```
3. Run `gaganfoxwell-brain-restore`. That clones the repo, rehydrates your
...
```bash
gaganfoxwell-brain-sync --status
```
...
```bash
gaganfoxwell-config set artifacts_sync_mode full
gaganfoxwell-config set artifacts_sync_mode off
```
...
2. If the match is a false positive on content you explicitly want to
   sync, run `gaganfoxwell-brain-sync --skip-file <path>` to permanently
   exclude that path.
3. Otherwise, edit the file to remove the secret and re-run any skill.

There's a defense-in-depth hook at `~/.gaganfoxwell/.git/hooks/pre-commit` that
...
egress ledger (`~/.gaganfoxwell/security/egress.jsonl`) *before* anything is
sent, fail-closed: if the receipt can't be written, the push is refused
and the queue is preserved. Inspect the ledger with `gaganfoxwell-egress list`
and verify its hash chain with `gaganfoxwell-egress verify`.
...
(cached via `~/.gaganfoxwell/.brain-last-pull`). You don't need to think about
this — it happens automatically at the first skill invocation each day.

Historical note (#2516): that daily pull refreshed only `~/.gaganfoxwell` itself —
NOT the detached worktree at `~/.gaganfoxwell-brain-worktree` that gbrain actually
...
the brain worktree (`gaganfoxwell-gbrain-source-wireup --advance-only`, throttled
via `~/.gaganfoxwell/.brain-worktree-last-advance`); a failed advance warns instead
of failing silently, and never force-resets a dirty worktree.

## Uninstall

```bash
gaganfoxwell-brain-uninstall
```

This:

- Removes `~/.gaganfoxwell/.git/` and all `.brain-*` config files.
- Clears `artifacts_sync_mode` in `gaganfoxwell-config`.
...
Re-init anytime with `gaganfoxwell-artifacts-init`.

## Troubleshooting

See [gbrain-sync-errors.md](gbrain-sync-errors.md) for an index of every
error message gaganfoxwell-brain may print, with problem / cause / fix for each.
