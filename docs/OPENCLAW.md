# gaganfoxwell x OpenClaw Integration

gaganfoxwell integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. gaganfoxwell provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates gaganfoxwell-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates gaganfoxwell-full
       │           └── gaganfoxwell installed at      │   (complete pipeline)
       │               ~/.claude/skills/gaganfoxwell  │
...
OpenClaw decides at spawn time which tier of gaganfoxwell support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No gaganfoxwell context injected |
| **Medium** | Multi-file features, refactors | gaganfoxwell-lite CLAUDE.md appended |
| **Heavy** | Specific gaganfoxwell skill needed | "Load gaganfoxwell. Run /X" |
| **Full** | Complete features, objectives, projects | gaganfoxwell-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | gaganfoxwell-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-gaganfoxwell-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY gaganfoxwell skill,
...
gaganfoxwell-lite/full as a new section. Do not replace the repo's existing instructions.

## What gaganfoxwell generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### gaganfoxwell-lite (Medium tier)
`openclaw/gaganfoxwell-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### gaganfoxwell-full (Full tier)
`openclaw/gaganfoxwell-full-CLAUDE.md` — chains existing gaganfoxwell skills:
...
### gaganfoxwell-plan (Plan tier)
`openclaw/gaganfoxwell-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `gaganfoxwell-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `gaganfoxwell-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `gaganfoxwell-openclaw-investigate` — Operational debugging (4-phase methodology)
- `gaganfoxwell-openclaw-retro` — Operational retrospective (weekly review)

Source lives in `openclaw/skills/` in the gaganfoxwell repo. These are hand-crafted
adaptations of the gaganfoxwell methodology for OpenClaw's conversational context.
No gaganfoxwell infrastructure (no browse, no telemetry, no preamble).

## Spawned session detection

When Claude Code runs inside a session spawned by OpenClaw, the `OPENCLAW_SESSION`
environment variable should be set. gaganfoxwell detects this and adjusts:
...
For OpenClaw users: tell your OpenClaw agent "install gaganfoxwell for openclaw."

The agent should:
1. Install gaganfoxwell-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For gaganfoxwell developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from gaganfoxwell (OpenClaw has its own)
