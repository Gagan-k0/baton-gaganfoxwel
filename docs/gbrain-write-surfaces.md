...
|---|---|
| Any host + `gaganfoxwell-config gbrain-refresh` reports `gbrain_local_status: "ok"` | Compressed brain-aware blocks render. Agent reads this doc on-demand when it actually saves. ~250 token overhead per planning skill. |
...
`bin/gaganfoxwell-gbrain-sync.ts` for code-lookup resolvers; gaganfoxwell treats the
...
and writeback. Set via `gaganfoxwell-config set
...
- **Any other non-zero exit**: treat as transient failure. Do not retry
  ...
  `gaganfoxwell-config gbrain-refresh` if they suspect gbrain itself is
  misconfigured.
...
gbrain sources list                    # confirm gaganfoxwell-brain-<user> source
...
endpoint depending on the user's `~/.gbrain/config.json`. **gaganfoxwell
...
2. `gaganfoxwell-config get brain_trust_policy@<endpoint-hash>` must be
   `personal` for auto-write. Run `gaganfoxwell-config endpoint-hash` to get
...
3. If trust policy is `personal` and `gbrain doctor` is clean but the
   page still isn't there, file an issue against gbrain — gaganfoxwell's
...
- **`.gbrain-source` write semantics**: gaganfoxwell treats the documented
