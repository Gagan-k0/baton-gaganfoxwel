...
**Downstream consumers:** T9 (gaganfoxwell-codex-session-import)

## Question this spike answers

What's the actual on-disk format of Codex sessions, and how do we recover
AskUserQuestion-shaped events from it for `gaganfoxwell-codex-session-import`?
...
stream. Gaganfoxwell skills running on Codex emit AskUserQuestion-shaped
...
## Recovery strategy for `gaganfoxwell-codex-session-import`

**Two-tier extraction:**

1. **Marker-first (D18 mechanism).** Search `agent_message` text for the
   `<gaganfoxwell-qid:foo-bar>` marker. If present, we have an exact question_id
...
Per existing `bin/gaganfoxwell-question-log` schema, augmented with:
...
`bin/gaganfoxwell-slug` logic on the cwd path. Conductor worktrees have their
