...
**For our use case:**
- gaganfoxwell registers exactly one PreToolUse hook and one PostToolUse hook on
...
- Mitigation: document this constraint in `bin/gaganfoxwell-settings-hook`
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CLAUDE_PROJECT_DIR/.claude/skills/gaganfoxwell/hosts/claude/hooks/question-preference-hook",
            "timeout": 5
          }
        ]
      }
...
2. **Auto-decided event tagging.** When hook returns `updatedInput`, the
   ...
   tracking — write a marker file in `~/.gaganfoxwell/sessions/<id>/.auto-decided-<tool_use_id>`
...
- Existing `bin/gaganfoxwell-settings-hook` (SessionStart-only impl, to be
