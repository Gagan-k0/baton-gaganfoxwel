...
Detailed guides for every gaganfoxwell skill — philosophy, workflow, and examples.
...
| [`/learn`](#learn) | **Memory** | Manage what gaganfoxwell learned across sessions. Review, search, prune, and export project-specific patterns and preferences. |
...
| [`/open-gaganfoxwell-browser`](#open-gaganfoxwell-browser) | **Gaganfoxwell Browser** | Launch Gaganfoxwell Browser with sidebar, anti-bot stealth, auto model routing, cookie import, and Claude Code integration. Watch every action live. |
...
| [`/gaganfoxwell-upgrade`](#gaganfoxwell-upgrade) | **Self-Updater** | Upgrade gaganfoxwell to the latest version. Detects global vs vendored install, syncs both, shows what changed. |
...
| [`/ios-sync`](#ios-sync) | **iOS Bridge Resync** | Regenerate accessors and Swift templates against the latest upstream gaganfoxwell. Run when you add new `@Observable` classes or upgrade gaganfoxwell. |
...
Both modes end with a design doc written to `~/.gaganfoxwell/projects/` — and that doc feeds directly into `/plan-ceo-review` and `/plan-eng-review`. The full lifecycle is now: `office-hours → plan → implement → review → QA → ship → retro`.
...
Visions and decisions are persisted to `~/.gaganfoxwell/projects/` so they survive beyond the conversation. Exceptional visions can be promoted to `docs/designs/` in your repo for the team.
...
Eng Review is the only required gate (disable with `gaganfoxwell-config set skip_eng_review true`). CEO and Design are informational — recommended for product and UI changes respectively.

### Plan-to-QA flow

When `/plan-eng-review` finishes the test review section, it writes a test plan artifact to `~/.gaganfoxwell/projects/`. When you later run `/qa`, it picks up that test plan automatically — your engineering review feeds directly into QA testing with no manual copy-paste.
...
Claude: [Runs full 80-item visual audit on the live site]
        ...
        [Report with before/after screenshots saved to .gaganfoxwell/design-reports/]
```
...
6. The approved variant saves to `~/.gaganfoxwell/projects/$SLUG/designs/` with an `approved.json`
...
Claude: Approved Variant A. Saved to ~/.gaganfoxwell/projects/myapp/designs/
...
Claude: Saved to ~/.gaganfoxwell/projects/myapp/designs/hero-20260330/finalized.html
```
...
Claude: [Explores 12 pages, fills 3 forms, tests 2 flows]
        ...
        [Full report with screenshots saved to .gaganfoxwell/qa-reports/]
```
...
`/browse` runs headless by default. You don't see what the agent sees. `/open-gaganfoxwell-browser` changes that. It launches Gaganfoxwell Browser (rebranded Chromium with anti-bot stealth) controlled by Playwright, with the sidebar extension auto-loaded. You watch every action in real time.

The sidebar chat is a Claude instance that controls the browser. It auto-routes to the right model: Sonnet for navigation and actions (click, goto, fill, screenshot), Opus for reading and analysis (summarize, find bugs, describe). One-click cookie import from the sidebar footer. The browser stays alive as long as the window is open... no idle timeout in headed mode. The menu bar says "Gaganfoxwell Browser" instead of "Chrome for Testing."
...
```
You:   /open-gaganfoxwell-browser

Claude: Launched Gaganfoxwell Browser with sidebar extension.
...
Keep gaganfoxwell current with one command. It detects your install type (global at `~/.claude/skills/gaganfoxwell` vs vendored in your project at `.claude/skills/gaganfoxwell`), runs the upgrade, syncs both copies if you have dual installs, and shows you what changed.

```
You:   /gaganfoxwell-upgrade
...
Set `auto_upgrade: true` in `~/.gaganfoxwell/config.yaml` to skip the prompt entirely — gaganfoxwell upgrades silently at the start of each session when a new version is available.
...
Install Greptile on your GitHub repo at [greptile.com](https://greptile.com) — it takes about 30 seconds. Once it's reviewing your PRs, gaganfoxwell picks up its comments automatically. No additional configuration.
...
gaganfoxwell solves this. `/review` and `/ship` are now Greptile-aware. They read Greptile's comments, classify each one, and take action:
...
Every false positive you confirm gets saved to `~/.gaganfoxwell/greptile-history.md`. Future runs auto-skip known FP patterns for your codebase. And `/retro` tracks Greptile's batting average over time — so you can see whether the signal-to-noise ratio is improving.
...
Run after upgrading gaganfoxwell or adding new `@Observable` classes. Detects what's installed, runs gen-accessors against the latest upstream templates, refreshes any changed Swift files, verifies the app rebuilds. Cache-key invalidation handles Swift version changes, generator git rev changes, and source changes.
