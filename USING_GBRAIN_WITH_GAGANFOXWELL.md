...
- **Cross-session memory.** Plans, retros, decisions, and learnings from past sessions live in `~/.gaganfoxwell/` and (if you opted in to artifacts sync) get pushed to a private git repo that gbrain indexes. `gbrain search "what did we decide about auth?"` actually finds the prior CEO plan instead of you re-describing context every session.
...
**Embedding model.** When `VOYAGE_API_KEY` is set, gaganfoxwell inits PGLite with `voyage-code-3` (1024-dim) — Voyage's code-specialized embedding model, which beats their general-purpose `voyage-4-large` and OpenAI `text-embedding-3-large` head-to-head on this codebase's symbol queries. Without `VOYAGE_API_KEY`, gbrain auto-selects (OpenAI 1536-dim when `OPENAI_API_KEY` is present, else falls down its provider chain). Either way, the embeddings call out to the chosen provider's API during sync — set the key for the provider you want before running `/sync-gbrain`.
...
The skill asks once per repo the first time you run a gaganfoxwell skill there. After that the decision is sticky — every worktree + branch of the same git remote shares the same policy, so you set it once and it follows you.
...
# Or directly:
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-repo-policy set "github.com/foo/bar" read-only
```

**To see every policy:**

```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-gbrain-repo-policy list
```

Storage: `~/.gaganfoxwell/gbrain-repo-policy.json`, mode 0600, schema-versioned so future migrations stay deterministic.
...
The skill runs three stages — code, memory, brain-sync — independently. A failure in one doesn't block the others. State persists to `~/.gaganfoxwell/.gbrain-sync-state.json` so re-running picks up cleanly.

Stages that can send data off-machine (code sync into a possibly-remote gbrain DB, memory ingest, the brain-sync push) each write a tamper-evident receipt to the egress ledger (`~/.gaganfoxwell/security/egress.jsonl`) before sending, fail-closed: if the receipt can't be written, the stage refuses with `EGRESS_RECEIPT_FAILED` instead of syncing unrecorded. Fix is usually `mkdir -p ~/.gaganfoxwell/security && chmod -R u+w ~/.gaganfoxwell/security`, then re-run. Inspect receipts with `gaganfoxwell-egress list`.
...
3. **Memory stage.** Stages your `~/.gaganfoxwell/` transcripts + curated memory. In local-stdio MCP mode, ingests into the local engine. In remote-http MCP mode, persists staged markdown to `~/.gaganfoxwell/transcripts/run-<pid>-<ts>/` for the remote brain admin's pull pipeline. The ingest timeout is 30 minutes by default; raise it for a big brain with `GAGANFOXWELL_INGEST_TIMEOUT_MS` (accepts 1 min–24h). On timeout the gbrain import checkpoint is preserved, so the next `/sync-gbrain` resumes instead of starting over.
...
Re-runnable, idempotent, safe to run from multiple terminals on the same machine (locked at `~/.gaganfoxwell/.sync-gbrain.lock`).
...
**If migration hangs:** another gaganfoxwell session may be holding a lock on the source brain. The timeout fires at 3 minutes with an actionable message. Close other workspaces and re-run.

## Gaganfoxwell memory sync (a separate concern)

This is different from gbrain itself. Your gaganfoxwell state (`~/.gaganfoxwell/` — learnings, plans, retros, timeline, developer profile) is machine-local by default. "Gaganfoxwell memory sync" optionally pushes a curated, secret-scanned subset to a private git repo so your memory follows you across machines — and, if you're running gbrain, that git repo becomes indexable there too.

Turn it on with:

```bash
gaganfoxwell-artifacts-init
```
...
**On a new machine:** Copy `~/.gaganfoxwell-artifacts-remote.txt` over (the legacy
`~/.gaganfoxwell-brain-remote.txt` name still works), run `gaganfoxwell-brain-restore`, and
...
| Bin | Purpose |
|---|---|
| `gaganfoxwell-gbrain-detect` | Emit current state as JSON: gbrain on PATH, version, config engine, doctor status, sync mode |
| `gaganfoxwell-gbrain-install` | Detect-first installer (probes `~/git/gbrain`, `~/gbrain`, then fresh clone). Has `--dry-run` and `--validate-only` flags. PATH-shadow check exits 3 with remediation menu. |
| `gaganfoxwell-gbrain-lib.sh` | Sourced, not executed. Provides `read_secret_to_env VARNAME "prompt" [--echo-redacted "<sed-expr>"]` |
| `gaganfoxwell-gbrain-supabase-verify` | Structural URL check. Rejects direct-connection URLs (`db.*.supabase.co:5432`) with exit 3 |
| `gaganfoxwell-gbrain-supabase-provision` | Management API wrapper. Subcommands: `list-orgs`, `create`, `wait`, `pooler-url`, `list-orphans`, `delete-project`. All require `SUPABASE_ACCESS_TOKEN` in env. `create` and `pooler-url` also require `DB_PASS`. `--json` mode available on every subcommand. |
| `gaganfoxwell-gbrain-repo-policy` | Per-remote trust triad. Subcommands: `get`, `set`, `list`, `normalize` |
| `gaganfoxwell-gbrain-source-wireup` | Registers your `~/.gaganfoxwell/` brain repo with gbrain as a federated source via `gbrain sources add` + `git worktree`, then runs an initial `gbrain sync`. Idempotent. Replaces the dead `consumers.json + /ingest-repo` HTTP wireup from v1.12.x. Flags: `--strict`, `--source-id <id>`, `--no-pull`, `--uninstall`, `--probe`. |

### gbrain CLI (upstream tool)

Gbrain itself ships with these that gaganfoxwell wraps:
...
| `~/.gaganfoxwell/gbrain-repo-policy.json` | Per-remote trust triad. Schema v2. Mode 0600. |
| `~/.gaganfoxwell/.setup-gbrain.lock.d` | Concurrent-run lock (atomic mkdir). Released on normal exit + SIGINT. |
| `~/.gaganfoxwell/.brain-queue.d/` | Pending sync records for gaganfoxwell memory sync — maildir-style spool, one file per record. A legacy `.brain-queue.jsonl` from older releases migrates automatically on the next drain. |
| `~/.gaganfoxwell/.brain-last-push` | Timestamp of last sync push (for `/health` scoring) |
| `~/.gaganfoxwell-artifacts-remote.txt` | URL of your gaganfoxwell memory sync remote (safe to copy between machines; legacy name `~/.gaganfoxwell-brain-remote.txt` still read) |
| `~/.gaganfoxwell/.setup-gbrain-inflight.json` | Reserved for future `--resume-provision` persisted state |

### Environment variables

| Var | Where it's read | What it does |
|---|---|---|
| `SUPABASE_ACCESS_TOKEN` | `gaganfoxwell-gbrain-supabase-provision` | PAT for Management API calls. Discarded after each setup run. |
| `DB_PASS` | `gaganfoxwell-gbrain-supabase-provision` (create, pooler-url) | Generated DB password. Never in argv. |
...
| `SUPABASE_API_BASE` | `gaganfoxwell-gbrain-supabase-provision` | Override the Management API host. Used by tests to point at a mock server. |
| `GBRAIN_INSTALL_DIR` | `gaganfoxwell-gbrain-install` | Override default install path (`~/gbrain`) |
| `GAGANFOXWELL_HOME` | every bin helper | Override `~/.gaganfoxwell` state dir. Heavy test use. |
| `VOYAGE_API_KEY` | `gbrain embed` subprocess; gaganfoxwell PGLite init | When set, gaganfoxwell inits PGLite with `voyage-code-3` (1024-dim), Voyage's code-specialized embedding model. Beats `voyage-4-large` and OpenAI `text-embedding-3-large` head-to-head on this codebase's symbol queries. See CHANGELOG v1.43.1.0 for the A/B numbers. |
...
| `GAGANFOXWELL_OPENAI_API_KEY` | `lib/conductor-env-shim.ts` | Conductor-injected fallback. Promoted to `OPENAI_API_KEY` when the canonical name is empty. |
| `GAGANFOXWELL_ANTHROPIC_API_KEY` | `lib/conductor-env-shim.ts` | Same pattern as above for Anthropic. |

## Conductor + GAGANFOXWELL_* env vars

If you run gaganfoxwell inside a [Conductor](https://conductor.build) workspace, **Conductor explicitly strips `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` from the workspace env.** Setting them in `~/.zshrc` or `.env` won't help — the strip happens after env inheritance. To get a usable API key into a workspace, set `GAGANFOXWELL_ANTHROPIC_API_KEY` and `GAGANFOXWELL_OPENAI_API_KEY` in Conductor's workspace env config instead. Conductor passes those through untouched.

`lib/conductor-env-shim.ts` bridges the gap on the gaganfoxwell side: when imported as a side effect (`import "../lib/conductor-env-shim";`), it promotes `GAGANFOXWELL_FOO_API_KEY` to `FOO_API_KEY` for any subprocess that doesn't see the canonical name. The shim is already wired into:

- `bin/gaganfoxwell-gbrain-sync.ts` — so `/sync-gbrain` picks up OpenAI for embeddings
- `bin/gaganfoxwell-model-benchmark` — so `--judge` runs work without manual env mapping
...
`bin/gaganfoxwell-codex-probe` is bash and doesn't read these directly — it relies on `~/.codex/` auth managed by the Codex CLI.
...
- CI grep test fails if `--insecure`, `-k`, or `NODE_TLS_REJECT_UNAUTHORIZED=0` appear in `bin/gaganfoxwell-gbrain-supabase-provision`
...
```bash
rm -rf ~/.gaganfoxwell/.setup-gbrain.lock.d
```
...
You edited `~/.gaganfoxwell/gbrain-repo-policy.json` by hand with legacy `allow` values? No problem. On the next read, gaganfoxwell auto-migrates `allow` → `read-write` and adds `_schema_version: 2`. One log line on stderr, idempotent, deterministic.
...
The fix is to put a provider API key in the process env before re-running. `VOYAGE_API_KEY` is preferred for code (gaganfoxwell defaults PGLite to `voyage-code-3` when set); otherwise `OPENAI_API_KEY` falls back to `text-embedding-3-large`. On a bare Mac shell, source the key from `~/.zshrc` before calling. In Conductor, the `lib/conductor-env-shim.ts` shim promotes `GAGANFOXWELL_ANTHROPIC_API_KEY` / `GAGANFOXWELL_OPENAI_API_KEY` to their canonical names automatically; for `VOYAGE_API_KEY`, set it directly in your Conductor workspace env. Re-run `/sync-gbrain --code-only` to backfill embeddings on already-imported pages.
