...
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `~/.claude/skills/gaganfoxwell/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 2" section from `~/.claude/skills/gaganfoxwell/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 3" section from `~/.claude/skills/gaganfoxwell/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `~/.claude/skills/gaganfoxwell/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `~/.claude/skills/gaganfoxwell/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `~/.claude/skills/gaganfoxwell/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `~/.claude/skills/gaganfoxwell/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `~/.claude/skills/gaganfoxwell/plan-devex-review/dx-hall-of-fame.md`.
...
This is NOT a scored pass. It's a checklist of proven patterns from gaganfoxwell's own DX.
...
(`gaganfoxwell-config set codex_reviews disabled`).
...
- **`disabled`** — the user turned Codex reviews off (`codex_reviews=disabled`). Skip this section entirely; do NOT fall back to a Claude subagent — disabled means no extra review step. Print: "Codex review skipped (codex_reviews disabled). Re-enable: `gaganfoxwell-config set codex_reviews enabled`."
...
stays discoverable: "Running the outside voice automatically (standard step). Disable: `gaganfoxwell-config set codex_reviews disabled`."
...
**Persist the result:**
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Persist after the DX Scorecard — the dashboard, the GAGANFOXWELL REVIEW REPORT, and the EXIT
PLAN MODE GATE's "review log was called" check depend on it. **PLAN MODE EXCEPTION — ALWAYS RUN** (writes to `~/.gaganfoxwell/`, not project files):

```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"plan-devex-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"PRODUCT_TYPE","tthw_current":"TTHW_CURRENT","tthw_target":"TTHW_TARGET","mode":"MODE","persona":"PERSONA","competitive_tier":"COMPETITIVE_TIER","unresolved":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
1. Brain trust policy for the active endpoint is `personal` (check via
   `~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-config get brain_trust_policy@<endpoint-hash>`).
...
a gaganfoxwell:takes fence block (documented but uglier path).
...
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)" 2>/dev/null || true
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-cache invalidate developer-persona --project "$SLUG" 2>/dev/null || true
```
...
```bash
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)" 2>/dev/null || true
(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-cache refresh --project "$SLUG" 2>/dev/null &) || true
```
