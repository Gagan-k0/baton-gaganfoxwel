...
This document explains **why** gaganfoxwell is built the way it is. For setup and commands, see CLAUDE.md. For contributing, see CONTRIBUTING.md.

## The core idea

gaganfoxwell gives Claude Code a persistent browser and a set of opinionated workflow skills. The browser is the hard part — everything else is Markdown.

The key insight: an AI agent interacting with a browser needs **sub-second latency** and **persistent state**. If every command cold-starts a browser, you're waiting 3-5 seconds per tool call. If the browser dies between commands, you lose cookies, tabs, and login sessions. So gaganfoxwell runs a long-lived Chromium daemon that the CLI talks to over localhost HTTP.

```
Claude Code                     gaganfoxwell
─────────                      ──────
                               ┌──────────────────────┐
...
1. **Compiled binaries.** `bun build --compile` produces a single ~58MB executable. No `node_modules` at runtime, no `npx`, no PATH configuration. The binary just runs. This matters because gaganfoxwell installs into `~/.claude/skills/` where users don't expect to manage a Node.js project.
...
| `POST /extension-token` | pinned Origin (`chrome-extension://<GAGANFOXWELL_EXTENSION_ID>`) + loopback Host | 404 | The only endpoint that hands out the root token |
...
Cookies are the most sensitive data gaganfoxwell handles. The design:

1. **Keychain access requires user approval.** First cookie import per browser triggers a macOS Keychain dialog. The user must click "Allow" or "Always Allow." gaganfoxwell never silently accesses credentials.
...
3. **Database is read-only.** gaganfoxwell copies the Chromium cookie DB to a temp file (to avoid SQLite lock conflicts with the running browser) and opens it read-only. It never modifies your real browser's cookie database.
...
Every enumerated gaganfoxwell-initiated off-machine sink writes a hash-chained, tamper-evident receipt to `~/.gaganfoxwell/security/egress.jsonl` BEFORE the send — `writeReceipt` in `lib/egress-receipt.ts` for TypeScript callers, `_receipted_curl` / `_receipted_git` from `bin/gaganfoxwell-egress-lib.sh` for shell scripts. Receipts record a sha256 of the exact bytes sent when the caller owns them (subprocess-owned sends like git pushes record `sha256: null`); they never store the body.
...
The Chrome sidebar agent has tools (Bash, Read, Glob, Grep, WebFetch) and reads hostile web pages, so it's the part of gaganfoxwell most exposed to prompt injection. Defense is layered, not single-point.
...
**Env knobs:** `GAGANFOXWELL_SECURITY_OFF=1` is a real kill switch (classifier stays off even if warmed; the L1-L3 filters keep running). Model cache at `~/.gaganfoxwell/models/testsavant-small/` (112MB, first run). Attack log at `~/.gaganfoxwell/security/attempts.jsonl` (salted sha256 + domain, rotates at 10MB, 5 generations). Per-device salt at `~/.gaganfoxwell/security/device-salt` (0600), cached in-process to survive FS-unwritable environments.

**Visibility.** A centered banner appears on canary leak or BLOCK verdict with the exact layer scores. `bin/gaganfoxwell-security-dashboard` aggregates local attempts; `supabase/functions/community-pulse` aggregates opt-in community telemetry across users. (The sidebar header's SEC shield icon and the `/health` `security` field were removed in #2557: their only data source — `~/.gaganfoxwell/security/session-state.json` — lost its only writer when the chat-path agent was ripped, so the shield reported stale or empty state. The live defenses report through their own call sites.)
...
```
Browser events → CircularBuffer (in-memory) → Async flush to .gaganfoxwell/*.log
```
...
1. **Update check** — calls `gaganfoxwell-update-check`, reports if an upgrade is available.
2. **Session tracking** — touches `~/.gaganfoxwell/sessions/$PPID` and counts active sessions (files modified in the last 2 hours). When 3+ sessions are running, all skills enter "ELI16 mode" — every question re-grounds the user on context because they're juggling windows.
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' ~/.gaganfoxwell/projects/<slug>/evals/_partial-e2e.json
```
