...
|----------------|-------------|-----------|-------|
| [screen name]  | ~/.gaganfoxwell/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)" 2>/dev/null || true
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-cache invalidate brand --project "$SLUG" 2>/dev/null || true
```
