 ...
 *   - prefer ~/.gaganfoxwell/openai.json over OPENAI_API_KEY
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "gaganfoxwell-design-auth-"));
...
describe("resolveApiKeyInfo", () => {
  test("uses ~/.gaganfoxwell/openai.json before OPENAI_API_KEY", () => {
    ...
    expect(describeApiKeySource(resolution!)).toBe("~/.gaganfoxwell/openai.json");
...
describe("saveApiKey", () => {
  test("stores the key file owner-only, even under a permissive umask", () => {
    ...
    const keyPath = path.join(tmpHome, ".gaganfoxwell", "openai.json");
