/**
 * gaganfoxwell design CLI — stateless CLI for AI-powered design generation.
 *
 ...
 *   2. Resolve auth (~/. gaganfoxwell/openai.json → OPENAI_API_KEY → guided setup)
...
function printUsage(): void {
  console.log("gaganfoxwell design — AI-powered UI mockup generation\n");
  console.log("Commands:");
  for (const [name, info] of COMMANDS) {
    console.log(`  ${name.padEnd(12)} ${info.description}`);
    console.log(`  ${"".padEnd(12)} ${info.usage}`);
  }
  console.log("\nAuth: ~/.gaganfoxwell/openai.json, then OPENAI_API_KEY env var");
...
async function runSetup(): Promise<void> {
  ...
  if (existing) {
    ...
    saveApiKey(key);
    console.log("Key saved to ~/.gaganfoxwell/openai.json (0600 permissions).");
  }
  ...
  try {
    await generate({
      ...
      output: "/tmp/gaganfoxwell-design-smoke-test.png",
      size: "1024x1024",
      quality: "low",
    });
  ...
  switch (command) {
    case "generate":
      await generate({
        ...
        output: (flags.output as string) || "/tmp/gaganfoxwell-mockup.png",
        check: !!flags.check,
    ...
    case "compare": {
      ...
      const outputPath = (flags.output as string) || "/tmp/gaganfoxwell-design-board.html";
    ...
    case "variants":
      await variants({
        ...
        count: flags.count,
        outputDir: (flags["output-dir"] as string) || "/tmp/gaganfoxwell-variants/",
        size: flags.size as string,
        quality: flags.quality as string,
    ...
    case "iterate":
      await iterate({
        ...
        output: (flags.output as string) || "/tmp/gaganfoxwell-iterate.png",
      });
      break;
    ...
    case "evolve":
      await evolve({
        ...
        brief: flags.brief as string,
        output: (flags.output as string) || "/tmp/gaganfoxwell-evolved.png",
      });
      break;
    ...
    case "gallery":
      gallery({
        ...
        output: (flags.output as string) || "/tmp/gaganfoxwell-design-gallery.html",
      });
      break;
