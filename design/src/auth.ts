 ...
 *
 * Resolution order:
 * 1. ~/.gaganfoxwell/openai.json → { "api_key": "sk-..." }
...
function configPath(): string {
  return path.join(process.env.HOME || "~", ".gaganfoxwell", "openai.json");
}
...
export function resolveApiKeyInfo(): ApiKeyResolution | null {
  // 1. Check ~/.gaganfoxwell/openai.json
  try {
  ...
  if (process.env.OPENAI_API_KEY) {
    ...
    const warning = envFile
      ? `Warning: OPENAI_API_KEY matches ${envFile} in the current directory. Design generation may bill that project's OpenAI account. Run $D setup to store a gaganfoxwell-specific key in ~/.gaganfoxwell/openai.json.`
      : undefined;
...
export function describeApiKeySource(resolution: ApiKeyResolution): string {
  if (resolution.source === "config") return "~/.gaganfoxwell/openai.json";
...
/**
 * Save an API key to ~/.gaganfoxwell/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!resolution) {
    ...
    console.error("  or save to ~/.gaganfoxwell/openai.json: { \"api_key\": \"sk-...\" }");
