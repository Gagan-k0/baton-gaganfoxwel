...
export const CMDLINE_MARKER = "gaganfoxwell-design-daemon";
...
export function resolveStateFilePath(): string {
  ...
  try {
    ...
    if (root) return path.join(root, ".gaganfoxwell", "design.json");
  } catch {
    // not in a git repo — fall through
  }
  return path.join(process.cwd(), ".gaganfoxwell", "design.json");
}
...
export function resolveDaemonLogPath(): string {
  return path.join(os.homedir(), ".gaganfoxwell", "design-daemon.log");
}

export function resolveStartupLogPath(): string {
  return path.join(os.homedir(), ".gaganfoxwell", "design-daemon-startup.log");
}

/**
 * Read the gaganfoxwell version both client and daemon should agree on. Looks
