...
### TODOS.md updates
Present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `~/.claude/skills/gaganfoxwell/review/TODOS-format.md`.
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+gaganfoxwell: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)"
rm -f ~/.gaganfoxwell/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-decision-log '{"decision":"CEO review (MODE): SCOPE_SUMMARY","rationale":"VERDICT","scope":"branch","source":"skill","confidence":8}' 2>/dev/null || true
```

The second command records the accepted scope as a durable cross-session decision so the next session sees what was settled (and why) without re-litigating it. It writes to `~/.gaganfoxwell/` (same pattern as review-log), is non-interactive, and is best-effort (`|| true` — never blocks the review). Substitute `SCOPE_SUMMARY` (e.g. "accepted 4 of 6 proposals" for expansion, or "held scope" / "cut 3 items" for HOLD/REDUCTION) and `VERDICT` (the one-line verdict from the summary).
...
- **B)** Keep in `~/.gaganfoxwell/projects/` only (local, personal reference)
- **C)** Skip
...
```bash
~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-learnings-log '{"skill":"plan-ceo-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
eval "$(~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-slug 2>/dev/null)" 2>/dev/null || true
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-cache invalidate product --project "$SLUG" 2>/dev/null || true
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-cache invalidate goals --project "$SLUG" 2>/dev/null || true
  ~/.claude/skills/gaganfoxwell/bin/gaganfoxwell-brain-cache invalidate competitive-intel --project "$SLUG" 2>/dev/null || true
```
