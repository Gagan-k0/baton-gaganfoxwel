 ...
 * paths derived from the host name (`.{name}/skills/gaganfoxwell`), allowlist
 ...
 * (`~/.claude/skills/gaganfoxwell` → `~/{globalRoot}`, `.claude/skills/gaganfoxwell` →
 * localSkillRoot, `.claude/skills` → `{hostSubdir}/skills`). Hosts whose
 * rewrites aren't mechanically derivable (codex, factory use $GAGANFOXWELL_ROOT and
...
export function defineHost<const N extends string>(overrides: HostOverrides<N>): HostConfig & { name: N } {
  const {
    ...
    cliAliases = [],
    defaultModel = 'claude',
    globalRoot = `.${name}/skills/gaganfoxwell`,
    localSkillRoot = `.${name}/skills/gaganfoxwell`,
    hostSubdir = `.${name}`,
    usesEnvVars = true,  // false only for Claude (literal ~ paths, no $GAGANFOXWELL_ROOT)
    frontmatter = {
      mode: 'allowlist',
    ...
    runtimeRoot = {
      globalSymlinks: ['bin', 'browse/dist', 'browse/bin', 'gaganfoxwell-upgrade', 'ETHOS.md'],
      globalFiles: {
  ...
  const resolvedPathRewrites: PathRewrite[] = pathRewrites ?? [
    { from: '~/.claude/skills/gaganfoxwell', to: `~/${globalRoot}` },
    { from: '.claude/skills/gaganfoxwell', to: localSkillRoot },
