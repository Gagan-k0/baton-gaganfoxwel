 ...
 *     stdin, unreadable slug). Errors go to ~/.gaganfoxwell/hook-errors.log,
 *     best-effort.
...
function main(): void {
  ...
  // Resolve the project slug the same way the preamble did (GAGANFOXWELL_PROJECT_SLUG
  // override, project-root walk, remote-derived slug).
  let slug = '';
  try {
    const r = runBin('gaganfoxwell-slug', [], { cwd, encoding: 'utf8', timeout: DEADLINE_MS });
