 ...
 *   1. Extract question_id via marker (<gaganfoxwell-qid:foo-bar>). If no marker,
 ...
 *
 * Always exits 0. Hook errors land in ~/.gaganfoxwell/hook-errors.log.
 ...
 * invalidates by being missing — gaganfoxwell-distill-apply doesn't touch the
...
function slugFromCwd(cwd: string | undefined): string {
  // Mirror gaganfoxwell-slug's basename fallback. The full slug resolver shells out
...
function logAutoDecided(
  ...
  try {
    ...
    runBin('gaganfoxwell-question-log', [JSON.stringify(payload)], {
      encoding: 'utf-8',
      stdio: ['ignore', 'pipe', 'pipe'],
      timeout: 3000,
      // cwd of the originating tool call so gaganfoxwell-slug resolves to the
...
async function main(): Promise<void> {
  ...
  if (isConductor()) {
    const conductorReason =
      ...
      'typed confirmation and do NOT proceed on a vague reply. Capture the decision with gaganfoxwell-question-log ' +
