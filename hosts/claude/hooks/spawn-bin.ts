/**
 * Windows-safe resolution + spawn for gaganfoxwell's bash bins. Two Windows-only
 ...
 * 2. `bin/gaganfoxwell-*` are extensionless bash scripts. Windows has no shebang
...
function bashExe(): string {
  return process.env.GAGANFOXWELL_BASH || (fs.existsSync(GIT_BASH) ? GIT_BASH : 'bash');
}

/** gaganfoxwell install root. This file lives at hosts/claude/hooks/. */
