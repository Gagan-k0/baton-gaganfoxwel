 ...
 * tool_input/tool_response, and writes them via gaganfoxwell-question-log so the
 ...
 *           "hooks": [
 *             { "type": "command",
 *               "command": "$CLAUDE_PROJECT_DIR/.claude/skills/gaganfoxwell/hosts/claude/hooks/question-log-hook",
 *               "timeout": 5 }
 *           ]
 *         }
 *       ]
 ...
 *     Errors land in ~/.gaganfoxwell/hook-errors.log for postmortem.
 *   - Spawns gaganfoxwell-question-log as a subprocess; that bin handles
 *     validation, dedup (source+tool_use_id), async derive.
 *   - Marker-first question_id (`<gaganfoxwell-qid:foo-bar>`), hash fallback
...
const MARKER_RE = /<gaganfoxwell-qid:([a-z0-9-]{1,64})>/i;
const RECOMMENDED_LABEL_RE = /\(recommended\)\s*$/i;

function logHookError(msg: string): void {
  try {
    const stateRoot =
      process.env.GAGANFOXWELL_STATE_ROOT ||
      process.env.GAGANFOXWELL_HOME ||
      path.join(os.homedir(), '.gaganfoxwell');
 ...
 * <gaganfoxwell-qid:...> wrapper) when present, else a hash-based hook- id.
...
function detectSkill(cwd: string | undefined): string {
  ...
  // skill marker (<gaganfoxwell-skill:NAME>) embedded in question text per
...
function spawnLog(payload: Record<string, unknown>, cwd?: string): void {
  const res = runBin('gaganfoxwell-question-log', [JSON.stringify(payload)], {
    encoding: 'utf-8',
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: 3000,
    // Run from the originating tool call's cwd so gaganfoxwell-slug resolves to
    // the project the user is actually in, not the hook script's location.
    cwd: cwd && fs.existsSync(cwd) ? cwd : undefined,
  });
  if (res.status !== 0) {
    logHookError(`gaganfoxwell-question-log exited ${res.status}: ${res.stderr || res.stdout}`);
  }
}
