...
const codex = defineHost({
  ...
  // .agents/skills/gaganfoxwell (symlinked runtime assets: bin, browse, review, qa,
  ...
  // Non-mechanical rewrites: the global path becomes $GAGANFOXWELL_ROOT (resolved by
  // the preamble env vars), plus an extra review-path rewrite the derived trio
  // doesn't cover.
  pathRewrites: [
    { from: '~/.claude/skills/gaganfoxwell', to: '$GAGANFOXWELL_ROOT' },
    { from: '.claude/skills/gaganfoxwell', to: '.agents/skills/gaganfoxwell' },
    { from: '.claude/skills/review', to: '.agents/skills/gaganfoxwell/review' },
